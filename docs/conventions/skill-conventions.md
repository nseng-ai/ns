...
Procedures for channel 2 are documented in `skills/skill-management/SKILL.md`; load that known first-party path directly. The canonical install flag is `--agent codex claude-code -y`. Do not maintain a duplicate skill index in `AGENTS.md`.
...
`<path...>` must identify a skill directory or its `SKILL.md` directly, such as `skills/code-gh` or `.agents/skills/writing-great-skills/SKILL.md`. There is no discovery, list, find, doctor, or whole-repository scan surface. `check` verifies only the declared paths' exposure overlays and command-backed replacement invariant; it does not validate `skills-lock.json` hashes, mirrors, acquisition, or install health.
...
- **Exposure is orthogonal to visibility.** `metadata.internal: true` is not an exposure policy.
