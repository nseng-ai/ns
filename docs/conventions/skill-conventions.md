...
| `unlisted`       | no — no ambient surface anywhere              | no — no human invocation surface   | invoke-only artifacts + `.pi/settings.json` `-skills/<name>`, and **both mirror symlinks removed** (`.agents/skills/<name>`, `.claude/skills/<name>`); no Pi replacement command |
