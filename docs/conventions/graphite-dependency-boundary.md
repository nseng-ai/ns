...
Local `ns dispatch prompt` source publication may consume the curated Flow minimal-submit client through `@nseng-ai/flow/api`, but only when the remote source is stale or missing, read-only structured planning has classified the branch as Graphite-tracked and computed the current/downstack impact, and the caller has supplied conditional Tier-3 authorization (TTY confirmation or dispatch `--force/-f`). Execution must remain bound to that exact authorized plan and fail before mutation if Flow's refreshed plan differs. Dispatch authorization is never forwarded as Graphite `--force`; Flow retains Graphite's remote-divergence safeguards.
