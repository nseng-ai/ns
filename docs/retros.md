...
`ns retros` collects compact, factual observations from local agent session logs so the `branch-retro` skill or another language-model workflow can write human-facing guidance for a branch. The standalone `retros` command is retired; use `ns retros exec ...`.
...
- `@nseng-ai/retros` package-local session adapters parse harness-specific logs into normalized session facts without retaining raw transcript content.
- `@nseng-ai/retros` evidence aggregation turns normalized session facts into deterministic observations.
- `ns retros exec collect-evidence --format json` is the supported branch-facing command envelope; `ns retros exec read-evidence-detail` reads targeted sanitized payload details.
