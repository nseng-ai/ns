...
5. **Integration — new standalone skill with cross-references.** Small edits point `objective-autorun`, including its ns-bookended parent orchestration, and planning doctrine at the new skill. No rewrite of Objective Runner behavior (the analyzed blowup was plan-driven, not runner-driven). No `orientation.md`: the discipline binds parent-orchestrator sessions only, which the skill plus plan contract already reach.
