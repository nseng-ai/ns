...
- **Consumer (consumer hat):** changing an *instance* — one concrete artifact that a capability loads and runs. Lives in `.sdl/*` (for example a single `.sdl/reviews/<slug>.md` review or a `.sdl/objectives/<slug>/` objective). It is configuration and content, not capability.
