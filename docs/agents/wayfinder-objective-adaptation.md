...
- Source: `mattpocock/skills`, upstream path `skills/in-progress/`, vendored from a
  post-1.0.1 upstream state; `skills-lock.json` records the source and hash. Upstream is
  under active development — expect it to move.
...
| **Fog** — suspected questions too coarse to state precisely      | **Fog** — a marked cluster under `## Open Questions`, never pre-sliced              |
...
- **Resolution mechanics** (answer as resolution comment, close the issue, append map
  ...
  `[x]`.
- **The `/prototype` skill dependency.** Not imported into ns; `prototype`-typed
  Question Rows describe the intended artifact generically.
