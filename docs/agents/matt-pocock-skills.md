...
| ------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------- | ---------------------------------------------------------------------------------- |
| `grilling`                                  | `skills/internal/pi-host/pi-grill-ui/SKILL.md`                                                                                                                                                                                                                                | Complete-frontier design-tree rounds through `grill_ask_round`             | Merged at the general Pi atomic-round cutover; review on refresh                   |
| `grilling`                                  | `skills/internal/pi-host/pi-grill-with-docs-ui/SKILL.md`                                                                                                                                                                                                                      | Same, composed with docs-aware behavior and reporting                      | Merged at the general Pi atomic-round cutover; review on refresh                   |
| `grilling`                                  | `ts/packages/internal/hosts/pi/tools/pi-tools/src/grill/prompts.ts` (`GRILL_UI_CONTRACT`)                                                                                                                                                                                     | Atomic complete-frontier and confirmation interaction contract             | Merged at the general Pi atomic-round cutover; pin behavior in grill UI tests      |
| `grilling`                                  | `ts/packages/incubating/hosts/pi/extensions/pi-ns-branch-context/src/saved-plan-commands.ts` (`buildWriteGrilledPlanPrompt`)                                                                                                                                                  | Saved Plan grilling prompt adapted to plan authoring and saving            | Pending semantic merge at the Saved Plan atomic-round cutover                      |
| `grilling`                                  | `skills/internal/agent-engineering/readme-driven-development/SKILL.md` (Grill step)                                                                                                                                                                                           | Operational adaptation with README-specific subject and completion bound   | Review domain additions on upstream `grilling` change                              |
| `grilling`                                  | `skills/incubating/objectives/objective-create/SKILL.md` (interview step), `docs/objective-system.md` (`objective-create` interview mirror)                                                                                                                                   | Operational adaptation with Objective-specific frontier and pattern menus  | Review domain additions and progressive disclosure on upstream `grilling` change   |
...
tracker bootstrap and prescribed throwaway-branch mechanics. The portable
`readme-driven-development` and Objective-creation adaptations now defer the common interview algorithm to the effective canonical `grilling` skill and retain
only their domain additions. The general Pi rows are merged through the atomic complete-frontier round cutover.
The Saved Plan prompt remains pending its separate cutover; its transitional singular activation
API is retained only for that consumer and the singular tool is not model-visible. The post-refresh semantic sweep otherwise remains deferred.

Dismissed near-misses from the semantic sweep — references by name or independent
vocabulary, not embeddings: `skills/incubating/objectives/objective-next/references/confirmed-execution.md`
...
(routes to the effective grilling skill by name), the `.ns/reviews/` definitions other
than `code-smell-review` (structurally independent of the two-axis review), "seam"
...
`skills/public/prs/pr-make-accountable/SKILL.md` is an intentional near-miss: its
evidence-gap interview deliberately omits grilling's recommendations and design-tree
frontier semantics, so it must remain independent rather than inherit the grilling loop.
...
- **Pi structured UI backends.** The portable upstream `grill-me` and
  ...
  `grill_ask_round` receives the complete ordered current frontier with stable IDs,
  2–5 choices, recommendations, rationales, and freeform; submitted rounds produce
  between-round status and reshape the tree; the empty frontier produces an explicit
  summary and only Confirm shared understanding / Return to grilling. General attempts
