...
This guide explains how SDL imports and adapts skills from `mattpocock/skills` without overwriting SDL-native agent workflows.
...
SDL first-party adaptations live under `skills/<name>/`. Their `.agents/skills/<name>` entries are symlinks back to `../../skills/<name>`, and `.claude/skills/<name>` symlinks through `.agents`.
...
- `review`: two-axis diff review against a fixed point, using upstream Standards and Spec sub-agent prompts.
...
Copy Matt-sourced reusable skills exactly when they remain general-purpose building blocks and do not conflict with SDL workflow ownership. Install or refresh them with the `skill-management` skill and canonical flags:
...
Use exact vendored copies for shared vocabulary or loops such as `grilling`, `domain-modeling`, `codebase-design`, and `writing-great-skills` unless SDL intentionally forks them.

## What to adapt locally

Adapt locally when SDL needs repo-specific behavior, tool use, or workflow ownership:

- Pi structured grill UI requirements (`grill_ask`, status paths, one-question tool calls, fallback prompts).
- SDL validation-scope policy.
- `CONTEXT-MAP.md` routing and SDL glossary conventions.
...
The portable upstream `grill-me` and `grill-with-docs` wrappers are intentionally tiny. SDL's Pi structured UI cannot depend on those wrappers for operational details.
...
`writing-great-skills` is imported as upstream provenance and a broader reference. SDL's operational audit checklist lives in `skills/skill-audit/`, especially `skills/skill-audit/references/writing-great-skills-adaptation.md`.

When the upstream skill-authoring vocabulary changes, update the SDL reference with the adapted concepts rather than turning `skill-audit/SKILL.md` into a tutorial.

## Future upstream update checklist

1. Read upstream `CHANGELOG.md`.
2. Compare upstream files for imported skills.
3. Classify changes as exact vendor refresh, SDL overlay update, fork required, or reject/defer.
...
- `handoff`: conflicts with SDL's Branch Memory handoff system.
- `setup-matt-pocock-skills`: conflicts with SDL `AGENTS.md`, `CONTEXT-MAP.md`, and skill-management conventions.
- `ask-matt`: routes through Matt's PRD/issue flow, not SDL Objectives, branch-context, Graphite, or SDL workflows.
- `to-prd`, `to-issues`, `triage`, `implement`: should be ported into SDL workflows only after separate design.
