...
The hidden read-only command `ns objective exec autorun-pr-title` computes a desired title and source facts without granting publication authority. Implementation children and `runner-finish` never edit pull requests. The Objective-definition PR is not an autorun slice and is not annotated.
