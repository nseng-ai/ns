...
Agent-facing progressive-disclosure details live in skill references: `skills/incubating/objectives/objective/references/execution-policy.md`, `skills/incubating/objectives/objective-create/references/execution-friendly-create.md`, and `skills/incubating/objectives/objective-next/references/confirmed-execution.md`.
...
- There is no separate public reopen command. On an explicit user request, `objective-close`'s guarded Reopen procedure deletes `closed.md`, amends `## Closure` into dated history (never rewriting close-time text), and re-runs the connected-Objective impact review in reverse with corrective Semantic Updates.
...
- Do not update a closed objective unless the user explicitly asks to amend the closed record; an explicit-request reopen routes through `objective-close`'s Reopen procedure — there is no separate public reopen command.
...
Accepted autorun pull requests carry the canonical title `[obj:<slug>] [autorun:<accepted ordinal>] <existing title>` (ADR 0052). The **accepted ordinal** is the checkpoint's 1-based position in the accepted cumulative autorun sequence; failed or recovery attempts that produce no accepted checkpoint consume no ordinal. The format is the default of the Objectives-owned cardinality-one Text-content Point `objective.autorun.pr-title` (ADR 0052), replaceable through `[points]` or `.ns/text-content/objective.autorun.pr-title.txt`; its descriptor-declared development override is `NS_OBJECTIVE_AUTORUN_PR_TITLE_TEXT_CONTENT`. Retitling is idempotent: exactly one existing canonical prefix is stripped before rendering, and empty, multiline, or over-120-character rendered titles are refused without truncation.

Title application always happens at a separately authorized external-write boundary, never inside an implementation child or `runner-finish`. In ns-bookended ADR publication, the trusted publisher computes and applies the title automatically alongside its managed body update, refusing before push on template failure or expected-title drift. Portable autorun only records the accepted order; when a later, separately authorized submit workflow creates PRs, the trusted parent computes titles with the hidden read-only compute command `ns objective exec autorun-pr-title` (objective slug, accepted ordinal, existing title in; desired title and source facts out) and applies them through the project's GitHub workflow. The Objective-definition PR is not an autorun slice and is not annotated.
