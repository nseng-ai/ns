...
> enforced by the `NS_TS_BAN_SNAKE_CASE_CLI_MACHINE_VALUE` style guard.
>
...
> ADR 0010 and ADR 0014, serialized ji machine values were migrated to kebab-case with
...
`@ji/slot` command group that is mounted only under `ns slot`; each leaf command
...
under the owning `ns` entrypoint. All `exec` subgroups use `isHidden: true`, and
the camelCase machine envelope, `0/1/2` exit codes, enveloped Zod usage errors,
and published `--json-schema` are inherited framework behavior (ADR 0010).
...
- **(d) `negative(...)` semantic correctness** (ADR 0010): `negative(...)`
...
`ns` umbrella note: the only commands physically defined under `ts/packages/kernel/src/`
are `shell show`/`shell install`. All other `ns ...` commands are either the
...
| -- | ---- | -------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------ |
| 1  | a    | Human-facing Tier 2 destructive/user-environment write lacks `--yes` / TTY-gated confirmation                                          | `brmem delete`, `ns shell install`                                                                                                                               | land-now-fix                                                 |
...
| 5  | a    | Single-PR land fast path merges to trunk with no confirmation while stack paths confirm                                                | `ccc land` (Pi surface)                                                                                                                                          | conformant (the documented Flow land exception: intentional) |
| 6  | a    | Hidden `exec` destructive/external write with no confirm flag; agent/script-only required args are sufficient intent                   | `ns branch-context exec delete`, `ns address exec reply-review-thread`, `ns address exec resolve-review-thread`                                                  | conformant (ADR 0014: args suffice)                          |
...
| 10 | d    | Query-miss (`ok(found:false)`) vs action-miss (`negative`) inconsistency across commands                                               | `pr-address` (pr-details/branch-pr/pr-checks vs download-feedback/map-branch-prs)                                                                                | conformant (ADR 0010: predicate vs action)                   |
...
| 14 | c/d  | `rawCommand` opts out of envelope entirely (no `errorType`/`resultSchema`; true failures exit 1 not 2)                                 | `packagechk` (all), `jicc cmux report`, `vibechk run`, `ccc exec autobranch`                                                                                     | land-now-fix (ADR 0010: narrow exemption; migrate)           |
...
2. **Danger-tier gaps cluster in human-facing command surfaces.** The live gaps are
   ...
   confirmation at all (`brmem delete`, `ns shell install`), and (ii) commands
   ...
   Hidden `exec` destructive/external writes are agent/script-only; by ADR 0014
...
5. **`rawCommand`/`isRawExit` packages opt out of the envelope contract.**
   ...
   now resolved by ADR 0010 (narrow exemption: only TUI/streaming/passthrough
...
| `exec check`     | No                       | d    | Absent entry → `ok(present:false)` (exit 0) — presence-predicate ratified by ADR 0010                                                                            | conformant     | `attach.ts:119-124`; `cli.ts:258,363-375`                  |
| `exec check`     | No                       | c    | Generic catch-all errorType, no `data`                                                                                                                           | land-now-fix   | `cli.ts:256`; `cli-entry.ts:156`                           |
| `exec delete`    | Yes                      | a    | Destructive Branch Memory deletion, but under hidden agent/script-only `exec`; operation args are sufficient intent and no prompt/`--yes` is required (ADR 0014) | conformant     | `cli.ts:159-165,262-270`; `attach.ts:127-139`              |
...
confirmation concern is reclassified as conformant by ADR 0014: hidden `exec`
...
`check` returning `ok(present:false)` is ratified as conformant (ADR 0010). No
...
confirmation while stack paths confirm — ratified intentional by the documented Flow land exception
(Pi surface). Lower-severity: `exec autobranch` rawCommand has no envelope
(ADR 0010: migrate, since it is a finite-result command not a raw surface).
...
| `exec pr-details`             | No                              | d    | PR-not-found → `ok(found:false)` — conformant predicate lookup (ADR 0010)                                                       | conformant (ADR 0010)             | `primitive-results.ts:30-37`; `primitive-commands.ts:160-167`       |
| `exec branch-pr`              | No                              | c/d  | Failures lack `data` (c); branch-not-found → `ok(found:false)` conformant predicate (ADR 0010)                                  | land-now-fix (c) / conformant (d) | `primitive-commands.ts:77-82,170-182`; `primitive-results.ts:30-37` |
...
| `exec pr-checks`              | No                              | d    | Miss → `ok(found:false)` (predicate lookup) vs `download-feedback` `negative` (action) — both correct under ADR 0010            | conformant (ADR 0010)             | `primitive-commands.ts:243-244`; `download-feedback.ts:62-66`       |
| `exec reply-review-thread`    | **Yes (Tier 2 external write)** | a    | Posts a comment to GitHub; agent-only hidden `exec` — operation args are sufficient intent, no confirm flag required (ADR 0014) | conformant (ADR 0014)             | `primitive-commands.ts:134-143,301-315`                             |
| `exec resolve-review-thread`  | **Yes (Tier 2 external write)** | a    | Resolves a GitHub review thread; same agent-only hidden `exec` rule (ADR 0014)                                                  | conformant (ADR 0014)             | `primitive-commands.ts:144-153,317-330`                             |
...
so (a) is resolved by ADR 0014 (agent-only hidden `exec`: required operation
...
action-miss (`negative`) split is now the ratified standard (ADR 0010), not an
...
- `ns shell show` — LOCAL (`cli.ts:284`; op `operations/shell.ts:30`)
- `ns shell install` — LOCAL (`cli.ts:294`; op `operations/shell.ts:36`)
- `ns slot shell show` / `ns slot shell install` — LOCAL (same group, dual-mounted via `cli.ts:182`)
- `ns slot ...` group — defined in `@ji/slot` (`slot/src/command-face.ts`); audited under **slot**: `list/ls/checkout/co/goto/claim/free/gc/init/resize`, `gt up/down/free-stack`, `gt exec stack-branches/stack-map-branches`
...
| Command                                        | Mutating? | Area | Finding                                                                                                                                         | Classification | Evidence (file:line)                                                 |
| ---------------------------------------------- | --------- | ---- | ----------------------------------------------------------------------------------------------------------------------------------------------- | -------------- | -------------------------------------------------------------------- |
| `ns shell show` (= `ns slot shell show`)       | No        | a    | Tier 0 read-only; resolves shell + renders script                                                                                               | conformant     | `operations/shell.ts:30-34`                                          |
| `ns shell show`                                | No        | c    | `failure("unsupported_shell", msg)` snake_case but no structured `data`                                                                         | land-now-fix   | `operations/shell.ts:32`; `sdl-core/src/shell-support.ts:135-140`    |
| `ns shell show`                                | No        | b/d  | Fixed bounded script; no negative path needed                                                                                                   | conformant     | `operations/shell.ts:33`                                             |
| `ns shell install` (= `ns slot shell install`) | Yes       | a    | Writes a managed marker block to a user dotfile outside the repo → **Tier 2** (ADR 0014); needs `--yes`/`-y` + `requireInteractiveOrUsageError` | land-now-fix   | `operations/shell.ts:36-50`; `sdl-core/src/shell-support.ts:114-129` |
| `ns shell install`                             | Yes       | c    | Shared `unsupported_shell` failure lacks structured `data`                                                                                      | land-now-fix   | `operations/shell.ts:38`; `sdl-core/src/shell-support.ts:135-140`    |
| `ns shell install`                             | Yes       | d    | Already-installed → `ok(is_already_installed:true)` — correct                                                                                   | conformant     | `operations/shell.ts:45-49`                                          |
| `ns shell install`                             | Yes       | b    | Single bounded result object                                                                                                                    | conformant     | `operations/shell.ts:45-49`                                          |
| `ns slot ...` (all slot/gt commands)           | n/a       | all  | defined in `@ji/slot`; audited under **slot**                                                                                                   | parked         | `slot/src/command-face.ts`                                           |
| dynamic extension commands                     | varies    | all  | runtime-loaded from extensions; not statically defined here                                                                                     | parked         | `cli.ts:185-217`                                                     |

**ns notes:** Substantive local surface is tiny — only `shell show`/`shell install`
(dual-mounted under `ns` and `ns slot`); the umbrella mostly composes the `@ji/slot`
...
danger tier of `shell install` is resolved to **Tier 2** by ADR 0014 (it mutates a
...
| Command       | Mutating?                                       | Area | Finding                                                                                                                                                                                   | Classification | Evidence (file:line)                          |
| ------------- | ----------------------------------------------- | ---- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------- | --------------------------------------------- |
| `cmux report` | Yes (external write: `cmux surface resume set`) | a    | Tier 1 reversible external metadata write keyed by env-provided IDs (not a user dotfile) — stays Tier 1 (ADR 0014); no prompt, no hang                                                    | conformant     | `cmux-report.ts:91-102,131`; `cli.ts:81-86`   |
...
exit 1, so ADR 0010 `errorType` and `negative(...)` semantics
...
1. **Area (a), land-now (safety):** add `--yes`/`-y` + `requireInteractiveOrUsageError`
   gating to human-facing `brmem delete` and `ns shell install` (Tier 2 per ADR
   ...
   solely to hidden `exec` destructive/external writes; ADR 0014 classifies their
...
## ADR-needed questions — current locators

The six contested design calls this audit surfaced are now resolved by
ADR 0010 and ADR 0014. Each row's classification
above has been updated accordingly.

1. **Agent-only `exec` destructive/external writes (`ns branch-context exec delete`,
   pr-address `reply`/`resolve-review-thread`):** resolved by ADR 0014 — the
...
2. **`ccc land` single-PR fast path:** resolved by the documented Flow land exception — auto-merge is
   intentional (Pi surface, not Clinkr). Row → conformant.
3. **`rawCommand`/`isRawExit` envelope exemption:** resolved by ADR 0010 — narrow
...
4. **Query-miss vs action-miss semantics (pr-address):** resolved by ADR 0010 —
   predicate/lookup misses use `ok(found:false)`; requested-target/action misses use
   `negative`. Rows → conformant.
5. **`brmem export`/`branch-context check` empty/absent as `ok`:** resolved by
   ADR 0010 — ratified as the standard. Rows → conformant.
6. **Dotfile/external-write danger tier (`ns shell install`, `jicc cmux report`):**
   resolved by ADR 0014 — user-dotfile writes are Tier 2 (`ns shell install` →
