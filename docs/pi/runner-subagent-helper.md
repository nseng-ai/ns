...
The helper API lives in `ts/packages/pi/src/runner-subagents/extension-api.ts` as `dispatchRunnerSubagent(...)`. Runtime/process internals live under `ts/packages/pi/src/runner-subagent/`. The generic LLM tool extension is `ts/packages/pi/src/runner-subagents/dispatch-extension.ts`, which registers `dispatch_runner_subagent` in final-text mode.
...
The project-local shim `.pi/extensions/dispatch-runner-subagent.ts` loads `ts/packages/pi/src/runner-subagents/dispatch-extension.ts`, registering the `dispatch_runner_subagent` tool.
