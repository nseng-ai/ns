...
This document describes the local runner-subagent helper tracked by the archived [Pi Core Subagent MVP Objective](../../.ji/objective-archive/pi-core-subagent-mvp/objective.md). It is a repo-local extension/package-layer primitive, not a Pi core API.
...
The `dispatch_runner_subagent` tool is backed by `.ji/pi/agents/runner.md`. The TypeScript extension owns execution, progress, cancellation, diagnostics, truncation, and result formatting; the Markdown definition owns runner-facing metadata and the child prompt wrapper.
...
The definition is loaded when the extension registers, so edits to `.ji/pi/agents/runner.md` require `/reload` or restarting Pi before the active tool metadata/prompt wrapper changes. Only `runner.md` is supported by this slice; additional agent variants remain future work.
