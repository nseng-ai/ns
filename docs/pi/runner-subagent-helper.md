...
The project-local shim `.pi/extensions/dispatch-runner-subagent.ts` loads `ts/packages/local-pi-tools/runner-subagents/src/extension.ts`, registering the `dispatch_runner_subagent` tool.
