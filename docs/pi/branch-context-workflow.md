...
1. Save a source-branch plan with `/ji:plan:save`, `/ji:plan:grill-and-save`, or `enriched-plan exec save`.
2. Create a branch and attach its branch context with `/ji:branch-context:from-plan` or `ji branch-context exec from-plan`.
...
4. Load and implement with `/ji:branch-context:impl-attached-plan` or `ji branch-context exec load`.

For Pi users, `/ji:branch-context:upstack-impl-from-plan` creates or reuses a branch with attached branch context, checks out the target branch, starts a fresh Pi session, and sends `/ji:branch-context:impl-attached-plan <key>` in that session. It uses Graphite by default, with `--plain-git` as an escape hatch.

## Save a source-branch plan

Pi users run `/ji:plan:save`. The static planning-policy body lives at:

```text
.ji/prompts/plans-write.md
```

For `/ji:plan:save`, the TypeScript Pi extension resolves this file from the current Git root and falls back to its built-in prompt body if Git root discovery, file reading, empty content, or symlink safety checks fail.

The structured grilling variant is `/ji:plan:grill-and-save`. It uses Pi's structured `grill_ask` UI and writes the same Saved plan artifact through `write_saved_plan_file`. The grill should resolve product/design requirements, not routine validation coverage; ordinary test/check scope is deferred to the downstream implementation agent's project policy and changed-file judgment.
...
When `XDG_STATE_HOME` is unset, the default is `$HOME/.local/state/ji/enriched-plan/...`. Legacy `~/.ji/enriched-plan/...` files are not read, migrated, or dual-written.
...
Pi users run `/ji:branch-context:from-plan`. CLI/agent workflows use:

```text
ji branch-context exec from-plan --slug <branch-context-slug> --plan-file <path> [--branch <branch>] [--branch-creation plain-git|graphite] [--summary <text>] [--format json]
```
...
Pi users can run `/ji:branch-context:upstack-impl-from-plan` after saving a plan. The command resolves a Saved plan, creates or reuses a branch with attached branch context, checks out the target branch, starts a new Pi session, and sends `/ji:branch-context:impl-attached-plan`.
...
→ send /ji:branch-context:impl-attached-plan [<key>] in that new session
```
...
Pi users run `/ji:branch-context:impl-attached-plan`. CLI/agent workflows use:
...
The code-adjacent rollback note for the trial protocol lives in `ts/packages/capabilities/branch-context/README.md`.

## Branch-context primitives

`ji branch-context exec from-plan` is documented sugar over primitive branch-context operations. The primitive operation set is also available for repair, admin, and non-plan entries:

```text
ji branch-context exec attach --plan <saved-plan-slug> [--branch <branch>] [--format json]
ji branch-context exec attach <key> --file <path> [--branch <branch>] [--format json]
ji branch-context exec list [--branch <branch>] [--format json]
ji branch-context exec check <key> [--branch <branch>] [--format json]
ji branch-context exec delete <key> [--branch <branch>] [--format json]
```
...
```text
/ji:branch-context:upstack-impl-from-plan --dry-run
```
...
```text
/ji:branch-context:upstack-impl-from-plan --branch <target-branch>
```
...
- If no Saved plan is found, run `/ji:plan:save` or pass an explicit saved-plan path.
...
- Pi commands: `/ji:plan:save`, `/ji:plan:grill-and-save`, `/ji:branch-context:from-plan`, `/ji:branch-context:upstack-impl-from-plan`, `/ji:branch-context:impl-attached-plan`.
