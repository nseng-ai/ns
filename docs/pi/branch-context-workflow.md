...
- **Local plan store**: `$XDG_STATE_HOME/ns/enriched-plan/<repo>/<encoded-source-branch>/<slug>--YY-MM-DDTHH-mm-ss--<sequence>.md` (default `$HOME/.local/state/ns/enriched-plan/...`), owned by `@nseng-ai/plans` and the `enriched-plan` CLI. Timestamped files are the only Saved Plans.
...
The caller owns the temporary transport file. It removes that exact path only after the command exits successfully and returns complete Saved Plan evidence; on save failure, it retains and reports the path.
...
An explicit plan path may be absolute or current-user home-relative with `~` or `~/`; a leading `@` is accepted and stripped. The normalized path must be absolute, match valid timestamped Saved Plan filename grammar, be a regular file, and remain lexically and physically inside the current repository/source-branch Local Plan Store.
