...
| `.pi/extensions/cmux.ts`                                       | Project-local adapter over engineered behavior      | Registers `/ns:cmux:sidebar:objective-summary`, `/ns:cmux:workspace:dispatch-plan`, `/ns:cmux:surface:dispatch-plan`, `/ns:cmux:workspace:open-branch`, and `/ns:cmux:workspace:dispatch-prompt` through the private cmux capability command suite. The retired `/ns:cmux:sidebar:session-summary` and `/ns:cmux:sidebar:branch-state-summary` commands and their backing `ns-cmux-sidebar` skill are not registered.                                                                                                                                   |
...
- `/ns:cmux:*` — command-and-control orchestration, especially cmux/session/workspace flows. Current examples: `/ns:cmux:handoff-tab`, `/ns:cmux:sidebar:objective-summary`, and `/ns:cmux:workspace:*`.
...
| cmux capability workspace/sidebar command suite (`/ns:cmux:handoff-tab`, `/ns:cmux:sidebar:objective-summary`, `/ns:cmux:workspace:dispatch-plan`, `/ns:cmux:surface:dispatch-plan`, `/ns:cmux:workspace:open-branch`, `/ns:cmux:workspace:dispatch-prompt`) | Promoted to `.pi/extensions/cmux.ts` plus engineered cmux capability-backed adapters. `/ns:cmux:sidebar:session-summary`, `/ns:cmux:sidebar:branch-state-summary`, and the `ns-cmux-sidebar` skill are retired and not registered. Workspace-opening commands do not auto-queue sidebar updates pending cmux workspace-targeting rearchitecture. `/ns:cmux:surface:dispatch-plan` launches the dispatch-plan child Pi in a new cmux surface in the caller workspace instead of opening a new workspace. `/ns:cmux:handoff-tab` creates a handoff via the portable handoff workflow, then opens a focused cmux tab; old cmux-prefixed compatibility aliases and old `/handoff-tab` are not current project commands.                |
