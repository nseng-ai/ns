...
| `.pi/extensions/code.ts`                                                  | Project-local adapter over engineered behavior | Adds `/code:changes`, `/code:cp`, `/code:submit`, `/code:autobranch`, and unified `/code:land` code/source-control commands.                                                                                                                                        |
| `.pi/extensions/checkpoint-preview.ts`                                    | Project-local adapter over engineered behavior | Adds `/cp-preview` and `/checkpoint-preview` to preview a dev-checkpoint `[cp]` commit message without staging or committing. `/cp-preview` is canonical; `/checkpoint-preview` is the currently registered alias.                                                  |
...
| `.pi/extensions/planned-branch.ts`                                        | Project-local adapter over engineered behavior | Adds `/plans:write`, `/plans:grill-and-write`, `/planned-branch:create`, `/planned-branch:upstack-impl-session`, and `/planned-branch:impl`; this repo defaults `/planned-branch:create` to Graphite, while `upstack-impl-session` is Graphite by built-in default. |
...
| `.pi/extensions/grill-ui.ts`                                              | Project-local adapter over engineered behavior | Adds `/grill-ui`, `/grill-with-docs-ui`, and the `grill_ask` structured question tool for Pi grill sessions.                                                                                                                                                        |
...
| `ts/packages/pi-extensions/src/checkpoint-preview.ts`                     | Engineered implementation                      | Package-tested checkpoint-message preview behavior for `/cp-preview` and `/checkpoint-preview`; sends a preview-only prompt and does not stage, commit, or amend.                                                                                                   |
...
6. Consolidate the local codebase/source-control commands under `/code:*` without legacy aliases. Completed; fresh RPC inventory should report `/code:changes`, `/code:cp`, `/code:submit`, `/code:autobranch`, and `/code:land` for the code extension, plus the non-code `asdl-dev` mirror `/dev:preview-url`, with no legacy `/dev:cp`, `/dev:submit`, `/cp`, `/newbr`, `/submit`, `/gh:land`, `/land-stack`, or `/gt:land-stack` aliases. The mirrored behavior is still sourced from the `asdl-dev` command table.
7. Categorize the remaining repo-owned workflow families. Completed; planned-branch now uses the `/planned-branch:*` Pi command family plus public installed agent skills, handoff artifacts use final `/handoff:*` command names and `handoff-*` skills, and branch retrospective remains intentionally skill/CLI-centered around `/skill:branch-retro` plus `aretro exec collect-evidence`.
...
| `/grill-ui`, `/grill-with-docs-ui`, plus `grill_ask`                                                                                                                                              | Pi-only structured grill UI surface: `/grill-ui` invokes internal `pi-grill-ui` for plain grilling, `/grill-with-docs-ui` invokes internal `pi-grill-with-docs-ui` for docs-aware grilling, and both share the `grill_ask` tool. Portable non-Pi routes remain the installed `grill-me` and `grill-with-docs` skills, not the internal Pi wrapper skills directly.                                                                                                                                                                                            |
...
| Checkpoint preview (`/cp-preview`, `/checkpoint-preview`)                                                                                                                                         | Preview-only checkpoint-message surface: `/cp-preview` is the canonical short command; `/checkpoint-preview` is the currently registered alias/alternate name. The command asks the agent to draft a dev-checkpoint `[cp]` message without staging, committing, amending, or mutating the repository.                                                                                                                                                                                                                                                         |
...
| Planned branch workflow (`/plans:write`, `/plans:grill-and-write`, `/planned-branch:create`, `/planned-branch:upstack-impl-session`, `/planned-branch:impl`)                                      | Final public Pi planning-layer command sequence. Portable core: the `@asdl/plans` package and `plans` bin for Saved plan authoring/inspection/resolution, the `@asdl/planned-branch` package and `planned-branch` bin for branch workflow, local store `~/.asdl/planned-branch/plans/...`, and Branch Memory namespace `planned-branch`. Installed agent skills use `/skill:plans-write`, `/skill:planned-branch-create`, and `/skill:planned-branch-impl` over the same CLI contract; the grilled interaction itself is Pi-only structured UI orchestration. |
| Handoff artifact workflow (`/handoff:create`, `/handoff:pickup`, `/handoff:list`)                                                                                                                 | Final project-local Pi handoff surface. Users create, pick up, list, and review directed handoff artifacts without Branch Memory vocabulary; pickup presents a summary before further action. Storage details are technical locators only. Python CLI exposes `handoff delete`; no Pi delete command yet. No old `brmem` aliases are retained.                                                                                                                                                                                                                |
...
| CCC workspace/sidebar command suite (`/ccc:sidebar:pr-summary`, `/ccc:sidebar:objective-summary`, `/ccc:workspace:dispatch-plan`, `/ccc:workspace:open-branch`, `/ccc:workspace:dispatch-prompt`) | Promoted to `.pi/extensions/ccc.ts` plus `ts/packages/ccc/src/cmux/`. Manual sidebar commands remain available; workspace-opening commands do not auto-queue sidebar updates pending CCC/cmux targeting rearchitecture. Old cmux-prefixed compatibility aliases are not current project commands.                                                                                                                                                                                                                                                             |
...
The planned-branch workflow uses `/plans:write`, `/plans:grill-and-write`, `/planned-branch:create`, `/planned-branch:upstack-impl-session`, and `/planned-branch:impl` in Pi, plus the `plans-write`, `planned-branch-create`, and `planned-branch-impl` skills for other agents, to save reviewed plans, create implementation branches, attach plans through Branch Memory, and load them for implementation. Saved-plan TypeScript APIs now live in `@asdl/plans`; `@asdl/planned-branch` owns branch creation, Branch Memory attachment, and implementation loading. The static `/plans:write` prompt body is repo-editable at `.asdl/prompts/plans-write.md` and resolved with `asdl exec resolve-prompt plans-write --format json`; `/plans:grill-and-write` is Pi-only structured UI orchestration over the same Saved plan artifact. `/planned-branch:upstack-impl-session` is the Pi-only convenience flow for Graphite-stacked branch creation on the current branch by default, exact `git checkout <branch>`, starting a fresh Pi session, implementation kickoff, and narrow re-run reuse of an existing Planned branch with an Attached plan when the Local plan store is missing.
