...
| `.pi/extensions/agents.ts`                                     | Project-local adapter over engineered behavior      | Registers the single `subagent` tool with `explorer` and `task` agent types plus the `/ns:agents:fleet` navigator through `@nseng-ai/ns-pi-subagents/extension`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| `.pi/extensions/grill-ui.ts`                                   | Project-local adapter over Internal Pi-tool package | Adds `/pi:grill-me`, `/pi:grill-with-docs`, and the `grill_ask` structured question tool for Pi grill sessions through `@internal/pi-tools/grill`, plus the side-quest workflow: `sq: <topic>` in the freeform answer path (or `/pi:grill-sidequest <topic>` when idle) marks the spot and pauses the interview, return happens via a session-tree jump to the ⚑ mark or `/pi:grill-return` with a Fold in / Note / Discard summary disposition, and a below-editor status widget shows pending question, answered count, and quest state. A dedicated `grill_ask` side-quest row is the documented promotion step if the workflow proves out. |
