...
| Planned branch workflow (`/planned-branch:write-plan`, `/planned-branch:write-grilled-plan`, `/planned-branch:create`, `/planned-branch:up-and-impl`, `/planned-branch:impl`)                     | Final public Pi planning-layer command sequence. Portable core: the `@asdl/planned-branch` package, `planned-branch` bin, local store `~/.asdl/planned-branch/plans/...`, and Branch Memory namespace `planned-branch`. Installed agent skills use `/skill:planned-branch-write-plan`, `/skill:planned-branch-create`, and `/skill:planned-branch-impl` over the same CLI contract; the grilled interaction itself is Pi-only structured UI orchestration. |
