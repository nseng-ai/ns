...
7. Categorize the remaining repo-owned workflow families. Completed; planned-branch commands and Branch Memory handoff commands are retained under their existing names, while branch retrospective remains intentionally skill/CLI-centered around `/skill:branch-retro` plus `aretro exec collect-evidence`.
...
| Branch Memory handoffs (`/brmem-handoff`, `/brmem-pickup-handoff`)                        | Retained as skill/CLI-centered workflow frontends. The Pi commands mirror the installed `brmem-handoff` and `brmem-pickup-handoff` skills and add picker/prompt ergonomics; Codex/Claude use those skills directly; all paths share `brmem` namespace `session-artifacts` with keys `handoffs/<slug>.md`. No `/brmem:*` rename is planned now because the existing names are explicit and match the skill names.          |
