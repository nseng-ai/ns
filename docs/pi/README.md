...
| `.pi/extensions/herdr.ts`                                      | Project-local adapter over engineered behavior      | Registers the resource-first `/ns:herdr:space:*` and `/ns:herdr:tab:*` catalog from `@nseng-ai/herdr/pi`; `/ns:herdr:tab:handoff` is conditional on the curated Handoffs integration.                                                                                                                                                                                                                                                                                                                                                                   |
...
- Repo-owned first-party product/orchestration Pi extension command families should use `/ns:<extension>:<action...>` names when introduced or renamed, such as `/ns:objective:list`, `/ns:objective:next`, or `/ns:herdr:launch:plan:space`. Reserve `/skill:<name>` for Pi's skill-command namespace, and keep `/pi:*` for Pi-native UI/session affordances.
...
- `/ns:herdr:space:*` and `/ns:herdr:tab:*` — direct Herdr resource operations. Launches use `/ns:herdr:launch:prompt:space`, `/ns:herdr:launch:plan:space`, and `/ns:herdr:launch:plan:tab`; each selects current branch or existing local trunk contextually at invocation time without fetching or refreshing trunk. Handoff Artifact launch remains the optional `/ns:herdr:tab:handoff` integration.
...
| Herdr resource command suite (`/ns:herdr:space:*`, `/ns:herdr:tab:*`)                                                                                                                      | Engineered Herdr capability adapters expose the exact nine-command resource-first catalog. Space and tab identity are explicit; `/ns:herdr:tab:handoff` is optional and is the only Herdr command that creates a Handoff Artifact. See [`../herdr/cmux-parity-checklist.md`](../herdr/cmux-parity-checklist.md) for the exact catalog and migration history.                                                                                                                                                                                                                                                                                                                                                                       |
...
Herdr commands are organized by destination resource: `/ns:herdr:space:*` for spaces and `/ns:herdr:tab:*` for tabs, with destination/payload launch commands under `/ns:herdr:launch:*`. Launch commands select current branch or existing local trunk contextually and never update trunk; no branch-basis flags or aliases are registered. Use `HERDR_WORKSPACE_ID` for explicit caller-space targeting and `HERDR_TAB_ID` when mutating the exact caller tab; never fall back to UI focus. The implementation lives in `ts/packages/capabilities/herdr/`, while Pi contributes only neutral host helpers and registration/presentation infrastructure.

See [Herdr migration history and current command catalog](../herdr/cmux-parity-checklist.md).
