...
| Handoff/pickup workflow (`/brmem-handoff`, `/brmem-pickup-handoff` today)                 | Carved out to follow-up Objective `directed-handoff-artifacts`. The target UX is a directed saved handoff artifact that users save and load without thinking in Branch Memory terms; the current `brmem`-named commands remain the present implementation until that Objective replaces, deprecates, or explicitly retains them.                                                                                          |
