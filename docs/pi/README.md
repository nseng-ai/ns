...
| `.pi/extensions/code.ts`                                                  | Project-local adapter over engineered behavior | Adds `/code:changes`, `/code:push`, `/code:autobranch`, and unified `/code:land` code/source-control commands. Submit now lives under `/sdl:submit`.                                                                                                                                    |
...
6. Consolidate the local codebase/source-control commands under `/code:*` without legacy aliases. Completed; after the native `sdl cp` slice, fresh RPC inventory should report `/code:changes`, `/code:push`, `/code:autobranch`, and `/code:land` for the code extension, `/sdl:cp` and `/sdl:submit` for SDL workflows, plus the non-code `asdl-dev` mirror `/dev:preview-url`, with no legacy `/code:cp`, `/dev:cp`, `/dev:submit`, `/cp`, `/newbr`, `/submit`, `/gh:land`, `/land-stack`, or `/gt:land-stack` aliases. Checkpoint and submit behavior are now sourced from `sdl`; `pr-regen` remains sourced from the `asdl-dev` command table.
...
Durable headless developer commands should live in a native CLI first, then be mirrored into Pi under the domain namespace chosen by the project: currently `/dev:preview-url` for preview URL lookup, `/sdl:cp` for checkpoint creation, and `/sdl:submit` for the source-control submit workflow.
