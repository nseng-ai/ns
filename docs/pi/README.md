...
| `ts/packages/hosts/pi/src/handoff/extension.ts` | Engineered implementation | Package-tested `/handoff:create`, `/handoff:pickup`, and `/handoff:list` presentation adapters over the portable `ji handoff ...` command face and Handoff Capability API. |
| `ts/packages/hosts/pi/src/branch-context/extension.ts` | Engineered implementation | Branch-context Pi command/tool wiring over the `@ji/branch-context` package. |
