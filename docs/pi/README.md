...
| `.pi/extensions/agents.ts`                                     | Project-local adapter over engineered behavior      | Registers the `explore` and `forked_pi_agent` tools plus the `/ns:agents:fleet` navigator through `@nseng-ai/ns-pi-subagents/extension`.                                                                                                                                                                                                                                      |
...
The `@nseng-ai/pi` typed parity gate tracks package-owned Pi **command** registrations. Pi model-visible tools are host-native bridges and do not require standalone parity metadata rows. If a command depends on a tool, put the fallback and parity rationale on the command row. Examples of tool bridges that are intentionally not rows: `grill_ask`, `forked_pi_agent`, `write_saved_plan_file`, `derive_handoff_slug_from_content`, and `handoff_tab_launch`.
