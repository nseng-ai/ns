...
ts/packages/local-pi-tools/<tool>/
```

The engineered layer is for durable behavior that benefits from tests, fake adapters, shared modules, or package-level validation. Project-local discovery adapters can stay in `.pi/extensions/` while the implementation lives in `@sdl/pi` or a private Local Pi-tool package.
...
| `.pi/extensions/dispatch-runner-subagent.ts`                                | Project-local adapter over Local Pi-tool package | Registers the `dispatch_runner_subagent` final-text tool for agents through `@local-pi-tools/runner-subagents`.                                                                                                                                                                                                 |
| `.pi/extensions/grill-ui.ts`                                                | Project-local adapter over Local Pi-tool package | Adds `/pi:grill-me`, `/pi:grill-with-docs`, and the `grill_ask` structured question tool for Pi grill sessions through `@local-pi-tools/grill`.                                                                                                                                                                 |
| `.pi/extensions/backing-skill-commands.ts`                                  | Project-local adapter over Local Pi-tool package | Registers generated command-backed skill commands through `@local-pi-tools/backing-skill-commands`.                                                                                                                                                                                                             |
...
| `ts/packages/hosts/pi/src/flow/sdl-extension.ts`                            | Engineered implementation                        | Mirrors native SDL flow commands into Pi as `/sdl:flow:*` commands and intentionally does not register flat `/sdl:*` or `/sdl:code:*` lifecycle aliases.                                                                                                                                                        |
...
| `ts/packages/hosts/pi/src/pr/code-extension.ts`                             | Engineered implementation                        | Aggregates the remaining `/code:*` review-feedback watch surface for the project adapter; migrated code-lifecycle commands live under SDL mirrors.                                                                                                                                                              |
...
| `ts/packages/hosts/pi/src/flow/land.ts`                                     | Engineered implementation                        | Package-tested adapter for `/sdl:flow:land`, delegating Graphite-proven single-PR and stack landing behavior to CCC.                                                                                                                                                                                            |
...
| `ts/packages/hosts/pi/src/runner-subagents/extension-api.ts` and submodules | Engineered implementation                        | Runner-subagent subprocess, JSON-event parsing, generated runtime extension, terminal capture, and final-text results.                                                                                                                                                                                          |
