...
| `.pi/extensions/asdl-dev.ts`                                              | Project-local adapter over engineered behavior | Mirrors `asdl-dev` commands into Pi as `/dev:preview-url`, `/dev:cp`, and `/dev:submit`.                                                |
| `.pi/extensions/dev.ts`                                                   | Project-local adapter over engineered behavior | Adds `/dev:changes`, `/dev:autobranch`, `/dev:land`, and `/dev:land-stack` local dev/source-control commands.                           |
...
| `ts/packages/pi-extensions/src/asdl-dev-extension.ts`                     | Engineered implementation                      | Registers the `asdl-dev` command table under Pi's `/dev:*` namespace through the generic CLI-command adapter.                           |
...
| `ts/packages/pi-extensions/src/dev.ts`                                    | Engineered implementation                      | Aggregates the package-tested Pi-only `/dev:*` local dev/source-control command family for the project adapter.                         |
...
| `ts/packages/asdl-dev/src/submit.ts`                                      | Engineered implementation                      | Headless Graphite submit command used by `asdl-dev submit` and mirrored into Pi as `/dev:submit`.                                       |
| `ts/packages/pi-extensions/src/submit.ts`                                 | Engineered implementation                      | Retained package-tested Pi submit implementation from the pre-promotion hybrid flow; not registered by the current project adapter.     |
...
6. Consolidate the local development/source-control commands under `/dev:*` without legacy aliases. Completed; fresh RPC inventory reports `/dev:preview-url`, `/dev:cp`, `/dev:changes`, `/dev:autobranch`, `/dev:submit`, `/dev:land`, and `/dev:land-stack`, with no legacy `/cp`, `/newbr`, `/submit`, `/gh:land`, or `/gt:land-stack` aliases. `/dev:submit` is now sourced from `.pi/extensions/asdl-dev.ts` through the `asdl-dev submit` command table entry.
...
| `/dev:preview-url`                                                                        | Mirrored `asdl-dev preview-url` command: prints the Vercel preview URL for a branch. It is exposed through `.pi/extensions/asdl-dev.ts`, not the Pi-only `dev.ts` extension.                                                                                                                                                                                                                                              |
| `/dev:cp`                                                                                 | Mirrored `asdl-dev cp` command: creates a checkpoint commit for the current diff with model-authored message generation. Legacy `/cp` is removed by the `/dev:*` migration.                                                                                                                                                                                                                                               |
| `/dev:changes`                                                                            | Read-only outstanding changes summary: presents the current pending worktree snapshot without staging, committing, stashing, or switching branches.                                                                                                                                                                                                                                                                       |
...
| `/dev:submit`                                                                             | Mirrored `asdl-dev submit` command: submits or updates the current Graphite stack with guarded headless submit behavior. Legacy `/submit` is removed by the `/dev:*` migration.                                                                                                                                                                                                                                           |
...
Durable headless developer commands should live in the `asdl-dev` CLI first, then be mirrored into Pi as `/dev:*` commands through `.pi/extensions/asdl-dev.ts`.
