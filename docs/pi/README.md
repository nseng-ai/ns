...
| `.pi/extensions/planned-branch.ts`                                        | Project-local adapter over engineered behavior | Adds `/planned-branch:write-plan`, `/planned-branch:write-grilled-plan`, `/planned-branch:create`, `/planned-branch:up-and-impl`, and `/planned-branch:impl`, defaulting this repo to Graphite creation.           |
...
| Planned branch workflow (`/planned-branch:write-plan`, `/planned-branch:write-grilled-plan`, `/planned-branch:create`, `/planned-branch:up-and-impl`, `/planned-branch:impl`)                     | Final public Pi planning-layer command sequence. Portable core: the `@asdl/plans` package and `plans` bin for local saved-plan inspection, the `@asdl/planned-branch` package and `planned-branch` bin for branch workflow, local store `~/.asdl/planned-branch/plans/...`, and Branch Memory namespace `planned-branch`. Installed agent skills use `/skill:planned-branch-write-plan`, `/skill:planned-branch-create`, and `/skill:planned-branch-impl` over the same CLI contract; the grilled interaction itself is Pi-only structured UI orchestration. |
...
The planned-branch workflow uses `/planned-branch:write-plan`, `/planned-branch:write-grilled-plan`, `/planned-branch:create`, `/planned-branch:up-and-impl`, and `/planned-branch:impl` in Pi, plus the `planned-branch-write-plan`, `planned-branch-create`, and `planned-branch-impl` skills for other agents, to save reviewed plans, create implementation branches, attach plans through Branch Memory, and load them for implementation. Saved-plan TypeScript APIs now live in `@asdl/plans`; `@asdl/planned-branch` owns branch creation, Branch Memory attachment, and implementation loading. The static `/planned-branch:write-plan` prompt body is repo-editable at `.asdl/prompts/planned-branch-write-plan.md` and resolved with `asdl exec resolve-prompt planned-branch-write-plan --format json`; `/planned-branch:write-grilled-plan` is Pi-only structured UI orchestration over the same Saved plan artifact. `/planned-branch:up-and-impl` is the Pi-only convenience flow for branch creation, exact `git checkout <branch>`, starting a fresh Pi session, and implementation kickoff.
