...
| `.pi/extensions/gt.ts`                                            | Project-local adapter over engineered behavior | Registers `/gt:land-stack` as the Pi-only Graphite stack landing command backed by package-tested behavior.            |
...
| `.pi/extensions/gh.ts`                                            | Project-local adapter over engineered behavior | Registers `/gh:land` as the GitHub single-PR landing command backed by package-tested behavior.                        |
...
| `ts/packages/pi-extensions/src/land.ts`                           | Engineered implementation                      | Package-tested GitHub single-PR squash landing behavior for `/gh:land`.                                                |
| `ts/packages/pi-extensions/src/land-stack.ts` and `land-stack/*`  | Engineered implementation                      | Package-tested Graphite stack landing behavior for Pi-only `/gt:land-stack`; internals are split under `land-stack/`.  |
...
4. Re-run Pi RPC command inventory after material changes and record the final surface as closure evidence. Completed.
...
| `/gh:land`                                                 | Resolved GitHub landing surface: Pi single-PR squash merge entrypoint backed by package tests; legacy `/land` alias removed. Codex/Claude should use the equivalent `gh pr view` plus `gh pr merge -s --match-head-commit ... --subject ... --body ...` flow rather than a Pi slash command. |
| `/gt:land-stack`                                           | Pi-only Graphite stack landing surface backed by package tests; legacy `/land-stack` alias removed. No Codex/Claude stack-landing workflow is claimed yet.                                                                                                                                   |
