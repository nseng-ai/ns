...
- `/ns:herdr:space:*` and `/ns:herdr:tab:*` — direct Herdr resource operations. Prompt, session, and Saved Plan implementation use symmetric `/ns:herdr:impl:*:{space,tab}` commands. Prompt and plan workflows select current branch or Local trunk contextually without fetching or refreshing trunk; session workflows prefill the matching prompt command without auto-submission or mutation. Handoff launch remains the optional `/ns:herdr:tab:handoff` integration.
...
Herdr direct operations are organized by destination resource: `/ns:herdr:space:*` for spaces and `/ns:herdr:tab:*` for tabs. Prompt, session, and Saved Plan implementation are symmetric across `/ns:herdr:impl:*:{space,tab}` destinations. Prompt and Saved Plan commands select the current branch or Local trunk contextually and never fetch or update trunk; Local trunk uses cached `origin/HEAD` plus the local branch, not a Graphite trunk query. Tab-targeted prompt and plan commands capture and validate `HERDR_WORKSPACE_ID` immediately after acknowledgement, before idle waiting, Git inspection, interaction, or mutation, then create a focused tab in that explicit caller space.

Session commands create a visible model summary turn and prefill the matching prompt command in the editor. A shared pending coordinator covers both destinations; the commands never auto-submit or mutate Herdr, Git, Handoff, or Branch Memory state. Launch vocabulary remains valid for lower-level mechanics: Prepared Herdr Launch owns destination startup, Pi launch mechanics start the process, and `ns-impl` identifies prompt transport/storage. Handoff launch remains the optional `/ns:herdr:tab:handoff` flow over a durable artifact. User-facing terms are **space**, **tab**, and **caller space**; reserve **workspace** for `HERDR_WORKSPACE_ID` and upstream mechanics. Use `HERDR_TAB_ID` when mutating the exact caller tab, and never fall back to UI focus.
