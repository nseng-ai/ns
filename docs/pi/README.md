...
| `ts/packages/capabilities/flow/src/pi/ji-extension.ts`         | Engineered implementation                        | Mirrors native SDL flow commands into Pi as `/ji:flow:*` commands and intentionally does not register flat `/ji:*` or `/ji:code:*` lifecycle aliases.                                                                                                                                                                                                                                                     |
...
| `ji flow submit` | SDL project-local extension command | SDK-only project-local Graphite submit command mirrored into Pi as `/ji:flow:submit`; no `/ji:flow:submit` or legacy submit aliases are retained. |
...
6. Consolidate the local codebase/source-control commands into grouped SDL flow surfaces without legacy aliases. Fresh RPC inventory should report `/code:pr-feedback-watch` as the remaining non-SDL review watch surface and `/ji:flow:changes`, `/ji:flow:cp`, `/ji:flow:autobranch`, `/ji:flow:branch-latest-commit`, `/ji:flow:autoslot`, `/ji:flow:submit`, `/ji:flow:regenerate-pr`, `/ji:flow:push`, `/ji:flow:land`, and `/ji:flow:pull-trunk` as the SDL lifecycle mirrors, with no legacy `/code:*`, flat lifecycle `/ji:*`, or `/ji:code:*` aliases.
...
| `/ji:flow:regenerate-pr`                                                                                                                                                                                                                                    | Primary mirrored `ji flow regenerate-pr` command: regenerates the current branch PR title and SDL-managed generated description region, preserves human body text outside that region, and asks before `gh pr edit`. Legacy `/ji:regenerate-pr`, `/ji:code:regenerate-pr`, `/code:pr-regen`, and `/ji:pr-regen` aliases are not registered.                                                                                                                                                                                                                                                                                                                                                                            |
...
| `/ji:flow:push`                                                                                                                                                                                                                                             | Primary mirrored `ji flow push` command: requires a clean worktree, runs plain `git push`, and renders generic SDL CLI output. It does not update Graphite metadata and is not a replacement for `/ji:flow:submit`; do not use it on Graphite-tracked PR branches or anywhere Graphite submission/PR metadata updates are desired. Legacy `/ji:push`, `/ji:code:push`, and `/code:push` aliases are not registered.                                                                                                                                                                                                                                                                                                    |
...
Durable headless developer commands should live in a native CLI first where practical, then be mirrored into Pi under the domain namespace chosen by the project. SDL lifecycle mirrors now use the grouped flow family: `/ji:flow:changes`, `/ji:flow:cp`, `/ji:flow:autobranch`, `/ji:flow:branch-latest-commit`, `/ji:flow:autoslot`, `/ji:flow:submit`, `/ji:flow:regenerate-pr`, `/ji:flow:push`, `/ji:flow:land`, and `/ji:flow:pull-trunk`.

SDL extension discovery is currently CLI-only. Pi does not dynamically register arbitrary `/ji:<name>` commands from `.ji/extensions` at runtime; exact Pi mirrors are static engineered adapters over selected SDL commands and need package tests/parity metadata when added or renamed. Current Flow mirrors are owned by `@ji/flow/pi`, whose `.pi/extensions/ji.ts` discovery adapter imports the package through its exports. Treat dynamic Pi mirror discovery as future SDL/Pi design work, not an implicit property of project-local SDL extensions.
