...
| `.pi/extensions/code.ts`                                                  | Project-local adapter over engineered behavior | Adds `/code:changes`, `/code:submit`, `/code:push`, `/code:autobranch`, and unified `/code:land` code/source-control commands.                                                                                                                                                          |
| `.pi/extensions/sdl.ts`                                                   | Project-local adapter over engineered behavior | Adds native `/sdl:cp` checkpoint creation over the `sdl cp` CLI.                                                                                                                                                                                                                        |
...
| `ts/packages/pi-extensions/src/asdl-dev-extension.ts`                     | Engineered implementation                      | Splits the remaining `asdl-dev` command table by Pi domain namespace: `/dev:preview-url` plus `/code:submit` and `/code:pr-regen`.                                                                                                                                                      |
...
- `/code:*` — codebase/source-control management workflows such as pending changes, checkpoints, branch movement, submit, and landing.
...
6. Consolidate the local codebase/source-control commands under `/code:*` without legacy aliases. Completed; after the native `sdl cp` slice, fresh RPC inventory should report `/code:changes`, `/code:submit`, `/code:push`, `/code:autobranch`, and `/code:land` for the code extension, `/sdl:cp` for checkpoint creation, plus the non-code `asdl-dev` mirror `/dev:preview-url`, with no legacy `/code:cp`, `/dev:cp`, `/dev:submit`, `/cp`, `/newbr`, `/submit`, `/gh:land`, `/land-stack`, or `/gt:land-stack` aliases. Checkpoint behavior is now sourced from `sdl`; remaining submit/pr-regen mirrors are still sourced from the `asdl-dev` command table.
...
| `/code:push`                                                                                                                                                                                                                  | Git push convenience command: requires a clean worktree, runs `git push`, and renders bounded output. It is not a replacement for `/code:submit`; use it only for already-committed branch pushes where Graphite submission/PR metadata updates are not desired.                                                                                                                                                                                                                                                                                                                                          |
...
| Handoff artifact workflow (`/handoff:create`, `/handoff:pickup`, `/handoff:list`)                                                                                                                                             | Final project-local Pi handoff lifecycle surface. Users create, pick up, list, and review directed handoff artifacts without Branch Memory vocabulary; pickup presents a summary before further action. Storage details are technical locators only. Python CLI exposes `handoff delete`; no Pi delete command yet. No old `brmem` aliases are retained. Focused tab launch is `/ccc:handoff-tab`, not part of the artifact lifecycle namespace.                                                                                                                                                          |
...
Durable headless developer commands should live in a native CLI first, then be mirrored into Pi under the domain namespace chosen by the project: currently `/dev:preview-url` for preview URL lookup, `/sdl:cp` for checkpoint creation, and `/code:submit` for the remaining source-control submit workflow.
