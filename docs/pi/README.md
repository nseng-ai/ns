...
| `ts/packages/incubating/extensions/flow/src/autobranch/*` and `checkpoint/*` | Engineered implementation | Provider-explicit autobranch workflow engine over pending-worktree, preparation, latest-commit extraction, and transaction helpers; Graphite uses `ns flow gt ...` / `/ns:flow:gt:*`, while official github/gh-stack v0.1.0 uses `ns flow gs ...` / `/ns:flow:gs:*` for autobranch, branch-latest-commit, and autoslot. |
...
6. Consolidate the local codebase/source-control commands into grouped ns flow surfaces without legacy aliases. Fresh RPC inventory should report `/code:pr-feedback-watch` as the remaining non-ns review watch surface and `/ns:flow:changes`, `/ns:flow:cp`, `/ns:flow:gt:autobranch`, `/ns:flow:gt:branch-latest-commit`, `/ns:flow:gt:autoslot`, `/ns:flow:gt:submit`, `/ns:flow:generate-pr-inventory`, `/ns:flow:push`, `/ns:flow:gt:land`, `/ns:flow:pull-trunk`, `/ns:flow:gs:autobranch`, `/ns:flow:gs:branch-latest-commit`, `/ns:flow:gs:autoslot`, and `/ns:flow:gt:squash-stack` as the ns lifecycle mirrors, with no legacy `/code:*`, flat lifecycle `/ns:*`, or `/ns:code:*` aliases.
...
| `/ns:flow:gs:autoslot`                                                                                                                                                                     | Autoslot command backed by the official `github/gh-stack` extension: creates or adopts a provider child, then performs the same managed Slot checkout when clean. Install it with `gh extension install github/gh-stack`. Untracked non-trunk sources run `gh stack init`, which durably enables rerere; untracked Git trunk refuses before Slot checkout. Recover by checking out a non-trunk source, or inspect retained provider/Git branches after a later failure.                                                                                                                                                                                                                                                                                                                                                |
...
Durable headless developer commands should live in a native CLI first where practical, then be mirrored into Pi under the domain namespace chosen by the project. ns lifecycle mirrors now use the grouped flow family: `/ns:flow:changes`, `/ns:flow:cp`, `/ns:flow:gt:autobranch`, `/ns:flow:gt:branch-latest-commit`, `/ns:flow:gt:autoslot`, `/ns:flow:gt:submit`, `/ns:flow:generate-pr-inventory`, `/ns:flow:push`, `/ns:flow:gt:land`, `/ns:flow:pull-trunk`, `/ns:flow:gs:autobranch`, `/ns:flow:gs:branch-latest-commit`, `/ns:flow:gs:autoslot`, and `/ns:flow:gt:squash-stack`.
