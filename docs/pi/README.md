...
| `.pi/extensions/proto.ts`                                                 | Project-local adapter over engineered behavior | Adds the parked prototype Objective runner wrapper `/proto:objective-impl`.                                                                                         |
...
| `.pi/extensions/roast.ts`                                                 | Project-local adapter over engineered behavior | Adds `/roast`, which invokes the local `roaster review run-matching` CLI for changed-path-selected reviewers.                                                       |
...
| `ts/packages/pi-extensions/src/proto.ts`                                  | Engineered implementation                      | Prototype Objective implementation wrapper for `/proto:objective-impl`; lifecycle disposition is parked.                                                            |
...
| `ts/packages/pi-extensions/src/cmux.ts` and `cmux/*`                      | Engineered implementation                      | Project cmux command suite, caller-workspace sidebar controller, slot helpers, and workspace-opening PR-sidebar hooks.                                              |
...
| `ts/packages/pi-extensions/src/roast.ts`                                  | Engineered implementation                      | Package-tested `/roast` command wrapper over the local `roaster review run-matching` CLI.                                                                           |
...
| `/proto:objective-impl` plus `/skill:proto-objective-impl`                                                                                         | Parked prototype Objective runner surface: Pi uses the explicit prototype wrapper and other harnesses can invoke the internal prototype skill by name. It is not canonical `/objective:*` behavior, and merge/promote/retire disposition remains parked unless explicitly unparked.                                                                                                                                       |
...
| Project cmux command suite (`/cmux:pr-sidebar`, `/cmux:objective-sidebar`, `/cmux-slot:dispatch-plan`, `/cmux-slot:open-branch`, `/cmux-dispatch`) | Promoted to `.pi/extensions/cmux.ts` plus `ts/packages/pi-extensions/src/cmux/`. Workspace-opening commands queue a caller-workspace PR-sidebar hook after success; `/cmux:set-workspace-summary` and `/cmux-refresh-meta` are not retained as aliases.                                                                                                                                                                   |
...
cmux-related Pi commands should target the caller workspace for summary updates, keep cmux mutations behind deterministic `asdl exec` commands, and avoid relying on stale local cmux source checkouts. The project cmux suite lives behind `.pi/extensions/cmux.ts` and `ts/packages/pi-extensions/src/cmux/`.
