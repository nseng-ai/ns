...
| Branch context workflow (`/enriched-plan:save`, `/enriched-plan:grill-and-save`, `/branch-context:from-plan`, `/branch-context:upstack-impl-session`, `/branch-context:impl`)                                                                               | Final public Pi planning-layer command sequence. Portable core: the `@sdl/plans` package and `enriched-plan` bin for Saved plan authoring/inspection/resolution, the `@sdl/branch-context` package and `branch-context` bin for branch workflow, local store `~/.sdl/enriched-plan/...`, and Branch Memory namespace `branch-context` with named Markdown attached-plan keys (`plan.md` remains legacy readable storage). Installed agent skills use `/skill:enriched-plan-save`, `/skill:branch-context-from-plan`, and `/skill:branch-context-impl` over the same CLI contract; the grilled interaction itself is Pi-only structured UI orchestration. |
