...
| `.pi/extensions/sdl.ts`                                           | Project-local adapter over engineered behavior | Adds grouped `/sdl:flow:*` lifecycle mirrors over the `sdl flow` CLI family: changes, cp, autobranch, autoslot, submit, regenerate-pr, push, land, and pull-trunk.                                                                                                                                              |
...
| `ts/packages/pi-extensions/src/sdl-extension.ts`                  | Engineered implementation                      | Mirrors native SDL flow commands into Pi as `/sdl:flow:*` commands and intentionally does not register flat `/sdl:*` or `/sdl:code:*` lifecycle aliases.                                                                                                                                                        |
...
| `ts/packages/pi-extensions/src/land.ts`                           | Engineered implementation                      | Package-tested adapter for `/sdl:flow:land`, delegating Graphite-proven single-PR and stack landing behavior to CCC.                                                                                                                                                                                            |
...
| `/sdl:flow:push`                                                                                                                                                                                                                                            | Primary mirrored `sdl flow push` command: requires a clean worktree, runs plain `git push`, and renders generic SDL CLI output. It is not a replacement for `/sdl:flow:submit`; use it only for already-committed branch pushes where Graphite submission/PR metadata updates are not desired. Legacy `/sdl:push`, `/sdl:code:push`, and `/code:push` aliases are not registered.                                                                                                                                                                                                                                                                                                                      |
...
Durable headless developer commands should live in a native CLI first where practical, then be mirrored into Pi under the domain namespace chosen by the project. SDL lifecycle mirrors now use the grouped flow family: `/sdl:flow:changes`, `/sdl:flow:cp`, `/sdl:flow:autobranch`, `/sdl:flow:autoslot`, `/sdl:flow:submit`, `/sdl:flow:regenerate-pr`, `/sdl:flow:push`, `/sdl:flow:land`, and `/sdl:flow:pull-trunk`.
