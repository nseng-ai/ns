...
| `.pi/extensions/subagents.ts`                                  | Project-local adapter over Internal Pi-tool package | Registers the `explore` and `dispatch_runner_subagent` tools plus the `/ns:subagents:fleet` navigator through `@nseng-ai/ns-pi-subagents/extension`.                                                                                                                                                                                                                                      |
...
| `ts/packages/extensions/ns-pi-subagents/src/runner-subagents/extension-api.ts` and submodules | Internal Pi-tool package | Runner-subagent subprocess, JSON-event parsing, generated runtime extension, terminal capture, and final-text results. |
