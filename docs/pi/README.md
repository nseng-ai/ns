...
- Repo-owned Pi slash commands must show immediate acknowledgement that the command was received, before waiting for Pi idle state or doing slow I/O. Engineered commands should use `registerCommandWithImmediateAck` from `@sdl/pi-extension-runtime/command-ack` at each command registration site. Default acknowledgement delivery is an above-fold dim transcript message when the host supports rendered custom messages; it falls back to a transient status line for minimal hosts. Use `{ delivery: "status" }` only for commands whose acknowledgement must stay out of the transcript.
