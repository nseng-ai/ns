...
| `.pi/extensions/sdl.ts`                                           | Project-local adapter over engineered behavior | Adds primary native `/sdl:changes`, `/sdl:cp`, `/sdl:autobranch`, `/sdl:submit`, and `/sdl:regenerate-pr` commands over the `sdl` CLI, plus nested `/sdl:code:*` code-lifecycle surfaces for changes, autoslot, land, push, and trunk pull.                                                                     |
...
| `ts/packages/pi-extensions/src/sdl-extension.ts`                  | Engineered implementation                      | Mirrors native SDL commands into Pi as primary `/sdl:changes`, `/sdl:cp`, `/sdl:autobranch`, `/sdl:submit`, and `/sdl:regenerate-pr` commands, plus the currently restored nested `/sdl:code:*` code-lifecycle commands.                                                                                        |
...
Durable headless developer commands should live in a native CLI first where practical, then be mirrored into Pi under the domain namespace chosen by the project: primary `/sdl:changes`, `/sdl:cp`, `/sdl:submit`, and `/sdl:regenerate-pr` SDL mirrors; and nested `/sdl:code:*` surfaces for the code-lifecycle taxonomy.
