...
A handoff is a directed, saved work-context artifact for a specific future continuation. Use save/pickup/list/resume language in normal user-facing copy; Branch Memory is the current technical storage layer, not the public model. The project-local Pi command surface remains `/handoff:create`, `/handoff:pickup`, and `/handoff:list`; explicit single-handoff deletion is currently available through the Python CLI as `handoff delete [--branch <branch>] [-f|--force] <slug>`.
