...
| `.pi/extensions/code.ts`                                                  | Project-local adapter over engineered behavior | Adds `/code:changes`, `/code:cp`, `/code:submit`, `/code:push`, `/code:autobranch`, and unified `/code:land` code/source-control commands.                                                                                                                                              |
...
6. Consolidate the local codebase/source-control commands under `/code:*` without legacy aliases. Completed; fresh RPC inventory should report `/code:changes`, `/code:cp`, `/code:submit`, `/code:push`, `/code:autobranch`, and `/code:land` for the code extension, plus the non-code `asdl-dev` mirror `/dev:preview-url`, with no legacy `/dev:cp`, `/dev:submit`, `/cp`, `/newbr`, `/submit`, `/gh:land`, `/land-stack`, or `/gt:land-stack` aliases. The mirrored behavior is still sourced from the `asdl-dev` command table where applicable.
