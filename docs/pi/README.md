...
| `.pi/extensions/ji.ts`                                         | Project-local adapter over engineered behavior   | Delegates to `sdl-flow/pi/sdl-extension` for grouped `/ji:flow:*` lifecycle mirrors over the `ji flow` CLI family: changes, cp, autobranch, branch-latest-commit, autoslot, submit, regenerate-pr, push, land, and pull-trunk.                                                                                                                                                                            |
...
| `ts/packages/capabilities/flow/src/pi/sdl-extension.ts`        | Engineered implementation                        | Mirrors native SDL flow commands into Pi as `/ji:flow:*` commands and intentionally does not register flat `/ji:*` or `/ji:code:*` lifecycle aliases.                                                                                                                                                                                                                                                     |
...
| Branch context workflow (`/ji:plan:save`, `/ji:plan:grill-and-save`, `/ji:branch-context:from-plan`, `/ji:branch-context:upstack-impl-from-plan`, `/ji:branch-context:impl-attached-plan`)                                                                  | Final public Pi planning-layer command sequence. Portable core: the `@sdl/plans` package and `enriched-plan` bin for Saved plan authoring/inspection/resolution, the `@sdl/branch-context` package and `ji branch-context exec` command surface for branch workflow, XDG local store `$XDG_STATE_HOME/ji/enriched-plan/...` (default `$HOME/.local/state/ji/enriched-plan/...`), and Branch Memory namespace `branch-context` with named Markdown attached-plan keys such as `<slug>.md`. Installed agent skills use `/skill:enriched-plan-save`, `/skill:branch-context-from-plan`, and `/skill:branch-context-impl` over the same CLI contract; the grilled interaction itself is Pi-only structured UI orchestration. |
