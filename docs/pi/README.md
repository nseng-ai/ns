...
| `ts/packages/capabilities/herdr/src/pi/extension.ts` and `core/*` | Engineered implementation | Private Herdr resource command suite, explicit caller space/tab targeting, responsible implementation workflows, Prepared Herdr Launch composition, and optional Handoff Artifact tab launch. |
...
| Herdr resource and implementation suite (`/ns:herdr:space:*`, `/ns:herdr:tab:*`, `/ns:herdr:impl:*`)                                                                                       | Engineered Herdr capability adapters expose the exact nine-command catalog. Space and tab identity are explicit; implementation commands own responsible attempts rather than process startup alone; `/ns:herdr:tab:handoff` is optional and is the only Herdr command that creates a Handoff Artifact. See [`../herdr/command-catalog.md`](../herdr/command-catalog.md) for the exact catalog and migration history.                                                                                                                                                                                                                                                                                                              |
...
Herdr direct operations are organized by destination resource: `/ns:herdr:space:*` for spaces and `/ns:herdr:tab:*` for tabs. Implementation workflows are `/ns:herdr:impl:prompt:space`, `/ns:herdr:impl:plan:space`, and `/ns:herdr:impl:plan:tab`. They select current branch or existing local trunk contextually and never update trunk; no branch-basis flags or aliases are registered. Their semantic contract is a responsible implementation attempt: inspect the supplied input and repository, implement when valid, otherwise stop with a concrete blocker or clarification request.

Launch vocabulary remains valid for the mechanics below that contract. Prepared Herdr Launch owns destination startup, Pi launch mechanics start the process, and `ns-launch` remains prompt transport/storage. Handoff launch remains the distinct `/ns:herdr:tab:handoff` flow over a durable artifact. Use `HERDR_WORKSPACE_ID` for explicit caller-space targeting and `HERDR_TAB_ID` when mutating the exact caller tab; never fall back to UI focus. The implementation lives in `ts/packages/capabilities/herdr/`, while Pi contributes only neutral host helpers and registration/presentation infrastructure.
