...
| `/ns:flow:land`                                                                                                                                                                            | Unified landing surface backed by package tests: requires Graphite stack proof, single-branch fast path squash-merges a PR into `gt trunk`, and stack-mode lands bottom-to-current while performing Post-Merge Stack Reconciliation after each successful merge. Landed-Branch Cleanup remains a separate opt-in resource-disposition step. Legacy `/code:land`, `/land`, `/gh:land`, `/land-stack`, and `/gt:land-stack` aliases are not registered. Codex/Claude should use equivalent `gt`/`gh` CLI flows rather than a Pi slash command.                                                                                                                                                                                                                                                                           |
