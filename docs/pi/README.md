...
| `ns flow submit` | ns project-local extension command | SDK-only submit command mirrored into Pi as `/ns:flow:submit`; absent `[workflow].stack-provider` targets one current-branch Git/GitHub PR, while configured `graphite` targets the current Graphite stack. No flat or legacy submit aliases are retained. |
...
| `/ns:flow:submit`                                                                                                                                                                          | Primary mirrored `ns flow submit` command: checkpoints pending work, then targets one current-branch Git/GitHub PR when `[workflow].stack-provider` is absent or the current Graphite stack when it is `graphite`. Newly created PRs receive generated metadata and optional normalized `--title-prefix`; existing PR prose is preserved by default. `--generate-pr-inventory` explicitly replaces every selected PR's complete title/body after early confirmation. Legacy aliases are not registered.                                                                                                                                                                                                                                                                                                                |
...
| `/ns:flow:land`                                                                                                                                                                            | Unified landing surface backed by package tests: absent `[workflow].stack-provider`, squash-merges the current branch's GitHub PR into Git trunk; configured `graphite` lands the current stack bottom-to-current and preserves descendant maintenance. Legacy `/code:land`, `/land`, `/gh:land`, `/land-stack`, and `/gt:land-stack` aliases are not registered.                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
