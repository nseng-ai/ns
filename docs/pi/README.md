...
| `ts/packages/pi-extensions/src/asdl-dev-extension.ts`                     | Engineered implementation                      | Mirrors `/dev:preview-url` and exposes the remaining PR-description implementation through `/sdl:code:regenerate-pr` while the lower implementation still lives in `asdl-dev pr-regen`.                                                                                                 |
...
| `sdl submit`                                                              | SDL built-in command                           | Headless Graphite submit command mirrored into Pi as `/sdl:submit`; replaces the transitional `asdl-dev submit` / `/code:submit` surfaces.                                                                                                                                              |
...
- `/code:*` — review/watch or code-management workflows intentionally outside the SDL code-lifecycle family. Current project-owned use is `/code:pr-feedback-watch`; migrated code-lifecycle workflows should not keep `/code:*` compatibility aliases.
...
6. Consolidate the local codebase/source-control commands into SDL code-lifecycle surfaces without legacy aliases. Completed; fresh RPC inventory should report `/code:pr-feedback-watch` as the remaining non-SDL review watch surface, primary `/sdl:changes`, `/sdl:cp`, and `/sdl:submit` SDL mirrors, nested `/sdl:code:changes`, `/sdl:code:checkpoint`, `/sdl:code:submit`, `/sdl:code:autobranch`, `/sdl:code:autoslot`, `/sdl:code:land`, `/sdl:code:push`, and `/sdl:code:regenerate-pr`, plus the non-code `asdl-dev` mirror `/dev:preview-url`, with no legacy `/code:changes`, `/code:cp`, `/code:checkpoint`, `/code:autobranch`, `/code:autoslot`, `/code:land`, `/code:push`, `/code:pr-regen`, `/dev:cp`, `/dev:submit`, `/cp`, `/newbr`, `/submit`, `/gh:land`, `/land-stack`, or `/gt:land-stack` aliases.
...
| `/sdl:code:regenerate-pr`                                                                                                                                                                                                     | PR metadata regeneration surface over the remaining `asdl-dev pr-regen` implementation. Legacy `/code:pr-regen` is not registered; review-feedback watch remains outside the SDL code-lifecycle family.                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
...
Durable headless developer commands should live in a native CLI first where practical, then be mirrored into Pi under the domain namespace chosen by the project: currently `/dev:preview-url` for preview URL lookup; primary `/sdl:changes`, `/sdl:cp`, and `/sdl:submit` SDL mirrors; and nested `/sdl:code:*` surfaces for the code-lifecycle taxonomy.
