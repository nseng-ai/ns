...
The engineered layer is for durable behavior that benefits from tests, fake adapters, shared modules, or package-level validation. Project-local discovery adapters can stay in `.pi/extensions/` while the implementation lives in `@ns/pi`, a private capability-pi package such as `@ns/flow/pi`, or a private Local Pi-tool package.
...
| `ts/packages/local/pi-tools/src/pr-previews/extension.ts`      | Local Pi-tool package                            | Registers read-only `/pr:preview-feedback` and `/pr:preview-checks` modal previews; download/editor-prefill and feedback watch behavior remain in `@ns/pi`.                                                                                                                                                                                                                     |
