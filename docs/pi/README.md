...
| `ts/packages/capability-pi/flow/src/sdl-extension.ts`          | Engineered implementation                        | Mirrors native SDL flow commands into Pi as `/sdl:flow:*` commands and intentionally does not register flat `/sdl:*` or `/sdl:code:*` lifecycle aliases.                                                                                                                                                        |
...
| `ts/packages/capability-pi/flow/src/code-extension.ts`         | Engineered implementation                        | Aggregates Flow/Code workflow Pi presentation for `/code:gt-restack-resolve` and `/gt:squash-stack` in `@sdl/flow-pi`; `@sdl/pi` does not import the extracted package.                                                                                                                                         |
