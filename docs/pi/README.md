...
The branch-context workflow uses `/ns:plan:save`, `/ns:plan:grill-and-save`, the Git/GT/GS `new-branch-from-plan` and `impl-branch-from-plan` families, and `/ns:branch-context:impl-attached-plan` in Pi. The `git`, `gt`, and `gs` namespaces select plain Git, Graphite, and GitHub Stacks without provider flags. `new-branch-from-plan` retains or restores the original branch; `impl-branch-from-plan` leaves the target checked out and starts provider-independent Attached Plan implementation in a fresh Pi session. The `branch-context-from-plan` and `branch-context-impl` skills provide the portable agent workflow. Saved-plan TypeScript APIs live in `@nseng-ai/plans`; `@nseng-ai/branch-context` owns preparation, attachment, reuse, and implementation loading. `@nseng-ai/pi-ns-branch-context` owns shared/ Git Pi presentation, while `@nseng-ai/pi-ns-gt` and `@nseng-ai/pi-ns-gs` own their provider namespaces. GitHub Stacks support here is limited to branch creation and local topology inspection; it does not imply reconciliation, submit, land, or publication parity.
