...
ts/packages/incubating/extensions/<extension>/src/pi/
ts/packages/internal/hosts/pi/tools/pi-tools/src/<tool>/
```

The engineered layer is for durable behavior that benefits from tests, fake adapters, shared modules, or package-level validation. Project-local discovery adapters can stay in `.pi/extensions/` while the implementation lives in `@nseng-ai/pi-runtime`, an extension `pi` subpackage such as `@nseng-ai/flow/pi`, or a private Internal Pi-tool package.
...
| -------------------------------------------------------------------------------------------- | --------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `.pi/extensions/objective.ts`                                                                | Project-local discovery adapter (migration state)   | Currently delegates namespaced `/ns:objective:*` wrappers to Objective-package code. ADR 0049's destination is the required-`ns` host package `@nseng-ai/pi-ns-objectives`, consumed through this thin discovery seam.                                                                                                                                                                                                                                                                                                                                           |
| `.pi/extensions/claude.ts`                                                                   | Project-local adapter over engineered behavior      | Delegates interactive Claude Code Handoff launch behavior to `@nseng-ai/handoffs/pi/claude-extension`.                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
...
| `.pi/extensions/code.ts`                                                                     | Project-local host composition adapter              | Composes internal `@internal/pi-tools/code-workflows/smart-restack` for `/code:gt-restack-resolve` with Flow-owned `stackSquashExtension` for `/gt:squash-stack`; neither owner imports the other.                                                                                                                                                                                                                                                                                                                                                               |
...
| `.pi/extensions/pr.ts`                                                                       | Project-local host composition adapter              | Composes Pi-runtime PR commands with the internal PR-feedback-watch extension; neither owner imports the other.                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| `.pi/extensions/ns.ts`                                                                       | Project-local adapter over engineered behavior      | Delegates to `@nseng-ai/flow/pi/ns-extension` for grouped `/ns:flow:*` lifecycle mirrors over the `ns flow` CLI family: changes, cp, autobranch, branch-latest-commit, autoslot, submit, generate-pr-inventory, push, land, pull-trunk, and squash-stack.                                                                                                                                                                                                                                                                                                        |
| `.pi/extensions/handoff.ts`                                                                  | Project-local adapter over engineered behavior      | Adds `/ns:handoff:create`, `/ns:handoff:pickup`, and `/ns:handoff:list` for directed Handoff Artifacts, including a custom list renderer.                                                                                                                                                                                                                                                                                                                                                                                                                        |
...
| `.pi/extensions/herdr.ts`                                                                    | Project-local adapter over engineered behavior      | Registers the exact Herdr catalog from `@nseng-ai/herdr/pi`, including resource operations, three `/ns:herdr:impl:*` workflows, and conditional `/ns:herdr:tab:handoff`.                                                                                                                                                                                                                                                                                                                                                                                         |
...
| `.pi/extensions/objective-autorun.ts`                                                        | Vibecoded implementation                            | Registers only the `objective_runner_step` tool (mechanically wraps one runner step: runner-begin → implementation subagent with live widget → runner-finish, returning the Runner Checkpoint). Implementation children inherit parent routing by default; the only explicit intent is approved same-provider `cheap`, chosen before dispatch, with no failure rerouting. The canonical `/ns:objective:autorun` command lives in `ts/packages/incubating/extensions/objectives/src/pi/extension.ts`. Tool promotion path stated in-file; not yet package-tested. |
...
| `ts/packages/incubating/extensions/objectives/src/pi/extension.ts`                           | Engineered migration source                         | Current package-tested Objective Pi behavior awaiting extraction to `@nseng-ai/pi-ns-objectives`. The destination host package owns `/ns:objective:*` presentation and orchestration and consumes only `@nseng-ai/objectives/api` plus neutral Pi runtime interfaces.                                                                                                                                                                                                                                                                                            |
...
| `ts/packages/incubating/extensions/flow/src/pi/ns-extension.ts`                              | Engineered implementation                           | Mirrors native ns flow commands into Pi as `/ns:flow:*` commands and intentionally does not register flat `/ns:*` or `/ns:code:*` lifecycle aliases.                                                                                                                                                                                                                                                                                                                                                                                                             |
...
| `ts/packages/incubating/extensions/handoffs/src/pi/extension.ts` | Engineered implementation | Package-tested `/ns:handoff:create`, `/ns:handoff:pickup`, and `/ns:handoff:list` presentation adapters over the portable `ns handoff ...` command face and Handoff extension package API. |
| `ts/packages/incubating/extensions/branch-context/src/pi/extension.ts` | Engineered implementation | Branch-context Pi command/tool wiring over the `@nseng-ai/branch-context` package. |
...
| `ts/packages/incubating/extensions/herdr/src/pi/extension.ts` and `core/*` | Engineered implementation | Private Herdr resource command suite, explicit caller space/tab targeting, behavior-preserving implementation workflows, Prepared Herdr Launch composition, and optional Handoff Artifact tab launch. |
...
- `/ns:herdr:space:*` and `/ns:herdr:tab:*` — direct Herdr resource operations. Implementation uses `/ns:herdr:impl:prompt:space`, `/ns:herdr:impl:plan:space`, and `/ns:herdr:impl:plan:tab`; each selects current branch or existing local trunk contextually without fetching or refreshing trunk, then implements a prompt or Saved Plan with the existing agent instructions and workflow behavior. Handoff launch remains the optional `/ns:herdr:tab:handoff` integration.
...
| Herdr resource and implementation suite (`/ns:herdr:space:*`, `/ns:herdr:tab:*`, `/ns:herdr:impl:*`)                                                                                       | Engineered Herdr extension adapters expose the exact nine-command catalog. Space and tab identity are explicit; implementation commands preserve existing behavior while naming the prompt or Saved Plan outcome accurately; `/ns:herdr:tab:handoff` is optional and is the only Herdr command that creates a Handoff Artifact. See [`../herdr/command-catalog.md`](../herdr/command-catalog.md) for the exact catalog and migration history.                                                                                                                                                                                                                                                                                      |
...
Herdr direct operations are organized by destination resource: `/ns:herdr:space:*` for spaces and `/ns:herdr:tab:*` for tabs. Implementation workflows are `/ns:herdr:impl:prompt:space`, `/ns:herdr:impl:plan:space`, and `/ns:herdr:impl:plan:tab`. They select current branch or existing local trunk contextually and never update trunk; no branch-basis flags or aliases are registered. `impl` is shorter, avoids collision with remote-system dispatch terminology, and describes implementing a prompt or Saved Plan more accurately than `launch`. The rename preserves existing agent instructions and workflow behavior.

Launch vocabulary remains valid for the mechanics below that contract. Prepared Herdr Launch owns destination startup, Pi launch mechanics start the process, and `ns-impl` identifies prompt transport/storage for implementation workflows. Handoff launch remains the distinct `/ns:herdr:tab:handoff` flow over a durable artifact. Use `HERDR_WORKSPACE_ID` for explicit caller-space targeting and `HERDR_TAB_ID` when mutating the exact caller tab; never fall back to UI focus. The implementation lives in `ts/packages/incubating/extensions/herdr/`, while Pi contributes only neutral host helpers and registration/presentation infrastructure.
...
- **Executing one:** the supported runner is `/ns:objective:autorun [<slug>] [scope / step budget / standing guidance]`; without a slug it opens the same active-Objective picker logic as `/ns:objective:next`, and with a slug it expands the enhanced `objective-autorun` skill so the parent session agent owns the loop and every between-step judgment. Each mechanical step (runner-begin → implementation subagent → runner-finish) runs through the local-only `objective_runner_step` tool and returns a Runner Checkpoint to judge. Current Objective-package and project-local Pi sources are migration state; ADR 0049 assigns the command, skill expansion, and orchestration to `@nseng-ai/pi-ns-objectives`, while `@nseng-ai/objectives` retains deterministic begin/finish and ADR 0037 publication safeguards. Parent publication is off by default and remains a separately authorized post-checkpoint action rather than tool behavior.
