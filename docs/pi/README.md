...
The engineered layer is for durable behavior that benefits from tests, fake adapters, shared modules, or package-level validation. Project-local discovery adapters can stay in `.pi/extensions/` while the implementation lives in `@ns/pi`, a private capability-pi package such as `@ns/flow/pi`, or a private Internal Pi-tool package.
...
| `.pi/extensions/code.ts`                                       | Project-local adapter over engineered behavior      | Delegates to `@ns/flow/pi/code-extension` for `/code:gt-restack-resolve` and `/gt:squash-stack`. Review feedback commands live elsewhere.                                                                                                                                                                                                                                       |
| `.pi/extensions/ns.ts`                                         | Project-local adapter over engineered behavior      | Delegates to `@ns/flow/pi/ji-extension` for grouped `/ns:flow:*` lifecycle mirrors over the `ns flow` CLI family: changes, cp, autobranch, branch-latest-commit, autoslot, submit, regenerate-pr, push, land, and pull-trunk.                                                                                                                                                   |
| `.pi/extensions/handoff.ts`                                    | Project-local adapter over engineered behavior      | Adds `/ns:handoff:create`, `/ns:handoff:pickup`, and `/handoff:list` for directed handoff artifacts, including a custom list renderer; also registers `/ccc:handoff-tab` when tool support is available.                                                                                                                                                                        |
...
| `.pi/extensions/dispatch-runner-subagent.ts`                   | Project-local adapter over Internal Pi-tool package | Registers the `dispatch_runner_subagent` final-text tool for agents through `@internal/pi-tools/runner-subagents`.                                                                                                                                                                                                                                                              |
| `.pi/extensions/grill-ui.ts`                                   | Project-local adapter over Internal Pi-tool package | Adds `/pi:grill-me`, `/pi:grill-with-docs`, and the `grill_ask` structured question tool for Pi grill sessions through `@internal/pi-tools/grill`.                                                                                                                                                                                                                              |
...
| `.pi/extensions/objective-autorun.ts`                          | Vibecoded implementation                            | Registers `/objective:autorun` (expands the repo `objective-autorun` skill and hands the parent-judgment loop to the session agent) and the `objective_runner_step` tool (mechanically wraps one runner step: runner-begin → implementation subagent with live widget → runner-finish, returning the Runner Checkpoint). Promotion path stated in-file; not yet package-tested. |
...
| `ts/packages/capabilities/objective/src/pi/extension.ts`       | Engineered implementation                           | Package-tested Objective extension behavior, including the unified `/ns:objective:next` front door.                                                                                                                                                                                                                                                                             |
...
| `ts/packages/capabilities/flow/src/pi/ji-extension.ts`         | Engineered implementation                           | Mirrors native ns flow commands into Pi as `/ns:flow:*` commands and intentionally does not register flat `/ns:*` or `/ns:code:*` lifecycle aliases.                                                                                                                                                                                                                            |
...
| `ts/packages/capabilities/flow/src/pi/code-extension.ts`       | Engineered implementation                           | Aggregates Flow/Code workflow Pi presentation for `/code:gt-restack-resolve` and `/gt:squash-stack` in `@ns/flow/pi`; `@ns/pi` does not import the extracted package.                                                                                                                                                                                                           |
| `ts/packages/internal/pi-tools/src/pr-previews/extension.ts`   | Internal Pi-tool package                            | Registers read-only `/pr:preview-feedback` and `/pr:preview-checks` modal previews; download/editor-prefill and feedback watch behavior remain in `@ns/pi`.                                                                                                                                                                                                                     |
...
| `ts/packages/capabilities/handoff/src/pi/extension.ts` | Engineered implementation | Package-tested `/ns:handoff:create`, `/ns:handoff:pickup`, and `/handoff:list` presentation adapters over the portable `ns handoff ...` command face and Handoff Capability API. |
| `ts/packages/capabilities/branch-context/src/pi/extension.ts` | Engineered implementation | Branch-context Pi command/tool wiring over the `@ns/branch-context` package. |
...
| `ns flow submit` | ji project-local extension command | SDK-only project-local Graphite submit command mirrored into Pi as `/ns:flow:submit`; no flat or legacy submit aliases are retained. |
| `ts/packages/internal/pi-tools/src/runner-subagents/extension-api.ts` and submodules | Internal Pi-tool package | Runner-subagent subprocess, JSON-event parsing, generated runtime extension, terminal capture, and final-text results. |
...
- `/ns:handoff:*` plus `/handoff:list` — durable Handoff artifact lifecycle: create, pick up, and list. Session launchers built on handoffs belong in the orchestration namespace, not the artifact lifecycle namespace.
...
2. Resolve the duplicate Objective stack implementation surface by using the namespaced Pi wrapper `/ns:objective:stack-impl` and the portable skill `/skill:objective-stack-impl`. Completed.
...
6. Consolidate the local codebase/source-control commands into grouped ns flow surfaces without legacy aliases. Fresh RPC inventory should report `/code:pr-feedback-watch` as the remaining non-ji review watch surface and `/ns:flow:changes`, `/ns:flow:cp`, `/ns:flow:autobranch`, `/ns:flow:branch-latest-commit`, `/ns:flow:autoslot`, `/ns:flow:submit`, `/ns:flow:regenerate-pr`, `/ns:flow:push`, `/ns:flow:land`, and `/ns:flow:pull-trunk` as the ji lifecycle mirrors, with no legacy `/code:*`, flat lifecycle `/ns:*`, or `/ns:code:*` aliases.
7. Categorize the remaining repo-owned workflow families. Completed; branch-context planning now uses the `/ns:plan:*` and `/ns:branch-context:*` Pi command families plus public installed agent skills, handoff artifacts use final `/ns:handoff:*` plus `/handoff:list` command names and `handoff-*` skills, and branch retrospective remains intentionally skill/CLI-centered around `/skill:branch-retro` plus `ns aretro exec collect-evidence`.
...
| `/ns:objective:stack-impl` plus `/skill:objective-stack-impl`                                                                                                                                                                                               | Resolved Objective stack implementation surface: Pi uses the namespaced picker wrapper; Codex/Claude use the portable skill; no public prompt-template duplicate remains.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
...
| `/ns:flow:regenerate-pr`                                                                                                                                                                                                                                    | Primary mirrored `ns flow regenerate-pr` command: regenerates the current branch PR title and ji-managed generated description region, preserves human body text outside that region, and asks before `gh pr edit`. Legacy `/ns:regenerate-pr`, `/ns:code:regenerate-pr`, `/code:pr-regen`, and `/ns:pr-regen` aliases are not registered.                                                                                                                                                                                                                                                                                                                                                                             |
...
| `/ns:flow:push`                                                                                                                                                                                                                                             | Primary mirrored `ns flow push` command: requires a clean worktree, runs plain `git push`, and renders generic ji CLI output. It does not update Graphite metadata and is not a replacement for `/ns:flow:submit`; do not use it on Graphite-tracked PR branches or anywhere Graphite submission/PR metadata updates are desired. Legacy `/ns:push`, `/ns:code:push`, and `/code:push` aliases are not registered.                                                                                                                                                                                                                                                                                                     |
...
| Branch context workflow (`/ns:plan:save`, `/ns:plan:grill-and-save`, `/ns:branch-context:from-plan`, `/ns:branch-context:upstack-impl-from-plan`, `/ns:branch-context:impl-attached-plan`)                                                                  | Final public Pi planning-layer command sequence. Portable core: the `@ns/plans` package and `enriched-plan` bin for Saved plan authoring/inspection/resolution, the `@ns/branch-context` package and `ns branch-context exec` command surface for branch workflow, XDG local store `$XDG_STATE_HOME/ns/enriched-plan/...` (default `$HOME/.local/state/ns/enriched-plan/...`), and Branch Memory namespace `branch-context` with named Markdown attached-plan keys such as `<slug>.md`. Installed agent skills use `/skill:enriched-plan-save`, `/skill:branch-context-from-plan`, and `/skill:branch-context-impl` over the same CLI contract; the grilled interaction itself is Pi-only structured UI orchestration. |
| Handoff artifact workflow (`/ns:handoff:create`, `/ns:handoff:pickup`, `/handoff:list`)                                                                                                                                                                     | Final project-local Pi handoff lifecycle surface. Users create, pick up, list, and review directed handoff artifacts without Branch Memory vocabulary; pickup presents a summary before further action. Storage details are technical locators only. The portable command face is `ns handoff ...`; no Pi delete command yet. No old `brmem` aliases are retained. Focused tab launch is `/ccc:handoff-tab`, not part of the artifact lifecycle namespace.                                                                                                                                                                                                                                                             |
| Branch retrospectives (`/skill:branch-retro`, `ns aretro exec collect-evidence`)                                                                                                                                                                            | Intentionally skill/CLI-centered and retained. The user-facing capability remains a branch/session retrospective, so the skill stays `branch-retro`; `ns aretro exec collect-evidence` is the deterministic evidence-collection boundary used by the skill, not a Pi slash command. Codex/Claude use the same installed skill surface.                                                                                                                                                                                                                                                                                                                                                                                 |
...
Durable headless developer commands should live in a native CLI first where practical, then be mirrored into Pi under the domain namespace chosen by the project. ji lifecycle mirrors now use the grouped flow family: `/ns:flow:changes`, `/ns:flow:cp`, `/ns:flow:autobranch`, `/ns:flow:branch-latest-commit`, `/ns:flow:autoslot`, `/ns:flow:submit`, `/ns:flow:regenerate-pr`, `/ns:flow:push`, `/ns:flow:land`, and `/ns:flow:pull-trunk`.

ji extension discovery is currently CLI-only. Pi does not dynamically register arbitrary `/ns:<name>` commands from `.ns/extensions` at runtime; exact Pi mirrors are static engineered adapters over selected ji commands and need package tests/parity metadata when added or renamed. Current Flow mirrors are owned by `@ns/flow/pi`, whose `.pi/extensions/ns.ts` discovery adapter imports the package through its exports. Treat dynamic Pi mirror discovery as future ji/Pi design work, not an implicit property of project-local ji extensions.

## CCC workspace/sidebar pattern

Manual CCC sidebar commands should target the caller cmux workspace for sidebar updates, keep cmux mutations behind deterministic `ccc exec` commands, and avoid relying on stale local cmux source checkouts. Workspace-opening commands currently do not auto-run sidebar updates. The project command suite lives behind `.pi/extensions/ccc.ts` and `ts/packages/capabilities/ccc/src/cmux/`; `.pi/extensions/worktree-status.ts` remains the project-local adapter for the automatic worktree-status renderer backed by `@ns/pi`, which delegates operational facts and presentation to `@ns/ccc/worktree-status`.
...
- **Executing one:** the supported runner is the `/objective:autorun <slug> [scope / step budget / standing guidance]` Pi command (`.pi/extensions/objective-autorun.ts`) — it expands the repo `objective-autorun` skill so the parent session agent owns the loop and every between-step judgment, while each mechanical step (runner-begin → implementation subagent → runner-finish) runs through the `objective_runner_step` tool and returns a Runner Checkpoint to judge. The run ends with local stacked branches handed back to the normal Graphite/flow workflow; it never submits or lands.
