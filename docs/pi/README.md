...
| `/ns:gs:autobranch`                                                                                                                                                                        | Native mirror of `ns gs autobranch`: captures the exact `ns-gs-autobranch` skill, runs the fresh authorized JSON CLI, returns on completion, reports refusal, and hands only known-partial or ambiguous forward recovery to the skill. The duplicate Flow registration is retired.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
...
Durable headless developer commands should live in a native CLI first where practical, then be mirrored into Pi under the domain namespace chosen by the project. ns lifecycle mirrors now use the grouped flow family: `/ns:flow:changes`, `/ns:flow:cp`, `/ns:flow:gt:autobranch`, `/ns:flow:gt:branch-latest-commit`, `/ns:flow:gt:autoslot`, `/ns:flow:gt:submit`, `/ns:flow:generate-pr-inventory`, `/ns:flow:push`, `/ns:flow:gt:land`, `/ns:flow:pull-trunk`, and `/ns:flow:gt:squash-stack`. `/ns:gs:autobranch` is a native GS CLI mirror. `/ns:flow:gs:autoslot` remains the sole provisional GS Skill-Backed Command exception while Slot destination provider behavior is learned.
