...
| `.pi/extensions/code.ts`                                                  | Project-local adapter over engineered behavior | Adds `/code:changes`, `/code:autobranch`, `/code:land`, and `/code:land-stack` local code/source-control commands.                                   |
...
| `ts/packages/pi-extensions/src/code.ts`                                   | Engineered implementation                      | Aggregates the package-tested Pi-only `/code:*` local code/source-control command family for the project adapter.                                    |
...
6. Consolidate the local codebase/source-control commands under `/code:*` without legacy aliases. Completed; fresh RPC inventory reports `/code:changes`, `/code:autobranch`, `/code:land`, and `/code:land-stack` for the code extension, plus the `asdl-dev` mirrors `/dev:preview-url`, `/dev:cp`, and `/dev:submit`, with no legacy `/cp`, `/newbr`, `/submit`, `/gh:land`, or `/gt:land-stack` aliases. The `/dev:*` mirror commands are sourced from `.pi/extensions/asdl-dev.ts` through the `asdl-dev` command table.
