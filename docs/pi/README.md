...
| `.pi/extensions/grill-ui.ts`                                   | Project-local adapter over Internal Pi-tool package | Adds `/pi:grill-me`, `/pi:grill-with-docs`, and the `grill_ask` structured question tool through `@internal/pi-tools/grill`, with structured choice, freeform, status, and end-session paths.                                                                                                                                                                                                                                                                                                                                                           |
...
- Repo-owned first-party product/orchestration Pi extension command families should use `/ns:<extension>:<action...>` names when introduced or renamed, such as `/ns:objective:list`, `/ns:objective:next`, or `/ns:herdr:space:plan`. Reserve `/skill:<name>` for Pi's skill-command namespace, and keep `/pi:*` for Pi-native UI/session affordances.
...
| Herdr resource command suite (`/ns:herdr:space:*`, `/ns:herdr:tab:*`)                                                                                                                      | Engineered Herdr capability adapters expose the exact eleven-command resource-first catalog. Space and tab identity are explicit; `/ns:herdr:tab:handoff` is optional and is the only Herdr command that creates a Handoff Artifact. See [`../herdr/cmux-parity-checklist.md`](../herdr/cmux-parity-checklist.md) for the exact catalog and migration history.                                                                                                                                                                                                                                                                                                                                                                     |
