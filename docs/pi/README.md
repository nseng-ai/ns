...
| `ts/packages/capabilities/herdr/src/pi/extension.ts` and `core/*` | Engineered implementation | Private Herdr resource command suite, explicit caller space/tab targeting, behavior-preserving implementation workflows, Prepared Herdr Launch composition, and optional Handoff Artifact tab launch. |
