...
| `.pi/extensions/planned-branch.ts`                                        | Project-local adapter over engineered behavior | Adds `/plans:write`, `/plans:grill-and-write`, `/planned-branch:create`, `/planned-branch:up-and-impl`, and `/planned-branch:impl`; this repo defaults `/planned-branch:create` to Graphite, while `up-and-impl` is Graphite by built-in default. |
...
| Planned branch workflow (`/plans:write`, `/plans:grill-and-write`, `/planned-branch:create`, `/planned-branch:up-and-impl`, `/planned-branch:impl`)                                               | Final public Pi planning-layer command sequence. Portable core: the `@asdl/plans` package and `plans` bin for Saved plan authoring/inspection/resolution, the `@asdl/planned-branch` package and `planned-branch` bin for branch workflow, local store `~/.asdl/planned-branch/plans/...`, and Branch Memory namespace `planned-branch`. Installed agent skills use `/skill:plans-write`, `/skill:planned-branch-create`, and `/skill:planned-branch-impl` over the same CLI contract; the grilled interaction itself is Pi-only structured UI orchestration. |
...
The planned-branch workflow uses `/plans:write`, `/plans:grill-and-write`, `/planned-branch:create`, `/planned-branch:up-and-impl`, and `/planned-branch:impl` in Pi, plus the `plans-write`, `planned-branch-create`, and `planned-branch-impl` skills for other agents, to save reviewed plans, create implementation branches, attach plans through Branch Memory, and load them for implementation. Saved-plan TypeScript APIs now live in `@asdl/plans`; `@asdl/planned-branch` owns branch creation, Branch Memory attachment, and implementation loading. The static `/plans:write` prompt body is repo-editable at `.asdl/prompts/plans-write.md` and resolved with `asdl exec resolve-prompt plans-write --format json`; `/plans:grill-and-write` is Pi-only structured UI orchestration over the same Saved plan artifact. `/planned-branch:up-and-impl` is the Pi-only convenience flow for Graphite-stacked branch creation on the current branch by default, exact `git checkout <branch>`, starting a fresh Pi session, and implementation kickoff.
