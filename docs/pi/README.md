...
6. Consolidate the local codebase/source-control commands into SDL code-lifecycle surfaces without legacy aliases. Completed for the currently restored command set; fresh RPC inventory should report `/code:pr-feedback-watch` as the remaining non-SDL review watch surface, primary `/sdl:changes` and `/sdl:cp` SDL mirrors, nested `/sdl:code:changes`, `/sdl:code:autobranch`, `/sdl:code:autoslot`, `/sdl:code:land`, `/sdl:code:push`, and `/sdl:code:pull-trunk`, with no legacy `/code:changes`, `/code:cp`, `/code:checkpoint`, `/code:autobranch`, `/code:autoslot`, `/code:land`, `/code:push`, `/code:pr-regen`, `/dev:cp`, `/dev:submit`, `/dev:preview-url`, `/cp`, `/newbr`, `/submit`, `/gh:land`, `/land-stack`, or `/gt:land-stack` aliases. Submit and PR-regeneration mirrors remain parked until their SDL command entries migrate back.
...
| `/sdl:submit`                                                                                                                                                                                                                                               | Parked until the `sdl submit` project-local extension migrates back. Legacy `/dev:submit` and `/submit` aliases are not registered.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
