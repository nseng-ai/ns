...
| `@nseng-ai/pi-ns-branch-context`                                                             | Engineered package extension                        | Owns provider-independent Saved Plan and Attached Plan Pi presentation, including `/ns:plan:save`, `/ns:plan:grill-and-save`, `/ns:plan:impl-saved-plan`, `/ns:branch-context:impl-attached-plan`, and the plain-Git `/ns:git:*` branch-from-plan pair. It does not own or default to Graphite.                                                                                                                                                   |
| `@nseng-ai/pi-ns-gt`                                                                         | Engineered package extension                        | Owns the Graphite `/ns:gt:branch-from-plan` and `/ns:gt:branch-and-impl-from-plan` pair. Provider choice is encoded by the namespace; provider flags are unsupported.                                                                                                                                                                                                                                                                             |
| `@nseng-ai/pi-ns-gs`                                                                         | Engineered package extension                        | Owns the GitHub Stacks `/ns:gs:branch-from-plan` and `/ns:gs:branch-and-impl-from-plan` pair over local `gh stack` topology. It does not claim reconciliation or publication support.                                                                                                                                                                                                                                                             |
...
| Branch context workflow (`/ns:plan:*`, Git/GT/GS `branch-from-plan` and `branch-and-impl-from-plan`, `/ns:branch-context:impl-attached-plan`) | Final public Pi planning-layer command sequence. The `git`, `gt`, and `gs` namespaces explicitly select plain Git, Graphite, and GitHub Stacks; provider flags are unsupported. Every `branch-from-plan` retains or restores the original branch, while every `branch-and-impl-from-plan` leaves the target checked out and launches provider-independent attached-plan implementation in a fresh session. Portable core: the `@nseng-ai/plans` package and `enriched-plan` bin, the `@nseng-ai/branch-context` package and `ns branch-context exec`, the XDG Local Plan Store, and Branch Memory namespace `branch-context`. This is branch-creation parity only; it does not imply reconciliation, submit, land, or publication parity.                                                                              |
...
The branch-context workflow uses `/ns:plan:save`, `/ns:plan:grill-and-save`, the Git/GT/GS `branch-from-plan` and `branch-and-impl-from-plan` families, and `/ns:branch-context:impl-attached-plan` in Pi. The `git`, `gt`, and `gs` namespaces select plain Git, Graphite, and GitHub Stacks without provider flags. `branch-from-plan` retains or restores the original branch; `branch-and-impl-from-plan` leaves the target checked out and starts provider-independent Attached Plan implementation in a fresh Pi session. The `branch-context-from-plan` and `branch-context-impl` skills provide the portable agent workflow. Saved-plan TypeScript APIs live in `@nseng-ai/plans`; `@nseng-ai/branch-context` owns preparation, attachment, reuse, and implementation loading. `@nseng-ai/pi-ns-branch-context` owns shared/ Git Pi presentation, while `@nseng-ai/pi-ns-gt` and `@nseng-ai/pi-ns-gs` own their provider namespaces. GitHub Stacks support here is limited to branch creation and local topology inspection; it does not imply reconciliation, submit, land, or publication parity.
