...
| `@nseng-ai/pi-ns-branch-context`                                                             | Engineered package extension                        | Adds `/ns:plan:save`, `/ns:plan:grill-and-save`, `/ns:branch-context:from-plan`, `/ns:branch-context:upstack-impl-from-plan`, and `/ns:branch-context:impl-attached-plan`; its package entrypoint defaults `/ns:branch-context:from-plan` to Graphite. The write tool accepts the current plain-save kickoff, while grilled saves require confirmed five-round-policy evidence.                                                                                                                               |
...
| `.pi/extensions/grill-ui.ts`                                                                 | Project-local adapter over Internal Pi-tool package | Adds `/pi:grill-me`, `/pi:grill-with-docs`, and only the model-visible `grill_ask_round` tool through `@internal/pi-tools/grill`. General commands start unlimited, versioned attempts with fresh ID namespaces; each call owns a complete ordered decision frontier or final confirmation. The tool is inactive at session start and each general command activates it before send. `/ns:plan:grill-and-save` activates the same tool with a five-submitted-round policy and enforced confirmation evidence. |
...
| `/pi:grill-me`, `/pi:grill-with-docs`, plus `grill_ask_round`                                                                                                                              | Pi-only atomic-round grill UI. Both commands activate only `grill_ask_round` before send and emit a versioned general/unlimited kickoff with a fresh attempt ID. Decision rounds contain the whole ordered current design-tree frontier with stable IDs, recommendations, rationales, 2–5 choices, and freeform; between-round status precedes recomputation. An explicit summary then offers only Confirm shared understanding or Return to grilling. Cancel discards the current draft and pauses general grilling; End terminates it. UI failure and missing confirmation fail closed, and downstream action is prohibited until confirmation. Saved Plan grilling uses the same atomic round tool with a five-submitted-round cap, and saving requires explicit confirmation evidence from the current Saved Plan attempt. |
