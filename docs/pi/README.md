...
| `.pi/extensions/brmem-handoff.ts`                                 | Project-local adapter over engineered behavior | Adds the current `/brmem-handoff` and `/brmem-pickup-handoff` commands while the public handoff UX moves toward save/load vocabulary. |
...
| `ts/packages/pi-extensions/src/brmem-handoff.ts`                  | Engineered implementation                      | Package-tested handoff save/load command selection and prompt behavior over the current Branch Memory storage contract.               |
...
4. Re-run Pi RPC command inventory after material changes and record the final surface as closure evidence. Completed for the landing and `/dev:*` migration slices.
...
| Handoff/pickup workflow (`/brmem-handoff`, `/brmem-pickup-handoff` today)                 | Carved out to follow-up Objective `directed-handoff-artifacts`. The vocabulary is settled around directed handoff artifacts that users save, load, list, and resume from without thinking in Branch Memory terms; the current `brmem`-named commands remain the present implementation until that Objective replaces, deprecates, or explicitly retains them.                                                             |
...
A handoff is a directed, saved, loadable work-context artifact for a specific future continuation. Use save/load/list/resume language in normal user-facing copy; Branch Memory is the current technical storage layer, not the public model.
