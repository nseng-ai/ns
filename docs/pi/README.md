...
| `.pi/extensions/cmux.ts`                                                  | Project-local adapter over engineered behavior | Registers `/cmux:set-workspace-summary`, `/cmux-slot:dispatch-plan`, `/cmux-slot:open-branch`, and `/cmux-dispatch` through the package cmux suite.   |
...
| `ts/packages/pi-extensions/src/cmux.ts` and `cmux/*`                      | Engineered implementation                      | Project cmux command suite, caller-workspace summary controller, slot helpers, and workspace-opening summary hooks.                                   |
...
| Project cmux command suite (`/cmux:set-workspace-summary`, `/cmux-slot:dispatch-plan`, `/cmux-slot:open-branch`, `/cmux-dispatch`) | Promoted to `.pi/extensions/cmux.ts` plus `ts/packages/pi-extensions/src/cmux/`. Workspace-opening commands queue a caller-workspace summary hook after success; `/cmux-refresh-meta` was intentionally removed rather than migrated.                                                                                                                                                                                     |
