...
| `.pi/extensions/sdl.ts`                                                   | Project-local adapter over engineered behavior | Adds primary native `/sdl:changes`, `/sdl:cp`, and `/sdl:submit` commands over the `sdl` CLI, plus nested `/sdl:code:changes`, `/sdl:code:checkpoint`, and `/sdl:code:submit` aliases for the code-lifecycle taxonomy.                                                                  |
...
| `ts/packages/pi-extensions/src/sdl-extension.ts`                          | Engineered implementation                      | Mirrors native SDL commands into Pi as primary `/sdl:changes`, `/sdl:cp`, and `/sdl:submit` commands, plus nested `/sdl:code:changes`, `/sdl:code:checkpoint`, and `/sdl:code:submit` aliases.                                                                                          |
...
6. Consolidate the local codebase/source-control commands under `/code:*` without legacy aliases. Completed; after the nested SDL code-lifecycle alias slice, fresh RPC inventory should report `/code:push`, `/code:autobranch`, and `/code:land` for the code extension, primary `/sdl:changes`, `/sdl:cp`, and `/sdl:submit` SDL mirrors, nested `/sdl:code:changes`, `/sdl:code:checkpoint`, and `/sdl:code:submit` SDL code-lifecycle aliases, plus the non-code `asdl-dev` mirror `/dev:preview-url`, with no legacy `/code:changes`, `/code:cp`, `/code:checkpoint`, `/dev:cp`, `/dev:submit`, `/cp`, `/newbr`, `/submit`, `/gh:land`, `/land-stack`, or `/gt:land-stack` aliases. Changes, checkpoint, and submit behavior are now sourced from `sdl`; `pr-regen` remains sourced from the `asdl-dev` command table.
