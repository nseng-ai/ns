...
| `.pi/extensions/code.ts`                                          | Project-local adapter over engineered behavior | Adds `/code:pr-feedback-watch` only; migrated code-lifecycle commands have moved to SDL mirrors without `/code:*` compatibility aliases.                                                                                                                                                                        |
| `.pi/extensions/sdl.ts`                                           | Project-local adapter over engineered behavior | Adds primary native `/sdl:changes`, `/sdl:cp`, `/sdl:autobranch`, `/sdl:submit`, `/sdl:regenerate-pr`, and `/sdl:push` commands over the `sdl` CLI, plus nested `/sdl:code:*` code-lifecycle surfaces for changes, autoslot, land, and trunk pull.                                                              |
...
| `ts/packages/pi-extensions/src/sdl-extension.ts`                  | Engineered implementation                      | Mirrors native SDL commands into Pi as primary `/sdl:changes`, `/sdl:cp`, `/sdl:autobranch`, `/sdl:submit`, `/sdl:regenerate-pr`, and `/sdl:push` commands, plus the currently restored nested `/sdl:code:*` code-lifecycle commands.                                                                           |
...
| `ts/packages/pi-extensions/src/code.ts`                           | Engineered implementation                      | Aggregates the remaining `/code:*` review-feedback watch surface for the project adapter; migrated code-lifecycle commands live under SDL mirrors.                                                                                                                                                              |
| `ts/packages/pi-extensions/src/land.ts`                           | Engineered implementation                      | Package-tested adapter for `/sdl:code:land`, delegating Graphite-proven single-PR and stack landing behavior to CCC.                                                                                                                                                                                            |
| `ts/packages/ccc/src/land-stack/*`                                | Engineered implementation                      | Internal Graphite stack landing engine used by `/sdl:code:land`; internals remain split under `land-stack/`.                                                                                                                                                                                                    |
...
| `ts/packages/pi-extensions/src/branch-context-extension.ts`       | Engineered implementation                      | Branch-context Pi command/tool wiring over the `@sdl/branch-context` package.                                                                                                                                                                                                                                   |
| `ts/packages/ccc/src/autobranch/checkpoint.ts` / `autobranch*.ts` | Engineered implementation                      | Hidden `ccc exec autobranch` compatibility flow plus workflow evidence over pending-worktree, preparation, latest-commit extraction, and transaction helpers; public autobranch is `sdl autobranch` / `/sdl:autobranch`.                                                                                        |
| `sdl submit`                                                      | SDL project-local extension command            | SDK-only project-local Graphite submit command mirrored into Pi as `/sdl:submit`; no `/sdl:code:submit` or legacy submit aliases are retained.                                                                                                                                                                  |
...
- Repo-owned Pi slash commands must show immediate acknowledgement that the command was received, before waiting for Pi idle state or doing slow I/O. Engineered commands should use `withImmediateCommandAck` from `@sdl/pi-extension-runtime/command-ack` at the registration boundary. Default acknowledgement delivery is an above-fold dim transcript message when the host supports rendered custom messages; it falls back to a transient status line for minimal hosts. Use `{ delivery: "status" }` only for commands whose acknowledgement must stay out of the transcript.
- `sendCommandProgressOrNotify({ host, ctx, message, level, shouldNotifyWhenNoUi })` follows the active `withImmediateCommandAck` `progressDelivery` option: explicit `status` stays out of the transcript, `message` uses above-fold progress, `both` emits both, and `none` suppresses helper progress. For nested wrappers, sub-registrars may override `progressDelivery` for the commands they register; acknowledgement delivery remains conservative and message-capable/default acknowledgement is not suppressed by a later nested `delivery: "status"` wrapper.
...
3. Resolve the single-PR and stack landing surfaces without legacy top-level aliases; landing is now exposed as `/sdl:code:land` without old landing aliases. Completed.
...
6. Consolidate the local codebase/source-control commands into SDL code-lifecycle surfaces without legacy aliases. Completed for the currently restored command set; fresh RPC inventory should report `/code:pr-feedback-watch` as the remaining non-SDL review watch surface, primary `/sdl:changes`, `/sdl:cp`, `/sdl:autobranch`, `/sdl:submit`, `/sdl:regenerate-pr`, and `/sdl:push` SDL mirrors, nested `/sdl:code:changes`, `/sdl:code:autoslot`, `/sdl:code:land`, and `/sdl:code:pull-trunk`, with no legacy `/code:changes`, `/code:cp`, `/code:checkpoint`, `/code:autobranch`, `/code:autoslot`, `/code:land`, `/code:push`, `/code:pr-regen`, `/sdl:code:autobranch`, `/sdl:code:push`, `/sdl:code:regenerate-pr`, `/sdl:pr-regen`, `/dev:cp`, `/dev:submit`, `/dev:preview-url`, `/cp`, `/newbr`, `/submit`, `/gh:land`, `/land-stack`, or `/gt:land-stack` aliases.
...
| `/sdl:changes`                                                                                                                                                                                                                                              | Primary mirrored `sdl changes` command: presents the current pending worktree snapshot without staging, committing, stashing, or switching branches. Legacy `/code:changes` is not registered.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| `/sdl:cp`                                                                                                                                                                                                                                                   | Primary mirrored `sdl cp` command: creates a checkpoint commit for the current diff with model-authored message generation. Legacy `/code:cp`, `/code:checkpoint`, `/dev:cp`, and `/cp` aliases are not registered.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| `/sdl:code:changes`                                                                                                                                                                                                                                         | Nested code-lifecycle SDL alias over `sdl changes`. Checkpoint and autobranch do not have nested code-lifecycle aliases in the current project-local extension slice; use primary `/sdl:cp` and `/sdl:autobranch`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| `/sdl:autobranch`                                                                                                                                                                                                                                           | Primary mirrored `sdl autobranch` command from the project-local SDK-only SDL extension: creates a Graphite branch from current uncommitted changes, or when the worktree is clean extracts the latest unpushed single-parent commit into a new Graphite branch while preserving its SHA. It refuses trunk, pushed latest commits, root/merge commits, and Graphite child branches. Legacy `/sdl:code:autobranch`, `/code:autobranch`, and `/newbr` are not registered.                                                                                                                                                                                                                                |
| `/sdl:submit`                                                                                                                                                                                                                                               | Primary mirrored `sdl submit` command: checkpoints pending work, preflights/restacks/submits the current Graphite stack, verifies the submitted PR, and updates managed PR descriptions. `/sdl:code:submit`, `/dev:submit`, and `/submit` aliases are not registered.                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| `/sdl:regenerate-pr`                                                                                                                                                                                                                                        | Primary mirrored `sdl regenerate-pr` command: regenerates the current branch PR title and SDL-managed generated description region, preserves human body text outside that region, and asks before `gh pr edit`. Legacy `/code:pr-regen`, `/sdl:pr-regen`, and nested `/sdl:code:regenerate-pr` aliases are not registered.                                                                                                                                                                                                                                                                                                                                                                            |
| `/sdl:code:autoslot`                                                                                                                                                                                                                                        | Autoslot command: creates a Graphite branch from current work, then moves it into a managed slot worktree when the post-branch worktree is clean. Legacy `/code:autoslot` is not registered.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| `/sdl:push`                                                                                                                                                                                                                                                 | Primary mirrored `sdl push` command: requires a clean worktree, runs plain `git push`, and renders generic SDL CLI output. It is not a replacement for `/sdl:submit`; use it only for already-committed branch pushes where Graphite submission/PR metadata updates are not desired. Legacy `/sdl:code:push` and `/code:push` are not registered.                                                                                                                                                                                                                                                                                                                                                      |
| `/sdl:code:land`                                                                                                                                                                                                                                            | Unified landing surface backed by package tests: requires Graphite stack proof, fast-path squash merges an isolated single PR into `gt trunk`, and stack-mode lands bottom-to-current while preserving descendant maintenance. Legacy `/code:land`, `/land`, `/gh:land`, `/land-stack`, and `/gt:land-stack` aliases are not registered. Codex/Claude should use equivalent `gt`/`gh` CLI flows rather than a Pi slash command.                                                                                                                                                                                                                                                                        |
| Branch context workflow (`/sdl:plan:save`, `/sdl:plan:grill-and-save`, `/sdl:branch-context:from-plan`, `/sdl:branch-context:upstack-impl-from-plan`, `/sdl:branch-context:impl-attached-plan`)                                                             | Final public Pi planning-layer command sequence. Portable core: the `@sdl/plans` package and `enriched-plan` bin for Saved plan authoring/inspection/resolution, the `@sdl/branch-context` package and `branch-context` bin for branch workflow, XDG local store `$XDG_STATE_HOME/sdl/enriched-plan/...` (default `$HOME/.local/state/sdl/enriched-plan/...`), and Branch Memory namespace `branch-context` with named Markdown attached-plan keys such as `<slug>.md`. Installed agent skills use `/skill:enriched-plan-save`, `/skill:branch-context-from-plan`, and `/skill:branch-context-impl` over the same CLI contract; the grilled interaction itself is Pi-only structured UI orchestration. |
