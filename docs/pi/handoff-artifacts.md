...
- **Delete a handoff** when explicitly removing one artifact by exact slug through the portable `ns handoff delete` command.
...
The `ns handoff list` command's normal output should show compact handoff inventory tables with slug and recency:
...
When the user opts into deleted-branch recovery with `ns handoff list --all --include-deleted`, include deleted local branches and keep their state visible:
...
`/ns:handoff:create` requires a meaningful continuation focus. If the user omits it, the command asks:
...
```text
ns handoff list --branch <branch>
ns handoff list --all
ns handoff list --all --format json
ns handoff list --all --include-deleted
ns handoff list --all --include-deleted --format json
ns handoff pickup <semantic-slug> --branch <branch> --format json
ns handoff delete [--branch <branch>] [--yes] <semantic-slug>
ns handoff gc --dry-run
brmem get <semantic-slug>.md --namespace handoff --branch <branch>
```

`ns handoff delete` removes exactly one handoff from the target branch by exact slug. Pass `--branch <branch>` to remove a handoff from a non-current or locally deleted branch; pass `--yes` to skip the confirmation prompt.

`ns handoff gc` deletes handoffs whose local branch no longer exists. Use `ns handoff gc --dry-run` to preview candidates and `ns handoff gc --force` to delete without prompting. Garbage collection deletes handoff entries only; it does not delete git branches, remote branches, Graphite state, or non-handoff Branch Memory entries.
