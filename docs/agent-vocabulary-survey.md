...
- **Automations** (Mar 2026) — "run cloud agents in the background, either on a schedule or in response to events from GitHub, GitLab, Slack, webhooks, Linear, and more." **Trigger** is the official noun; triggers **fire**; an automation runs "when _any_ trigger fires." Sharing scopes: Private / Team Visible / Team Owned (verb: promote). (<https://cursor.com/docs/cloud-agent/automations>)
...
- **Replit** — Plan mode → ordered task list → "Start building"; **Checkpoints** (app-state snapshots, also the billing unit); "Agents & Automations" refers to artifacts the agent builds _for you_, not scheduled runs of the agent. (<https://docs.replit.com/replitai/plan-mode>)
