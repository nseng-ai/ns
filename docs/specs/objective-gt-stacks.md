...
A required read of repository or Objective data fails partway through computing the projection. The command fails rather than emitting a partial projection, with an `error_type` identifying the failed read and a message carrying the underlying detail.

Defined data-read identifiers:

- `git_objective_stack_touches_failed` — reading per-branch Objective touch slices failed.
- `git_objective_stack_status_failed` — reading trunk-projected Objective status failed.

The full stable `error_type` enumeration for this command is therefore `not_in_repo`, `gt_branch_graph_failed`, `git_objective_stack_touches_failed`, and `git_objective_stack_status_failed`.
