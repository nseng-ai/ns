...
The current ordered work checklist.
...
The roadmap is intentionally checklist-shaped so users and agents can see what
has been done so far and what remains, without reconstructing the plan from the
update log. Completed and in-progress items stay in the ordered checklist unless
the user asks to compact or condense it.
...
- [x] Completed work area.
  - Artifact: Delivered or verified output.

## Parked

- Work intentionally deferred, rejected, canceled, or waiting on external facts.
```
...
`[~]` is an initiative-specific marker and is intentional even though it is not
GitHub-native task-list syntax.

Parked items are plain bullets with no checkboxes. They are not part of the
active ordered checklist.

Large initiatives may use subheadings under `## Checklist`, but the canonical
top-level shape remains `## Checklist` followed by `## Parked`.
...
It should read the initiative state, identify the highest-value unblocked
checklist area, and recommend a concrete implementation shape. It may suggest
...
should be proactive but evidence-bound: update stale statuses, revise completion
criteria, park obsolete work, adjust sequencing, and update durable scope,
constraints, invariants, risks, or open questions when the initiative history and
current repository state justify it.
...
Completion criteria may use initiative status markers:

- `[ ]` not satisfied
- `[~]` partially satisfied or in progress
- `[x]` satisfied
...
- [~] Named in-progress work area.
  - Artifact: Expected reviewable or verifiable output.

- [x] Named completed work area.
  - Artifact: Delivered or verified output.
...
Large initiatives may add subheadings under `## Checklist` while preserving the
...
- Keep completed and in-progress roadmap items visible in the ordered checklist
  unless the user asks to compact them.
- Use `[ ]`, `[~]`, and `[x]` for active checklist status.
