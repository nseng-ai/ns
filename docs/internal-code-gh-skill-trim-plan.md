# Plan: Trim `internal-code-gh/SKILL.md` (skill-audit)

**Status:** Historical audit plan. The skill has since been renamed to `internal-code-gh`, and the trim described here appears applied in the current `SKILL.md`; retain this document as context rather than as a fresh execution checklist.
**Target:** `skills/internal-code-gh/SKILL.md` (canonical first-party source; `.agents/skills/internal-code-gh` and `.claude/skills/internal-code-gh` are symlinks to it).
...
`internal-code-gh` is an internal (`metadata.internal: true`), public-authored skill that wraps the GitHub CLI (`gh`). It ships a 360-line `SKILL.md` plus four large references:
...
The `internal-code-gh` body violates all of these: it is ~95% restatement of the four references, wrapped in "when the user asks X, load `references/gh.md` and re-list these commands" scaffolding.
...
2. **Coordination pointers (`internal-code-gh-ci-debug`, `graphite`): excluded.** (User declined.) Keeps the change purely subtractive plus the description rewrite — no invented content.
...
- Edit the canonical source at `skills/internal-code-gh/SKILL.md`; the symlinks resolve automatically.

---

## 8. Execution checklist

1. Rewrite `skills/internal-code-gh/SKILL.md` to the target in §6 (frontmatter description + lean body).
2. Confirm no remaining `](#graphql-api-reference)` or other intra-file anchors: `grep -n "](#" skills/internal-code-gh/SKILL.md` → expect no matches.
3. `dprint check skills/internal-code-gh/SKILL.md` (autofix with `just dprint-fix` if it reports formatting).
4. `git diff --check`.
...
Trigger `skill-audit` on `internal-code-gh`, or say "apply the internal-code-gh trim plan". Everything needed to execute without re-deriving is in this file.
