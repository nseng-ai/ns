...
- The caller space for space rename, tab creation, tab prompt or Saved Plan implementation, and Handoff tab launch is resolved through the typed Herdr gateway caller-context operation, backed by Herdr's caller-aware `pane current --current` query. Commands fail rather than target UI focus when resolution fails; there is no environment-variable transport. User-facing language says **space**, **tab**, and **caller space**; **workspace** is reserved for upstream Herdr mechanics such as workspace IDs.
