...
The two session commands privately run `pi --fork` against the persisted source Pi session, with tools disabled, to draft a directed, self-contained implementation prompt outside the source transcript. That private derivation process is the only forked process in the workflow. The commands then offer the same explicit approval menu for space and tab destinations and share one pending-generation coordinator. Before the menu, Herdr identifies the source checkout as context and explains that approval selects the branch basis, creates a new branch, and executes in an isolated Slot. Only **Implement on a new branch in an isolated Slot** enters the branch, Branch Memory, Slot-checkout, and destination-launch pipeline; **Load into editor for review/edit** explicitly prefills the matching `/ns:herdr:impl:prompt:{space,tab}` command; cancellation or dismissal performs no implementation mutation.
...
Session prompt transport is deliberately private, durable, and symmetric across space and tab destinations. The tool-less, non-interactive source derivation tells the generator to express repository anchors relative to the repository root and to treat absolute source-worktree paths as context. Herdr then presents a prompt-free menu: **Implement on a new branch in an isolated Slot**, **Load into editor for review/edit**, or **Cancel**. Implementing stores the prompt without overwrite at `ns-impl/prompt.md` on the collision-resolved destination branch, verifies an exact read, checks that branch out through Slots, and creates the Herdr destination with the returned Slot checkout worktree path. Loading into the editor is an explicit opt-in to inspect/edit it; cancel or menu dismissal performs no implementation mutation.
