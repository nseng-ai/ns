...
| cmux surface                            | Disposition | Herdr mirror                          | Notes                                                                                                                                                                                                                  |
| --------------------------------------- | ----------- | ------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `/ns:cmux:sidebar:objective-summary`    | ✅ Selected | `/ns:herdr:sidebar:objective-summary` | Labels the explicit caller Herdr workspace with the Objective slug and reports the current slot as the explicit caller pane title; branch metadata remains omitted. No public generic workspace-summary command added. |
...
| -------------------------------- | ------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `ns cmux exec workspace-summary` | ✅ Retained in cmux | Used by the retained `/ns:cmux:sidebar:objective-summary` deterministic flow. Not mirrored in Herdr as a public command; `/ns:herdr:sidebar:objective-summary` directly composes workspace rename and caller-pane metadata reporting. |
...
| Apply workspace metadata (title + description)   | `ns cmux exec workspace-summary --title --description`         | ⚠️ Partial           | Herdr uses workspace rename for the Objective label and caller-pane `report-metadata --title` for the slot line. It does not expose the cmux workspace-summary shape or branch description. |
