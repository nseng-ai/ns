...
competing positions and identify decisions that SDL/Clinkr must resolve in ADRs
...
Important clig.dev philosophy for SDL: human-first and composable are compatible,
...
For SDL, this supports Clinkr's existing direction: typed schemas,
...
SDL already follows this pattern with hidden `exec` subgroups for skill-invoked
operations.
...
Decision needed: should SDL standardize richer numeric exit codes, keep simple
...
Decision needed: when should SDL commands vary output by TTY, and when must they
...
Decision needed: should SDL encode danger tiers in Clinkr APIs, document them as
