...
**Why it exists (historical):** this 2026-06-13 research originally informed the now-retired `areg` feature for converting imperative skills into invoke-only commands across Claude Code, Codex, and Pi. It records the underlying harness facts; `areg` references below are historical unless explicitly marked current.
...
**Current-state addendum:** Skill management now belongs directly to `npx skills`, including acquisition, installation, updates, removal, and `skills-lock.json`. Checked-in repository files own first-party sources and topology, Harness Overlays, and invocation metadata. ns workflows may invoke installed skills at runtime, but ns does not invoke or wrap `npx skills` and does not manage skills. The former `ns skills`, top-level `ns update`, and `ns skill-exposure` commands do not exist. References to those commands below are preserved historical findings, not current guidance.
...
## Current implications for Skill Exposure

> Historical note: an earlier implementation exposed five `areg` invocation kinds. That implementation and its `ambient-only` / `unlisted` options are retired. Current policy retains exactly `normal`, `invoke-only`, and `skill-backed-command`, applied to explicit skill-directory or `SKILL.md` paths with `ns skill-exposure`.
...
2. **Exposure is policy; command loading is mechanism.** `ns skill-exposure apply skill-backed-command <explicit-path>` reconciles `disable-model-invocation: true`, `agents/openai.yaml`, and `.pi/settings.json` `"-skills/<name>"`, then verifies a Skill-Backed Command Registration. A Skill-Backed Command can exist independently of that exposure policy; applying the policy makes it the preferred Pi surface. Do not hand-edit these artifacts.
3. **Skill-Backed Commands must require their skills directly.** A policy-hidden skill may be absent from `pi.getCommands()`, so specialized and generic `/ns:cmd` extensions read the required `SKILL.md` directly from its managed source (nested `skills/<disposition>/.../<name>/` for first-party skills, `.agents/skills/<name>/` for vendored skills) instead of expanding ambient `skill:<name>` registrations. The repo-local file is the workflow authority: missing, unreadable, or malformed required content fails before command-specific work and starts no model turn. A loaded Pi skill command and handwritten fallback instructions are not substitutes; see [ADR 0048](../adr/0048-required-skill-backed-workflows-fail-closed.md).
4. **Namespacing (`/ns:cmd`) is extension-owned in Pi.** Pi cannot file-namespace prompts or skills; repo-local TypeScript extensions register namespaced commands and either keep richer specialized behavior or use generic Skill-Backed Command wrappers.
5. **Codex ambient cost is unavoidable.** No current per-harness flag makes an explicitly invocable Codex skill zero-ambient.
6. **Mirror and installation health belong to acquisition/layout tooling.** `ns skill-exposure` does not remove mirrors or offer a hidden/unlisted policy; use `npx skills` for supported install/layout operations.
