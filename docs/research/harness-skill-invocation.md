...
> Historical note: an earlier implementation exposed five `areg` invocation kinds. That implementation and its `ambient-only` / `unlisted` options are retired. Current policy retains exactly `normal`, `invoke-only`, and `command-backed`, applied to explicit skill-directory or `SKILL.md` paths with `ns skill-exposure`.
...
2. **Command conversion is a three-artifact lifecycle plus replacement verification:** `ns skill-exposure apply command-backed <explicit-path>` reconciles `disable-model-invocation: true`, `agents/openai.yaml`, and `.pi/settings.json` `"-skills/<name>"`, and verifies the replacement Pi extension command. Do not hand-edit these artifacts.
3. **Replacement Pi commands must require backing skills directly.** A command-converted skill may be hidden from `pi.getCommands()`, so specialized and generic `/ns:cmd` extensions read the backing `SKILL.md` directly from its managed source (`skills/<name>/` for first-party skills, `.agents/skills/<name>/` for vendored skills) instead of expanding ambient `skill:<name>` registrations. The repo-local file is the workflow authority: missing, unreadable, or malformed required content fails before command-specific work and starts no model turn. A loaded Pi skill command and handwritten fallback instructions are not substitutes; see [ADR 0048](../adr/0048-required-skill-backed-workflows-fail-closed.md).
