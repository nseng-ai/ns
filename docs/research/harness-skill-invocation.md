...
**Since researched:** first-party provisioning (`ns skills` / `ns update`) materializes npm-module-bundled skills into these harness roots. `npx skills` owns repo-local and third-party acquisition/layout/lock checks. Current cross-harness policy is the three-policy `ns skill-exposure apply/show/check <explicit-path...>` surface documented in [Skill Conventions § Skill Exposure Policy and Harness Overlays](../conventions/skill-conventions.md#skill-exposure-policy-and-harness-overlays); it does not discover skills or validate install health.
