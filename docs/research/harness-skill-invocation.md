...
> These mechanics were since implemented as `areg`'s managed **skill invocation kinds** (`normal` / `invoke-only` / `command-backed` / `ambient-only` / `unlisted`). For the repo-facing taxonomy — how to inspect and change a skill's kind, and why you must never hand-edit the flags below — see [Skill Conventions § Skill Invocation Kinds](../conventions/skill-conventions.md#skill-invocation-kinds-areg). The notes here are the underlying harness research the kinds are built on.
...
2. **Command conversion is a three-artifact lifecycle plus replacement verification:** add `disable-model-invocation: true` to `SKILL.md`, write `agents/openai.yaml` with `allow_implicit_invocation: false`, and add `.pi/settings.json` `"-skills/<name>"`. Before writing those artifacts, `areg skill apply command-backed` verifies that a replacement Pi extension command exists so hiding `/skill:<name>` does not remove the user's invocable Pi surface.
...
6. **Mirror removal is the only lever that hides a skill from every typeahead.** The invocation flags above drop a skill from ambient *model* context but leave the human-facing invocation surfaces — Codex `$name`, Claude Code `/name` typeahead, Pi `/skill:name` — reachable because those read the mirror directories (`.agents/skills/<name>`, `.claude/skills/<name>`). Deleting both mirror symlinks is the only thing that removes those entries. This is implemented as the areg **`unlisted`** kind: it removes both mirrors, keeps the Pi `-skills/<name>` exclusion as belt-and-suspenders (a mirror could be re-created by an unrelated install), and provides no replacement command — discovery moves to an ambient router skill that reads the leaf's canonical `skills/<name>/` source directly.
