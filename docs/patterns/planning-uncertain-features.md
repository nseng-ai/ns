...
- Marker evidence: marked additions in `.ji/prompts/plans-write.md`, `skills/enriched-plan-save/SKILL.md`, `ts/packages/pi/src/branch-context/enriched-plan-save.ts`, and associated TypeScript prompt tests.
...
```bash
rg -n "PLAN-VERIFICATION-WORKSTREAM:(START|END)" .ji skills ts/packages
rg -n "refactor-execution-strategy-guidance|enriched-plan-save-plan-quality" .ji skills ts/packages
```
