...
Extension manifests declare static `ns.points` metadata beside `ns.commands`. The full point id is `<group>.<path segments joined with .>`, with the extension group as the enforced namespace root. First-party extensions normally use `<group>.<workflow>.<leaf>`, but that is a convention rather than a platform rule. Each point declares two type axes: `accepts: hook | prompt` and `semantics: additive | override`. Override prompt points may declare a package-relative markdown `default` file.
