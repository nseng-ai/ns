...
Extension descriptors declare static point metadata. The original implementation used static `package.json` `ns.points` metadata beside `ns.commands`; that legacy manifest surface has since been removed in favor of descriptor modules exposed as `exports["./ns-extension"]` and selected through `ns.toml` `extensions`. First-party extensions normally use ids shaped like `<group>.<workflow>.<leaf>`, but that is a convention rather than a platform rule. Each point declares two type axes: `accepts: hook | prompt` and descriptor `cardinality: many | one` (catalog semantics: additive | override). Override prompt points may declare a package-relative markdown `default` file.
