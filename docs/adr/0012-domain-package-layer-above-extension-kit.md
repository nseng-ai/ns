...
runtime host rather than in its owning capability — for example
`@sdl/pi/objectives` owns the Objective selection/diff/list rules, and `ccc` consumes
them via `@sdl/pi/objectives/selection`. That is CCC depending on the `@sdl/pi` runtime
...
through the Capability API; the `@sdl/pi` runtime host keeps only UI and runtime registration.

```text
CCC (highest-fan-out consumer)             @sdl/pi  (runtime host, off-axis)
         \                                       /
