...
| 5  | a    | Single-PR land fast path merges to trunk with no confirmation while stack paths confirm                                                | `ccc land` (Pi surface)                                                                                                                                          | ADR-needed                                    |
| 6  | a    | External GitHub write (Tier 2) with no confirm flag / interactive gate; agent-only hidden `exec`                                       | `pr-address exec reply-review-thread`, `resolve-review-thread`                                                                                                   | ADR-needed                                    |
...
| 10 | d    | Query-miss (`ok(found:false)`) vs action-miss (`negative`) inconsistency across commands                                               | `pr-address` (pr-details/branch-pr/pr-checks vs download-feedback/map-branch-prs)                                                                                | ADR-needed                                    |
...
| 14 | c/d  | `rawCommand` opts out of envelope entirely (no `errorType`/`resultSchema`; true failures exit 1 not 2)                                 | `packagechk` (all), `sdlcc cmux report`, `vibechk run`, `roaster exec publish-findings`, `ccc exec autobranch`                                                   | land-now-fix + ADR-needed (see cross-cutting) |
...
5. **`rawCommand`/`isRawExit` packages opt out of the envelope contract.**
   ...
   should migrate onto the envelope, is a **cross-cutting ADR-needed** question.
...
| `exec check`     | No                       | d    | Absent entry → `ok(present:false)` (exit 0) — defensible presence-predicate vs ADR 0013 exit-code branching                                                             | ADR-needed     | `attach.ts:119-124`; `cli.ts:258,363-375`                  |
...
is correct. `check` returning `ok(present:false)` is the one contestable (ADR-needed)
...
| `land` / `land-stack` (Pi `/sdl:flow:land`)                                   | Yes (squash-merge to trunk, `gt delete -f`, frees slots) | a    | Tier 3 high blast radius. Stack/chunked paths confirm, gate on `ctx.hasUI`, fail fast naming `--yes`. BUT isolated single-PR fast path merges to trunk with NO confirmation and ignores `--yes`. Non-interactive refusal is custom `LandStackFailure` exit 1 not Clinkr `usageError` exit 2 | ADR-needed       | `land.ts:171-176,336-399` (merge `:368`); confirm `land.ts:287-304`; `land-stack.ts:198-214,312-340` |
...
confirmation while stack paths confirm (ADR-needed — is skipping confirmation on
single-PR intentional?). Lower-severity: `exec autobranch` rawCommand has no envelope.
...
| `exec pr-details`             | No                              | d    | PR-not-found → `ok(found:false)`, not `negative` — inconsistent with `download-feedback`                                            | ADR-needed                        | `primitive-results.ts:30-37`; `primitive-commands.ts:160-167`       |
| `exec branch-pr`              | No                              | c/d  | Failures lack `data`; branch-not-found → `ok(found:false)`                                                                          | land-now-fix / ADR-needed         | `primitive-commands.ts:77-82,170-182`; `primitive-results.ts:30-37` |
...
| `exec pr-checks`              | No                              | d    | Miss → `ok(found:false)` while `download-feedback` returns `negative` for same intent                                               | ADR-needed                        | `primitive-commands.ts:243-244`; `download-feedback.ts:62-66`       |
| `exec reply-review-thread`    | **Yes (Tier 2 external write)** | a    | Posts a comment to GitHub. No `--yes`/`-y`, no `isInteractive()` gate, no `requireInteractiveOrUsageError`; mutates unconditionally | ADR-needed                        | `primitive-commands.ts:134-143,301-315`                             |
| `exec resolve-review-thread`  | **Yes (Tier 2 external write)** | a    | Resolves a GitHub review thread; same gap                                                                                           | ADR-needed                        | `primitive-commands.ts:144-153,317-330`                             |
...
so (a) is ADR-needed. Cross-cutting (c): every `failure(...)` uses snake_case but none
pass `data`. (b): list commands expose no completion state (consistent with ADR 0012
parked); `pr-checks` alone carries `has_more`. (d): real inconsistency between
lookup-style misses (`ok(found:false)`) and action-misses (`negative`).
...
| `sdl shell install` (= `sdl slot shell install`) | Yes       | a    | Tier 1: idempotent/additive/reversible managed marker-block write; no confirm. Ambiguity: writes to user dotfile outside repo (possible Tier 2 external write) | conformant     | `operations/shell.ts:36-50`; `sdl-core/src/shell-support.ts:114-129` |
...
`data` on the shared `unsupported_shell` failure (e.g. `{shell, supportedShells}`). One
judgment call: danger tier of `shell install` (Tier 1 idempotent marker-block write vs
Tier 2 external write because it mutates a file outside the repo). Command tree is
...
| Command       | Mutating?                                       | Area | Finding                                                                                                                                                                                   | Classification | Evidence (file:line)                          |
| ------------- | ----------------------------------------------- | ---- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------- | --------------------------------------------- |
| `cmux report` | Yes (external write: `cmux surface resume set`) | a    | Tier 1 reversible external metadata write (idempotent); no prompt, no hang. (Alternate Tier 2 "external write" reading defensible)                                                        | conformant     | `cmux-report.ts:91-102,131`; `cli.ts:81-86`   |
...
1. **Area (a), land-now (safety):** add `--yes`/`-y` + `requireInteractiveOrUsageError`
   gating to `branch-context exec delete` and `brmem delete`; re-gate `areg init`,
...
## Open ADR-needed questions

These should be resolved (or explicitly parked with rationale) before the corresponding
remediations:

1. **Agent-only `exec` destructive writes (pr-address `reply`/`resolve-review-thread`):**
   does the human-tier Tier 2 confirm model apply to hidden `exec` commands, or is the
   `agent-exec-tier.md` "required flag + `usageError`" rule sufficient/exempting?
2. **`ccc land` single-PR fast path:** is auto-merge to trunk without confirmation
   intentional, or should it gain the same confirm gate as the stack paths?
3. **`rawCommand`/`isRawExit` envelope exemption:** are raw-exit commands (`packagechk`,
   `sdlcc cmux report`, `vibechk run`, `roaster publish-findings`, `ccc autobranch`) a
   sanctioned exception to the envelope/`resultSchema`/`--json-schema` pre-ship items, or
   should they migrate onto the Clinkr envelope?
4. **Query-miss vs action-miss semantics (pr-address):** standardize whether a lookup
   miss is `ok(found:false)` or `negative` (currently both are used for equivalent "not
   found" outcomes).
5. **`brmem export`/`branch-context check` empty/absent as `ok`:** ratify presence-query
   `ok(present:false)` and empty-success `ok` as the standard, distinct from semantic
   `negative`.
6. **Dotfile/external-write danger tier (`sdl shell install`, `sdlcc cmux report`):** is an
   idempotent write outside the repo Tier 1 or a Tier 2 external write requiring `--yes`?
