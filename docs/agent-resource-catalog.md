...
| Handoff artifacts     | Final first-party Pi surface: `/handoff:create`, `/handoff:pickup`, `/handoff:list`, `/skill:handoff-save`, and `/skill:handoff-load`. The Python CLI also exposes `handoff delete` for exact-slug single-artifact removal; no Pi delete command entry is exposed yet. List output uses grouped cards with copyable pickup commands. No old `brmem`-named handoff aliases are retained. |
