...
- **@asdl/pi-extensions → Pi runtime + asdl-dev + @asdl/sdl + repo CLIs**: Pi extensions own discovery adapters, argument restoration/UI behavior, worktree-status lifecycle/footer plumbing, runtime cmux helpers, and runtime CLI bridging over `asdl-dev`, `sdl`, `git`/`gt`/`gh`, `brmem`, `objective`, `slot`, and related repo commands. For `/sdl:*` and `/sdl:code:*` mirrors, Pi extensions should use thin adapters over SDL-owned command behavior or lower orchestration seams. For `/sdl:code:autobranch`, `/sdl:code:land`, and worktree-status operational presentation, pi-extensions preserves public adapters while delegating repo-opinionated orchestration/observability to CCC where appropriate.
...
- **@asdl/ccc → @asdl/pi-extension-runtime + lower capabilities**: CCC owns multi-capability command orchestration behind the `ccc` Pi command prefix, selected repo workflow orchestration behind current non-`ccc` public surfaces such as `/objective:stack-impl`, `/sdl:code:autobranch`, and `/sdl:code:land`, and worktree-status observability facts/presentation. CCC composes lower `asdl-dev` pending-worktree/checkpoint primitives, injected GitHub/command execution capabilities, Graphite/GitHub/slot landing orchestration policy, passive Graphite metadata facts, and neutral `@asdl/pi-extension-runtime` helpers; lower capabilities such as `@asdl/pi-extension-runtime`, `@asdl/branch-context`, `asdl-dev`, handoff, Objective, brmem, Git, and Graphite must not import `@asdl/ccc`.
