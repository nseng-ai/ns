...
- **Checkpoint / autobranch / submit / preview**: distinguish current `asdl-dev` command semantics, SDL public lifecycle command ownership for migrated workflows, CCC-owned repo source-control orchestration where the flow crosses primitives, and Pi discovery/presentation adapters in `@asdl/pi-extensions`.
