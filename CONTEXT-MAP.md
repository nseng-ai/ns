...
- **Domain placement / layer**: capability domain logic lives only in the **Extensions** (first-party extension packages built on the **Extension Kit**, `@sdl/extension-kit`); the `@sdl/pi` **Presentation Host**, the Extension Kit, and the `@sdl/sdl` kernel are not domain homes. Domain stranded in `@sdl/pi/*` (e.g. `@sdl/pi/objectives` selection rules consumed by **CCC**) is relocated into its owning extension and re-consumed via the Peer API. The SDK boundary is permeable downward over time but gated on proven generality, and opinionated above-SDK patterns such as gateways are expected to stay above the SDK (ADR 0012).
