...
- [@asdl/pi-extensions](./ts/packages/pi-extensions/CONTEXT.md) — repo-local Pi discovery adapters, engineered extension package, worktree-status/Pi footer lifecycle adapter language, saved-plan/planned-branch/checkpoint/handoff language, runner subagents, CCC command-prefix boundary, and CLI bridge vocabulary. This file exists, but still needs the later refresh against the current extension inventory and `asdl-dev` command mirror boundary.
- [@asdl/ccc](./ts/packages/ccc/CONTEXT.md) — CCC (Cmux Command and Control) vocabulary for the private TypeScript orchestration layer that composes Pi, cmux, Graphite, Objective, handoff, planned-branch, autobranch/land, and owns worktree-status observability without lower packages importing it.
...
- **@asdl/pi-extensions → Pi runtime + asdl-dev + repo CLIs**: Pi extensions own discovery adapters, argument restoration/UI behavior, worktree-status lifecycle/footer plumbing, runtime cmux helpers, and runtime CLI bridging over `asdl-dev`, `git`/`gt`/`gh`, `brmem`, `objective`, `slot`, and related repo commands. For `/code:autobranch`, `/code:land`, `/code:land-stack`, and worktree-status operational presentation, pi-extensions preserves public adapters while delegating repo-opinionated orchestration/observability to CCC.
- **@asdl/ccc → @asdl/pi-extension-runtime + lower capabilities**: CCC owns multi-capability command orchestration behind the `ccc` Pi command prefix, selected repo workflow orchestration behind stable non-`ccc` public surfaces such as `/objective:stack-impl`, `/code:autobranch`, `/code:land`, and `/code:land-stack`, and worktree-status observability facts/presentation. CCC composes lower `asdl-dev` pending-worktree/checkpoint primitives, injected GitHub/command execution capabilities, Graphite/GitHub/slot landing orchestration policy, passive Graphite metadata facts, and neutral `@asdl/pi-extension-runtime` helpers; lower capabilities such as `@asdl/pi-extension-runtime`, `@asdl/planned-branch`, `asdl-dev`, handoff, Objective, brmem, Git, and Graphite must not import `@asdl/ccc`.
