...
- **Extension API**: when a user says "ASDL extension API," assume the SDL extension API (`@asdl/sdl/sdk`) unless they mention Pi, TUI, slash commands registered with `pi.registerCommand`, model-visible tools, or `.pi/extensions`; use "Pi runtime extension API" for the Pi surface and "SDL extension API" for SDL.
