...
- `packages/asdl-handoff/CONTEXT.md` — directed handoff artifact vocabulary over Branch Memory storage: continuation focus, handoff slug/key, `handoffs` namespace, `HandoffSummary`, branch state, list scope, all-branches inventory, garbage collection, Entry Locator, and updated-at ordering.
...
- `packages/asdl-objectives/CONTEXT.md` — Objective CLI package vocabulary, including Objective records/statuses, archive/unarchive, checked-in Markdown storage, hidden `exec` commands, path-touch attribution, and opt-in `objective gt` stack projection.
...
- **asdl-objectives → asdl-core.git + asdl-core.gt + asdl-core.clinkr + asdl-core.console/format/plugin**: Objective CLI inventory and stack projection use Git path-touch facts and opt-in Graphite stack projection. Do not reintroduce `asdl-objectives → brmem` as a storage edge.
...
- **Graphite stack projection**: distinguish `asdl-core.gt` Graphite vocabulary, `slot gt` operations, `objective gt stacks` projection, and `asdl-dev submit` Graphite verification.
