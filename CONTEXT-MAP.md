...
- **roaster** — [`packages/roaster/CONTEXT.md`](packages/roaster/CONTEXT.md). _Planned._
  - Roaster harness: `Roaster`, `ReviewDefinition`, `HarnessRuntime`, `HarnessDefinition`, `HarnessReviewRequest`, `ReviewCatalog`, `ReviewSource`, `ReviewFormat`, `Finding`, `InlineCommentability`, severity/frontmatter. Explicitly disambiguates roaster "Review" vocabulary against `gh.PRReview` and `pr-address` thread vocabulary.
...
- **Review** — `asdl-core.gh.PRReview` is a submitted review event; `asdl-pr-address` works mostly with core review threads/comments; `roaster` uses roaster/harness/review-definition vocabulary for local review runs.
