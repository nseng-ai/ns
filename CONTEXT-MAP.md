...
- **aretro → asdl-core.sessions + asdl-core.git + asdl-core.clinkr/plugin**: aretro collects deterministic branch/session/git evidence for the `/aretro:branch-retro` Pi command and its backing `branch-retro` skill, leaving recommendation judgment to the agent workflow.
