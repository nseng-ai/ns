...
- **@asdl/pi-extensions → Pi runtime + asdl-dev + repo CLIs**: Pi extensions own discovery adapters, argument restoration/UI behavior, runtime cmux helpers, and runtime CLI bridging over `asdl-dev`, `git`/`gt`/`gh`, `brmem`, `objective`, `slot`, and related repo commands. For `/code:autobranch` and `/code:land`, pi-extensions preserves the public `code` command-family adapters while delegating orchestration to CCC.
- **@asdl/ccc → @asdl/pi-extension-runtime + lower capabilities**: CCC owns multi-capability command orchestration behind the `ccc` Pi command prefix and selected repo workflow orchestration behind stable non-`ccc` public surfaces such as `/objective:stack-impl`, `/code:autobranch`, and `/code:land`. CCC composes lower `asdl-dev` pending-worktree/checkpoint primitives, injected GitHub/command execution capabilities, and neutral `@asdl/pi-extension-runtime` helpers; lower capabilities such as `@asdl/pi-extension-runtime`, `@asdl/planned-branch`, `asdl-dev`, handoff, Objective, brmem, Git, and Graphite must not import `@asdl/ccc`.
