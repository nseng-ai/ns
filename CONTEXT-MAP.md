...
- TypeScript workspace inventory: 11 repo-local packages under `ts/packages/`. This map's TypeScript package-context coverage is intentionally partial pending a focused rebaseline.
- Present context files: root `CONTEXT.md`, `packages/asdl-core/CONTEXT.md`, `packages/asdl-handoff/CONTEXT.md`, `ts/packages/handoff/CONTEXT.md`, `ts/packages/brmem/CONTEXT.md`, `ts/packages/pi-extension-runtime/CONTEXT.md`, `ts/packages/pi-extensions/CONTEXT.md`, `ts/packages/ccc/CONTEXT.md`, and `ts/packages/sdl/CONTEXT.md`.
...
- [@asdl/sdl](./ts/packages/sdl/CONTEXT.md) — Source Development Lifecycle CLI vocabulary for SDL command surfaces, project-specific SDL command extensions, command modules, `@asdl/sdl/sdk` as the public author API, internal migration exports, SDL Pi mirrors, hard cutover, and lower orchestration ownership.
...
- **asdl-dev → @asdl/sdl**: `asdl-dev` may consume SDL primitives during migration, but migrated lifecycle commands should move their public boundary to SDL rather than remain durable `asdl-dev` commands.
...
- **@asdl/pi-extensions → Pi runtime + asdl-dev + @asdl/sdl + repo CLIs**: Pi extensions own discovery adapters, argument restoration/UI behavior, worktree-status lifecycle/footer plumbing, runtime cmux helpers, and runtime CLI bridging over `asdl-dev`, `sdl`, `git`/`gt`/`gh`, `brmem`, `objective`, `slot`, and related repo commands. For `/sdl:*` mirrors, Pi extensions should use thin adapters over SDL-owned command behavior. For existing `/code:autobranch`, unified `/code:land`, and worktree-status operational presentation, pi-extensions preserves public adapters while delegating repo-opinionated orchestration/observability to CCC until those flows migrate.
- **@asdl/sdl → @asdl/pi-extensions**: Pi extensions mirror SDL commands as `/sdl:*` through thin adapters; SDL owns command behavior, and Pi owns runtime registration and presentation.
- **@asdl/sdl → @asdl/ccc**: CCC may own project-specific orchestration internals for some workflows, while SDL can still own the public lifecycle command surface.
- **@asdl/ccc → @asdl/pi-extension-runtime + lower capabilities**: CCC owns multi-capability command orchestration behind the `ccc` Pi command prefix, selected repo workflow orchestration behind current non-`ccc` public surfaces such as `/objective:stack-impl`, `/code:autobranch`, and unified `/code:land` pending migration decisions, and worktree-status observability facts/presentation. CCC composes lower `asdl-dev` pending-worktree/checkpoint primitives, injected GitHub/command execution capabilities, Graphite/GitHub/slot landing orchestration policy, passive Graphite metadata facts, and neutral `@asdl/pi-extension-runtime` helpers; lower capabilities such as `@asdl/pi-extension-runtime`, `@asdl/branch-context`, `asdl-dev`, handoff, Objective, brmem, Git, and Graphite must not import `@asdl/ccc`.
...
- **Changes / checkpoint / autobranch / submit / preview**: distinguish current `asdl-dev` command semantics, SDL public lifecycle command ownership for migrated workflows such as `changes`, `cp`, and `submit`, CCC-owned repo source-control orchestration where the flow crosses primitives, and Pi discovery/presentation adapters in `@asdl/pi-extensions`.
