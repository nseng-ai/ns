...
- Host API objects (the ns extension API object, Pi runtime `ExtensionAPI`, and narrowed views of either) are gateway-like collaborators. When the diff shows a **gateway clump** — two or more gateways or host API objects repeatedly traveling together — form an extension-owned `*Context`; a single collaborator or one-site grouping stays as-is. Canonical rules: `docs/conventions/consumer-gateways-and-command-shape.md`.
