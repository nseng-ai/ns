...
Use `.ns/reviews/dignified-python-tripwire/review.md` as the authoritative review definition. Do not duplicate or reinterpret the review rules from memory.

First read `.ns/reviews/dignified-python-tripwire/review.md`, then apply that review definition exactly to the supplied diff or current branch changes in this same session. Stay read-only and keep findings grounded in the diff.

Use explicit automation only when the user asks for isolated runner execution or automatic review logs:

```bash
ns roaster review run dignified-python-tripwire
```
...
```bash
ns roaster exec record-findings --review-key dignified-python-tripwire --format json < findings.json
```
