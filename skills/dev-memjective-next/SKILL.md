...
description: "Run on a fresh slice branch: carry the memjective snapshot forward onto the current branch via `brmem put`, choose the next slice from the roadmap, and implement it in-session. Refuses to run on a branch that already has a snapshot. See `dev-memjective` for the subsystem overview."
allowed-tools:
  - "Bash(git rev-parse *)"
...
- **Writes exactly one brmem entry: the carry-forward.** The only brmem
  ...
  time; no writes to the master-branch snapshot; no writes to any other
  branch.
...
- **2+ matches** → abort; the branch is in an invalid state. Tell the
