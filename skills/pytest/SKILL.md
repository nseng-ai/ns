...
the architectural guidance in `python-fake-driven-testing`.
...
gateway first (see `python-fake-driven-testing`) and mock the gateway.
...
read `python-fake-driven-testing`. Introducing an ABC gateway and a fake
...
- Refactoring existing `unittest.mock.patch` code into the
  gateway/fake pattern → `python-fake-driven-testing`
