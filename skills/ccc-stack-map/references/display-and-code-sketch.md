...
- Preserve the structured Graphite source rule: use `ji slot gt exec stack-branches --format json`, `gt parent --no-interactive`, or `gt children --no-interactive` for facts.
