...
- do not run `sdl objective exec runner-subagent-usage`;
...
```bash
sdl objective exec runner-subagent-usage --format md <session-file>...
```
...
- Include `sdl objective exec runner-subagent-usage --format md ...` output, a compact transcription, or the explicit unavailable reason.
