...
When you hit a stop condition and are about to write the final response, read `references/final-response.md` first. It holds the runner-subagent digest-telemetry procedure (when and how to run `ns objective exec runner-subagent-usage`) and the exact `## Stack implementation digest` structure the final response must emit.
