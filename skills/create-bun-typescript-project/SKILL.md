...
description: "Scaffold a well-structured Bun-centric TypeScript project. Use when the user wants to create a new TypeScript project, set up a Bun project, bootstrap a TS library or CLI, or start a new TS repo with modern tooling. Creates package.json (Bun, no build step, no dist/), strict ESM tsconfig.json, oxlint + oxfmt orchestrated by ultracite, bun test --sequential, bunfig.toml, and .gitignore — passing `bun run check`, `bunx tsc --noEmit`, and `bun test --sequential` immediately."
metadata:
