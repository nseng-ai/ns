...
- For separate fix commits, use `ji flow cp`; do not hand-roll checkpoint commits or amend/squash unless the user explicitly asks for a different commit mode.
...
3. **Validate each branch bottom-to-top**
   ...
   - When a branch required edits and now passes, run `ji flow cp` on that same branch to create a separate checkpoint-style fix commit. Capture the printed commit summary. If `ji flow cp` refuses because the worktree is clean, record that no commit was needed; for any other refusal, stop and report stdout/stderr.
...
4. **Restack after every fix commit**
   ...
   - If restack fails because an in-scope branch is checked out in another slot, ask before running `ji slot gt free-stack`. Only retry restack after the user authorizes slot freeing.
...
- any `ji flow cp` commit summaries;
