...
1. **Begin** — fast, read-only, LBYL:
   ...
   ```bash
   ns objective exec runner-begin <owner>/<slug> [--recover] [--guidance <text|@file>] \
     --report-path <scratch>/step-<n>-report.json --format json > <scratch>/step-<n>-facts.json
   ```

   On exit 0 the facts file holds the machine envelope: `data.prompt` (the subagent prompt), `data.baseBranch`, `data.headAtDispatch`, `data.reportPath`. Non-zero means nothing was dispatched: exit 1 is a precondition refusal (read the message), exit 2 a usage error (bad locator, report path inside the repo or already existing, unreadable `@file` guidance).
...
3. **Finish** — the deterministic verdict and local commit handoff, run by you, exactly once:
   ...
   The report path defaults from the facts (`--report @path` overrides). Finish validates the report fail-closed, runs the verification gate, creates the runner-owned **local-only** commit with provenance trailers (`Objective-Runner-Step: <owner>/<slug>`, plus `Objective-Runner-Mode: recover` for recovered attempts), and prints the **Runner Checkpoint** to stdout. That checkpoint is the handoff from runner to parent: the parent judges the commit and any later Objective update, push, submit, PR, or human handoff decision. **Finish is terminal**: never re-run it after `committed` — a second run deterministically fails verification (`head-unchanged`, `worktree-dirty`) by design.

Flags on begin:

- `<owner>/<slug>` — the Objective Locator (required positional on both commands; a bare slug means `<current-owner>/<slug>`).
...
- **The facts file is the step's identity.** Save begin's stdout verbatim and replay it to finish untouched. Finish cross-checks the locator and takes mode, base branch, and dispatch-time HEAD from it.
...
| 2    | Usage error (invalid/unknown locator, missing/existing/in-repo report path, unreadable `@file` guidance) or infrastructure failure |
...
| 2    | Usage error (missing/malformed facts, locator mismatch, non-ok saved envelope — nothing judged) or malfunction (missing/invalid report file, commit failure) | malfunction checkpoint when one could be produced |
...
A trusted parent may publish afterward only when the selected Objective's durable Runner Policy permits it, a human confirms an exact launch preview, and the implemented parent publisher binds that one invocation to the Objective Locator, current non-trunk branch, already-existing PR, and launch/last-published heads. Before publication, the parent must read the committed checkpoint, complete its continue/recover/stop and Semantic Update judgments, commit any material Objective tracking, and supply the cumulative PR-ready summary. If the parent publication capability is unavailable, the run remains local-only; never substitute raw `git push`, `gt submit`, or `gh` mutation.
