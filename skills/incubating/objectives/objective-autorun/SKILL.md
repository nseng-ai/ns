...
This is parent-judgment iteration per ADR 0024, not batch mode. The runner deliberately has no multi-step or retry behavior; the loop exists only as your repeated, judged re-invocation.
...
- Implementation children inherit the parent provider, model, and thinking policy by default. The only orchestrator-selected alternative is the named provider-local `cheap` route chosen before dispatch; failure never authorizes rerouting.
