...
- For `## Prompting Instructions`, row-level `Prompting:` prose, or shaping the prompts produced for an Objective, read `references/prompting-instructions.md`.
...
Separately, any Objective — regardless of pattern or execution policy — may carry an optional `## Prompting Instructions` section shaping how prompts produced for it are serialized; it grants no execution permission and never selects the next step (see `references/prompting-instructions.md`).
