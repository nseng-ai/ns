...
3. Load every edge-connected Objective before authoring closure: run `ns objective show <slug> --format md` to inventory counterpart states and both Edge Annotations, then run `ns objective exec read-objective <counterpart> --format md` for each counterpart. Read each active counterpart's `objective.md`, `roadmap.md`, and recent relevant updates; read `orientation.md` when present.
...
10. Run `ns objective check --all` after any Record Frontmatter edit. Treat a warning naming this closure as a missed re-judgment, not noise.
...
- If any Record Frontmatter was edited, confirm `ns objective check --all` reports no structural errors or missed blocked/closed-counterpart warning.
