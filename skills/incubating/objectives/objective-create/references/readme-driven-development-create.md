...
Create the Objective through the normal objective-create workflow, then fill the full bundle under `.ns/objectives/<owner>/<slug>/`:
...
- state the promotion in `objective.md` `## Completion Criteria` — the Objective is not complete while the canonical contract lives only under `.ns/objectives/<owner>/<slug>/references/`.
...
- **Pass report** additionally names the created Objective Locator and the roadmap rows added.
