...
- Independently of execution policy, any pattern may opt into an optional `## Prompting Instructions` section (semantics: the `objective` skill's `references/prompting-instructions.md`). Offer it only when the interview surfaces durable prompting facts — standing context, validation gates, executor preference, standing hazards — rather than as a mandatory question.
