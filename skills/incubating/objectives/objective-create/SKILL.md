...
- Independently of execution policy, any pattern may opt into an optional `## Metaprompt` section (semantics: the `objective` skill's `references/metaprompt.md`). Offer it only when the interview surfaces durable metaprompt facts — standing context, validation gates, executor preference, standing hazards — rather than as a mandatory question.
