...
- Prefer `ns handoff list` for inventory.
- Prefer `ns handoff gc --dry-run` before deleting stale deleted-branch handoffs.
- Use direct `brmem` commands only for storage/recovery/admin cases where no `ns handoff ...` helper exists.
...
Do not add `--overwrite` unless the user explicitly wants replacement. After copying, verify with `ns handoff list --branch <destination-branch> --format json` or `brmem check`.
...
Prefer the first-party ns command face, which deletes one exact-slug handoff (pass the slug without `.md`):
...
Use `ns handoff gc --dry-run` first to preview handoffs whose local branch is deleted. Use `ns handoff gc --force` only after the user confirms deletion or explicitly asked to force cleanup.
