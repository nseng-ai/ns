...
3. Treat the attached plan in that prompt as authoritative unless current repo state proves it stale; if stale, explain the discrepancy before changing scope. Plan findings with provenance and anchors are inherited evidence: verify the anchors, do not rerun reconnaissance the plan captured, and name the trigger before expanding beyond that evidence. See `docs/conventions/agent-context-economy.md`. If you go beyond or against what the plan settled, anything the plan ruled out becomes an open question again — recheck why it was ruled out, and look for an existing sibling that already does what you are about to build, before designing the extension.
...
7. Implement in focused steps; investigate validation failures, rerun appropriate gates after fixes, report unresolved failures accurately, and note any skipped declared gate and why.
