...
Use `.sdl/reviews/reinvented-abstractions-tripwire/review.md` as the authoritative review definition. Do not duplicate or reinterpret the review rules from memory.

First read `.sdl/reviews/reinvented-abstractions-tripwire/review.md`, then apply that review definition exactly to the supplied diff or current branch changes in this same session. Stay read-only and keep findings grounded in the diff.

Use explicit automation only when the user asks for isolated runner execution or automatic review logs:

```bash
sdl roaster review run reinvented-abstractions-tripwire
```
...
```bash
sdl roaster exec record-findings --review-key reinvented-abstractions-tripwire --format json < findings.json
```
