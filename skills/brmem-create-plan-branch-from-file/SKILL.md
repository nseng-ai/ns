...
from repo policy, invoking `create_brmem_plan_branch_from_file`, and reporting
the tool evidence.

**The repo policy prompt owns only policy guidance:** branch naming conventions,
slug restrictions, or extra pre-invocation review checks. A prompt must not
create branches, run `git`, run `gt`, run `brmem put`, publish work, or otherwise
perform the mutations owned by the tool.

**The tool owns the mutations:** creating a plain local Git branch and storing
the reviewed plan in Branch Memory namespace `brmem-plans` with key
`<slug>.md` on that branch.
...
guidance only. If resolution fails and policy is required to proceed, stop and
surface the error. Do not try the old prompt name.
...
### 4. Choose slug and optional branch name
...
The tool preflights the source file, target branch, and Branch Memory entry. If
it refuses the request, report the refusal and do not perform manual fallback
mutations.
...
3. **Invalid source file rejected** — pass a missing, relative, non-Markdown, or
   in-repo path and verify the tool refuses before creating a branch or entry.
4. **Existing target refused** — try an existing branch or existing `<slug>.md`
   entry in namespace `brmem-plans` and verify the tool refuses without
   clobbering.
5. **No old prompt fallback** — remove or ignore old prompt assets and verify the
