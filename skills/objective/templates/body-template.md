...
- the `Status:` line is for human reading only; canonical "open"/"closed" state is signaled by whether the objective lives in open or closed storage via `objective close` / `objective reopen`, not by this prose
