...
| `references/graphql.md`             | porcelain can't do it: Projects V2, Discussions, batch/multi-repo, deep nesting, advanced search, body/comment edit history     | `Projects V2` · `Discussion` · `Batch` · `Pagination` · `Example [0-9]` · `userContentEdits`     |
...
- In this Graphite-backed repo, do not use generic `git push` / `gh pr create` examples to publish or update Graphite-tracked PR branches. Use `ns flow gt submit` or `gt submit --no-interactive` so Graphite metadata stays synchronized.
