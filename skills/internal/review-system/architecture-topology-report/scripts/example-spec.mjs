...
export default {
  ...
  class T pen;`,
    ...
    { title: `<span class="font-mono text-base">pr-address</span> ships an API but no command face`,
      ...
      solution: `Confirm intent during the pr-address capability migration: either it is a pure provider seam owned per ADR 0004 (acceptable, document it), or it is a capability still missing its command face (then add one).`,
