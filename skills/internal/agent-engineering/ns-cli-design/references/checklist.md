...
- [ ] The command declares `renderHuman` over the schema-typed result (required
      by the SDK and by Clinkr `/app` for data-bearing commands). Handlers return
      semantic data, never pre-rendered terminal text; an intentionally
      machine-oriented command uses an explicit deterministic JSON renderer.
