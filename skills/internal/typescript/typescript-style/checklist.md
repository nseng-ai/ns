...
- [ ] No **`undefined` with domain meaning** remains: meaningful absence has a named discriminated
      variant, defaultable absence is resolved, and impossible absence is an invariant failure. Ordinary
      optional metadata, callbacks, DI overrides, and `AbortSignal` inputs remain optional where useful.
...
- [ ] A small set of required core data and dependencies stays positional only when order is clear;
      optional/defaulted behavior and multiple optional values use a named `*Options` object; five or
      more positional inputs normally trigger interface redesign or a named `*Options` object.
...
- [ ] Public root barrels, if present, are small curated package-level surfaces, not root-only catchalls;
      remaining barrels use named exports and `export type {}`.
