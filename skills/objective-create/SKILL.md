...
2. Conduct a user interview before writing, inspired by [Matt Pocock's `grill-me` skill](https://github.com/mattpocock/skills/blob/main/skills/productivity/grill-me/SKILL.md):
   ...
   - Numbered menus should include the recommended path first, the main alternative(s) next, and a final "Stop and create Objective" option. Use domain-specific labels so the choices are concrete (for example: `1) Skill-only steelthread`, `2) Dedicated CLI commands`, `3) Stop and create Objective`). Tell the user they can answer with a number or a custom correction.
