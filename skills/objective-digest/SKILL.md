...
- **Slice attribution.** Each branch is opened to land exactly one slice.
  ...
  multiple slices). Use this for the slice table's PR / Branch / Obj
  columns.
...
is **not** attached to this objective's snapshot tree. The CLI does not
...
| #   | ✓ | Slice         | PR          | PR state | Branch     | Obj     |
| --- | - | ------------- | ----------- | -------- | ---------- | ------- |
| 1   | ✓ | <slice title> | [#NNN](url) | open     | `<branch>` | `fresh` |
| ... |   |               |             |          |            |         |

**Obj legend:** `fresh` snapshot pinned at branch HEAD (or branch deleted
post-merge — pin can no longer drift) · `stale` branch advanced past
snapshot pin, run `objective-update` · `—` no snapshot for this
...
- PR / PR state / Branch / Obj — origin-branch attribution by
  ...
  `branch`, and Obj directly from `obj_state`.
  - Do **not** use per-branch checked overlays to pick the origin. Once a
    ...
    authored the slice.
  - Slices with no matching branch render PR / PR state / Branch / Obj
    all as `—`. Heuristic drift signals never appear in the slice rows;
    they live in the trailing `> ⚠` block.
- Obj is taken verbatim from the matched branch's `obj_state` (`fresh`
...
- For each `unclaimed_pr_candidates[i]`, decide whether `title` plausibly
  ...
  unchecked slice but has no snapshot. <url> — run objective-claim
  if it belongs to this objective.`
