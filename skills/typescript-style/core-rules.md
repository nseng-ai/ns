...
- **Preserve first-party import names for greppability.** `NS_TS_BAN_IMPORT_ALIAS_FOR_FIRST_PARTY`:
...
- **Do not hide import aliases in local constants.** `NS_TS_BAN_IMPORTED_BINDING_LOCAL_ALIAS`: do not
...
- **Do not launder types through `as unknown as T`.** `NS_TS_BAN_AS_UNKNOWN_AS`: a double-cast hides
