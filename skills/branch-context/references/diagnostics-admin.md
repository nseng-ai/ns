...
- Prefer `sdl branch-context exec load` for attached-plan loading.
- Prefer `sdl branch-context exec list/check` for read-only branch-context entry inspection.
...
- Scope filesystem inspection narrowly to the relevant repo key under `$XDG_STATE_HOME/sdl/enriched-plan/` (default `$HOME/.local/state/sdl/enriched-plan/`); do not perform broad home-directory traversals.
...
- Missing or unexpected attached plan key: inspect the `branch-context` namespace and rerun `sdl branch-context exec load <key>` only when a non-default key is explicitly intended.
...
```bash
sdl branch-context exec retarget-plan \
  --plan-file <path> \
