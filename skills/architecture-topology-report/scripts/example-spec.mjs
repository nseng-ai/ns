...
export default {
  ...
  verdict: {
    ...
    read:
      `The layering is <em>right</em> and now provably so: neutral infra sits at the bottom, capabilities sit above the
       ...
       <em>drift</em>, not distance — the SDK kernel imports a capability (<span class="font-mono text-sm">@ns/kernel → @ns/slot</span>),
  ...
  northStar: {
    ...
    bands: [
      { label: "Consumer", bg: "#1e293b", labelBg: "#0f172a", noteColor: "#cbd5e1", chipBorder: "#334155",
        packages: [{ name: "ccc" }], note: "highest-fan-out consumer (13) · holds no privileged tier · depends on providers through their /api only" },
      ...
      { label: "SDK", bg: "#e0e7ff", labelBg: "#6366f1", noteColor: "#4338ca", chipBorder: "#c7d2fe",
        packages: ["@ns/kernel"], note: "extension kernel/host + the thin author API of host primitives — must not import a capability" },
    ...
    offAxis:
      `Off this axis (not modeled as capabilities): the <span class="font-mono">@ns/pi</span> presentation host, standalone tools
  ...
  graphIntro:
    `Every runtime edge, live. Node <strong>area ∝ source LOC</strong>. The graph is now <strong>acyclic</strong> — there are no red
     ...
     <em>upward</em>: <span class="font-mono text-sm">ccc → @ns/pi</span> (a capability into the host) and
     <span class="font-mono text-sm">@ns/kernel → @ns/slot</span> (the SDK into a capability). Drag to pin, scroll to zoom,
  ...
  graphCaption:
    `The two heaviest nodes are the <span class="font-mono">@ns/pi</span> presentation host (11.7k LOC) and <span class="font-mono">ccc</span> (8.1k) —
     ...
     is the smallest node on purpose. The fan-in spine is <span class="font-mono">@ns/core</span> (28) and <span class="font-mono">@ns/clinkr</span> (19):
  ...
  scorecard: [
    ...
    { invariant: `Capability Kit owns the <span class="font-mono text-xs">ctx</span>→gateway adapter + result/error shapes; flow domain tested against <span class="font-mono text-xs">InMemoryGitGateway</span>`,
      status: "holds", statusKind: "holds",
      evidence: `<span class="font-mono text-xs">@ns/capability-kit</span> exists (180 LOC); consumed by <span class="font-mono text-xs">flow, handoff, objective</span>. <span class="font-mono text-xs">runPushCore</span>/<span class="font-mono text-xs">runCpCore</span> are gateway-injected with fake-gateway unit coverage.` },
    { invariant: `Capability API convention (<span class="font-mono text-xs">@ns/&lt;cap&gt;/api</span>), gateway-core rule, and deep-import/cycle guard documented + enforced`,
      status: "holds", statusKind: "holds",
    ...
    { invariant: `<span class="font-mono text-xs">ccc</span> depends on providers through their Capability APIs, not <span class="font-mono text-xs">@ns/pi</span>/<span class="font-mono text-xs">@ns/kernel</span> internals; no privileged tier`,
      status: "partial", statusKind: "partial",
      evidence: `Fan-out leader (13, highest in the graph) ✓ and reaches <span class="font-mono text-xs">objective/slot/branch-context/plans</span> via <span class="font-mono text-xs">/api</span>. But still imports <span class="font-mono text-xs">@ns/pi/commands/ack</span> (×9), <span class="font-mono text-xs">@ns/pi/terminal/presentation</span>, <span class="font-mono text-xs">@ns/pi/runtime/machine-envelope</span>, and <span class="font-mono text-xs">domain-primitives-transitional</span>.` },
    { invariant: `Declared package tiers validated; <strong>no hard tier violations</strong> (SDK must not depend on a Capability; a Capability must not depend on the Host)`,
      ...
      evidence: `Tier guard reports two hard edges: <span class="font-mono text-xs text-red-600">@ns/kernel → @ns/slot</span> (sdk-must-not-depend-on-capability) and <span class="font-mono text-xs text-red-600">@ns/ccc → @ns/pi</span> (capability-must-not-depend-on-host), plus 4 depends-on-transitional debt edges.` },
  ...
  findings: [
    { title: "The SDK kernel imports the slot capability directly",
      ...
      beforeAfter: {
        ...
        now: `flowchart TD
  kernel["@ns/kernel (SDK)"] -->|command-face| slot["@ns/slot (capability)"]
  classDef bad fill:#fee2e2,stroke:#dc2626,color:#991b1b;
  class kernel,slot bad;`,
        afterLabel: "after — mounted via the loader",
        after: `flowchart TD
  kernel["@ns/kernel (SDK)"] --> kit["capability-kit / loader"]
  slot["@ns/slot"] --> kit
  classDef ok fill:#dcfce7,stroke:#16a34a,color:#14532d;
  class kernel,slot,kit ok;`,
      },
      problem: `<span class="font-mono text-sm">@ns/kernel/cli.ts</span> imports <span class="font-mono text-sm">buildSlotCommandGroup</span> from <span class="font-mono text-sm">@ns/slot/command-face</span> and <span class="font-mono text-sm">createRealSlotContext</span> from <span class="font-mono text-sm">@ns/slot</span> — the kernel depends <em>up</em> on a capability, the one true inversion in the graph.`,
      solution: `Mount slot's command face through the same extension-discovery path every other capability uses, so <span class="font-mono text-sm">@ns/kernel</span> depends on the loader/kit, not on <span class="font-mono text-sm">@ns/slot</span>.`,
    ...
    { title: `<span class="font-mono text-base">ccc</span> still reaches up into the presentation host`,
      ...
      chipRows: [
        ...
        { label: "still reaches into:", items: [
          { text: "@ns/pi/commands/ack ×9", state: "pending" }, { text: "@ns/pi/terminal/presentation", state: "pending" },
          { text: "@ns/pi/runtime/machine-envelope", state: "pending" }, { text: "domain-primitives-transitional", state: "pending" }] },
      ],
      problem: `The model wants <span class="font-mono text-sm">ccc</span> to hold no privileged tier and reach providers through their Capability API only. It already does for migrated capabilities, but it imports 12 host internals from <span class="font-mono text-sm">@ns/pi</span> (command-ack, terminal presentation, machine envelope, skill expansion) — a capability depending on the host.`,
  ...
  class T pen;`,
      problem: `<span class="font-mono text-sm">@ns/domain-primitives-transitional</span> is the endgame's done-signal — it must delete to zero — yet 4 packages still import its primitives (checkpoint, pending-worktree, temp-files, text-generation).`,
    ...
    { title: `<span class="font-mono text-base">pr-address</span> ships an API but no command face`,
      ...
      problem: `The model says a capability <em>mandatorily</em> exposes a command face and adds <span class="font-mono text-sm">/api</span> only where a consumer needs it. <span class="font-mono text-sm">@ns/pr-address</span> exposes <em>only</em> <span class="font-mono text-sm">./api</span> — the inverse — and has zero runtime fan-in, so the API it ships is currently unconsumed.`,
  ...
  keystone: {
    title: `Convert <span class="font-mono text-xl text-emerald-300">ccc</span> into a clean consumer — sever its <span class="font-mono text-xl text-rose-300">@ns/pi</span> host edges.`,
    paras: [
      `With the graph now acyclic, the densest remaining knot is <span class="font-mono text-sm text-emerald-200">ccc</span>. It is the
       designated highest-fan-out consumer (13), yet it still imports 12 host internals from <span class="font-mono text-sm text-rose-200">@ns/pi</span>
      ...
      `It is the keystone because <span class="font-mono text-sm">ccc</span> is the only node that sits in three open invariants at once —
       ...
       deleting the transitional package). The smaller <span class="font-mono text-sm">@ns/kernel → @ns/slot</span> inversion is sharp but isolated;
       fix it alongside as a scoped loader-mount cut, but ccc is where the leverage is.`,
    ],
    sequence: [
      `1 · ccc off <span class="font-mono">@ns/pi</span> host internals`,
