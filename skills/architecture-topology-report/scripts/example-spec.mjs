...
export default {
  ...
  northStar: {
    ...
    bands: [
      ...
      { label: "Capabilities", bg: "#ecfdf5", labelBg: "#10b981", noteColor: "#047857", chipBorder: "#6ee7b7",
        packages: ["flow*", "handoff", "objective", "branch-context", "plans", "slot", "pr-address", "roaster", "retros"],
  ...
  scorecard: [
    ...
    { invariant: `All nine capabilities are above-SDK Capabilities (command face + gateway-injected core), each via a completed child Objective (flow excepted)`,
      ...
      evidence: `Done: <span class="font-mono text-xs">flow</span>(ref) + closed children <span class="font-mono text-xs">slot, branch-context, plans, objective, handoff</span>. Pending: <span class="font-mono text-xs">pr-address, roaster, retros</span> — no child Objective, no kit dependency, no gateway-injected core.` },
    { invariant: `<span class="font-mono text-xs">ccc</span> depends on providers through their Capability APIs, not <span class="font-mono text-xs">@nseng-ai/pi</span>/<span class="font-mono text-xs">@nseng-ai/kernel</span> internals; no privileged tier`,
  ...
  class kernel,slot,kit ok;`,
    ...
    { title: `<span class="font-mono text-base">ccc</span> still reaches up into the presentation host`,
      ...
      problem: `The model wants <span class="font-mono text-sm">ccc</span> to hold no privileged tier and reach providers through their Capability API only. It already does for migrated capabilities, but it imports 12 host internals from <span class="font-mono text-sm">@nseng-ai/pi</span> (command-ack, terminal presentation, machine envelope, skill expansion) — a capability depending on the host.`,
  ...
  keystone: {
    title: `Convert <span class="font-mono text-xl text-emerald-300">ccc</span> into a clean consumer — sever its <span class="font-mono text-xl text-rose-300">@nseng-ai/pi</span> host edges.`,
    paras: [
    ...
    sequence: [
      `1 · ccc off <span class="font-mono">@nseng-ai/pi</span> host internals`,
      `2 · isolate-and-cut <span class="font-mono">kernel → slot</span> via loader mount`,
      `3 · migrate pr-address / roaster / retros`,
