...
// ── 2. mechanical graph assembly ────────────────────────────────────────────
// Two datasets feed the in-report granularity toggle: the package view (runtime
// manifest edges — the default) and the topology-circle view (static TS import
// edges). Graph JSONs without circle facts render the package view only.
function validateTierOverrides(tierOverrides) {
  const invalidOverrides = Object.entries(tierOverrides)
    .filter(([, tier]) => !Object.hasOwn(TIERS, tier));
...
// Package-granularity view: one node per workspace package, runtime edges.
function assemblePackageGraph(analysis, tierOverrides = {}) {
  ...
  (analysis.cycles ?? []).forEach((scc, i) => scc.forEach((id) => sccIndex.set(id, i)));
  ...
  const nodes = ids.map((id) => {
    ...
    const declared = tierOverrides[id] ?? pkg.tier;
    if (!Object.hasOwn(TIERS, declared)) warnings.push(id);
  ...
  for (const [src, deps] of Object.entries(analysis.graph)) {
    for (const t of deps) {
      links.push({
        source: src,
        target: t,
        cycle: sccIndex.has(src) && sccIndex.get(src) === sccIndex.get(t),
      });
    }
  }
...
// Circle-granularity view: topology circles over static TS import edges.
function assembleGraph(analysis, tierOverrides = {}) {
  const circleFacts = analysis.topologyCircles;
  const edgeGraph = analysis.circleGraph ?? {};
  const cycleFacts = analysis.circleCycles ?? [];
  const ids = Object.keys(circleFacts).sort();
  const { fanOut, fanIn } = fanCounts(edgeGraph, ids);
  ...
  const colors = packageColorsForNodes(nodes);
  for (const node of nodes) {
    node.fill = colors[node.packageColorKey].fill;
    node.stroke = colors[node.packageColorKey].stroke;
  }
  const links = [];
  for (const [src, deps] of Object.entries(edgeGraph)) {
...
function GRAPH_RENDERER() {
  ...
  function build(viewKey) {
    ...
    // package view: fill = tier color; circle view: fill = enclosing package
...
function renderHeader(s, analysis) {
  ...
  <header class="space-y-5">
    ...
    <div class="flex flex-wrap items-center gap-x-5 gap-y-2 text-xs text-slate-600 border-t border-slate-200 pt-4">
      ...
      <span class="inline-flex items-center gap-1.5 text-slate-400">package view: fill = tier · circle view: fill = enclosing package</span>
    </div>
  </header>`;
}
...
async function main() {
  ...
  const circleView = analysis.topologyCircles ? assembleGraph(analysis, tierOverrides) : null;
