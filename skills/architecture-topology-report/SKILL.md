...
- `topologyCircles` / `circleGraph` — source topology circles discovered from root
  ...
  (nodes sized by circle LOC, placed in tier lanes, colored by enclosing package).
...
toggle — static import edges, node fill = enclosing package; node area ∝ LOC, tier lanes/filters,
