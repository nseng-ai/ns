...
| `tiers?`                       | `{ "<pkg>": "<tierId>" }`                                                                          | optional report-only overrides. Canonical tier ids: `capability capability-kit sdk transitional neutral-infra host capability-pi standalone-tool local-pi-tool`. Omit this field to use declared package tiers. Invalid override values fail fast. |
