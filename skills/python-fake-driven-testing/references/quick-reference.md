...
See `python-fake-driven-test-layout` for the canonical directory convention.
...
Per `python-fake-driven-test-layout`:
...
`src/<package>/gateways/<domain>/fake.py` — see `python-fake-driven-test-layout`
for the full layout.

Test data builders and helper utilities live next to the tests that use them
(per `pytest`); avoid a top-level `tests/helpers/` shed.
