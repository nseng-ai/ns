...
1. Draft a complete, self-contained Markdown plan for a fresh downstream session.
   ...
   - If implementation includes same-shape edits across multiple files, choose
     the execution mode explicitly: deterministic AST/codemod tooling for
     fitting TypeScript syntax work, `ts-morph-analyze` for AST inspection,
     precise edits for 1-4 files or semantic docs/spec changes, and
     `refactor-swarm` for 5+ file-local prose-aware or mixed code/docs/tests
     edits. Require final grep/equivalent stale-term checks for name or concept
     changes.
