...
description: Use when a user explicitly wants to write and save a Saved plan/source-branch plan file as an enriched plan for later branch-context attachment — "write a plan", "save an enriched plan", "prepare a plan for a fresh implementation session" — or to continue a Pi `/ns:plan:save` or `/ns:plan:grill-and-save` handoff.
---
