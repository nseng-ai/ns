...
- In this Graphite-backed repo, do not use generic `git push` / `gh pr create` examples to publish or update Graphite-tracked PR branches. Use `sdl flow submit` or `gt submit --no-interactive` so Graphite metadata stays synchronized.
