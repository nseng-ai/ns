...
9. **Re-derive `orientation.md` from the verified contract when one exists.** If the slug has an `orientation.md`, rewrite it against the verified contract: shrink or drop the `What you see now`/`Avoid` lines as the migration lands, and keep the durable `Direction`/`Getting to` lines accurate. If the verified contract shows the Objective has become cross-cutting and it lacks one, add `orientation.md` using the `objective` umbrella skill's format. Never close, so never drop an `orientation.md` here; the existing `git add .sdl/objectives/<slug>/` stages it.
