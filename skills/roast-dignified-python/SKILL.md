...
First read `reviews/dignified-python.md`, then apply that review definition exactly to the supplied diff or current branch changes in this same session. Stay read-only and keep findings grounded in the diff.
...
In Pi, the equivalent isolated runner surface is `roaster:run:dignified-python`.

For durable logging or publication after a same-session review, convert findings to `{ "findings": [...] }` and run:

```bash
roaster exec record-findings --review-key dignified-python --format json < findings.json
```
