...
- **Layers, bottom to top:** **Neutral Infra**, including the generally applicable `@nseng-ai/clinkr`, and the temporarily same-tier `@nseng-ai/ns-foundation` for shared ns-family infrastructure; the SDK (`@nseng-ai/sdk` plus its `sdk` subpackage); the **Extension Kit**; and the first-party **ns extensions** built on it. The ns-foundation tier mismatch is explicit deferred debt, not a redefinition of Neutral Infra.
- **ns-shaped gateways:** external-tool gateways live in ns-foundation when they are cohesive shared ns-family infrastructure, or as **Extension Kit** subpackages when they are first-party extension-building substrate. The former standalone **Capability Gateway Backend** tier is retired.
- **Current shared gateway homes:** `@nseng-ai/ns-foundation/exec` and `@nseng-ai/ns-foundation/git` are existing shared infrastructure doors, with the Extension Kit keeping the ctx→gateway adapter (`createNsGitGateway`). Moving any gateway still requires focused ownership and consumer-channel analysis.
...
- **Decision sources:** ADR 0009 holds extension layering, domain ownership, and the dependency-graph invariant. ADR 0019 holds the consumption classification and package-placement gate. ADR 0032 records the historical Foundation admission rule; ADR 0049 supersedes that placement rule for ns-foundation. ADR 0031 holds the point-system decision.
...
**Neutral Infra**:
The generally applicable infrastructure floor below the SDK, exemplified by `@nseng-ai/clinkr`. Neutral means independent of ns, not effect-free. `@nseng-ai/ns-foundation` still declares `ns.tier: "neutral-infra"` temporarily, but it is shared ns-family infrastructure; ADR 0049 records this mismatch as deferred taxonomy and classification debt rather than broadening Neutral Infra to include ns-specific concepts.
*Avoid*: pure floor, purity-defined neutral infra, "no real-world I/O" as the admission test, ns-foundation as proof that neutral includes ns-specific infrastructure, silent treatment of the temporary tier mismatch as resolved

**ns-foundation**:
The shared infrastructure and conventions package for the ns product family, `@nseng-ai/ns-foundation`, kept below workflow-specific behavior and permitted to depend on lower generally applicable infrastructure such as Clinkr. Independently valuable generic residents may later graduate outward through focused work.
*Avoid*: Foundation, generic-only package, Neutral Infra synonym, Extension Kit substitute, rename-time module redistribution

**Pure Utility**:
A deterministic transform with no I/O and no ns runtime knowledge. Pure utilities may live in ns-foundation when shared across the ns family, but purity alone does not determine package ownership or tier.
*Avoid*: gateway, host service, runtime harness, synonym for Neutral Infra

**Kit Gateway**:
The per-domain *seam* for an external tool, external protocol, or precise filesystem-backed gateway — its contract, fake/testing support, `ctx`→gateway adapter, and real adapter — owned at an **Extension Kit** subpackage (currently `@nseng-ai/extension-kit/<domain>`) when it is first-party ns extension-building substrate. Existing shared ns-family gateways may instead live in ns-foundation; moving one between owners requires explicit follow-up work.
*Avoid*: product extension domain, generic filesystem gateway, automatic home for every gateway, neutral-infra gateway
...
**Runtime Harness**:
Program boot code that creates or wires the vended API object and is never reached through `ctx`. Runtime harness code belongs in the SDK or a named CLI-runtime infrastructure home, not in ns-foundation by default.
...
**Package Tier**:
The declared architecture classification of a TypeScript workspace package, stored in its `package.json` at `ns.tier` and enforced by the TypeScript style guard. The canonical live tiers are `neutral-infra`, `sdk`, `extension-kit`, `extension`, `host`, `standalone-tool`, and `internal-tool`. Tier is orthogonal to **Release disposition**: it no longer projects onto any directory segment, and the guard rule that once did (`NS_TS_TIER_DIRECTORY_PROJECTION`) is retired (ADR 0045). A package lives in a single tier: `ns.tier` is the tier for the package and every declared **Subpackage**, `ns.subpackageTiers` overrides are rejected by the guard, and cross-package layering is enforced against the owning package's tier for every **Topology circle**. ns-foundation temporarily remains `neutral-infra` even though that tier's name and semantics do not fit its ns-specific role; ADR 0049 records taxonomy/classification repair as deferred debt. Hosts and tools are off-axis: hosts present/register/consume ns extensions, while tools may depend broadly without becoming part of the Extension Dependency Graph. The former `transitional` and `capability-gateway-backend` tiers are deleted, `capability-pi` is deleted (an extension's Pi presentation lives in its own `pi` subpackage), and `internal-pi-tool` is merged into `internal-tool`; do not reintroduce a live transitional/backend tier as a debt label, and do not add a `platform` tier for I/O-performing Neutral Infra.
...
**API-kind subpackage**:
A declared **Subpackage** with supported cross-package runtime exports, regardless of its name — a **Container package** may have several, each earning its rank from the inbound edge class it anchors, not source size (ADR 0023). ns-foundation's precise public doors (`exec`, `time`, …) are API-kind. The literal `api` subpackage is the required naming specialization for an **extension package API**, where extension consumer/provider rules apply. Private implementation features stay folders inside the owning API-kind subpackage.
