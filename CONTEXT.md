...
**Gateway**:
The canonical interface to an external or non-deterministic capability: process execution, Git, GitHub, filesystem, network, plus the system clock and timers. Gateway types are semantic and capability-shaped: they expose domain operations and values rather than raw substrate mechanisms or stdout for domain logic to parse. Domain logic lives above Gateways, consumes them through manual injection, and decides policy from their semantic results; fake the Gateway beneath domain logic, never the domain logic itself. Real adapters live at the edge and own subprocess, filesystem, HTTP, environment, and wire-format parsing. External-service boundaries carry the `Gateway` suffix (`GitGateway`, `GithubPrFeedbackGateway`); bare runtime primitives (`Clock`, `TimerScheduler`) are Gateways by category but named without the suffix, and incumbent generic names win absent confusion — foundation's exec seam is `CommandExecApi`, and `ExecGateway` is retired. Filesystem-backed Gateways must be domain-specific seams above raw filesystem operations, such as `ObjectiveStorageGateway` or plan-store storage, not a pure/shared `FileSystemGateway`. Avoid Gateways that are too high-level when selection or workflow policy belongs in domain logic. `typescript-fake-driven-testing` is the canonical source for the fuller mechanics; this definition must remain aligned with its Core model and Gateway style sections.
*Avoid*: port, generic service locator, dependency-injection bag, pure filesystem gateway, shared `FileSystemGateway`, substrate-shaped capability gateway, high-level policy gateway, `ExecGateway` (retired name), gateway as a synonym for every injected collaborator
