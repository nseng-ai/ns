...
The SDL extension stack, bottom to top: **Neutral Infra** below the SDK (`@sdl/core`, `@sdl/clinkr`, `@sdl/graphite`, `@sdl/brmem`), the SDK (`@sdl/sdl` kernel + `@sdl/sdl/sdk`), the **Extension Kit**, and the **Extensions** built on it. ADR 0012 holds the layering diagram, the rule that capability domain lives in extensions and never in the **Presentation Host** or kernel, and the rule that the SDK boundary is permeable downward only to concepts that prove general worth (opinionated patterns such as gateways stay above the SDK); these terms name its parts.

**Extension**:
A package that adds an SDL capability on top of the **Extension Kit**, exposing a **Command Face** and, where a sibling needs it, a **Peer API** over a gateway-injected **Domain Core**.
*Avoid*: plugin, built-in, bundled command

**First-party extension**:
An SDL-shipped, SDL-owned **Extension** (flow, objective, handoff, slot, branch-context, plans, pr-address, roaster, aretro, and **CCC**), as opposed to a third-party extension.
*Avoid*: built-in extension, bundled extension (reserve for packaging), core extension

**Extension Kit**:
The shared substrate (`@sdl/extension-kit`) that **Extensions** are built on — the `ctx`→**Gateway** adapter and shared result/error shapes; it holds no capability domain.
*Avoid*: extension framework, above-SDK substrate, extension-kit core

**Command Face**:
An **Extension**'s kernel-loaded CLI/Pi command contributions — the thin shell that converts `ctx`→**Gateways** and calls the **Domain Core**.
*Avoid*: CLI surface (alone), command layer

**Peer API**:
An **Extension**'s curated, typed in-process export at the required `@sdl/<cap>/api` subpath, consumed by sibling extensions (chiefly **CCC**) — never package roots or internals.
*Avoid*: public API, package-root export, internal subpath

**Domain Core**:
The gateway-injected **Domain logic** behind an **Extension**'s **Peer API** and **Command Face** that takes `GitGateway`-style **Gateways**, never raw `ctx`, so it is unit-testable with in-memory fakes.
...
**CCC**:
A **first-party extension** (the cmux command-and-control surface) that composes cmux, Graphite, and other first-party extensions through their **Peer APIs**; it depends on those extensions but holds no privileged tier or status.
