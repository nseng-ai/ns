...
async function loadWorktreeStatus(pi: ExtensionAPI, cwd: string, signal?: AbortSignal): Promise<WorktreeStatus> {
	const down = await loadDownBranch(pi, cwd, signal);
	const [brmem, up, commits, dirty] = await Promise.all([
	...
	return {
		brmem,
		gt: { down, up, commits, dirty },
	};
}

async function loadBrmemStatus(pi: ExtensionAPI, cwd: string, signal?: AbortSignal): Promise<string | undefined> {
...
async function loadDownBranch(pi: ExtensionAPI, cwd: string, signal?: AbortSignal): Promise<string> {
...
async function loadUpBranch(pi: ExtensionAPI, cwd: string, signal?: AbortSignal): Promise<string> {
	if (signal?.aborted) return "-";
...
async function loadDirty(pi: ExtensionAPI, cwd: string, signal?: AbortSignal): Promise<"yes" | "no"> {
...
function formatWorktreeStatus(status: WorktreeStatus): string[] {
	const lines: string[] = [];
...
function formatGtStatus(status: GtStatus): string {
...
function dimLinesWidget(lines: string[]) {
	return (_tui: unknown, theme: { fg: (color: "dim", value: string) => string }) => ({
