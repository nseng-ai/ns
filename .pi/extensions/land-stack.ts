...
async function runMergeLoop(pi: ExtensionAPI, ctx: ExtensionCommandContext, plan: LandingPlan, landed: LandedPr[]): Promise<void> {
	...
	for (let index = 0; index < stack.landingBranches.length; index += 1) {
		...
		setStatus(ctx, "refreshing trunk...");
		const getArgs = ["get", stack.trunk, "--downstack", "--no-restack", "--no-checkout", "--force", "--no-interactive"];
...
function parseGtStackOutput(stdout: string): ParsedStackOutput | undefined {
...
function validateInitialPrPreflight(branchPlans: BranchPlan[], trunk: string): void {
...
function validateStrictMergeGate(input: { branch: string; localSha: string; pr: PullRequestSnapshot; trunk: string }): void {
...
function validateOpenPrBasics(input: { branch: string; localSha: string; pr: PullRequestSnapshot }): void {
...
function isManagedSlotPath(path: string): boolean {
	const normalized = path.replaceAll("\\", "/");
	return normalized.includes("/.slots/repos/") && /\/worktrees\/slot-[^/]+(?:\/|$)/.test(normalized);
}

function slotNameFromPath(path: string): string | undefined {
	const normalized = path.replaceAll("\\", "/");
...
function formatPlan(plan: LandingPlan): string {
	...
	lines.push(
		...
		`  - gt get ${stack.trunk} --downstack --no-restack --no-checkout --force --no-interactive`,
...
function formatFailure(error: unknown, landed: LandedPr[]): string {
