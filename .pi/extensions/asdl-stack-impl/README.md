...
- `/stack-impl [--replace] <local-plan-file-or-branch-memory-key>` stores or loads a canonical stack plan, starts or resumes the first incomplete slice branch, writes/validates its pointer ledger, and opens a fresh Pi session with a kickoff prompt.
...
- `/stack-impl-status <local-plan-file-or-branch-memory-key>` shows plan, branch, ledger, handoff, worktree, git, and Graphite diagnostics. With no argument, it tries to infer the plan from the current branch's `stack-impls` ledger.
- `/stack-impl-closeout <tool-call-id>` is an internal follow-up queued by `stack_impl_slice_done`; it stores the agent-drafted completion handoff.
...
4. The extension detects the marker, validates the plan, presents a controlled confirmation, stores the plan in Branch Memory, and queues `/objective-stack-impl <objective-slug>` as a follow-up on the user's behalf. The follow-up validates and loads the Branch Memory plan, then reuses the normal stack-impl orchestration.
...
Branch: `asdl-stack-impl-extension/plan-storage`
```
...
- `stack-impls`: branch-local pointer ledgers.
- `session-artifacts`: completion handoffs.

Keys:

- Plan key: `<objective>.md` in `stack-plans` on the plan branch.
- Ledger key: `<objective>/<escaped-branch>.md` in `stack-impls` on the slice branch.
...
Use it only after code, tests, Objective update, Graphite commit/amend, and handoff draft are ready. It queues `/stack-impl-closeout <tool-call-id>`, which verifies the current branch ledger and plan hash before storing the handoff under the derived `session-artifacts` key.

`stack_impl_slice_blocked` accepts:
...
`/stack-impl` resumes by recomputing the first planned branch without a derived completion handoff. If that branch already exists and has a valid ledger matching the canonical plan hash, the extension checks it out and starts a fresh session instead of recreating it. Plan/ledger hash drift fails closed.

`/stack-impl-status` reports:
...
- `stack_impl_slice_done` is trusted for v1; mechanical verification of Objective updates, validation reruns, changed files, and clean worktree remains parked.
- Pending closeout payloads are in memory. If the runtime reloads before `/stack-impl-closeout`, call `stack_impl_slice_done` again.
- Branch rename/restack repair is fail-closed; use `/stack-impl-status` diagnostics and repair manually.
