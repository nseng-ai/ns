...
function usage(): string {
	return [
		...
		"Default iterations: 1. Submission requires --submit. Submit uses ns flow submit --no-restack; --dry-run never commits or submits.",
	].join("\n");
}
...
function objectiveDirectory(objective: string): string {
	if (objective.startsWith(".ns/objectives/")) return objective;
	return path.join(".ns", "objectives", objective);
}
...
- Create the implementation branch with the repo's branch-context Graphite creation path: use \`ns branch-context exec from-plan --branch-creation graphite ...\` or \`/ns:branch-context:from-plan --graphite ...\`.
...
async function verifyAfterChild(options: VerifyAfterChildOptions): Promise<RepoChangeFacts> {
	...
	const nonObjectiveChanges = facts.changedFiles.some((file) => !file.startsWith(".ns/objectives/"));
...
async function commitAndMaybeSubmit(options: CommitAndMaybeSubmitOptions): Promise<{ commitOutput: string; submitOutput?: string; recoveryNotes: string[] }> {
	...
	try {
		const submitOutput = await execChecked({ env, command: "ns", args: ["flow", "submit", "--no-restack"] });
