...
const { requireEffectiveSkillBlock, requireEffectiveSkillSource } =
...
async function runJustThenInvokeSkill(
	...
	let skillSource: ReturnType<typeof requireEffectiveSkillSource>;
	try {
		skillSource = requireEffectiveSkillSource(ctx, SKILL_NAME);
	} catch (error) {
		if (ctx.hasUI) {
	...
	let skill: Awaited<ReturnType<typeof requireEffectiveSkillBlock>>;
	try {
		skill = await requireEffectiveSkillBlock(skillSource);
	} catch (error) {
		if (ctx.hasUI) {
			ctx.ui.notify(error instanceof Error ? error.message : String(error), "error");
		}
