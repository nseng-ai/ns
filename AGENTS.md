...
- Pre-launch, user-facing product documentation lives in package `README`s — treat them as the canonical install/quickstart/usage surface.
...
- Prefer tested CLI commands over harness-specific custom tools for deterministic, portable operations. Add a custom tool only when the harness integration provides a necessary capability that a CLI cannot provide. Keep the agent tool surface as small as practical because too many tools make tool selection harder and can confuse agents.
...
- Keep code or other authoritative ground truth and `CONTEXT.md` synchronized. Do not update `CONTEXT.md` ahead of implementation; during planning or design, retain proposed vocabulary in the plan or discussion. When ground truth changes, update the relevant `CONTEXT.md` in the same change. A documentation-only edit is appropriate only to bring stale glossary language back into sync with already-existing ground truth (for example paths, counts, package names, vocabulary, or `Avoid` aliases). If the drift is broad, ambiguous, or decision-bearing, record/report it instead of silently widening the work.
...
- This repo uses Graphite (`gt`) as the default tool for branch and PR workflow; for branch creation, commits/amends, PR submit/update, and stack navigation/reshaping, load `skills/internal/code/code-graphite/SKILL.md` if `code-graphite` is absent from the available-skill inventory.
...
- For GitHub backend work via GraphQL, REST, or `gh`, load `skills/internal/code/code-gh/SKILL.md` if `code-gh` is absent from the available-skill inventory.
