...
**`--help` MUST come before any `npx skills` subcommand, never after it.** Subcommands like `update`, `add`, and `remove` execute their side effects even when `--help` is appended (e.g., `npx skills update --help` actually performs the update instead of printing help). Use `npx skills --help` (no subcommand) to discover commands and flags. Do not invoke `npx skills <subcommand> --help` to "see what it does" — read the `skill-management` skill or ask the user instead. Tracked upstream as [vercel-labs/skills#960](https://github.com/vercel-labs/skills/issues/960); remove this warning once that issue is fixed and a release with the fix is available.
