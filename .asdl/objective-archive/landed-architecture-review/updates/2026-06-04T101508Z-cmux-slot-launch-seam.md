...
This improves Locality for future changes to slot checkout, cmux workspace command arguments, worktree descriptions, and workspace-open failure formatting. It also reduces the risk that the plan-dispatch and branch-open workspace commands drift when cmux launch behavior changes.

## Follow-Ups

- Continue the Objective with the Pi CLI command lifecycle cluster next.
- If the plan-dispatch workspace command remains too large in later work, consider a separate planned-branch creation/attachment seam around Graphite branch creation and Branch Memory plan storage rather than folding that into the cmux slot-launch seam.
