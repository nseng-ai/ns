...
Reusable session parsing and analysis should live in `asdl-core`, not inside `asdl-retro`. The `asdl-retro` package remains the standalone/plugin CLI boundary for `branch-retro`, while the provider-neutral session source/query interfaces, normalized parser models, Pi JSONL adapter, and deterministic analysis helpers are shared core library code.
...
This shifts roadmap PR 2 from an `asdl-retro`-local parser to a reusable `asdl-core` session library. Later `branch-retro exec collect-evidence` work should consume that shared library rather than owning provider-specific parsing or aggregation logic.
...
- Keep `asdl-retro` thin when `collect-evidence` is added; it should orchestrate CLI context and render the evidence envelope.
