...
- Minimal, need-driven v1 feature set (design settled in the 2026-06-10 grilling; see the corresponding Semantic Update):
  ...
  - Per-command legacy machine-output escape hatch, deprecated from birth: `legacyMachine?: (exit: ClinkrExit<T>) => { body, exitCode }`. When present, `--format json` routes the whole exit union through it so migrated commands keep their current `{"success": ...}` shapes and exit-code semantics; usage-error and crash channels adopt clinkr semantics immediately. Kill tracked in the umbrella's `migration-debt.md`.
  ...
  - Generated help at every level, replacing all hardcoded help functions. Both `--flag value` and `--flag=value` syntaxes work everywhere (commander native behavior).
...
- Commander's programmatic API can host schema-first generation (building options/arguments from Zod schema fields) without fighting the framework; Next.js's commander adoption validates commander at scale but not this generation pattern specifically. De-risked (2026-06-10, v1 build): generation worked without fighting commander — `exitOverride()`/`configureOutput()` contain it, `addCommand(cmd, {hidden})` covers hidden subgroups, `Option.attributeName()` gives the kebab→camel reverse map, and zod stays the sole required/default enforcer (commander declares everything optional, keeping the usage-error channel uniform and `--json-schema` eager). One click-parity divergence needed absorbing: commander errors a bare group invocation to stderr/exit 1, so clinkr gives group nodes an action that prints help to stdout and exits 0 (click behavior); like click, there is no `help` subcommand.
...
- Schema-first parameter generation could overfit to the first migrated CLI; hardening across all four migrations is the mitigation, with `pr-address` (fallback dispatch) as the remaining stress case. Partially de-risked: the `asdl-dev` mixed `--flag=value` / `--flag value` syntax concern dissolved — commander supports both natively, so clinkr offers both globally.
