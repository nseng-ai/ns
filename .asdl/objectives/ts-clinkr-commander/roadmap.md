...
- [x] Pin current CLI behavior with scenario tests where coverage is missing.
  ...
  - Suites written for all four CLIs (PR #1223): help/version bytes, unknown-command/exec dispatch, inline `--flag=value` quirks, duplicate-flag precedence, parse-failure channels, and exact legacy `--format json` envelope bytes (including pr-address's indent-2 `ensure_ascii` formatting and machine-envelope `--format` detection rules).
  - Evidence: the stale `--runtime` help-byte fixtures across `plans`, `planned-branch`, `asdl-dev`, and `pr-address` were refreshed, and the four package scenario suites passed together.
- [x] Migrate `@asdl/plans` to clinkr.
  - First and smallest migration completed: `plans` now builds a `ClinkrGroup<PlansCliContext>` with schema-derived `list`, hidden `exec write`, and hidden `exec resolve` commands; its hand-rolled parser and hardcoded help functions are gone.
  - Feed-forward clinkr corrections landed with tests: root `--version`, root `--runtime`, compact `legacyMachine` serialization for byte-exact legacy JSON, no generated `help` subcommand, and pre-parse bare-group help handling that preserves commander's unknown-command errors.
  - The migrated CLI preserves byte-exact compact `--format json` success/domain-failure bodies and human success output; accepted clinkr surface divergences are pinned in the `plans` scenario suite and recorded in the umbrella migration-debt ledger.
