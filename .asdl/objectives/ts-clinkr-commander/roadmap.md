...
- [~] Pin current CLI behavior with scenario tests where coverage is missing.
  ...
  - Blocker before `[x]`: the help-byte fixtures were captured against pre-`--runtime` help text even though the `--runtime` diagnostic option already exists on the branch's ancestry; 15 tests fail across all four packages (stale `Usage:`/options-block fixtures). Refresh the fixtures to the current help bytes — `--runtime` is intended CLI surface, not a regression. See the 2026-06-10 pinning Semantic Update.
