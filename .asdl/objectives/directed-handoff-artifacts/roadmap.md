...
- [x] Rework save/load user-facing copy so normal descriptions, notifications, injected prompts, and success output say "handoff" rather than exposing Branch Memory storage details.
...
- [x] Rename and update Codex/Claude skill surfaces so non-Pi agents can save and load handoffs using non-`brmem` handoff names while still using low-level `brmem` only as storage/recovery machinery.
