...
- [x] Update Objective skills and docs to delegate deterministic mechanics for the two shipped commands.
  - Evidence: candidate listing (`objective exec list --format md`) and record reading (`objective exec read-objective <slug> --format md`) delegations from PR #467 remain in place across the five Objective skills and `docs/objective-system.md`. The umbrella skill and canonical doc describe shipped versus future CLI responsibilities and keep the "do not parse Markdown headings, roadmap checkboxes, or prose meaning in CLI code" guard explicit.
  - Descope cleanup landed on master via `finish-objetict-pushdown`: `skills/objective/SKILL.md` Tracking Gate paragraph and Non-goals CLI inventory no longer mention `objective exec tracking-gate-facts`; `docs/objective-system.md` operations summary lists `(list, read-objective)`, the `objective-next` section keeps only closed-record filtering under Shipped CLI and moves Tracking Gate items back under Future CLI pushdown candidates, the Tracking Gate section restores "deterministic git comparison ... left as future CLI work" wording, and the Future CLI Pushdown Principle list flips the changed-path and Tracking Gate items back to `_(future.)_`.
- [x] Validate the reduced steelthread (two commands only).
  - Evidence: `uv run pytest packages/asdl-objectives/tests tests/scenario/test_plugins.py` reports 30 passed and `just` (ruff, ruff format, dprint, ty, `uv run pytest -n auto --ignore-glob='*/integration/*'` with 1197 passed) is green on `finish-objetict-pushdown`.
  - The 9 `tracking-gate-facts`-coupled scenario tests from `598105c8` were already removed from master via PR #468; this verification confirms the reduced steelthread (two commands) holds against the updated skill and doc surface.
