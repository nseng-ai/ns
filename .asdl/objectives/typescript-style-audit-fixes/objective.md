...
- Broad conversion from object-literal `type` aliases to `interface` is acceptable despite churn, provided behavior and public exports remain compatible. Re-confirmed on a second package (`ts/packages/asdl-dev`, branch `asdl-dev-stack-omnibus-roaster-fixes`): behavior and public exports preserved with gates green. The final closeout audit found and fixed a small set of later direct object-shape alias drift in `asdl-dev`, `ccc`, and `pi-extensions` source/tests without changing behavior.
- Most JSON/runtime casts can be replaced with small local guards without needing a large schema library. Confirmed for the CLI/process and runner/grill boundary slices: `land`, `land-stack/pr-facts`, `worktree-status`, `runner-subagent/json-events`, `runner-subagent/subagent-runtime`, and `grill-ui/inline-ui` were hardened with small local guards and normalizers, with no schema dependency added. The final closeout audit applied the same pattern to the last broad JSON parse casts in test helpers.
...
- Some existing public exported type aliases needed compatibility-preserving treatment; the remaining scan hits are unions, function-style compositions, or the documented `RunnerSubagentOptions` object-base-plus-mutually-exclusive-union alias rather than plain object-shape contracts.
- Dependency-injection over-abstraction is now an accepted closeout risk rather than an open design blocker: `asdl-dev/src/checkpoint-flow.ts`, `pi-extensions/src/pr-feedback-watch.ts`, and explicit CLI/command-runner/gateway modules own their direct runtime interactions. The final audit did not find a narrower remaining domain seam outside the injected `pathExists`, `normalizePath`, and `readTextFile` collaborators.

## Open Questions

Resolved or accepted at closeout:

- `erasableSyntaxOnly` or another compiler/lint guard remains a possible future supplement only if the Roaster reviewer plus root agent guidance prove insufficient; it is not required to complete this Objective.
- No plain exported object-shape alias remains that needs a compatibility alias. Remaining alias scan hits are unions, function-style compositions, or the documented `RunnerSubagentOptions` shape/union composition.
- Direct Node/Pi globals are either adapter-owned or exposed through explicit injected collaborators; the accepted boundaries are the compliance line for this Objective.
- Machine-envelope and CLI JSON parsing should stay as local guards for now. Shared decoder helpers remain parked unless repeated local guard duplication becomes a real maintenance problem.

## Closure

Outcome: completed.

The TypeScript style audit remediation is complete across the existing TypeScript surface covered by this Objective. Hard non-erasable constructs and ordinary explicit `any` were removed, object-shape contracts were converted to interfaces while preserving unions and function aliases as `type`, untrusted JSON/runtime boundaries now narrow from `unknown`, expected async/system failures were moved toward discriminated returned data at the relevant branch points, and remaining direct Node/Pi runtime usage is either adapter-owned or reachable through explicit injected collaborators.

Final closeout evidence: the audit loop fixed the last small object-shape alias drift in `asdl-dev`, `ccc`, and `pi-extensions`, and tightened the remaining broad JSON parse casts in test helpers to parse as `unknown` before local narrowing. Focused scans found no ordinary explicit `any`, no broad JSON parse casts, no unaccepted object-shape alias hits, and no narrower direct-runtime domain seams beyond the accepted adapter/default-collaborator boundaries. Targeted package checks/tests, `just ts-check`, `just ts-test`, and `just dprint-check` passed. Remaining caveats are intentionally accepted: `RunnerSubagentOptions` stays a `type` because it composes a base object with mutually exclusive option variants; two double-cast scan hits remain as test-only malformed-shape fixtures; direct runtime modules such as CLIs, command runners, gateways, extension controllers, and filesystem watchers own their adapter roles; and additional compiler/lint guardrails remain optional future hardening rather than required Objective work.
