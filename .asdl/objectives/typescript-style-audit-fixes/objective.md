...
- Reworking throw-based APIs into returned data can cascade through tests and command handlers; careless conversion could weaken user-facing error messages. Partially de-risked on the first two failure-as-data slices: the brmem/planned-branch conversion (`runFirstAvailableBrmemCommand` → `FirstAvailableBrmemCommandRun`, discriminated `BrmemRun` through `attached-plan`/`plan-persistence`/`planned-branch-creation`) and the `asdl-dev` submit-gateway conversion (presentation strings → typed `SubmitSemanticFailureCause` / `CurrentPrVerificationFailureCause` causes) both preserved user-facing messages by moving English prose into formatter helpers and updated tests to assert on returned `type`/`cause` fields rather than caught throws, with gates green. The cascade risk remains open for the still-untouched `land-stack`, `handoff`/`objective` parsing, and runner runtime parsing slices.
