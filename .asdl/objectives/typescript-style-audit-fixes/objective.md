...
- Reworking throw-based APIs into returned data can cascade through tests and command handlers; careless conversion could weaken user-facing error messages. Partially de-risked across three failure-as-data slices: the brmem/planned-branch conversion (`runFirstAvailableBrmemCommand` → `FirstAvailableBrmemCommandRun`, discriminated `BrmemRun` through `attached-plan`/`plan-persistence`/`planned-branch-creation`), the `asdl-dev` submit-gateway conversion (presentation strings → typed `SubmitSemanticFailureCause` / `CurrentPrVerificationFailureCause` causes), and the `land-stack` conversion (`LandStackFailure` / `LandStackResult<T>` through preflight, PR facts, worktrees, planning, mutation, merge-loop, presentation, and command streaming) all preserved user-facing messages by moving English prose into formatter helpers and updated tests to assert on returned `type`/failure fields rather than caught throws, with gates green. The cascade risk remains open for the still-untouched `handoff`/`objective` parsing and runner runtime parsing slices.
