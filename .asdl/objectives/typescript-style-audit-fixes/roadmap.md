...
- [~] Harden untyped JSON, tool, and runtime boundaries with `unknown` plus guards or decoders.
  Evidence: the CLI/process JSON parse sites are hardened — `land.ts`, `land-stack/pr-facts.ts`, and `worktree-status.ts` now parse `gh pr view` / `brmem list` output into `unknown` and narrow it with local guards before property use, removing the `as PullRequestView`, `as Partial<PullRequestSnapshot>`, and `as BrmemEntry[]` casts and the `Boolean(raw.isDraft)` coercion. Validation: `bun run --cwd ts/packages/pi-extensions check`, `bun run --cwd ts/packages/pi-extensions test`, `just ts-check`, and `just ts-test` passed. Remaining for this row: runner/grill runtime inputs (`runner-subagent/json-events.ts`, `runner-subagent/subagent-runtime.ts`, `grill-ui/inline-ui.ts` dynamic import) still rely on broad runtime casts and are deferred to a later slice.
