...
- Sibling TypeScript consumers can continue shelling out to the `brmem` CLI through the shared `@asdl/core/brmem-cli` launcher until their own follow-up migration; native brmem does not need to rewire them to be the default. PR #1466 (`branch-memory-dispatch-prompt-delivery`) corroborates this boundary by storing `ccc` dispatch prompt payloads in Branch Memory through the CLI launcher rather than a direct native-library dependency. PR #1473 centralized neutral shell-out command handling and `put` envelope parsing for those consumers without changing this assumption or replacing CLI shell-out with the native TypeScript brmem library. After public cutover, that launcher resolves only PATH `brmem`, so consumers use the same TypeScript-backed public shim rather than Python `.venv` or `uv run brmem` fallbacks. PR #1495 extends the same boundary to handoff launch verification by routing `checkHandoffExists` through `runAvailableBrmemCommand` and shared machine-envelope parsing rather than introducing a native brmem dependency.

Risks:

- Git-ref plumbing parity was the central migration risk: branch encoding (`/` → `---`), snapshot-tree construction, `commit-tree`/`update-ref` semantics, and Entry Locator formatting had to match Python exactly while both implementations existed. Mitigation is now historical: cross-language round-trip parity probes ran before the Python reference was deleted, and the TypeScript real/fake/scenario tests plus public shim smoke are the active regression evidence.
...
- `packages/brmem` is now deleted from active paths after parity, distribution, and skill-doc evidence landed. Rollback/reference evidence is available in git history at commit `44c3e9992b424c4b174ccaeb9f4567bb8f611dc1`, the last pre-deletion commit containing the Python package.
- Scope creep into direct native-library consumer migration (`@asdl/core` launcher, branch-context gateway, `ccc` dispatch prompt storage, handoff launch verification, or similar callers) would blur this Objective's boundary; that work is deliberately deferred. CLI-shellout consumer integrations are acceptable evidence for public `brmem` usability, but they do not complete the native brmem cutover rows. PR #1473 and PR #1495 de-risk narrower duplication issues by sharing command wrapper and envelope parsing mechanics across shell-out consumers, but they do not complete or start the native-consumer migration parked below.
- The temporary TypeScript CI Python/uv dependency was removed with the Python reference and cross-language parity oracle. The `typescript` job now installs only Node/pnpm dependencies.
