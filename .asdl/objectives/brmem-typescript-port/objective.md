...
- Sibling TypeScript consumers can continue shelling out to the `brmem` CLI through the shared `@asdl/core/brmem-cli` launcher until their own follow-up migration; native brmem does not need to rewire them to be the default. PR #1466 (`branch-memory-dispatch-prompt-delivery`) corroborates this boundary by storing `ccc` dispatch prompt payloads in Branch Memory through the CLI launcher rather than a direct native-library dependency. PR #1473 centralized neutral shell-out command handling and `put` envelope parsing for those consumers without changing this assumption or replacing CLI shell-out with the native TypeScript brmem library.
