...
- Keeping the Python fallback too long undermines the single-language goal and leaves two implementations writing the same refs; the retirement phase should be short and explicit once parity is proven. Public invocation and generic shell-out defaults are now TypeScript-only; the remaining Python fallback scope is the checked-in reference/parity oracle until the retirement row deletes it from active paths.
