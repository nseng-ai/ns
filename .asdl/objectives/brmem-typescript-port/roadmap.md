...
- Migrating TypeScript consumers (`@asdl/core/brmem-cli.ts` launcher, `branch-context/brmem-gateway.ts`, `ccc` dispatch prompt storage, handoff launch verification, and similar callers) from CLI shell-out onto the native brmem library — deliberate follow-up outside this Objective; revisit once native brmem is TS-default. PR #1466 demonstrates `ccc` can consume Branch Memory through the shared CLI launcher boundary, but it does not move this parked direct-library migration into the active Objective scope. PR #1473 only centralized neutral shell-out command handling and `brmem put` parsing for existing CLI-backed consumers; PR #1495 similarly routes handoff launch verification through the shared brmem command runner and shared envelope parser. Neither replaces those consumers with native brmem library calls.
