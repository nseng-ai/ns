...
- [x] Categorize the remaining repo-owned workflow command families before closure: planned branches (`/write-plan`, `/create-planned-branch`, `/impl-planned-branch`) are retained as the Pi planning-layer sequence; Branch Memory handoffs (`/brmem-handoff`, `/brmem-pickup-handoff`) are retained as Pi frontends over skill/CLI-centered workflows; branch retrospectives remain intentionally skill/CLI-centered around `/skill:branch-retro` and `aretro exec collect-evidence`.
