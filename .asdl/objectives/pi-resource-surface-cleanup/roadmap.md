...
- [ ] Audit first-party skills cluster-by-cluster and assign dispositions, starting with the highest-confusion clusters: Objective/prototype/standing runners, handoff and Branch Memory, branch retrospective and `aretro`, dev/source-control/GitHub/Graphite, PR-address/review, Pi UI/internal helpers, planned-branch commands, and command-wrapper skills.
- [ ] Apply low-risk first-party skill cleanup found by the audit: fix stale H1s and `Original description` scaffolding, tighten trigger descriptions, normalize command-skill frontmatter, add or remove internal metadata where appropriate, settle `PENDING_REGEN` lock entries, and split or push down oversized procedural bodies when the win is clear.
