...
- [x] Categorize the remaining repo-owned workflow command families before closure: planned branches (`/write-plan`, `/create-planned-branch`, `/impl-planned-branch`) are retained as the Pi planning-layer sequence; handoff/pickup is carved out to the follow-up Objective `directed-handoff-artifacts` to make saved handoff artifacts the public UX and Branch Memory an implementation detail; branch retrospectives remain intentionally skill/CLI-centered around `/skill:branch-retro` and `aretro exec collect-evidence`.
...
- [x] Re-run fresh Pi RPC command inventory after the workflow-family categorization slice and confirm the dispositions did not require command renames or legacy aliases.
...
- [x] Run docs-only validation for the workflow-family categorization slice with `just dprint-check` and `git diff --check`.
