...
- Collapsing the four asdl-reviewer gateways may erase a seam that a future harness or environment variant actually wants. _Status_: PR #486 now tests this risk by replacing the four gateways with a `review_environment` seam backed by real and fake adapters; not fully de-risked until review/merge confirms the new seam still preserves the useful variation point. _Mitigation_: keep the workflow thin over the composite environment interface and re-litigate the two-adapter rule if a future harness/environment variant needs a seam that disappeared.
