...
Seven candidates surfaced across the architecture-deepening passes:
...
6. **asdl-slots lifecycle package split** — after lifecycle became the coherent state-machine owner, the 719-line module crossed the threshold where the deeper shape was a package with operation-specific submodules (`outcomes.py`, `pool.py`, `checkout.py`, `free.py`, `gc.py`) rather than one oversized file.
7. **review environment seam split** — after harness invocation moved behind `HarnessRuntime`, the composite `ReviewEnvironmentGateway` bundled unrelated fake shapes. The deeper shape is `ReviewCatalogGateway`, `LocalDiffGateway`, and direct `HarnessRuntime` injection into `Workflow`.

Roadmap was an **open list** while active: deepening one candidate could surface adjacent shallowness, and new rows could be added to `## Work` with a deletion-test argument recorded in this `## Scope` section.
...
Both architecture-deepening rounds reached a definite shipped state. No candidates were parked-with-reason or rejected-with-ADR, and no additional open-list candidates remained when the Objective closed.

Round 1 shipped the original five candidates:

- `gateway_access.py` pass-throughs in `brmem/` and `asdl-pr-address/` were deleted; op call sites read fields directly from `load_typed_context(...)`; brmem branch resolution uses `Ensure.ideal_state(...)` on canonical `DetachedHead` / `GitCommandFailure`.
- asdl-slots `checkout`, `init`, `resize`, `free`, and `gc` route through `asdl_slots.lifecycle` semantic entry points returning `Slot<Op>Outcome | SlotLifecycleFailure`; `lifecycle` became the sole importer of the init/resize planning helpers, and the standalone `gc` module disappeared. `slot list`, `slot goto`, and selector-specific reads in `slot free` intentionally remain thin inventory/selection seams until a second caller justifies a lifecycle query API.
- asdl-reviewer's four thin gateways were replaced by one `review_environment` seam with real and fake adapters; workflow, CLI, gateway, scenario, and plugin tests target that interface.
- clinkr operation registration moved onto `ClinkrGroup._register_operation(...)`; the standalone params bridge and direct tests were deleted after behavior coverage moved through registered commands; public decorator/group APIs stayed unchanged, and the full `just` gate passed (`1302 passed`).
- asdl-reviewer harness invocation unified behind `asdl_reviewer.harness.invocation.HarnessRuntime`, exposed to workflow as `list_harnesses()` / `run_review(HarnessReviewRequest)`. The old adapter, registry, prompting module, and `harness/claude/` package were deleted. PR #502 and targeted reviewer/plugin validation (`135 passed`) plus full `just` (`1285 passed`) are the recorded evidence.

Round 2 shipped the two follow-up shapes surfaced by the independent assessment of the Round 1 stack:

- `asdl_slots/lifecycle.py` split into a five-file `lifecycle/` package. CLI handlers and tests now import operations from canonical submodules and shared outcome types from `asdl_slots.lifecycle.outcomes`. The optional test-layout cleanup remains a non-blocking follow-up.
- `ReviewEnvironmentGateway` split into `ReviewCatalogGateway` and `LocalDiffGateway`, while `HarnessRuntime` is injected directly into `Workflow` and `ReviewerCliContext`. Workflow, CLI, gateway, scenario, and plugin tests now target the three-dependency shape.

Residual risks and follow-ups are outside this closed Objective:

- If a second harness or review-environment variant appears, re-litigate the two-adapter rule against `HarnessRuntime` rather than pre-emptively reviving a harness-registry shape.
...
- A second-pass architecture review for `asdl-objectives` and `asdl-dispatcher` was deferred. Open a fresh Objective if those packages warrant their own deepening pass.
- The question of whether future mixed-outcome deepening Objectives need an explicit resolution-mode column remains unresolved because this Objective closed cleanly without it.
