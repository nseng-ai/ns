...
- [~] **Collapse asdl-reviewer gateways into one review-environment seam** — replace `harness_detection`, `local_diff`, `review_definition`, `review_execution` with a composite gateway. Workflow becomes a thin caller of one interface. Real + fake adapters live at the new seam. _In progress: PR #486 introduces `review_environment`, deletes the four thin gateway packages, and refactors workflow/CLI/tests around real + fake adapters. Keep `[~]` until the PR lands and the tests target the merged interface._
