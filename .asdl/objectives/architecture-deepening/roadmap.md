...
- [ ] **Deepen `brmem/gateway_access.py`** — fold the three one-line accessors and the error translator into `BrmemCliContext` (or wherever the CLI envelope already lives) and delete the file. *Start here — smallest, cleanest deletion-test signal, exercises the deepening pattern before touching busier packages.*
