...
- [~] **Consolidate asdl-slots slot lifecycle** — introduce a single coordinating module that owns repo discovery → inventory → planning → execution. Intermediate dataclasses become implementation details; CLI commands call semantic operations (`checkout`, `init`, `resize`, `free`, `gc`). _Status: `checkout`, `init`, `resize`, and `free` consolidated — `lifecycle.checkout_branch` / `checkout_current` / `initialize_pool` / `resize_pool` / `free_slots` now own the mutating slot workflows and return `Slot<Op>Outcome | SlotLifecycleFailure` discriminated unions; the corresponding CLI ops are translation/rendering or selector-resolution shells. `lifecycle` is the sole importer of `build_init_plan` / `build_resize_plan`. `free` still performs selector-specific inventory reads in the CLI wrappers, while `gc`, `goto`, and `list` still call `build_slot_inventory` directly and remain to be resolved (`goto` and `list` are read-only and share a weaker deletion-test signal than the mutating ops)._
