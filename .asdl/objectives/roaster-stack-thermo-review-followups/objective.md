...
- The first decomposition slice de-risked the dry-run/result-shaping extraction path: dry-run result contracts and projection helpers now live outside `stack_workflow.py`, shared triage views are pure helpers, and full repo validation passed. Remaining decomposition slices still need the same safety-ordering guardrail.
