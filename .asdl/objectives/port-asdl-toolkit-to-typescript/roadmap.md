...
- [ ] Repeat the capability subobjective pattern until all active first-party user-facing capabilities are TS-default.
  ...
  - The out-of-sequence `areg` exception is complete. The current default capability, `asdl-dispatcher`, is tracked by an active child Objective unless new evidence changes the order.
  - Evidence: active subobjective `.asdl/objectives/dispatcher-typescript-port/` tracks the dispatcher contract inventory and port-or-retire decision. Initial inventory found only help/version/plugin discoverability, `operations=[]`, and no active callers beyond package tests and workspace/build wiring.
