...
- [ ] Repeat the capability subobjective pattern until all active first-party user-facing capabilities are TS-default.
  ...
  - Current exception: `areg` is active out of sequence through `areg-typescript-port`. After that slice completes or parks again, resume the remaining order deliberately; the next default capability is now `objective` unless new evidence changes the order.
