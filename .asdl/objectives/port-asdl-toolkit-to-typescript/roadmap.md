...
- [ ] Repeat the capability subobjective pattern until all active first-party user-facing capabilities are TS-default.
  ...
  - Evidence: `areg` completed as an out-of-sequence TS-default capability and fed repo-local TS source/shim cutover, hidden `exec skillx` Clinkr envelope handling, package-local skill/project mutation seams, external `npx skills` boundary handling, and Python deletion sequencing lessons back into [`porting-playbook.md`](porting-playbook.md), the migration ledger, and Semantic Update `updates/2026-06-16T012156Z-areg-cutover-playbook-lessons.md`.
