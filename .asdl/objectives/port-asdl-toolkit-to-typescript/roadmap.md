...
- [x] Select the next capability by the persisted capability order and fresh integration-leverage evidence.
  ...
  - Remaining order after `brmem`: `handoff`, `objective`, `asdl-dispatcher`, roaster, `slot`, `vibechk`, then `aretro` last. Dispatcher has since completed by retirement/no-port; revisit the remaining order only when new evidence materially changes usage, dependency, or strategic value.
- [x] Promote `areg` from parked status into an out-of-sequence subobjective.
  - The default post-`brmem` sequence remains valuable, but present strategic interest in making `areg` TypeScript-default now justifies an explicit exception rather than leaving it parked pending evidence.
  - Evidence: `objective.md` now marks `areg` as an active out-of-sequence subobjective, explains the exception to the default `handoff` / `objective` order, and resolves the parked-entry open question for `areg`; `areg-typescript-port` records the detailed contract, distribution, and Python-retirement scope for the slice.
- [ ] Repeat the capability subobjective pattern until all active first-party user-facing capabilities are TS-default.
  ...
  - Evidence: `areg` completed as an explicit out-of-sequence TS-default capability and fed repo-local TS source/shim cutover, hidden `exec skillx` Clinkr envelope handling, skill-artifact contract inventory, package-local skill/project mutation seams, managed project-file mutation safety, external `npx skills` boundary handling, Python deletion sequencing/reference, and post-cutover cleanup-separation lessons back into [`porting-playbook.md`](porting-playbook.md), the migration ledger, and Semantic Updates `updates/2026-06-15T191151Z-areg-cutover-and-playbook-lessons.md` and `updates/2026-06-16T012156Z-areg-cutover-playbook-lessons.md`.
  - Evidence: `objective` completed as the fourth default TS capability and fed checked-in Markdown record parity, plugin retirement, TypeScript source-shim install, Python deletion/rollback lessons, and the follow-up TS-native camelCase JSON cleanup back into [`porting-playbook.md`](porting-playbook.md), the migration ledger, migration debt, and Semantic Updates `updates/2026-06-17T045528Z-objective-cutover-playbook-lessons.md` and `updates/2026-06-17T113000Z-objective-json-debt-burned-down.md`.
  - Evidence: dispatcher completed by retirement/no-port through `.asdl/objectives/dispatcher-typescript-port/`: fresh caller discovery found no active consumers beyond package tests and workspace/build wiring, the Python placeholder had `operations=[]` and no operation contract, `packages/asdl-dispatcher` and active root references were removed, and no TypeScript package was created.
  - Evidence: `slot` completed through `.asdl/objectives/slot-typescript-port/` and fed the first OS/worktree/shell-coupled port lessons back into [`porting-playbook.md`](porting-playbook.md), the migration ledger, and Semantic Update `updates/2026-06-18T172324Z-slot-cutover-playbook-lessons.md`; rollback/reference evidence is commit `9164ef9ea562`.
  - Evidence: Roaster / review workflows completed through `.asdl/objectives/roaster-typescript-port/`: standalone TypeScript `@asdl/roaster` now covers the CI PR-diff findings slice, hidden publication `exec` commands, and the GitHub Actions workflow; the green real-PR workflow run `27610014374` preceded Python deletion; `packages/roaster` is deleted; `just install-roaster` / `install-tools` provide the accepted source shim; and TS plugin mounting remains parked outside the completed standalone-CLI scope.
  - Evidence: Vibe check / `vibechk` completed through `.asdl/objectives/vibechk-typescript-port/`: standalone TypeScript `@asdl/vibechk` now covers `run`, `runs`, `show`, `diff`, the `claude` runner, schema-version-1 bundle compatibility, local result branches, Markdown reports, and manual PR-paste workflow; `just install-vibechk` is an opt-in source shim, deliberately outside `install-tools`; `packages/vibechk` is deleted; rollback/reference evidence is commit `25c748681`; and missing `publish`, `codex`, `pi`, and real publish smoke work remains parked in `vibechk-v1`.
  - The out-of-sequence `areg` exception is complete, `slot` also completed out of the previous order after dispatcher retirement, Roaster completed as a default capability, and Vibe check / `vibechk` completed as the next default capability. The next default unstarted capability is Branch retrospectives / `aretro` unless new evidence changes the order.
