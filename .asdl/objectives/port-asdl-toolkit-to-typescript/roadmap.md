...
- [ ] Repeat the capability subobjective pattern until all active first-party user-facing capabilities are TS-default.
  ...
  - The out-of-sequence `areg` exception is complete, `slot` also completed out of the previous order after dispatcher retirement, and Roaster completed as the next default capability. The next default unstarted capability is Vibe check / `vibechk` unless new evidence changes the order.
