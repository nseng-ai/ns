...
- [ ] Repeat the capability subobjective pattern until all active first-party user-facing capabilities are TS-default.
  ...
  - Evidence: `objective-typescript-port` was created as the active child Objective for the next default capability slice, and `contract-inventory.md` records the initial public contract baseline for standalone `objective`, hidden `objective exec`, checked-in Markdown storage, skill/Pi/CCC consumers, the current `asdl objective` plugin path, distribution assumptions, and incidental Python details.
  - The out-of-sequence `areg` exception is complete. The active default capability is now `objective`; next remaining default capability after it is `asdl-dispatcher` unless new evidence changes the order.
