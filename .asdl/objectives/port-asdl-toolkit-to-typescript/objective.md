...
| Slots / `slot`                                | Active subobjective (planning)                | Active subobjective `.asdl/objectives/slot-typescript-port/` tracks the detailed `slot` TypeScript port: contract inventory, a roaster-style `prework/` suite, and the migration through the first OS-coupled surface (worktree pool, `~/.slots` state, parent-shell `cd` integration, clipboard). First-party `asdl-slots` CLI and plugin support slot/Graphite workflow concepts referenced by repo policy.                                                                                                  |
...
| `asdl-dispatcher` / `dispatcher`              | Active child objective (planning)             | Active subobjective `.asdl/objectives/dispatcher-typescript-port/` records that the current Python surface is a thin standalone CLI/plugin placeholder with no operations; the child Objective owns the port-or-retire decision before any TypeScript implementation work.                                                                                                                                                                                                                                     |
...
Completed exception: `areg` ran out of sequence through the `areg-typescript-port` subobjective and is now TS-default. This exception did not rewrite the default sequence; it recorded that strategic interest in making `areg` TypeScript-default was strong enough to promote it from parked status before the remaining default work. With the exception complete, the default sequence resumed at `objective`, and `objective` has now completed its TypeScript cutover. The current default capability is `asdl-dispatcher`, tracked by the active `dispatcher-typescript-port` child Objective unless new evidence materially changes the sequence.
...
- The completed `pr-address`, `brmem`, `handoff`, `areg`, and `objective` cutovers confirm this direction: package-local seams can prove runtime, payload/reference, git-gateway, wrapper, project-file mutation, skill-management, checked-in Markdown record handling, canonical TS-native JSON cleanup, and deletion behavior before only repeated framework gaps move into shared TS packages.
...
- Distribution, installed-skill wrapper behavior, and Pi integration may expose packaging constraints not visible in the previous Python/uvx model. `pr-address`, `brmem`, `handoff`, and `areg` deliberately accepted repo-local TypeScript source/shim models with checkout and `ts/node_modules` preconditions where consumers did not require registry publishing; future ports must make their own consumer-backed distribution decision.
