...
- The completed `pr-address`, `brmem`, `handoff`, `areg`, `objective`, `slot`, `roaster`, and `vibechk` cutovers confirm this direction: package-local seams can prove runtime, payload/reference, git-gateway, wrapper, project-file mutation, skill-management, checked-in Markdown record handling, canonical TS-native JSON cleanup, OS/worktree/shell-coupled behavior, GitHub publication, CI workflow cutover, local bundle compatibility, runner subprocess boundaries, and deletion behavior before only repeated framework gaps move into shared TS packages.
