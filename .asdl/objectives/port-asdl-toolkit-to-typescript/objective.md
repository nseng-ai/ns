...
| `areg`                                        | TS-default; completed out-of-sequence cutover | Standalone TypeScript `@asdl/areg` CLI is the active repo-local surface; `just areg-check`, `refresh-skills`, skill caller docs, and hidden `exec skillx` guidance use the TS path; `packages/areg` is deleted from active tracked paths; external installed distribution beyond the repo-local TS shim remains parked as follow-up.                                                                                               |
...
Resolved exception: `areg` completed its out-of-sequence `areg-typescript-port` slice and is now TypeScript-default for repo-local use. This did not rewrite the default sequence; after recording the exception and its reusable lessons, the remaining order resumes deliberately with Objectives / `objective` unless new evidence changes the order.
