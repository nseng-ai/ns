...
| `areg`                                        | Active out-of-sequence subobjective     | Promoted from parked status by explicit strategic-value decision in `areg-typescript-port`, which intentionally jumps ahead of the default `handoff` / `objective` sequence. The active subobjective records the standalone CLI contract scope, skill-management surfaces, hidden `exec skillx` helpers, and distribution uncertainty that must be resolved before TS cutover.                                                     |
...
Current exception: `areg` is active out of sequence through the `areg-typescript-port` subobjective. This exception does not rewrite the default sequence; it records that present strategic interest in making `areg` TypeScript-default now is strong enough to promote it from parked status before `handoff` and `objective`. After the `areg` slice completes or parks again, resume the remaining order deliberately rather than silently inheriting the old "next planned capability" note.
...
- Resolved (2026-06-14): `areg` has enough strategic-value evidence to promote from parked status into the active out-of-sequence `areg-typescript-port` subobjective. `packagechk` remains parked pending evidence.
