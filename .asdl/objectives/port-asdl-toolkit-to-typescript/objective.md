...
| Objectives / `objective`                      | In progress; child Objective active           | Objective skill family and `asdl-objectives` CLI are active coordination surfaces; `.asdl/objectives/objective-typescript-port/` now owns the detailed contract inventory and port planning for this capability slice.                                                                                                                                                                                                                                                                                         |
...
Completed exception: `areg` ran out of sequence through the `areg-typescript-port` subobjective and is now TS-default. This exception did not rewrite the default sequence; it recorded that strategic interest in making `areg` TypeScript-default was strong enough to promote it from parked status before the remaining default work. With the exception complete, the default sequence has resumed at `objective`; `.asdl/objectives/objective-typescript-port/` tracks the active slice, and the next remaining default capability after it is `asdl-dispatcher` unless new evidence materially changes the sequence.
...
- The completed `pr-address`, `brmem`, `handoff`, and `areg` cutovers confirm this direction: package-local seams can prove runtime, payload/reference, git-gateway, wrapper, project-file mutation, skill-management, and deletion behavior before only repeated framework gaps move into shared TS packages.
