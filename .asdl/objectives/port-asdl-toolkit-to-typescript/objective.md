...
Completed exception: `areg` ran out of sequence through the `areg-typescript-port` subobjective and is now TS-default. This exception did not rewrite the default sequence; it recorded that strategic interest in making `areg` TypeScript-default was strong enough to promote it from parked status before the remaining default work. With the exception complete, resume the remaining order deliberately: the next default capability is `objective` unless new evidence materially changes the sequence.
