...
| Objectives / `objective`                      | TS-default; completed fourth default cutover  | Standalone TypeScript `@asdl/objective` CLI is the active public surface through the `just install-objective` / `install-tools` source shim; the `asdl objective` plugin is retired, `packages/asdl-objectives` is deleted from tracked active paths, first-party Pi/CCC JSON consumers remain compatible through Objective-local legacy machine projections, and rollback/reference evidence is in-repo commit `1b1bb1fa44ad`.                                                                                |
...
- The completed `pr-address`, `brmem`, `handoff`, `areg`, and `objective` cutovers confirm this direction: package-local seams can prove runtime, payload/reference, git-gateway, wrapper, project-file mutation, skill-management, checked-in Markdown record handling, and deletion behavior before only repeated framework gaps move into shared TS packages.
