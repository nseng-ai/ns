...
| Branch retrospectives / `aretro`              | TS-default; completed default cutover         | Standalone TypeScript `@asdl/aretro` is the active evidence CLI for the public `branch-retro` skill; the runner uses the repo-local TypeScript source CLI or a PATH `aretro` shim, Python `packages/aretro` and `uv run` / `uvx` fallbacks are retired, `just install-aretro` remains opt-in rather than part of `install-tools`, rollback/reference evidence is in-repo commit `dd1c69ac85f9f836a9c12cd1da219099a2683273`, and semantic retrospective judgment remains in the skill.                          |
...
`aretro` stayed last so retrospective/evidence-analysis work benefited from mature git, command-runtime, and evidence payload foundations rather than driving early architecture. It has now completed through `.asdl/objectives/aretro-typescript-port/`: standalone `@asdl/aretro` is the active deterministic evidence CLI, `branch-retro` uses the TypeScript source/PATH runner, the Python package and `uvx` fallback are retired, and semantic recommendations remain in the skill.

Completed exceptions: `areg` ran out of sequence through the `areg-typescript-port` subobjective and is now TS-default; `slot` also completed out of the previously listed order after dispatcher retirement through `.asdl/objectives/slot-typescript-port/`. These exceptions did not rewrite the default sequence. With Branch retrospectives / `aretro` complete, there is no remaining default unstarted capability in the persisted sequence; final migration cleanup should focus on ledger/playbook consistency, remaining parked/out-of-scope Python paths, and any end-of-migration debt.
...
- Distribution, installed-skill wrapper behavior, and Pi integration may expose packaging constraints not visible in the previous Python/uvx model. `pr-address`, `brmem`, `handoff`, `areg`, `objective`, `slot`, `roaster`, and `vibechk` deliberately accepted repo-local TypeScript source/shim models with checkout and `ts/node_modules` preconditions where consumers did not require registry publishing; future ports must make their own consumer-backed distribution decision.
