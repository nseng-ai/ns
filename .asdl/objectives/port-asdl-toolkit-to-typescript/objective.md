...
| Branch Memory / `brmem`                       | In progress; subobjective active        | Selected as the second capability. Active subobjective `.asdl/objectives/brmem-typescript-port/` tracks the port. Integration-leverage evidence: existing TS code already shells out to `brmem` (the `@asdl/core` launcher, `branch-context` gateway), while the capability itself is still Python-backed. Standalone `ts/packages/brmem` (library + CLI), git-ref storage parity is the central concern, run-from-source shim distribution, no plugin to retire.                                          |
...
- The `pr-address` cutover confirms this direction: package-local seams can prove runtime, payload/reference, gateway, wrapper, and deletion behavior before only repeated framework gaps move into shared TS packages.
...
- Distribution, installed-skill wrapper behavior, and Pi integration may expose packaging constraints not visible in the previous Python/uvx model. `pr-address` deliberately accepted a run-from-source shim with checkout and `ts/node_modules` preconditions; future ports must make their own consumer-backed distribution decision.
...
- Resolved (2026-06-13): `brmem` follows `pr-address`, holding the persisted order. Fresh integration-leverage evidence confirmed rather than changed it — existing TypeScript code already depends on `brmem` (the `@asdl/core` launcher and `branch-context` gateway) while the capability is still Python-backed. Tracked by the `brmem-typescript-port` subobjective.
