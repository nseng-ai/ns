...
- [ ] Decide and implement `asdl objective` plugin retirement or preservation.
  ...
  - Include the coordinated JSON-shape cutover in this phase or the adjacent install/caller cutover: inventory `objective --format json` consumers, migrate/update Pi/CCC/skills/tests as needed, then remove compatibility `legacyMachine` usage so Objective commands use normal `@asdl/clinkr` JSON output deliberately rather than command-by-command opportunistically.
  - Policy: steer-first. Do not execute plugin removal until the preview includes current grep evidence, the replacement/deletion plan for `tests/scenario/test_plugins.py`, JSON consumer/cutover evidence, and the expected user-visible compatibility note.
  - Evidence: updated plugin smoke tests or deliberate deletion/replacement, current grep showing no active `asdl objective` callers, JSON-shape cutover evidence, and a Semantic Update recording the decision.
