...
The first roadmap row is complete. `areg init` now has explicit scenario coverage for the safer preflight/planning/apply contract: successful initialization, prompt behavior, malformed managed markers (including doubled-start, doubled-end, and end-before-start variants) before install, path-shape validation before install (wrong-type `asdl.toml`, `AGENTS.md`, `CLAUDE.md`, `.claude`, and `.claude/settings.local.json`), `asdl.toml` `[areg].agents` preservation/replacement semantics, and non-destructive behavior when `npx skills add` fails — including preservation of pre-existing planned files.
...
The first roadmap row is marked complete. The config decision is now explicit for `init`: project config lives in `asdl.toml` (`[areg].agents`), legacy `areg.json` is still read for agent migration, unknown `asdl.toml` sections are preserved, and the `[areg].agents` value is replaced with the requested agent list.
...
- Symlink/canonical path hardening for managed writes, `.claude` settings, and `skillx cleanup` already landed in the next roadmap row (commit `7379b9ae`).
