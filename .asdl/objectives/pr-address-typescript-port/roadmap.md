...
- [ ] Shared XOR-source resolver in `json-input.ts`.
...
- [ ] Dead-code sweep.
      Group 3 — structural/dedup. Delete `LEGACY_EXEC_OPERATIONS`, `isTsManaged`, the `raw-exit` dispatch variant, `isActionComplexity`, `getFeedbackInlineResultSchema` (feedback-collection copy), `validResolutionModesText`, `stackFeedbackPrFixture`, the verbatim `parseOptions` copy in `classification-core.ts`, and unreachable `--json-schema` skip branches; prune speculative `index.ts` barrel exports to the real embedding contract. Decide `writeTextArtifact`'s fate per the open question.
