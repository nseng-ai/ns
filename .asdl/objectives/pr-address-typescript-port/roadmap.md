...
- [~] Cut over public skill, wrapper, plugin, and distribution paths to TypeScript default.
  - Decided 2026-06-09: the `asdl pr-address ...` plugin is retired outright.
  - Revised 2026-06-13: checkout-free bundled JavaScript distribution is no longer required. The accepted installed CLI model is the existing run-from-source shim (`ts/packages/pr-address/scripts/pr-address-shim` → `~/.local/bin/pr-address` via `just install-pr-address`) that runs `node <checkout>/ts/packages/pr-address/src/cli.ts` from the enclosing worktree or a baked canonical checkout. There is no `local|prod` mode, no `uvx`/Python wrapper path, no npm publish requirement, and no bundle gate. The shim may require an asdl checkout with `ts/node_modules`.
  - Policy: docs, wrapper behavior and tests, local checkout behavior, and plugin retirement are directly executable after preview. Live npm/PyPI publishing, pushing installed-skill artifacts to external stores, and adding checkout-free bundle machinery are out of scope unless a new requirement is explicitly accepted.
  - Evidence should include shim resolution checks (enclosing-worktree vs baked canonical checkout, missing-`node_modules` failure), documentation updates for the run-from-source requirement, and plugin-retirement evidence.
  - Progress evidence: `updates/2026-06-09T155412Z-cutover-retirement-playbook.md` records local TypeScript operation status, wrapper alias coverage, public docs/playbook updates, and explicit deferral of npm/prod/plugin cutover decisions. `updates/2026-06-13T115653Z-checkout-free-bundle-requirement-dropped.md` records the later decision to drop checkout-free bundling entirely.
- [x] Fix CLI argument compat gaps: `--format=json` and strict integer parsing.
      ...
      Follow-up evidence: `updates/2026-06-13T064142Z-clinkr-test-polish-and-integer-contract.md` records the accepted stricter-than-click integer edge policy for `+5`, whitespace, and underscores, plus TS workspace validation.
...
- [x] Shared operation-support layer.
      ...
      Completion evidence: updates/2026-06-12T221500Z-structural-dedup-group.md — gateway helpers dissolved entirely (required gateways made the indirection moot); `isRecord` in `operation-support.ts`; trims in `string-values.ts`; `errorMessage` had already dissolved in the dead-code sweep; argv-parser portion dissolved in the clinkr migration as predicted.
...
- [x] Make `PrAddressContext` gateways required; delete the `missing_gateway` branch class.
      ...
      Completion evidence: updates/2026-06-12T221500Z-structural-dedup-group.md — `fakePrAddressContext` supplies default in-memory gateways; omitted-gateway tests deleted as type-impossible; no fixture carried a `missing_gateway` envelope.
- [x] Decompose the two 1k-line files.
      ...
      Completion evidence: updates/2026-06-12T221500Z-structural-dedup-group.md — pure-move decompositions with byte-unchanged fixtures; judo folded in (the `requiredAt` joins lived in `stack-feedback.ts` and are replaced with zipped pairs).
- [x] Shared thread-decision engine and Python shared-module ports.
      ...
      Completion evidence: updates/2026-06-12T221500Z-structural-dedup-group.md — `string-values.ts` and `stack-feedback-thread-index.ts` ported; throw-on-null unification proven safe; the decision engine stays in `resolve-thread-batch-payload.ts` (already shared via import; extraction would not improve the import graph).
- [x] Split `stack-feedback-prep` into parallel fetch planning and ordered artifact writes.
      ...
      Completion evidence: updates/2026-06-12T221500Z-structural-dedup-group.md — concurrency on by default (open question resolved); first-failure-in-input-order pinned by test; prep fixture byte-unchanged.
- [x] Consolidate test scaffolding and align test layout.
      ...
      Completion evidence: updates/2026-06-12T221500Z-structural-dedup-group.md — shared support modules landed; layout moves verified by collected-count comparison (331→332, one deliberate split); `@asdl/clinkr/testing` `runForTest` and `@asdl/core/testing` `createTempDirTracker` evaluated and not adopted for recorded reasons.
      Follow-up evidence: `updates/2026-06-13T064142Z-clinkr-test-polish-and-integer-contract.md` records the shared golden wrapper helper, zero-case guard, wrapper-temp reuse, and local helper cleanup that tighten the completed scaffolding row without reopening it.
- [x] Migrate the `pr-address` CLI shell to `@asdl/clinkr`.
      Ownership transferred from `ts-cli-foundation` on 2026-06-12. This record owns the consumer migration: build the `pr-address` command tree through `@asdl/clinkr`, preserve legacy-Python fallback dispatch until the retirement rows, keep package-specific payload/reference behavior local, and coordinate only reusable framework gaps back to `ts-cli-foundation`. Sequence after the group-2 payload spec and before the endgame's `schema-routes` and retirement branches as evidence allows.
...
- [ ] Retire active Python fallback paths and fully delete `packages/asdl-pr-address`.
  - Decided 2026-06-09, revised 2026-06-13: the end state is full in-repo deletion within the endgame stack, gated on all operations being TypeScript-managed, all `--json-schema` routes TypeScript-owned, the run-from-source wrapper accepted and documented, plugin retirement landed, and docs/tests free of Python invocation paths. PyPI `asdl-pr-address==0.1.1` is the external frozen rollback after deletion.
...
- Branches 1-6 validate with `pnpm --dir ts/packages/pr-address run check` and `run test` plus targeted golden/parity probes; branches 7-8 broaden to full `just` because they remove Python packages, plugin wiring, and asdl-scope tests.
...
- Checkout-free bundled JavaScript distribution — explicitly dropped from this Objective on 2026-06-13; revisit only if installed use without an asdl checkout becomes a real consumer requirement.
- npm registry publishing of `@asdl/pr-address` — not needed under the accepted run-from-source wrapper model; revisit only if a registry consumer appears.
