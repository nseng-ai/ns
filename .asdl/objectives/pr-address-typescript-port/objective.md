...
- Materialized 2026-06-09: the wrapper's prod mode pins `asdl-pr-address==0.1.0`, which was never published — only `0.1.1` exists on PyPI — so installed/prod invocation is currently broken. This lowers the regression risk of prod cutover (there is no working prod Python path to preserve) and raises the priority of the bundle-distribution branch. Resolved later on 2026-06-09: prod mode executes the checked-in bundle; the `legacy-python` rollback mode pins published `0.1.1`.
- npm distribution risk is retired by the bundle decision; the replacement risk is bundling machinery itself (build correctness, runtime requirements of the bundled artifact, skill-directory size and staleness between releases). De-risked 2026-06-09: the bundle build is byte-deterministic (esbuild, lockfile-pinned, stable banner), and a freshness test byte-compares a rebuild against the checked-in artifact so staleness fails CI. Accepted residual: ~880 KB checked-in artifact and regenerate-on-every-src-change diffs.
- Plugin retirement is a deliberate breaking change for any `asdl pr-address ...` callers; the cutover must update docs/tests so no active caller path remains, rather than preserving compatibility. Executed 2026-06-09: entry point, plugin module, and asdl-scope integration test removed; a retirement-guard test asserts pr-address never re-mounts under the asdl umbrella.
...
Resolved 2026-06-09 (endgame stack):

- Golden parity: existing golden JSON outputs were treated as byte-for-byte targets (envelopes, artifacts) with explicit normalization only for root-length-dependent `payload_bytes` and live timestamps; generated `--json-schema` documents use structured semantic parity verified by a dedicated comparator.
