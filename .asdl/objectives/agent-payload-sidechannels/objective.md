...
- A shared payload-store primitive in `asdl-core` for agent payload artifacts, used by `pr-address` first. The future namespace is `asdl_core.payloads`.
...
- Payload root/session controls:
  ...
  - payload mode requires a supplied valid session id from an explicit option such as `--payload-session-id` or from `ASDL_PAYLOAD_SESSION_ID`;
...
- Payload file safety and behavior:
  ...
  - payload write failure fails closed with a stable payload error and no compact result data, rather than falling back to full stdout.
...
- Changing `pr-address exec prepare-run` and `pr-address exec get-feedback` defaults because backward compatibility is not required for this change:
  ...
  - the complete raw machine envelope is written to a `.raw.json` payload;
  - an explicit `--payload-mode inline|payload` escape hatch exists, with `payload` as the default and `inline` intentionally printing the full raw payload;
  - payload mode requires a valid payload session id and inline mode does not;
  - machine output uses a deterministic discriminator such as `payload_mode` so callers can branch on `payload` versus `inline`;
...
- Detail retrieval and payload-artifact summarization:
...
- Updating the `pr-address` skill/workflow documentation once payload command behavior exists so agents use the payload artifact design by default: run the feedback command with a supplied session id, pass the raw payload path to a subagent or equivalent payload-aware summarizer when available, validate the returned classification, use selected-detail lookup for targeted bodies, and use explicit inline/full-output mode only as a debugging or migration escape hatch.
- Functional tests for correctness of pathing, payload writing, locator manifests, detail lookup, prompt resolution, `pr-address` completeness invariants, and validation behavior. This Objective does not require numeric token/character budget tests or a measurement script.
...
- Do not add command-level LLM invocation. CLI commands produce deterministic manifests and payload files; agents or harness-specific subagents perform semantic judgment.
- Do not make Clinkr automatically spool all large machine outputs. The payload artifact behavior is opt-in so each command can define an appropriate compact manifest.
- Do not add bounded body previews or a `--body-preview-chars` escape hatch in v1. Full text comes from payload/subagent access, selected-detail lookup, or explicit inline mode.
...
- Do not store credentials, secrets, or unrelated long-lived archives in payload artifacts. The payload artifact store is a temporary workflow artifact mechanism, not durable memory.
...
- Raw payload artifacts store full raw Clinkr machine envelopes as schema-equivalent JSON to explicit inline machine-envelope output.
- Payload write failures produce Clinkr failures with stable payload error types and no result data; commands never fall back to printing raw payloads inline after payload write failure.
- `pr-address exec prepare-run` and `pr-address exec get-feedback` default to payload mode and no longer print full review/comment bodies to stdout in normal human or JSON operation. They print compact locator manifests with PR metadata, counts, every feedback item, every unresolved thread, non-body item metadata, per-body `body_chars`, body locators, and payload references.
- `--payload-mode inline|payload` or an equivalent explicit option exists, with `payload` as default and `inline` as an intentional full-stdout debugging/migration path. Inline mode bypasses payload session preflight.
- `pr-address` provides a tested way to retrieve selected full details from a raw payload by locator through a `pr-address exec` surface, backed by reusable core RFC 6901 JSON Pointer lookup against validated payload artifacts.
- The `pr-address` classification workflow is updated so a payload-aware agent/subagent can read the raw payload path and return a strict full classification packet. The main workflow validates that every manifest feedback item is accounted for exactly once, rejects duplicates/missing ids/unknown ids/invalid enum values, retries once for correction, and fails closed if the packet remains invalid.
...
- The `pr-address` skill/reference documentation reflects the new workflow: use compact payload defaults with a supplied session id, avoid printing raw JSON into the main transcript, prefer payload-aware subagent/helper summarization when available, use selected-detail lookup for targeted body text, use explicit inline/full-output mode only as an escape hatch, and preserve the per-item completeness invariant.
- Functional tests cover payload path/session validation, relative root rejection, payload file creation, payload failure behavior, compact manifest shape, body elision/no body text in stdout, locator detail retrieval, prompt resolution behavior, embedded prompt drift, and `pr-address` classification completeness validation. Relevant targeted repo checks pass for changed areas.
...
- Agents and subagents in Pi, Claude, and Codex can reliably receive a file path and read a temp payload in the same local environment, or can fall back to selected-detail lookup or explicit inline mode when they cannot.
...
- Required session IDs may make default payload mode fail in plain terminal use until harnesses/skills pass `ASDL_PAYLOAD_SESSION_ID` or `--payload-session-id`. Inline mode remains the explicit debugging/migration escape hatch.
- Raw payload files can contain sensitive PR discussion, code snippets, bot output, and review text. Private temp permissions and temp-root scoping reduce exposure, but relying on OS cleanup means the files can outlive the immediate command.
...
- A harness that cannot run a payload-aware summarizer may have to use selected-detail lookup or explicit inline mode. The Objective intentionally avoids bounded inline body previews in v1, so docs must make the fallback path clear.
- Strict directory-permission checks can fail on unusual filesystems or preexisting temp roots. The shared store now fails closed for unsafe managed directories and has POSIX-focused tests for private directory/file behavior, but unusual filesystem behavior remains a consumer-facing risk to keep explicit through `payload_directory_unsafe` or `payload_root_invalid` rather than silently weakening the safety contract.
...
- Bounded previews: no bounded body-preview escape hatch is included in v1. Fallbacks are payload/subagent access, selected-detail lookup, and explicit inline mode.
...
- Compact manifest field names: the `pr-address` payload manifest models now establish `payload_mode`, `payload_reference`, `counts`, review items, review-thread items, discussion-comment items, and per-body `body_locator` objects with `body_chars`, `json_pointer`, optional `item_pointer`, and PR-domain locator metadata.
...
Completed. The Objective landed the payload artifact and prompt-pluggability steelthread around `pr-address`: a durable specification, shared payload store, opt-in Clinkr payload helper, repo-local launch policy with embedded fallback, compact `prepare-run` / `get-feedback` manifests, selected-detail lookup, deterministic feedback-classification validation, and public `pr-address` skill/reference workflow wiring.
