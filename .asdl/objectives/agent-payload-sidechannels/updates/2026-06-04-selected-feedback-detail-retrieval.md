...
Added selected-detail retrieval for compact `pr-address` feedback sidecars. `asdl_core.payloads.lookup` now provides a reusable RFC 6901 JSON Pointer resolver plus a payload-aware JSON artifact reader that validates explicit absolute sidecar paths before loading `.raw.json` or `.summary.json` values.
