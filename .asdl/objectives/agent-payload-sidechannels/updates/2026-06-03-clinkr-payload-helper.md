# Clinkr Payload Helper Implemented

## Summary

Implemented the opt-in Clinkr payload helper layer in `asdl_core.payloads.clinkr`. The new helper surface lets future payload-enabled Clinkr commands explicitly open/preflight a payload store, write a full Clinkr machine envelope to a `.raw.json` payload artifact, and embed the returned `PayloadReference` in command-specific compact manifests.
...
The roadmap row “Add the opt-in Clinkr/helper surface for payload writes” is complete because `open_clinkr_payload_store(...)` maps payload preflight failures to `ClinkrFailure` with stable payload error types, and `write_clinkr_raw_payload_artifact(...)` writes `ClinkrExit.to_envelope_dict()` as the raw payload artifact while returning the store-owned reference.
...
- Convert `pr-address exec prepare-run` and `get-feedback` to compact payload defaults in a later slice.
