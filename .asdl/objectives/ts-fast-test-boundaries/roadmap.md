...
- [~] Move sqlite-backed Graphite/worktree-status coverage into the integration suite or replace default-path sqlite setup with injected fakes.
  ...
  - Remaining work: the separate `packages/slot/test/gateways/real-git-gateway-movement.test.ts`, `packages/slot/test/gateways/real-gt-gateway.test.ts`, and `packages/sdl/test/scenario/cp-cli.test.ts` coverage is still outside this completed CCC/Pi slice and should be assessed separately.
