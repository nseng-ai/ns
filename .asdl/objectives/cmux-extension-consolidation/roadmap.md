...
- [ ] Reuse canonical command helpers in the cmux suite. Replace `slot-dispatch-plan.ts`'s
...
- [ ] Collapse the duplicated cmux primitives into one shared home. Remove the per-file
      ...
      private `shellQuote`/`formatShellArg` in favor of the canonical quoter. Decide and record
      whether the shared home is a cmux-local `internal.ts` or an existing package module.
      Evidence: ts-check/ts-test green; each primitive defined once in the cmux suite.

- [ ] Delegate slot-checkout envelope parsing to `machine-envelope.ts`. Rewrite
...
- [ ] De-duplicate the branch-slug helpers. Make `cmux/branch-slug.ts` import
