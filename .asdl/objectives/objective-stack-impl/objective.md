...
- The generic child-session text tool should stay minimal for v1: `title` and `prompt` only, with same-worktree sequencing and safety checks handled by parent instructions rather than tool options.
...
- Capturing final assistant text still depends on Pi JSON event shapes. The helper/parser slice de-risks known shapes with fake-driven tests for `message_end`, `turn_end`, and `agent_end`, but future Pi event-shape changes should be treated as compatibility risks.
- The earlier terminal-capture baseline has now been superseded for the helper/parser, generic tool, and prompt-template slices: local branch evidence adds a `returnMode: "final-text"` API, `ChildSessionFinalTextResult`, JSON final-assistant-text capture, `stopped-without-useful-text` diagnostics, the `run_child_session_text` custom tool, a project-local discovery shim, fake-driven coverage while preserving terminal-capture behavior, and `.pi/prompts/objective-stack-impl.md` as the v1 entry point.
- The generic child-session text tool is intentionally permissive but minimal. V1 exposes only `title` and `prompt`; the Objective-stack prompt must keep same-worktree child runs sequential and require parent-side git/worktree checks.
- Current-session-only orchestration can inherit noisy or stale context. The prompt now explicitly asks the parent agent to compact relevant context and discard distractions before planning, but live usage may still reveal unclear context boundaries.
- If the child stops without useful final text, the helper and `run_child_session_text` tool preserve `stopped-without-useful-text` diagnostics; the parent must still treat that as non-complete and inspect the child session file rather than advancing blindly.

## Open Questions

- Should the final-text result shape remain an option on `runChildSession` as more caller-facing tools are added, or does a wrapper become useful once usage patterns settle?
- Should future `run_child_session_text` versions support options such as `requireCleanWorktree`, or should the minimal title/prompt contract remain the long-term boundary?
- Resolved for v1: when `/objective-stack-impl` is invoked without an Objective slug, the prompt tells the parent agent to run `objective list --format md`, show open candidates, and ask the user to choose instead of inferring.
- Resolved for v1: the prompt provides a required child-prompt checklist while leaving exact wording and slice-specific context generation to the parent agent.
- After the steelthread is inspected, should any repeated parent-agent steps graduate into deterministic extension tools?

## Closure

Outcome: completed. The prompt-first Objective stack implementation steelthread is ready for user inspection: child sessions can return final assistant text, the generic `run_child_session_text` tool is available through project Pi extension discovery, and `/objective-stack-impl [objective-slug]` exists as the current-session-only prompt-template entry point.

Key evidence:

- `runChildSession` supports `returnMode: "final-text"` with `final-text`, `stopped-without-useful-text`, cancellation, error, protocol-error, and terminal-capture compatibility outcomes.
- `ts/packages/pi-extensions/src/run-child-session-text.ts` registers the generic `run_child_session_text` tool, and `.pi/extensions/run-child-session-text.ts` exposes it through project-local discovery.
- `.pi/prompts/objective-stack-impl.md` defines `/objective-stack-impl [objective-slug]` with current-session-only orchestration, explicit Objective selection, in-session context compaction, Graphite-aware slice planning, sequential same-worktree child delegation, parent-side status inspection and validation, Objective update guidance, manual recovery notes, and no automatic PR submission.
- The v1 workflow remains Branch Memory-free and does not introduce hidden ledgers, stack schemas, stack-specific child terminal tools, deterministic parsing of child prose, parallel same-worktree children, or a closed-loop extension command.
- Verification passed: `dprint check .pi/prompts/objective-stack-impl.md`, `just dprint-check`, and `cd ts/packages/pi-extensions && bun test && bun run check`.

Remaining assumptions, risks, and caveats:

- Live use may reveal that some parent-orchestrator steps should later graduate into deterministic helper commands, but that is intentionally outside this v1 steelthread.
- Freeform child final text remains judgment-based evidence; the parent prompt mitigates this by requiring status/session-file inspection and independent validation.
- Manual recovery remains the accepted v1 recovery model through git/Graphite state, Objective updates, the parent transcript, and child session files.

Follow-ups:

- Inspect the current branch's prompt wording and Objective updates before merge.
- Optionally run a live `/objective-stack-impl <objective-slug>` smoke test in Pi after inspection.
