...
- The earlier terminal-capture baseline has now been superseded for the helper/parser and generic tool slices: local branch evidence adds a `returnMode: "final-text"` API, `ChildSessionFinalTextResult`, JSON final-assistant-text capture, `stopped-without-useful-text` diagnostics, the `run_child_session_text` custom tool, a project-local discovery shim, and fake-driven coverage while preserving terminal-capture behavior. The `/objective-stack-impl` prompt remains active de-risking work.
