...
- [~] Migrate the mutation flow to explicit artifact references.
  - Single-PR mutation now uses the explicit reference contract: `build-resolve-thread-batch-payload --pr-number --batch-id --commit-sha --decisions-file` resolves the latest plan, writes a PR/batch resolve-build artifact for every valid build result, and `resolve-thread-batch --from-build <payload-path>` refuses implicit resolution with `explicit_artifact_required` when the reference is omitted. `resolve-thread-batch` now writes a managed resolution-result artifact after successful or partial mutation attempts and rejects non-ready build artifacts before mutation. Stack build/resolution still needs the same contract completed.
...
- [~] Make compact stdout the default across all exec helpers.
  ...
  - Current branch `compact-default-stdout-pr-address-exec-helpers` adds shared compact-output infrastructure (`ts/packages/pr-address/src/stdout-mode.ts`), extends `--stdout-mode compact|full` across collection, planning, mutation, stack, payload, and summary helpers, flips compact to the default, preserves full output through session artifacts, and updates schemas plus CLI reference docs.
  - Evidence: local branch diff against Graphite parent `pr-address-session-resolved-lifecycle-artifacts`, PR #1567, and commit `7d293759b`. The row remains in progress because PR TypeScript CI is failing against the new compact-output shapes; completion requires fixing that validation drift and rerunning the TypeScript gate.
- [~] Remove composed-payload input styles for pipeline-produced artifacts.
  ...
  - Cutovers landed for `validate-feedback-classification`, `record-batch-checkpoint`, and `finalize-run`: removed lifecycle flags are usage errors, and lifecycle file input survives only for the agent-authored checkpoint evidence file. Remaining helpers with compatibility modes still need deliberate removal.
