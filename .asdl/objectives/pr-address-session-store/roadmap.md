...
- [~] Rewrite the pr-address skill and CLI references for the session-store flow.
  ...
  - First doc cutover landed in `skills/pr-address/references/cli-planning.md` for `validate-feedback-classification`. The lifecycle and mutation references now document session-only `record-batch-checkpoint`, session-only `finalize-run`, resolution-result artifacts, and removed-flag usage errors. The broader skill and remaining CLI references still need the final session-store rewrite.
