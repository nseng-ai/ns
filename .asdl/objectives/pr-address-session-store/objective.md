...
The remaining composed-payload glue is concentrated in compatibility paths and stale skill/docs guidance: `plan-feedback` still accepts `{manifest, classification}` for manual/debug compatibility, planning/read helpers still expose pipeline-produced explicit-input fallbacks, and the skill still teaches legacy wrapper/file flows. Compact-by-default stdout is implemented and locally validated after the schema-fixture follow-up reconciled the generated JSON schemas and stack orchestration fixtures with the compact-output contract. Stack batch-build now mirrors the session-resolved explicit-artifact mutation contract by resolving the latest stack plan and writing per-PR resolve-build artifacts for `resolve-thread-batch --from-build`.
...
- Clean input-style cutover: `--payload-file`/`--payload-json`/stdin composition is removed for pipeline-produced artifacts (manifests, plans, build payloads, checkpoints, feedback). This has started with `validate-feedback-classification`, where explicit manifest and wrapper sources were removed and file input survives only for the agent-authored classification packet. File input survives elsewhere only for agent-authored content: decisions files, classification answers, validation results.
