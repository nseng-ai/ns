...
1. The **findings-publication** workflow has no module and no end-to-end test surface — it lives in untested bash in `.github/workflows/roaster.yml`, glued across three `exec` commands by a temp file and a doubly-parsed run envelope.
...
3. The per-invocation execution environment (`cwd`, `env`, `signal`) is ambient to a roaster run yet re-threaded at every gateway call, on top of a `RoasterCliContext` that wraps `RoasterContext` (the `ctx.context.X` double hop).
...
This Objective tracks turning these candidates into actual depth: smaller interfaces, more leverage for callers, and locality for maintainers. The strongest and first is #1 — it converts the riskiest, least-tested part of roaster into one interface that can be tested end-to-end.

## Scope

- **Candidate 1 — collapse the findings-publication pipeline into one deep module.** Introduce one in-process publication module (working name `publishFindings(ctx, { prNumber, envelope, runUrl })`) that parses the run envelope once, classifies and posts inline findings, renders the summary comment (including inline status threaded as an in-memory value), and posts/updates the findings comment. The three `exec` commands collapse to one thin adapter (or remain thin adapters over the new module). The CI bash in `.github/workflows/roaster.yml` shrinks accordingly. Files in play: `src/operations/cli-operations.ts`, `src/findings-publication.ts`, `src/cli.ts`, `.github/workflows/roaster.yml`.
...
- **Candidate 3 — bind the execution environment into the context.** Bind `cwd`/`env`/`signal` once (at `runCli`) and flatten the two context records so handlers call gateways with work-shaped arguments only (e.g. `ctx.localDiff.loadDiff({ baseRef })`), removing the repeated `{ cwd, env }` threading and the `ctx.context.X` double hop. Gateway seams stay; the caller-facing interface narrows.
...
- The findings-publication workflow's only external consumer is the roaster GitHub Actions workflow (`.github/workflows/roaster.yml`); no other caller depends on the three `exec` commands as separate steps. If false, collapsing them to one adapter needs a compatibility path.
...
- **Over-binding the environment (candidate 3):** binding `cwd` at construction conflicts with `createRealRoasterContext()` being built before argv is known. Bind at `runCli` time, not at context construction, to keep both real and fake adapters honest.
