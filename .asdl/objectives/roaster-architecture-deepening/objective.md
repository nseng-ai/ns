...
- **Candidate 3 — shipped: capture the execution environment in the context.** `runCli` now builds raw `RoasterGateways`, then combines them with `{ cwd, env, signal }` through `createRoasterContext()` into a run-scoped `RoasterContext`. `RoasterCliContext` is flat, operation handlers call review catalog, local diff, harness, and GitHub methods with work-shaped arguments only, and `publishFindings()` accepts a GitHub-capable context plus semantic publication options. Raw gateway seams and tests remain intact for adapter isolation.
...
- **Environment capture timing (candidate 3) — de-risked:** the shipped facade captures invocation facts at `runCli` time, not inside `createRealRoasterGateways()`, so real and fake adapters can still be constructed before argv/runtime facts are known. The cancellation decision is resolved: optional `CliDeps.signal` enters the same run environment and is forwarded by the facade to raw gateways that already support it.
