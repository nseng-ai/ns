...
3. The per-invocation execution environment is now bound once at `runCli`: `RoasterGateways` is the raw adapter/dependency bundle, while `RoasterContext` is the prebound caller-facing context that captures `cwd`, `env`, and optional `signal`. Operation handlers receive a flat `RoasterCliContext` and call work-shaped methods such as `ctx.localDiff.loadDiff({ baseRef })`, `ctx.harness.runReview(request)`, and `ctx.github.*(...)` without threading ambient process facts.
