...
3. The per-invocation execution environment is now bound once at `runCli`: the raw `RoasterContext` remains the adapter/dependency container, while `bindRoasterContext()` creates a run-scoped facade that captures `cwd`, `env`, and optional `signal`. Operation handlers now receive a flat `RoasterCliContext` and call work-shaped methods such as `ctx.localDiff.loadDiff({ baseRef })`, `ctx.harness.runReview(request)`, and `ctx.github.*(...)` without threading ambient process facts.
...
- **Candidate 3 — shipped: bind the execution environment into the context.** `runCli` now builds the raw `RoasterContext` as before, then binds it with `{ cwd, env, signal }` through `bindRoasterContext()` into a per-invocation `RoasterRunContext`. `RoasterCliContext` is flat, operation handlers call bound review catalog, local diff, harness, and GitHub methods with work-shaped arguments only, and `publishFindings()` accepts a bound GitHub-capable context plus semantic publication options. Raw gateway seams and tests remain intact for adapter isolation.
...
- **Over-binding the environment (candidate 3) — de-risked:** the shipped facade binds at `runCli` time, not inside `createRealRoasterContext()`, so real and fake adapters can still be constructed before argv/runtime facts are known. The cancellation decision is resolved: optional `CliDeps.signal` enters the same bound run environment and is forwarded by the facade to raw gateways that already support it.
