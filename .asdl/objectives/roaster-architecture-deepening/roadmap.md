...
- [ ] Collapse the findings-publication pipeline into one deep `publishFindings` module (candidate 1, top recommendation).
    Parse the run envelope once; classify + post inline findings; render the summary with inline status threaded in memory; post/update the findings comment. Collapse the three `exec` commands into one thin adapter and shrink the CI bash in `.github/workflows/roaster.yml`. Remove the temp-file round-trip and the double envelope parse.
...
- [ ] Bind the execution environment into the roaster context (candidate 3).
    Bind `cwd`/`env`/`signal` once at `runCli` and flatten `RoasterCliContext`/`RoasterContext` so handlers call gateways with work-shaped arguments only. Keep gateway seams; narrow the caller-facing interface. Bind at run time, not at context construction.
