...
- [x] Capture the execution environment in the roaster context (candidate 3).
      Shipped on branch `bind-roaster-run-context-facade`: `runCli` combines raw `RoasterGateways` with `cwd`, `env`, and optional `signal` into a run-scoped `RoasterContext`; operation handlers use a flat `RoasterCliContext` and work-shaped gateway methods; `publishFindings()` now accepts a GitHub-capable context and semantic options only. Raw gateway interfaces stay intact.
