...
The public `pr-address` skill, the internal stack-address skill, and the CLI reference now document that `build-resolve-thread-batch-payload` accepts single-PR `plan-feedback` data only. Until a stack-native payload builder exists, stack plans remain planning provenance and agents must use per-PR plan data for each PR/batch.
...
- Add stack-native resolution payload building from validated stack plans plus explicit decisions.
