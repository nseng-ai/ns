...
1. `aretro-ts-skill-distribution-cutover` — make the `branch-retro` runner prefer the TypeScript CLI for repo-local use, add an opt-in `just install-aretro` source shim, audit checkout-free/prod use, and update active docs away from Python/default `asdl aretro` examples.
2. `aretro-ts-retire-python` — only after prior parity and cutover gates pass, remove Python package/workspace/build/test/publish wiring, preserve rollback/reference evidence, grep for stale active references, and update the umbrella TypeScript migration Objective/playbook.
...
1. `aretro-ts-skill-distribution-cutover`
