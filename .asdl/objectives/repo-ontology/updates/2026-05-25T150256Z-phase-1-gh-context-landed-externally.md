...
Evidence: these changes are already merged on master (#555, #580); this update writes their post-landing effect on the Objective rather than tracking an open branch. `/CONTEXT-MAP.md` already links the Gh anchor as _Present_ (landed with #555), so no map edit was needed here.

## Objective Impact

- `roadmap.md`: Phase 1 `## Gh` task marked `[x]` with completion evidence noting external authorship via #555/#580, the three resolved disambiguations, and that the map already marks the Gh anchor _Present_. This clears the stale `PRDetails` / `IssueGateway` references the task framing carried.
