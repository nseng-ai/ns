...
The asdl monorepo has rich domain language in the root Objective-system context, across 7 tracked Python workspace packages, and now in the repo-local TypeScript/Pi extension package. One Python package, the multi-subpackage `asdl-core`, is itself several subdomains. The Clinkr H2 section of `packages/asdl-core/CONTEXT.md` (commit `8de0a295`) showed that the `grill-with-docs` skill produces high-signal glossaries; the ongoing improvement is to run the same workflow across the repo-level context and every package or extension surface with meaningful domain language, then tie them together with a root `CONTEXT-MAP.md`.

The outcome: a contributor or agent landing on the repo can navigate the asdl ontology from a single entry point, with cross-context naming collisions (Review/Comment/Feedback, State/status variants, active/root status, branch/ref/plan usage) explicitly flagged rather than buried in source. The Phase 0 scaffold was correct as an initial slice, but the repo has since gained `packagechk` as a standalone workspace package, substantial checked-in Pi extension workflows under `ts/packages/pi-extensions/`, and Objective archive/unarchive mechanics; the Objective therefore includes rebaselines as live repo language lands.
...
- Phase 3 — Create per-package `CONTEXT.md` for `asdl-pr-address`, `asdl-reviewer`, `asdl-slots`, `asdl-objectives`, and `packagechk`. The first four are peer asdl plugin packages; `packagechk` is a standalone CLI package whose registry/claimability vocabulary belongs in the repo map even though it has no `asdl-core` dependency. `@asdl/pi-extensions` is handled by Phase 0.6 because its implementation and docs already landed before the Python package sessions completed.
...
- `/CONTEXT-MAP.md` exists at repo root, links the existing root `CONTEXT.md`, lists all 7 in-scope Python package contexts (`asdl-core`, `brmem`, `asdl-pr-address`, `asdl-reviewer`, `asdl-slots`, `asdl-objectives`, and `packagechk`) plus the repo-local `@asdl/pi-extensions` context with active links as they exist, includes an explicit "Out of scope" note for `asdl-dispatcher`, and does not describe `asdl-initiatives` as a tracked package unless the map is explicitly noting its absence.
...
- `packages/asdl-core/CONTEXT.md` remains the single asdl-core context file and contains H2 sections for `Clinkr`, `Git`, `Gt`, `Gh`, and `Top-level utilities`. Each section follows the Clinkr pattern: Language entries with `Avoid:` aliases, followed by Relationships. The `packages/asdl-core/src/asdl_core/gt/CONTEXT.md` split must be reconciled rather than left as a competing per-subpackage glossary.
...
- Cross-context naming collisions surfaced during sessions (the "Review" overload across `gh` / `pr-address` / `reviewer`, "State/status" usage across `gh`, `format.state_badge`, Objectives, slots, and `packagechk.CheckStatus`, active/root status, branch/ref usage across `git`/`gt`/`slots`/`brmem`, and plan terminology across Objectives / planned branches / Branch Memory) are either resolved identically across files or recorded in the map's "Flagged ambiguities" section.
...
- The context inventory can drift during a multi-session documentation sweep. The current closure target is the root context, 7 in-scope Python package contexts, and the repo-local `@asdl/pi-extensions` context; if another tracked package or checked-in extension surface with real domain language lands before closure, the map should be rebaselined before more per-context sessions proceed.
...
- Package/context inventory drift already materialized twice: `packagechk` became a tracked standalone package while `asdl-initiatives` disappeared, and later `@asdl/pi-extensions` plus Objective archive mechanics became substantial repo language. Mitigation: Phase 0.5 and Phase 0.6 explicitly rebaseline the map, root context, and completion criteria before closure.
...
- The repo's domain language drifts as packages add new concepts after their sessions close. This materialized again before closure: the not-yet-written `asdl-objectives` context gained the opt-in `objective gt` Graphite stack-projection surface and a known-real `asdl-objectives → asdl-core.gt` edge before Phase 3. Mitigation: this ground-truth rebaseline updates Phase 3/4 so later package/map sessions capture it; future changes after a package session remain closure-note or follow-on material unless they invalidate the map's package inventory or relationship edges, in which case rebaseline before closure.
- Non-Goal conflict surfaced: `packages/asdl-core/src/asdl_core/gt/CONTEXT.md` now exists as a Graphite metadata-store contract doc, which conflicts with the single-file-H2 Non-Goal for asdl-core context. Mitigation: Phase 1 now includes a reconcile task to fold that contract into, or carve it out against, the `## Gt` H2 section; resolve this before closure because the readback/completion criteria assume one asdl-core context file.
