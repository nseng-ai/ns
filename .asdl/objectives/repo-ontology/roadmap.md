...
- [ ] Update `/CONTEXT-MAP.md`'s Inventory Baseline from the stale four-package TypeScript claim to the current nine tracked `ts/packages/*` workspace packages: `asdl-dev`, `@asdl/pi-extensions`, `@asdl/pi-extension-runtime`, `@asdl/ccc`, `@asdl/core`, `@asdl/clinkr`, `@asdl/planned-branch`, `@asdl/plans`, and `@asdl/pr-address`.
- [ ] Decide and record a context status for each new TypeScript package — planned context, accepted-from-adjacent-Objective context, or explicitly out of scope with a revisit trigger — instead of leaving `@asdl/core`, `@asdl/clinkr`, `@asdl/planned-branch`, `@asdl/plans`, and `@asdl/pr-address` absent from the map.
