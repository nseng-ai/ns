...
- [x] Create `packages/asdl-handoff/CONTEXT.md` for directed handoff artifact vocabulary: Handoff artifact, continuation focus, handoff slug/key, the handoff namespace (now singular `handoff` after the 2026-06 rename), `HandoffSummary`, branch state (`active`/`deleted`), list scope, all-branches inventory, garbage-collection preview/delete actions, Entry Locator, and updated-at ordering.
      Evidence: `packages/asdl-handoff/CONTEXT.md` exists and currently defines Handoff Artifact, Continuation Focus, Create a Handoff, Pick Up a Handoff, Handoff Slug, Handoff Key, Handoff Namespace (`handoff`, with `handoffs` as an `Avoid:` alias), Handoff Summary, List Handoffs, Handoff Technical Locator, Branch State, List Scope, All-Branches Inventory, Delete a Handoff, and Handoff Garbage Collection with `Avoid:` aliases. The glossary tracked the trunk-side singular-namespace and Create/Pick Up/List/Delete vocabulary renames.
...
- [ ] Create `packages/asdl-pr-address/CONTEXT.md` for PR addressing behavior around core `PRReviewThread`, `PRReviewComment`, `PRDiscussionComment`, reactions, feedback, thread resolution, and replies.
      Status: the `pr-address-typescript-port` Objective now owns the `pr-address` consumer migration to `@asdl/pr-address`; re-derive this slice against that cutover plan (Phase 15.5 decision) before executing it, so the context targets the surviving surface.
...
- [ ] Refresh `ts/packages/pi-extensions/CONTEXT.md` against the current extension inventory: `.pi/extensions/asdl-dev.ts`, `code.ts`, handoff, objective, `branch-context.ts` (renamed on trunk from the `planned-branch` extension), runner-subagent, grill-ui, the unified Graphite-aware `/code:land`, CLI-command bridge, worktree status, terminal presentation, command-output rendering/summarization surfaces, and the CCC delegation boundary for autobranch/land/worktree-status orchestration.
...
Phase 15.5 — TypeScript workspace rebaseline

- [ ] Update `/CONTEXT-MAP.md`'s Inventory Baseline from the stale four-package TypeScript claim to the current nine tracked `ts/packages/*` workspace packages: `asdl-dev`, `@asdl/pi-extensions`, `@asdl/pi-extension-runtime`, `@asdl/ccc`, `@asdl/core`, `@asdl/clinkr`, `@asdl/branch-context` (dir `ts/packages/branch-context`, renamed on trunk from `@asdl/planned-branch`), `@asdl/plans` (CLI/skills now `enriched-plan`), and `@asdl/pr-address`. The map's Relationships section already uses `@asdl/branch-context`, so this catch-up also makes the baseline consistent with the rest of the map.
- [ ] Decide and record a context status for each new TypeScript package — planned context, accepted-from-adjacent-Objective context, or explicitly out of scope with a revisit trigger — instead of leaving `@asdl/core`, `@asdl/clinkr`, `@asdl/branch-context`, `@asdl/plans`, and `@asdl/pr-address` absent from the map.
- [ ] Verify the adjacent-Objective contexts `ts/packages/ccc/CONTEXT.md` and `ts/packages/pi-extension-runtime/CONTEXT.md` against this Objective's contract (Language entries with `Avoid:` aliases where relevant, followed by Relationships); record gaps as follow-up rows rather than silently accepting the files.
- [ ] Re-derive the Phase 7 `asdl-pr-address` slice against the `pr-address-typescript-port` cutover plan: decide whether the PR-addressing context target stays on the Python package, moves to `@asdl/pr-address`, or waits for the cutover.

Phase 16 — final map and readback

- [ ] Populate the final Relationships section of `/CONTEXT-MAP.md` with concrete cross-package and extension edges only: `areg → asdl-core.project_config` plus external `gh`/`npx skills`; `brmem → asdl-core.git + asdl-core.clinkr`; `asdl-handoff → brmem + asdl-core.git + asdl-core.clinkr + asdl-core.console/format/plugin`; `asdl-pr-address → asdl-core.gh + asdl-core.git + asdl-core.clinkr/plugin`; `roaster → asdl-core.gh + asdl-core.git + asdl-core.clinkr/plugin`; `asdl-slots → asdl-core.git + asdl-core.gh + asdl-core.gt + asdl-core.clinkr/console/plugin`; `asdl-objectives → asdl-core.git + asdl-core.clinkr + asdl-core.console/format/plugin`; `aretro → asdl-core.sessions + asdl-core.git + asdl-core.clinkr/plugin`; `vibechk` as standalone/no-`asdl-core` with git/runner/store boundaries; `asdl-dev → git + gt + Vercel + Pi text generation`; `@asdl/pi-extensions → Pi runtime + asdl-dev + git/gt/gh/brmem/objective/slot CLIs`; and `packagechk` as standalone/no-`asdl-core`. Refresh this enumeration with the Phase 15.5 TypeScript decisions (`@asdl/pi-extension-runtime`, `@asdl/ccc`, and any accepted new TypeScript contexts) before finalizing.
