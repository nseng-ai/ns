...
The roadmap item to make GitHub diff/file discovery large-PR aware is now complete for roaster's current paths. Roaster avoids the observed `gh pr diff` / `PullRequest.diff too_large` failure mode by using local checkout diffs for review discovery/execution and paginated PR file metadata only for inline-comment placement.
