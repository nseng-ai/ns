...
- Model-text steps can move onto the backend-neutral text-generation abstraction already used by `asdl-dev cp`/`submit`, so slug/summary generation need not stay Pi-locked. Partially validated by the 2026-06-09 sweep: the abstraction exists and `ASDL_DEV_CHECKPOINT_MODEL` overrides the checkpoint default, but slug generation still hardcodes `openai-codex/gpt-5.4-mini` in three duplicated `model-slug.ts` helpers (`pi-extensions`, `ccc`, `planned-branch`), so the assumption is not yet realized for slugs.
