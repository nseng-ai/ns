...
- Parity-table rot: the table is worthless if updating it is not enforced. This risk has now materialized **five** times: stale `/objective:gt-stacks` rows, the stale `/handoff:list` PARTIAL row, the pre-2026-06-09 backlog (`/cmux:*` → `/ccc:*` rename, the `/code:land` unification, several unlisted surfaces), the pre-2026-06-10 backlog (the `/model:*` family, `/ccc:claude-plan-tab`, `/code:pr-regen`, and the `/planned-branch:upstack-impl-session` rename all accumulated unlisted), and the post-2026-06-10 drift caught 2026-06-12 (the new `/claude:handoff` command landed unlisted within two days of the full record rewrite, alongside the `/internal-code-workflows` → `/code-workflows` rename and router restructuring leaving stale references in this record). The full-sweep workflow works as a corrective control, but refreshes still lag surface changes; diff-scoped review at change time remains the weak point. The 2026-06-10 update's standing trigger — consider prioritizing the parked CI parity gate on a fifth materialization — has now fired.
...
- The fifth parity-table-rot materialization (2026-06-12) fired the 2026-06-10 follow-up trigger: should the parked machine-checkable CI parity gate be promoted to active work now, ahead of or alongside the push-down rows?
