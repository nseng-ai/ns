...
- Parity-table rot: the table is worthless if updating it is not enforced. This risk has already materialized once: the table still listed removed `/objective:gt-stacks` / `objective gt stacks` as FULL after the live CLI/Pi surface had been removed. The specific stale row is corrected by this refresh, but the discipline risk remains until the parity-review skill's full-sweep mode is in place.
