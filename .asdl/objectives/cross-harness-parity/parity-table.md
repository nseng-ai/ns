...
| `/objective:gt-stacks` | Objective × Graphite view | `objective gt stacks --format md` | `objective` | FULL | Projection + render in Python. |
...
| `/cmux:pr-sidebar` | Sidebar from current PR work | `asdl exec cmux-workspace-summary` | `cmux-sidebar` | FULL | Mutation CLI is scenario-tested; Pi adds fast-model swap + auto-trigger. |
| `/cmux:objective-sidebar` | Sidebar from an Objective | `asdl exec cmux-workspace-summary` | `cmux-sidebar` | FULL | |
...
| `/cmux-slot:dispatch-plan` | Plan → branch → slot → cmux | `slot`/`brmem`/`cmux` prims; no orchestration CLI | _none_ | NONE | ~1020 lines of TS sequence + Pi-session plan source. Push down with explicit inputs. |
| `/cmux-dispatch` | Prompt → branch → slot → cmux | `git`/`gt`/`slot`/`cmux` prims | _none_ | NONE | Hardcodes `pi @file` launch; needs orchestration CLI + agent-neutral launch. |
| `/cmux-slot:open-branch` | Branch → slot → cmux | `slot checkout` + `cmux new-workspace` | _none_ | PARTIAL | Explicit-branch case is two runnable CLIs; needs a thin skill. Planned-branch inference is Pi-only. |
