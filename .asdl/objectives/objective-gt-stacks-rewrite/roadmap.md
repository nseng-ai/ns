...
- [~] Add failing projection tests for `parent..branch` slice attribution, many-to-many branch/Objectives, `also_touches`, active-root-only touches, archive-root ignores, Objective status projection from trunk, latest-work tie-breaking, deterministic ordering, and warning de-duplication. Evidence so far: projection-core tests cover parent-slice attribution, many-to-many grouping, `also_touches`, open/closed/in-flight trunk status projection, latest-work tie-breaking, group/row ordering, and warning de-duplication. Remaining follow-up: explicit projection-level archive-root-only and any additional edge cases that emerge during CLI/renderer coverage.
- [~] Implement and refactor the deep semantic projection module until the projection tests pass with locality: Graphite graph and Git/Objective reads in, semantic Objective stack projection out. Evidence so far: `gt_stack_projection.py` now exposes semantic dataclasses and `build_objective_gt_stack_projection(repo_root, git, gt)` over `GtGateway.branch_graph()` and `GitGateway` reads, with JSON-data conversion free of glyphs/rendering annotations. Remaining follow-up: harden failure envelopes and any uncovered edge cases when wiring CLI/renderers.
