...
- [x] Build the steel thread: a first-party handoff workflow inspired by the existing handoff skill that writes the next-session handoff document into Branch Memory for the current branch (`skills/brmem-handoff/SKILL.md`).
- [x] Decide and document the Branch Memory storage convention, including namespace/base area, entry key, overwrite policy, and expected retrieval command (`handoff/current.md`, branch-scoped, overwrite current, load with `brmem get`).
- [x] Add next-session loading guidance so an agent can find the stored handoff and continue from it reliably.
...
- [x] Update skill documentation so agents know when to use the Branch Memory handoff workflow instead of the vendored temp-file handoff.
