...
- Implement `initiative exec read-initiative <slug-or-path>` to read one explicit Initiative record. It should require an explicit slug or path, validate paths under `.asdl/initiatives/<slug>/`, report file inventory, detect `closed.md`, and support both `--format json` and `--format md`. JSON should return stable facts and paths by default; Markdown output should include the raw `initiative.md`, `roadmap.md`, and all update Markdown by default.
...
- Add tests for JSON contracts, Markdown renderers, path validation, closed-marker detection, missing files, explicit missing-slug behavior, filesystem inventory edge cases, and changed-path classification for Tracking Gate facts.
...
- `initiative exec read-initiative <slug-or-path> --format json` handles explicit slug/path selection, invalid paths, missing slug/path, closed records, and missing expected files with stable JSON facts and paths; `--format md` includes raw `initiative.md`, `roadmap.md`, and all update Markdown by default.
