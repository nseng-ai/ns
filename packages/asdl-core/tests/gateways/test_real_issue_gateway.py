...
from asdl_core.gh import real_gateway_helpers, real_issue_gateway
from asdl_core.gh.real_issue_gateway import RealIssueGateway
from asdl_core.gh.types import PRInlineCommentInput, PRLookupError
    ...
    *,
    ...
    def fake_run(
        ...
        **kwargs: object,
        ...
        if cmd[:2] == ["gh", "api"] and cmd[2] == "repos/dagster-io/asdl/pulls/47/files":
            assert "--paginate" in cmd
...
def test_add_comment_posts_body_and_returns_issue_comment(
    ...
    assert comment.author == "schrockn"
    assert comment.url == "https://github.com/dagster-io/asdl/pull/47#issuecomment-5555"
    ...
    assert post_cmd[4] == "repos/dagster-io/asdl/issues/47/comments"
    ...
    monkeypatch.setattr(
        ...
        _make_fake_run(
            add_comment_response={
                ...
                "body": "ghost comment",
                "user": None,
                "html_url": "https://github.com/dagster-io/asdl/pull/47#issuecomment-6666",
            }
        ),
    )
...
def test_add_reaction_targets_comment_id_in_path(
    ...
    post_calls = [c for c in calls if c[:4] == ["gh", "api", "--method", "POST"]]
    assert len(post_calls) == 1
    assert post_calls[0][4] == "repos/dagster-io/asdl/issues/comments/99999/reactions"
...
def test_get_pr_for_branch_returns_summary(
    ...
    result = RealIssueGateway().get_pr_for_branch("asdl-pr-address-skill")

    assert result is not None
    assert not isinstance(result, PRLookupError)
    assert result.number == 47
    assert result.title == "Port pr-address skill"
    assert result.url == "https://github.com/dagster-io/asdl/pull/47"
    ...
    discussion_comment_pages = [
        [
            ...
            },
            {
                ...
                "body": "<!-- asdl-reviewer:dignified-python -->\nFake!",
                "user": {"login": "alice"},
                "html_url": "https://github.com/dagster-io/asdl/pull/47#issuecomment-102",
            },
        ]
    ]
    monkeypatch.setattr(
        ...
        [
            {
                ...
                "body": "<!-- asdl-reviewer:dignified-python -->\nI'm a human!",
                "user": {"login": "alice"},
                "html_url": "https://github.com/dagster-io/asdl/pull/47#issuecomment-999",
            },
        ]
    ]
    monkeypatch.setattr(
...
def test_update_comment_sends_patch_and_returns_updated_comment(
    ...
    cmd = patch_calls[0]
    assert cmd[4] == "repos/dagster-io/asdl/issues/comments/101"
...
def test_get_pr_changed_files_fetches_paginated_rest_files(
    ...
    assert file_calls == [["gh", "api", "repos/dagster-io/asdl/pulls/47/files", "--paginate"]]
...
def test_get_pr_review_comments_fetches_paginated_rest_comments(
    ...
    assert comment_calls == [["gh", "api", "repos/dagster-io/asdl/pulls/47/comments", "--paginate"]]
