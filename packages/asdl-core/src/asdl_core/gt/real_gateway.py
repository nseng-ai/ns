...
kept the required slice stable while adding nullable columns via Kysely
migrations. If a future migration renames or removes one of the required
columns, metadata reads return a schema-mismatch ``GtCommandFailure`` instead of
falling back to human-facing ``gt`` log text parsing.
"""
