...
When the harness exposes per-dispatch model selection, bounded artifact-backed classification or summarization tasks may request the harness's configured cheap/fast model (for example, `openai-codex/gpt-5.4-mini:medium` on an OpenAI Codex-backed setup, or `claude-haiku-4-5` on an Anthropic Claude-backed setup); when a workflow needs a concrete stronger escalation target, use the harness's configured stronger model (for example, `openai-codex/gpt-5.5:high` or `claude-opus-4-8`). The parent must still validate the result against deterministic manifests, schemas, counts, or locators, and must escalate to a stronger model when validation fails or judgment requires broader code context.
