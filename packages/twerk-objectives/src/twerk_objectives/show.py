...
def _format_file_header(file: ObjectiveFile) -> str:
    ...
    """
    if file.source_branch == FALLBACK_TRUNK_BRANCH:
        source = f"canonical: {FALLBACK_TRUNK_BRANCH}"
    else:
...
def _read_file(
    ...
    *,
    ...
    if current_branch is not None and current_branch != FALLBACK_TRUNK_BRANCH:
    ...
    if canonical_present:
        content = gateway.get(OBJECTIVE_NAMESPACE, key, FALLBACK_TRUNK_BRANCH)
        if content is not None:
            return ObjectiveFile(filename=filename, source_branch=FALLBACK_TRUNK_BRANCH, content=content)
    return None
