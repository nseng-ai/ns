...
def nsx_group() -> None:
    """Run skills from dagster-io/asdl-tools without installing."""


@nsx_group.command("list")
@click.pass_obj
def nsx_list_cmd(ctx: AregContext) -> None:
    """List available skills from dagster-io/asdl-tools."""
    requires_gh()
    result = list_skills(_DEFAULT_SKILL_REPO, gh=ctx.gh)
    _emit(result)
    if not result.success:
        raise SystemExit(1)
...
@click.option("--skill", "skill_name", default=None, help="Specific skill to fetch.")
@click.pass_obj
def nsx_fetch_cmd(ctx: AregContext, skill_name: str | None) -> None:
    """Fetch a skill from dagster-io/asdl-tools into a temp directory."""
    requires_npx()
    result = fetch_skill(_DEFAULT_SKILL_REPO, skill_name, npx_skills=ctx.npx_skills)
    _emit(result)
    if not result.success:
        raise SystemExit(1)
