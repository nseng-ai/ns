...
class GetFeedbackResult:
    ...
    discussion_comments: tuple[GhIssueComment, ...]
...
def run_get_feedback(
    ...
    return GetFeedbackResult(
        ...
        review_threads=gateway.get_pr_review_threads(
            request.pr_number, include_resolved=request.include_resolved
        ),
        discussion_comments=gateway.get_pr_discussion_comments(request.pr_number),
    )
