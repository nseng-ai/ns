...
def _build_digest_prompt(
    ...
    return (
        ...
        ">>>\n"
        "\n"
        "# Step 4 — Remaining work (one bullet per unfinished roadmap entry)\n"
        "\n"
        "Render one bullet per remaining unfinished numbered entry in the canonical\n"
        "roadmap below. Bullet shape:\n"
        "`- **<entry headline>.** <one short sentence>`. The headline copies\n"
        "the numbered entry title (e.g. `3. Inline line-level comments`); the\n"
        "sentence summarizes what's left in that entry in ≤25 words. Skip\n"
        "entries whose bullets are all checked. If every entry is done, render\n"
