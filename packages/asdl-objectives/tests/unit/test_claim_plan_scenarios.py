"""Scenario-style coverage for the internal claim planner.
...
def _assert_hard_failure(
    ...
    *,
    ...
    with pytest.raises(ClinkrFailure) as exc_info:
        plan_claim_objective(ctx, request)

    assert exc_info.value.error_type == error_type


def test_claim_plan_explicit_slug_falls_back_to_canonical() -> None:
    gateway = FakeBranchMemoryGateway()
...
def test_claim_plan_explicit_slug_explicit_from_branch() -> None:
    gateway = FakeBranchMemoryGateway()
    gateway.put("objectives", "widget-rewrite/body.md", "master", "# canonical\n")
    ...
    data = _plan_data(
        ctx,
        ClaimPlanRequest(slug="widget-rewrite", from_branch="feat/source"),
    )
...
def test_claim_plan_explicit_slug_explicit_from_file(tmp_path: Path) -> None:
    ...
    data = _plan_data(
        ctx,
        ClaimPlanRequest(slug="widget-rewrite", from_file=str(body_path)),
    )
...
def test_claim_plan_no_slug_single_candidate_ancestor() -> None:
    gateway = FakeBranchMemoryGateway()
    ...
    data = _plan_data(ctx, ClaimPlanRequest())
...
def test_claim_plan_no_slug_multi_candidate_ancestor() -> None:
    gateway = FakeBranchMemoryGateway()
...
def test_claim_plan_no_slug_canonical_fallback_lists_master_slugs() -> None:
    gateway = FakeBranchMemoryGateway()
...
def test_claim_plan_no_slug_nothing_reachable() -> None:
...
def test_claim_plan_explicit_slug_not_found_anywhere() -> None:
    ctx = _make_ctx(gateway=FakeBranchMemoryGateway(), branches=("master", "feat/x"))

    data = _plan_data(ctx, ClaimPlanRequest(slug="ghost"))
...
def test_claim_plan_from_branch_missing_slug() -> None:
    gateway = FakeBranchMemoryGateway()
...
def test_claim_plan_from_file_missing(tmp_path: Path) -> None:
    ctx = _make_ctx(gateway=FakeBranchMemoryGateway())

    data = _plan_data(
        ctx,
        ClaimPlanRequest(slug="widget-rewrite", from_file=str(tmp_path / "does-not-exist.md")),
    )
...
def test_claim_plan_detached_head_without_target_fails_hard() -> None:
    ctx = _make_ctx(
        gateway=FakeBranchMemoryGateway(),
    ...
    _assert_hard_failure(
        ctx,
        ClaimPlanRequest(slug="widget-rewrite"),
        error_type="detached_head",
    )


def test_claim_plan_target_trunk_fails_hard() -> None:
    ctx = _make_ctx(gateway=FakeBranchMemoryGateway())

    _assert_hard_failure(
        ctx,
        ClaimPlanRequest(slug="widget-rewrite", target="master"),
        error_type="target_is_trunk",
    )


def test_claim_plan_from_branch_and_from_file_together_fail_hard(tmp_path: Path) -> None:
    body_path = tmp_path / "body.md"
...
def test_claim_plan_from_branch_without_slug_fails_hard() -> None:
    ctx = _make_ctx(gateway=FakeBranchMemoryGateway())

    _assert_hard_failure(
        ctx,
        ClaimPlanRequest(from_branch="feat/source"),
        error_type="source_flag_without_slug",
    )


def test_claim_plan_from_file_without_slug_fails_hard(tmp_path: Path) -> None:
    body_path = tmp_path / "body.md"
    body_path.write_text("# body\n", encoding="utf-8")
    ctx = _make_ctx(gateway=FakeBranchMemoryGateway())
    ...
    _assert_hard_failure(
        ctx,
        ClaimPlanRequest(from_file=str(body_path)),
        error_type="source_flag_without_slug",
    )


def test_claim_plan_envelope_shape_is_deterministic(tmp_path: Path) -> None:
    body_path = tmp_path / "body.md"
    body_path.write_text("# body\n", encoding="utf-8")
    ctx = _make_ctx(gateway=FakeBranchMemoryGateway())
...
def test_claim_plan_nearest_ancestor_wins() -> None:
    gateway = FakeBranchMemoryGateway()
...
def test_claim_plan_tied_ancestors_are_ambiguous() -> None:
    gateway = FakeBranchMemoryGateway()
