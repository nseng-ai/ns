...
def test_objective_gt_stacks_json_contract_groups_objective_segments(
    cli_group: ClinkrGroup,
    tmp_path: Path,
) -> None:
    graph = _gt_graph(
        _gt_branch("main", None, children=("feat/a", "feat/b")),
        _gt_branch("feat/a", "main", children=("feat/connector",), validation_result="OK"),
        _gt_branch(
            "feat/connector",
            "feat/a",
            children=("feat/c",),
...
def test_objective_gt_stacks_markdown_renders_objective_segments(
    cli_group: ClinkrGroup,
    tmp_path: Path,
) -> None:
...
def test_objective_gt_stacks_ignores_archive_root_and_includes_active_root_touches(
    ...
    graph = _gt_graph(
        _gt_branch("main", None, children=("feat/delete", "feat/archive")),
        _gt_branch("feat/delete", "main"),
        _gt_branch("feat/archive", "main"),
    )
    ctx = _gt_context(
        repo_root=tmp_path,
        graph=graph,
        path_change_touches_by_ref_path={
            ("main..feat/delete", ".asdl/objectives"): (
                _change_touch(
                    "delete-active",
                    "2026-05-20T10:00:00Z",
            ...
            ("main..feat/archive", ".asdl/objectives"): (
                _change_touch(
                    "archive-only",
                    "2026-05-20T11:00:00Z",
    ...
    assert result.exit_code == 0, result.output
    objectives = json.loads(result.output)["data"]["objectives"]
    assert [objective["slug"] for objective in objectives] == ["deleted"]
    assert objectives[0]["latest_work"]["branch"] == "feat/delete"


def test_objective_gt_stacks_skips_graphite_branches_without_local_refs(
    cli_group: ClinkrGroup,
    tmp_path: Path,
) -> None:
    graph = _gt_graph(
        _gt_branch("main", None, children=("feat/local", "feat/stale")),
        _gt_branch("feat/local", "main"),
        _gt_branch("feat/stale", "main"),
    )
    ctx = _gt_context(
        repo_root=tmp_path,
        graph=graph,
        local_branches=("main", "feat/local"),
        path_change_touches_by_ref_path={
            ("main..feat/local", ".asdl/objectives"): (
                _change_touch(
                    "local-alpha",
                    "2026-05-20T10:00:00Z",
                    ".asdl/objectives/alpha/objective.md",
            ...
            ("main..feat/stale", ".asdl/objectives"): (
                _change_touch(
                    "stale-beta",
                    "2026-05-20T11:00:00Z",
                    ".asdl/objectives/beta/objective.md",
...
def test_objective_gt_stacks_graphite_failure_returns_failure_envelope(
    cli_group: ClinkrGroup,
    tmp_path: Path,
) -> None:
    ctx = _gt_context(
        repo_root=tmp_path,
        graph=GtCommandFailure(message="metadata missing", returncode=None),
    )
...
def _change_touch(oid: str, committed_iso: str, *paths: str) -> PathChangeTouch:
...
def _gt_graph(*branches: GtTrackedBranch) -> GtBranchGraph:
    return GtBranchGraph(trunk="main", branches=branches, warnings=())


def _local_branches_from_graph(
    graph: GtBranchGraph | GtCommandFailure,
    local_branches: tuple[str, ...] | None,
) -> tuple[str, ...]:
...
def _invoke_gt_stacks_json(
    cli_group: ClinkrGroup,
    ctx: ObjectiveGtCliContext | ObjectiveCliUnavailable,
) -> Result:
    return CliRunner().invoke(
        cli_group,
        ["gt", "stacks", "--format", "json"],
...
def _invoke_gt_stacks_md(cli_group: ClinkrGroup, ctx: ObjectiveGtCliContext) -> Result:
    return CliRunner().invoke(
        cli_group,
        ["gt", "stacks", "--format", "md"],
        obj=build_clinkr_context_object(lambda: ctx),
