...
def test_tree_help(cli_group: ClinkrGroup) -> None:
    ...
    assert "Usage: memjective tree" in result.output
    assert "Display the tree of branches carrying a memjective snapshot" in result.output
...
def _seed_sole_memjective_on_feat_x() -> FakeBranchMemoryGateway:
    gateway = FakeBranchMemoryGateway()
    gateway.put("memjectives", "widget-rewrite/body.md", "master", "seed\n")
    gateway.put("memjectives", "widget-rewrite/body.md", "feat/x", "snap\n")
...
def test_tree_auto_resolves_sole_memjective_on_current_branch(cli_group: ClinkrGroup) -> None:
    obj = _make_obj(
        gateway=_seed_sole_memjective_on_feat_x(),
...
def test_tree_auto_resolve_ambiguous_fails(cli_group: ClinkrGroup) -> None:
    gateway = FakeBranchMemoryGateway()
    gateway.put("memjectives", "slug-a/body.md", "feat/x", "snap\n")
    gateway.put("memjectives", "slug-b/body.md", "feat/x", "snap\n")
...
def test_tree_explicit_slug_happy_path(cli_group: ClinkrGroup) -> None:
    gateway = FakeBranchMemoryGateway()
    gateway.put("memjectives", "widget-rewrite/body.md", "master", "seed\n")
    gateway.put("memjectives", "widget-rewrite/body.md", "widget-rewrite-layer-1", "snap\n")
    obj = _make_obj(
        gateway=gateway,
...
def test_tree_unknown_slug_is_negative(cli_group: ClinkrGroup) -> None:
    obj = _make_obj(gateway=_seed_sole_memjective_on_feat_x(), live_branches=("master",))
...
def _seed_mixed() -> FakeBranchMemoryGateway:
    ...
    gateway.put("memjectives", f"{slug}/body.md", "widget-rewrite-groundwork", "snap\n")
    gateway.put("memjectives", f"{slug}/body.md", "widget-rewrite-layer-1", "snap\n")
    gateway.put("memjectives", f"{slug}/body.md", "widget-rewrite-layer-2", "snap\n")
    gateway.put("memjectives", f"{slug}/body.md", "widget-rewrite-closed", "snap\n")
    gateway.put("memjectives", f"{slug}/body.md", "widget-rewrite-stale-broken", "snap\n")
    return gateway
...
def test_tree_human_output_omits_stale_marker(cli_group: ClinkrGroup) -> None:
    gateway = FakeBranchMemoryGateway()
    gateway.put("memjectives", "widget-rewrite/body.md", "master", "seed\n")
    gateway.put("memjectives", "widget-rewrite/body.md", "widget-rewrite-stale", "snap\n")
    obj = _make_obj(
        gateway=gateway,
