...
`brmem` is a generic branch-scoped key/value store backed by git refs (entries live under `refs/brmem/<namespace>/<encoded-branch>:<key>`). It is consumed by other subpackages — including `memjective` — but the namespace is a parameter, not a `brmem` concern. `brmem` must remain agnostic of any specific namespace's schema, slug rules, or workflow.
...
- **No imports from `memjective` or any other sibling**. `memjective` builds on `brmem`, never the reverse.
