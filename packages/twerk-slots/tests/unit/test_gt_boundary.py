...
SRC_ROOT = Path("packages/twerk-slots/src/twerk_slots")
...
def _imports_forbidden_gt_module(tree: ast.AST, path: Path) -> bool:
    for node in ast.walk(tree):
        if isinstance(node, ast.ImportFrom):
            ...
            if module.startswith("twerk_slots.cli.slot.gt"):
                if path == SLOT_GROUP and module == "twerk_slots.cli.slot.gt.group":
        ...
        if isinstance(node, ast.Import):
            for alias in node.names:
                if alias.name.startswith("twerk_slots.cli.slot.gt"):
                    return True
    return False
...
def test_graphite_usage_stays_under_slot_gt_boundary() -> None:
    ...
    for path in _source_files():
        ...
        if _imports_forbidden_gt_module(tree, path):
            violations.append(f"{path}: imports twerk_slots.cli.slot.gt outside allowed mount")
