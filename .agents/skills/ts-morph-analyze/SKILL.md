...
```bash
pnpm install --dir .agents/skills/ts-morph-analyze --ignore-workspace
```
...
**Usage:**
```bash
pnpm --dir .agents/skills/ts-morph-analyze exec tsx scripts/summarize-class.ts <tsconfig> <file> <name> [--private] [--jsdoc]
```
...
**Example:**
```bash
pnpm --dir .agents/skills/ts-morph-analyze exec tsx scripts/summarize-class.ts "$PWD/tsconfig.json" "$PWD/src/nodes/types.ts" ExecutableNode
```
...
**Usage:**
```bash
pnpm --dir .agents/skills/ts-morph-analyze exec tsx scripts/list-exports.ts <tsconfig> <path> [--recursive] [--types-only] [--functions-only] [--classes-only]
```
...
**Example:**
```bash
pnpm --dir .agents/skills/ts-morph-analyze exec tsx scripts/list-exports.ts "$PWD/tsconfig.json" "$PWD/src/utils/"
```
...
**Usage:**
```bash
pnpm --dir .agents/skills/ts-morph-analyze exec tsx scripts/show-hierarchy.ts <tsconfig> <file> <class-name> [--descendants] [--depth=N]
```
...
**Example:**
```bash
pnpm --dir .agents/skills/ts-morph-analyze exec tsx scripts/show-hierarchy.ts "$PWD/tsconfig.json" "$PWD/src/nodes/text-node.ts" TextNode --descendants
```
...
**Usage:**
```bash
pnpm --dir .agents/skills/ts-morph-analyze exec tsx scripts/extract-interface.ts <tsconfig> <file> <class-name> [--name=InterfaceName] [--include-protected]
```
...
**Example:**
```bash
pnpm --dir .agents/skills/ts-morph-analyze exec tsx scripts/extract-interface.ts "$PWD/tsconfig.json" "$PWD/src/services/user.ts" UserService --name=IUserService
```
...
- Piped to a file: `pnpm --dir .agents/skills/ts-morph-analyze exec tsx scripts/summarize-class.ts ... > api.d.ts`
