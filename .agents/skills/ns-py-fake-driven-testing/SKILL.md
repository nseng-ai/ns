...
`ns-fake-driven-test-layout`; when path guidance conflicts, that skill wins.
For pytest mechanics, use `ns-pytest`. For Python implementation style, use
`ns-dignified-python`.
...
- Layer 1 `fake-check` (~5%): tests of fake implementations. Write when adding
  or changing fakes. Location: `tests/gateways/test_fakes.py`.
- Layer 2 `real-sanity` (~10%): fast tests of real implementations with mocked
  external systems. Write when adding or changing real gateways. Location:
  `tests/gateways/test_real_gateways.py`.
- Layer 3 `pure` (~10%): unit tests for utilities with zero dependencies, no
  fakes, no mocks. Location: `tests/unit/`.
- Layer 4 `logic` (~70%): business logic and top-level workflows tested over
  in-memory fakes. This is the default for features and bug fixes. Location:
  `tests/scenario/` for end-to-end workflows or `tests/unit/` for narrow logic.
- Layer 5 `smoke` (~5%): critical workflows over real systems, used sparingly.
...
- When a gateway interface changes, update ABC, Real, Fake, fake tests, real
  sanity tests, and affected business-logic tests together.
