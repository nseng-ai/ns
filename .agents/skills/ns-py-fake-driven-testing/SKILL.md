---
name: ns-py-fake-driven-testing
description:
...
**For directory placement** — which subfolder a test goes in (`tests/unit/`, `tests/integration/`, `tests/scenario/`, `tests/gateways/`) — see `ns-fake-driven-test-layout`. That skill owns the on-disk convention; this one focuses on the architectural strategy and gateway patterns. When the two seem to disagree on a path, `ns-fake-driven-test-layout` wins.
...
**Testing a CLI or end-to-end workflow?**
→ Read `fast-scenario-testing.md`
...
### 📖 `fast-scenario-testing.md`

**Read when**:

- Writing tests for a CLI command end-to-end
...
- The Arrange / Act / Assert three-phase scenario shape
...
3. **Scenario shape (especially for CLIs)**: `fast-scenario-testing.md`
4. **Patterns**: `patterns.md` (CliRunner, builders)
5. **Avoid**: `anti-patterns.md` (speculative tests, hardcoded paths)
...
│ │ Location: tests/scenario/ (e2e) or tests/unit/ (narrow)   │ │
│ └──────────────────────────────────────────────────────────┘ │
└──────────────────────────────────────────────────────────────┘
...
│ │ Speed: Fast (mocked)                                      │ │
│ │ Location: tests/gateways/test_real_gateways.py            │ │
│ └──────────────────────────────────────────────────────────┘ │
└──────────────────────────────────────────────────────────────┘
...
│ │ Speed: Milliseconds per test                              │ │
│ │ Location: tests/gateways/test_fakes.py                    │ │
│ └──────────────────────────────────────────────────────────┘ │
└──────────────────────────────────────────────────────────────┘
```
