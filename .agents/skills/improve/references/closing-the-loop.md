...
- The plan file exists and its dependencies show DONE in `shadcn-improve/plans/README.md`. If not: stop, name the missing dependency.
...
1. **The full plan file text, inlined.** The worktree contains only committed files — if `shadcn-improve/plans/` is uncommitted, the executor can't read it. Never assume; always inline.
...
> One override: SKIP the plan's instruction to update `shadcn-improve/plans/README.md` —
...
## `reconcile` — keep `shadcn-improve/plans/` alive

Process what happened since the last session. Read `shadcn-improve/plans/README.md` and every plan file, then per status:
