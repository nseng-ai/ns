...
4. `RoasterFailure` now intentionally exposes only the consumed failure shape: `{ type, message }`, plus typed failure-code aliases for gateway/source categories. The unused structured payload fields were dead weight, not latent capability.

This Objective tracked turning these candidates into actual depth: smaller interfaces, more leverage for callers, and locality for maintainers. Candidates #1, #2, #3, and #4 have shipped; no roadmap candidate remains in an indeterminate state.
...
- **Candidate 4 — shipped: shrink the failure union to the consumed shape.** The branch-local implementation confirms roaster command seams only consume `type` and `message`, then removes the unused structured payload fields (`command`, `stderr`, `code`, `path`, `reviewKey`, `model`) from `RoasterFailure` variants. `src/failures.ts` now defines semantic failure-type aliases and a single `RoasterFailure` interface with `{ type, message }`; gateway and operation code construct and consume that smaller shape directly, and the unused `failureMessage` / `isFailureOfType` helpers are gone.
...
- The unread structured fields on `RoasterFailure` were not consumed by roaster's command/output seams. Candidate 4 verified this by shrinking the failure shape and leaving callers on the same `type`/`message` behavior.
...
- **Candidate 4 premature shrink — de-risked:** the branch removed only fields that were unconsumed at roaster seams, kept semantic failure codes, and passed `pnpm --dir ts run check` plus `pnpm --dir ts run test`. If future callers need machine-readable diagnostics, that should be added deliberately as a new contract rather than preserved as unused internal payload.
...
- Candidate 4 is resolved: shrink, not deepen. The unused structured payloads were removed; no ADR was needed because the seam was simplified rather than preserving a knowingly rejected alternative.
...
Outcome: completed. All four architecture-deepening candidates reached a definite shipped state: findings publication is one end-to-end workflow, duplicated DTO definitions have one canonical source, CLI invocation context and operation runtime are separated intentionally, and the `RoasterFailure` union was shrunk to the consumed `{ type, message }` shape.

Completion evidence includes targeted roaster tests from the candidate branches, real PR evidence for the findings-publication workflow, and full TypeScript validation for the final candidate-4 branch (`pnpm --dir ts run check` and `pnpm --dir ts run test`). No active non-parked roadmap work remains. Future structured diagnostics for roaster failures, if needed, should be opened as a new Objective or roadmap item with an explicit machine-readable output contract rather than treated as unfinished work here.
