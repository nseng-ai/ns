// AST codemod for the package-scope sweep: rewrites old-name module
// specifiers via span splicing (no printer, zero formatting churn).
//
...
// old scope prefix, or ./sdl-style subpath): all files except EXCLUDED_FROM_MODE_B,
// whose old-name strings need per-site judgment.
//
...
const EXCLUDED_FROM_MODE_B: readonly string[] = [
	...
	"ts/packages/infra/core/test/support/typescript-style-guard/",
	"ts/packages/infra/core/test/cli-runtime/cli-entry.test.ts",
	"ts/packages/infra/core/test/cli-theme/package-boundary.test.ts",
];
...
function listSourceFiles(): string[] {
	...
	for (const root of SCAN_ROOTS) {
		walkFiles(join(REPO_ROOT, root), (name) => /\.(ts|tsx|mts|cts)$/.test(name), files);
	}
	return files.sort();
}
...
function main(): void {
	...
	console.log(`${write ? "APPLIED" : "DRY RUN"}: ${filesChanged} files, ${specifierEdits} specifier edits (mode A), ${stringEdits} string edits (mode B)`);
