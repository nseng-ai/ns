...
- [~] Keep `CONTEXT.md`, `CONTEXT-MAP.md`, and `grill-with-docs`-maintained docs up to date.
  - Guidance: Re-derive the next slice from current source/docs, existing context coverage, and unresolved map ambiguities. The repo is all-TypeScript under `ts/packages/` with `@sdl/*` names (unscoped `sdlcc`, `sdl-flow`, and `sdl-sdk` excepted); do not author against deleted Python `packages/*` paths or old `asdl-*` names, and do not treat old phase numbers as a hidden queue.
