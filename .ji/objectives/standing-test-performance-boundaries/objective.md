...
- Add or refine narrow testing seams when an existing production boundary makes default-path tests depend on real systems. Prefer explicit gateway injection, in-memory fakes, command/process/sqlite adapters, and existing `@sdl/core` `Clock` / `TimerScheduler` seams for TypeScript time-sensitive behavior.
...
- **Time and timeout behavior:** keep wall-clock reads (`Clock`) separate from one-shot timer scheduling (`TimerScheduler`). Default tests should advance `@sdl/core/testing` manual helpers rather than using sleeps, elapsed `Date.now()` assertions, or broad fake-timer state.
