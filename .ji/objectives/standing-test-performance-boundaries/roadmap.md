...
- [~] Rebaseline the remaining default suites for likely boundary leaks after the current TypeScript stack lands.
  ...
  - Latest evidence: the `asdl-core` real child-process `runCommand` split is completed with timing and discovery evidence in `updates/2026-06-20T184212Z-asdl-core-run-command-integration.md`; the source CLI shim subprocess split is completed with placement and validation evidence in `updates/2026-06-21T131815Z-source-cli-shim-subprocess-split.md`; the `areg` real-filesystem gateway suite was classified in `updates/2026-06-23T221633Z-default-suite-rebaseline.md` as fast/inert enough to avoid a migration for now; the `@sdl/core/testing` temp Git repo helper smoke split is completed with targeted timing and discovery evidence in `updates/2026-06-23T230148Z-sdl-core-temp-git-repo-integration-split.md`, so the next rebaseline should choose a different candidate.
