...
This Objective is the canonical home for tactical TypeScript structural cleanup and architecture-deepening findings that are *not* specifically extension-layering endgame work. It subsumes the still-live tactical rows from the closed `ts-cli-architecture-deepening` Objective. Architectural layering and capability-extension migration work routes to `sdl-extension-architecture` (ADR 0009). **Sequencing update:** the remaining backlog has been row-by-row reclassified after the extension-architecture endgame advanced. Only rows marked neutral structural cleanup in `roadmap.md` are ready candidates for this Objective after a narrow pickup re-check; capability-owned rows route to their owning capability context, and obsolete/disposed rows are historical evidence rather than implementation targets. The parent `sdl-extension-architecture` Objective remains open because `ccc` clean-consumer work and deletion of `@sdl/domain-primitives-transitional` are still incomplete.
...
- **Cross-package dedup (partly open after reclassification):** the original `resolveBranchOrCurrent` helper row is disposed as obsolete because no clear repeated policy-free helper shape remains. GitHub PR feedback leaf helpers still have duplication/drift and may be a neutral ADR 0016-safe cleanup: core `github-pr-feedback` owns `ghAuthorSchema` and private `normalizeAuthor`/`numericGithubIdentity` helpers, while `roaster` still carries divergent copies. The vestigial `@sdl/core` root `.` export remains a likely neutral next candidate, with a live bare importer in `ts/packages/hosts/pi/src/sessions/harness-session.ts`.
...
- **Boundary / Zod / type-contract cleanups (open, rehome-aware):** `sdlcc/stack-map-model-loader.ts`; `@sdl/aretro`'s `sessions/pi-jsonl-source.ts`; `@sdl/kernel`'s `extension-discovery.ts`; `@sdl/vibechk`'s `workflow.ts`; and Flow-owned submit/PR-description code remain candidates, while `@sdl/packagechk`'s `claim-command.ts` cleanup is complete. Each remaining candidate must be checked against current package ownership before implementation.
- **Small dedup / absorbed architecture-deepening rows (reclassified):** ccc/flow small helper dedup and slot-dispatch are capability-boundary work; slot inventory reconciliation, slot Graphite navigation, and Branch Memory entry-locator rows are disposed/absorbed by current Slot and brmem designs; objective-markdown validation and branch-context plan-attachment seams belong to Objective and Branch Context/Plans respectively.
...
- The remaining named cross-package duplications are either unified to a single canonical implementation, explicitly reclassified as no-longer-valid because current extension/capability ownership makes the original target wrong, or routed to their owning capability context in `roadmap.md`.
...
- Should the two strongest neutral next candidates be sequenced as `@sdl/core` root-export deletion first, then GitHub PR feedback leaf-helper sharing, or should `@sdl/areg` god-file decomposition take priority as a larger tool-local cleanup?
