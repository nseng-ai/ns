...
> (`.sdl/extensions/flow/`) is thin: each command is a one-line re-export of the
...
4. `land`'s slot dependency is **conditional and opt-in** (`--free`). It detects
   managed slots by path regex (`sdl/slots/repos/.../worktrees/slot-*`) and shells out
   to `sdl slot free` — it does **not** import `@sdl/slot`. Land works fully without
