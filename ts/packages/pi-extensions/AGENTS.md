...
Every repo-owned Pi slash command must acknowledge receipt synchronously, before awaiting `ctx.waitForIdle()` or starting slow work. Use `registerCommandWithImmediateAck` from `@sdl/pi-extension-runtime/command-ack` at each command registration site instead of hand-writing per-command acknowledgements or wrapping the host.
