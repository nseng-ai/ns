...
describe("code extension registration", () => {
	test("registers review-feedback watch and smart restack", () => {
		const pi = new FakePi();
		codeExtension(pi);

		expect([...pi.commands.keys()]).toEqual(["code:pr-feedback-watch", "gt-smart-restack"]);
		...
		expect(pi.commands.get("gt-smart-restack")?.description).toContain("gt restack");
	});
});
