...
describe("parseEpisodeAnalysisResponseText", () => {
	test("parses a clean JSON verdict pair without a summary", () => {
		const result = parseEpisodeAnalysisResponseText(
			JSON.stringify({ efficiency: "efficient", relevance: "load-bearing" }),
		);
	...
	test("parses and trims the opinionated summary; blank summaries become null", () => {
		const withSummary = parseEpisodeAnalysisResponseText(
			JSON.stringify({
				efficiency: "wasteful",
				relevance: "rot",
				summary: "  t2-t4 retry the same failing bash call; ≈1.2k tokens of churn.  ",
			}),
		);
		...
		const blank = parseEpisodeAnalysisResponseText(
			JSON.stringify({ efficiency: "mixed", relevance: "stale", summary: "   " }),
		);
		expect(blank).toEqual({
			ok: true,
			value: { efficiency: "mixed", relevance: "stale", summary: null },
		});
	});
	...
	test("reports malformed or invalid verdicts as errors", () => {
		expect(parseEpisodeAnalysisResponseText("no json here")).toEqual({
			ok: false,
			error: "response contains no JSON object",
		});
		const invalidJson = parseEpisodeAnalysisResponseText("{efficiency:nope}");
		expect(invalidJson.ok).toBe(false);
		const invalidVerdict = parseEpisodeAnalysisResponseText(
			JSON.stringify({ efficiency: "great", relevance: "current" }),
		);
...
describe("EPISODE_ANALYSIS_SYSTEM_PROMPT", () => {
	test("includes the delegation efficiency weighting rule", () => {
		expect(EPISODE_ANALYSIS_SYSTEM_PROMPT).toContain(
			"delegation that kept large work out of this context is efficient",
		);
	});

	test("demands decisive verdicts and an opinionated, never-advisory summary", () => {
		expect(EPISODE_ANALYSIS_SYSTEM_PROMPT).toContain(
			"mixed and still-useful are not safe defaults",
		);
...
describe("buildEpisodeAnalysisPayload", () => {
	test("serializes one target episode with full normalized turn text and compact episode map", () => {
		...
		expect(request.targetEpisode.delegations).toEqual([
			{ turn: 2, label: "delegate fix", confidence: "low" },
		]);
