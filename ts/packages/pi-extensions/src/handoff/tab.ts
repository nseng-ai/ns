...
import {
	...
	SAVE_HANDOFF_FALLBACK,
	SAVE_HANDOFF_SKILL_NAME,
	checkHandoffExists,
	currentBranch,
	errorMessage,
	expandHandoffSkill,
...
export function buildHandoffTabPrompt(
	...
	return `${options.skillBlock ?? SAVE_HANDOFF_FALLBACK}
...
export async function handleHandoffTabCommand(pi: ExtensionAPI, args: string, ctx: CommandContext): Promise<void> {
	...
	try {
		skill = await expandHandoffSkill(pi, SAVE_HANDOFF_SKILL_NAME);
	} catch (error) {
		skillReadError = errorMessage(error);
	}

	ctx.ui.notify(saveHandoffTabStartMessage(skill, skillReadError, request.slug), skill ? "info" : "warning");
...
function saveHandoffTabStartMessage(skill: Awaited<ReturnType<typeof expandHandoffSkill>>, skillReadError: string | undefined, slug: string): string {
	if (skill !== undefined) {
