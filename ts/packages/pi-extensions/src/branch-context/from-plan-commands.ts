...
Stack a branch context on the current branch with Graphite, attach the saved plan, check out that branch with exact git checkout <branch>, start a fresh Pi session, and run ${formatImplBranchContextCommand(BRANCH_CONTEXT_PLAN_KEY)} for the attached plan in that new session.
...
This command intentionally models the manual flow: /branch-context:from-plan --graphite, git checkout <branch>, /new, then ${formatImplBranchContextCommand(BRANCH_CONTEXT_PLAN_KEY)} in the new Pi session.`;
...
export function formatCreateBranchContextPreview(preview: CreateBranchContextPreview): string {
	...
	lines.push(`Branch Memory key: ${BRANCH_CONTEXT_PLAN_KEY}`);
	return lines.join("\n");
}
...
export async function handleCreateBranchContextCommand(
	...
	if (args.dryRun) {
		...
		presentBranchContextMessage(
			...
			{ status: "dry-run", targetBranch: preview.targetBranch, key: BRANCH_CONTEXT_PLAN_KEY },
			"info",
		);
		return;
	}
...
export async function handleUpAndImplCommand(
	...
	if (args.dryRun) {
		presentBranchContextMessage(
			pi,
			ctx,
			formatUpAndImplDryRunMessage(formatCreateBranchContextPreview(preview), preview.targetBranch, BRANCH_CONTEXT_PLAN_KEY),
			{ status: "dry-run", targetBranch: preview.targetBranch, key: BRANCH_CONTEXT_PLAN_KEY },
			"info",
		);
		return;
