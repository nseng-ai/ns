...
import { formatPlannedBranchUpAndImplDryRun, launchPlannedBranchUpAndImpl } from "@asdl/ccc/planned-branch-up-and-impl";
...
async function handleUpAndImplCommand(
	...
	if (args.dryRun) {
		presentPlannedBranchMessage(
			...
			formatPlannedBranchUpAndImplDryRun({
				previewText: formatCreatePlannedBranchPreview(preview),
	...
	const launchResult = await launchPlannedBranchUpAndImpl({
		host: pi,
		sessionLauncher: ctx,
		cwd: ctx.cwd,
		hasUI: ctx.hasUI,
