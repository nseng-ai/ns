...
export function applyInterrogationEvent(state: TranscriptState, event: InterrogationEvent): TranscriptState {
	switch (event.type) {
		...
		case "tool-end":
			return { ...state, entries: [...state.entries, { type: "tool", name: event.name, status: "end", summary: event.summary }] };
		case "retry":
