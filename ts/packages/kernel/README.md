...
```bash
ji shell show --shell zsh
ji shell install --shell zsh
ji shell install --shell bash
...
```bash
# bash (~/.bashrc)
eval "$(ji completion bash)"

# zsh (~/.zshrc)
eval "$(ji completion zsh)"
...
The proving consumer is `ji slot checkout` / `ji slot co`, which complete local branch names for the branch and base positionals without performing any checkout/create/delete mutation. Branch completion is local branches only; remote refs are out of scope.
...
```text
built-in command table < bundled first-party package manifests < $XDG_DATA_HOME/ji/extensions < <cwd>/.ji/extensions
```

Global and project roots support these one-level entry shapes:

```text
.ji/extensions/greet.ts
.ji/extensions/greet.js
.ji/extensions/greet/index.ts
.ji/extensions/greet/index.js
...
- `JI_CHECKPOINT_MODEL`: model reference for generated checkpoint messages.
- `JI_DEV_CHECKPOINT_MODEL`: transitional fallback for the old checkpoint model selection.

Pi exposes the same capability as `/ji:flow:cp`. Old compatibility aliases such as `/code:cp`, `/code:checkpoint`, `/ji:code:cp`, and `/ji:code:checkpoint` are not restored.
...
- `JI_SLUG_MODEL`: model reference for generated branch slugs.
- `JI_CHECKPOINT_MODEL`: model reference for generated checkpoint messages.
- `JI_DEV_CHECKPOINT_MODEL`: transitional fallback for the old checkpoint model selection.

Pi exposes the same capability as `/ji:flow:autobranch`. Flat `/ji:autobranch`, `/code:autobranch`, and `/newbr` are not restored as compatibility surfaces.
...
Pi exposes the same capability as `/ji:flow:branch-latest-commit`.
...
- `JI_CHANGES_MODEL`: model reference for generated changes summaries.
- `PI_DRAFT_MODEL`: transitional fallback for the old Pi changes-summary model selection.

Pi exposes the same capability as `/ji:flow:changes`; `/code:changes` is not retained as a compatibility alias.
...
```bash
ji flow submit [--no-restack] [--verbose]
```

Behavior:

- checkpoints pending worktree changes before submission using the same `[cp]` checkpoint-message policy as `ji flow cp`;
...
- `JI_DEV_PR_DESCRIPTION_PROMPT`: optional custom PR-description prompt file.
- `JI_SUBMIT_FAILURE_MODEL`: model reference for generated submit-failure summaries.
- `JI_SUBMIT_FAILURE_LOG_DIR`: optional directory for raw submit-failure transcripts.

Pi exposes the same capability as `/ji:flow:submit`. `/dev:submit`, `/submit`, and other legacy submit aliases are not restored.
...
Environment matches PR description generation for `ji flow submit`:

- `JI_DEV_PR_DESCRIPTION_MODEL`: model reference for generated PR descriptions.
- `JI_DEV_PR_DESCRIPTION_PROMPT`: optional custom PR-description prompt file.

Pi exposes the same capability as `/ji:flow:regenerate-pr`. `ji pr-regen`, `/ji:pr-regen`, and `/code:pr-regen` are not retained as compatibility surfaces.
