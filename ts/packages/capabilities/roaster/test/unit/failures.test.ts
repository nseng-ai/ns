...
import type { RoasterFailure, RoasterResult } from "../../src/core/failures.ts";
...
describe("roaster failures", () => {
	test("carry the consumed failure shape", () => {
		...
		expect(failure).toEqual({ type: "git-diff-failed", message: "git diff failed" });
	});

	test("uses result unions for expected failures", () => {
		const result: RoasterResult<string> = {
			type: "error",
			error: { type: "base-ref-unavailable", message: "Pass --base-ref explicitly." },
		};
