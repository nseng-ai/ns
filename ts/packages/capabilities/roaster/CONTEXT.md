...
The curated in-process consumer API exported as `@nseng-ai/roaster/api` for packages that need Roaster behavior without shelling out.

Avoid: importing private `@nseng-ai/roaster/src/...` modules or treating the broad package root as the Capability API.
