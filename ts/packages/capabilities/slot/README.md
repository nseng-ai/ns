...
`@ji/slot` owns Slot domain logic, operations, the Slot Capability API, and the bundled ji extension contribution for `ns slot ...`. The supported command-line surface is **only** through the ji binary:
