...
**Slot Capability API**:
The curated `@ji/slot/api` surface for downstream in-process consumers that need Slot behavior without invoking the CLI or importing private modules.
*Avoid*: command output parsing, `@ji/slot/src/...` import, package-root convenience import, `ctx`-passing API
