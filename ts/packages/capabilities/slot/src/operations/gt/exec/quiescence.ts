...
export async function runGtQuiescence(ctx: SlotCliContext, request: GtQuiescenceRequest) {
	...
	const integrity = validateStackIntegrity(stack, { scope: request.scope });
...
function forkedStackMessage(fork: StackFork): string {
	return `Graphite stack forks at '${fork.branch}' with children: ${fork.children.join(", ")}. Check out the intended tip and rerun, or pass \`--scope downstack\`.`;
}
