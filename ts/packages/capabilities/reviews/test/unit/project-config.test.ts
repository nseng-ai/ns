...
describe("parseReviewsProjectConfigToml", () => {
	test("loads shared model policy and defaults Reviews to fast", () => {
		...
		expect(config.modelPolicy.profiles).toEqual({});
	...
	test("parses Reviews diff and shared model profiles", () => {
		...
		expect(config.modelPolicy.profiles.deep?.model.modelId).toBe("claude-opus-4-6");
		expect(config.modelPolicy.profiles.deep?.thinking).toBe("high");
		expect(config.modelPolicy.profiles.architecture?.model.modelId).toBe("gpt-5.6-terra");
	});
