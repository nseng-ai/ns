...
import { createLocalDiff, type LocalDiff } from "../../src/core/models.ts";
...
function localDiff(
	files: readonly ReturnType<typeof diffFile>[],
...
describe("prompt-sized diff coverage", () => {
	test("returns the original diff byte-for-byte when nothing is omitted under the total cap", () => {
		...
		expect(result.inputCoverage).toMatchObject({
			includedFileCount: 1,
			omittedFileCount: 0,
			changedPathCount: 1,
		});
	...
	test("includes exact total-cap and exact per-file-cap inputs", () => {
		...
		const totalResult = promptSizedDiff(
			localDiff([diffFile("exact-total.ts", 1, exactTotalDiff)], exactTotalDiff),
		);
		const perFileResult = promptSizedDiff(
			localDiff([diffFile("exact-file.ts", MAX_PROMPT_DIFF_FILE_TOKENS)]),
		);
	...
	test("omits files over the per-file cap even when total input would fit", () => {
		const result = promptSizedDiff(
			localDiff([diffFile("large.ts", MAX_PROMPT_DIFF_FILE_TOKENS + 1)]),
		);
	...
	test("uses unknown path fallback and returns header only when all files are omitted", () => {
		const result = promptSizedDiff(
			localDiff([diffFile("", MAX_PROMPT_DIFF_FILE_TOKENS + 1, "large\n")]),
		);
