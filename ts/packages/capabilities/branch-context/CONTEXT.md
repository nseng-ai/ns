...
Concrete Pi slash-command registration and command names such as `/ns:plan:impl-saved-plan` and `/ns:branch-context:impl-attached-plan` are owned by the extension's own `pi` subpackage — `src/pi/surfaces.ts` defines the command-name constants and `registerBranchContextCommands` performs registration — with `hosts/pi` merely delegating via `@nseng-ai/branch-context/pi`. The first directly selects a local-store **Saved Plan** and embeds it in a fresh current-branch Pi session without creating **Branch Context** or an **Attached Plan**; the second loads through the branch-context workflow from branch-scoped Attached Plan context. Branch Context core (non-`pi` code) must not depend on the Pi package because Pi is a Presentation Host above ns extensions; importing Pi outside the `pi` subpackage would pull host-specific command surfaces into the Branch Context provider boundary.
*Avoid*: extension-package-API command-name export, duplicated Pi command string outside the `pi` subpackage, host-specific launch formatter in core
