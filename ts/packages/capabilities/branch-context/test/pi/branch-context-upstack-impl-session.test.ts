...
import { NoSavedPlanAvailableError } from "@ji/plans/api";
...
describe("branch-context-upstack-impl-session", () => {
	...
	test("ns:branch-context:upstack-impl-from-plan reuses one session-created attached plan when the local plan store is missing", async () => {
		const events: string[] = [];
	...
	test("ns:branch-context:upstack-impl-from-plan reuses a non-default session-created attached plan", async () => {
	...
	test("ns:branch-context:upstack-impl-from-plan reuses an explicit branch when the local plan store is missing", async () => {
	...
	test("ns:branch-context:upstack-impl-from-plan dry-run describes explicit branch reuse without checkout", async () => {
		const explicitBranch = "branch-contexts/explicit-target";
		const pi = new FakePi();
		const fakes = createBranchContextOperationFakes({
			async resolveSelectedSavedPlanFile() {
	...
	test("ns:branch-context:upstack-impl-from-plan dry-run includes non-default keys in the follow-up flow", async () => {
		const pi = new FakePi();
		const fakes = createBranchContextOperationFakes({
			async resolveSelectedSavedPlanFile() {
				throw missingPlanStoreError();
	...
	test("ns:branch-context:upstack-impl-from-plan reuses the current branch when the local plan store is missing", async () => {
	...
	test("ns:branch-context:upstack-impl-from-plan fails clearly for ambiguous session candidates", async () => {
	...
	test("ns:branch-context:upstack-impl-from-plan surfaces attached-plan key ambiguity on explicit reuse", async () => {
	...
	test("ns:branch-context:upstack-impl-from-plan falls through to the current branch when the session candidate fails verification", async () => {
		const currentBranch = "branch-contexts/current-target";
		const pi = new FakePi([gitCurrentBranchStep(currentBranch), gitCheckoutStep(currentBranch)]);
		const fakes = createBranchContextOperationFakes({
			async resolveSelectedSavedPlanFile() {
	...
	test("ns:branch-context:upstack-impl-from-plan aggregates session and current-branch failures into one error", async () => {
	...
	test("ns:branch-context:upstack-impl-from-plan resumes when the plan store directory exists but holds no plans", async () => {
		const explicitBranch = "branch-contexts/explicit-target";
		const pi = new FakePi([gitCheckoutStep(explicitBranch)]);
		const fakes = createBranchContextOperationFakes({
			async resolveSelectedSavedPlanFile() {
	...
	test("ns:branch-context:upstack-impl-from-plan reports keyed cancellation recovery", async () => {
		const pi = new FakePi([gitCheckoutStep(IMPL_BRANCH)]);
		const fakes = createBranchContextOperationFakes({
			async resolveSelectedSavedPlanFile() {
				throw missingPlanStoreError();
