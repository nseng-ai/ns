...
describe("branch-context-impl", () => {
	test("ns:branch-context:impl-attached-plan waits, loads the attached plan, and sends an implementation prompt", async () => {
		const events: string[] = [];
		...
		const command = pi.commands.get("ns:branch-context:impl-attached-plan");
		...
		expect(context.statuses).toEqual([
			{
				key: "ns:branch-context:impl-attached-plan",
				value: "loading attached branch-context plan…",
			},
			{ key: "ns:branch-context:impl-attached-plan", value: undefined },
		]);
	...
	test("ns:branch-context:impl-attached-plan passes a requested slug into attached-plan selection", async () => {
		const pi = new FakePi();
		const fakes = createBranchContextOperationFakes();
		registerBranchContextExtension(pi, { branchContextOperations: fakes.operations });
		const command = pi.commands.get("ns:branch-context:impl-attached-plan");
	...
	test("ns:branch-context:impl-attached-plan presents saved-plan fallback evidence", async () => {
	...
	test("ns:branch-context:impl-attached-plan presents load failures without sending an implementation prompt", async () => {
		const pi = new FakePi();
		const fakes = createBranchContextOperationFakes({
