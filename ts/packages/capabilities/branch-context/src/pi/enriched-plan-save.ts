...
<!-- PLAN-VERIFICATION-WORKSTREAM:START refactor-execution-strategy-guidance -->
- If the plan includes same-shape edits across multiple files, explicitly choose an execution strategy: \`ts-morph-analyze\` for TypeScript AST inspection where useful, repo-supported tested codemods or focused scripts with reviewable output for broad TypeScript symbol/API changes, precise edits for 1-4 files or semantic docs/specs, and \`refactor-swarm\` for 5+ file-local prose-aware or mixed code/docs/tests edits.
<!-- PLAN-VERIFICATION-WORKSTREAM:END refactor-execution-strategy-guidance -->
