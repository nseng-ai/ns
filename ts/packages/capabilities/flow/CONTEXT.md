...
This context captures Flow language for lifecycle commands, its curated Capability APIs, and the current boundary between Flow-owned presentation/orchestration and the `@nseng-ai/flow/land` domain core subpackage.
...
**Flow Capability API**:
The curated `@nseng-ai/flow/api` in-process compatibility surface consumed by downstream packages so they do not import Flow private source modules. Dispatch source publication is Vercel-owned and does not cross this boundary.
*Avoid*: package-root import, private `@nseng-ai/flow/src/...` import, narrowed land-only API, Vercel source-publication implementation
