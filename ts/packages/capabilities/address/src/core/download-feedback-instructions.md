...
- For straightforward fixes, edit the minimal files and run appropriate validation. If validation passes, close every addressed review thread with `ns address exec close-review-threads --thread-ids-json '{"threadIds":["<THREAD_ID>"]}' --format json`; include `--body <BODY>` when a reply is useful. If all downloaded feedback was handled by straightforward fixes and no ambiguous/complex/high-risk feedback remains, resubmit the PR with `ns flow submit` before the final summary; if submission fails, report that failure. Use single-thread reply/resolve primitives only for one-offs, and do not use raw `gh api graphql` for those mutations.
