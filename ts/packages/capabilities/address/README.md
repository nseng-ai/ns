...
`@nseng-ai/address/api` is the curated in-process Capability API for PR-feedback consumers. Consumers that handle PR lookup, review, discussion-comment, review-thread, review-thread mutation, or Address check payloads should import the gateway and DTO vocabulary from this subpath instead of `@nseng-ai/core/github-pr-feedback`, `@nseng-ai/core/github-pr-status`, Pi modules, command operation schemas, or private source paths.
...
- **Stable through the Address seam:** `GithubStatusChecks`, `GithubStatusCheckEntry`, `GithubCheckTally`, `GithubCheckBucket`, `GithubStatusCheckKind`. Import these from `@nseng-ai/address/api` when consuming `getPrChecks`/`pr-checks` payloads. The generic status normalization mechanics remain neutral infra in `@nseng-ai/core/github-pr-status`.
...
ADR 0016 keeps dependency direction as `@nseng-ai/address` → `@nseng-ai/core`: lower type declarations and real GitHub mechanics may live in core, while seam consumers import only the Address Capability API.
...
```bash
pnpm --dir ts --filter @nseng-ai/address run check
pnpm --dir ts --filter @nseng-ai/address run test
```
