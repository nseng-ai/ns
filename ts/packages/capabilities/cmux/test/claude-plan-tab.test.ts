...
} from "@nseng-ai/cmux/api";
import { registerCccClaudePlanTabCommand } from "@nseng-ai/cmux/pi";
import {
	FakeCommandContext,
	FakePi,
	makeTempDir,
...
describe("claude plan tab", () => {
	...
	test("writes the last assistant seed verbatim when invoked without arguments", async () => {
		...
		await pi.commands.get("ns:cmux:claude-plan-tab")?.handler("", ctx);
	...
	test("uses a non-empty command argument as the seed instead of the last assistant message", async () => {
		...
		await pi.commands.get("ns:cmux:claude-plan-tab")?.handler("Explicit plan", ctx);
