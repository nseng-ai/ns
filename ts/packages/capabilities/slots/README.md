...
- `ns slot foreach -- git clean -fd` runs a command in every managed slot worktree (sequentially, in slot-number order). Repeat `--exclude SLOT` (`-x SLOT`) to omit named slots from both the operation-in-progress preflight and execution. It aborts when any included slot has a git operation in progress, and prompts for confirmation unless `--yes` is passed. Pass the command after `--`; flag-bearing commands (e.g. `-fd`) require the `--` separator.
