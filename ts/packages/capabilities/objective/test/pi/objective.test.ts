...
const OBJECTIVE_SKILLS_BY_COMMAND: Record<ObjectiveCommandName, ObjectiveSkillName> = {
	"sdl:objective:next": "objective-next",
	"sdl:objective:update": "objective-update",
	"sdl:objective:close": "objective-close",
	"sdl:objective:stack-impl": "objective-stack-impl",
};

const ACTION_PROMPTS: Record<ObjectiveCommandName, string> = {
	"sdl:objective:next": "Run objective-next for this explicitly selected Objective slug or path:",
	"sdl:objective:update":
		"Run objective-update for this explicitly selected Objective slug or path:",
	"sdl:objective:close": "Run objective-close for this explicitly selected Objective slug or path:",
	"sdl:objective:stack-impl":
...
async function runObjectiveStackImpl(
	...
	const command = pi.commands.get("sdl:objective:stack-impl");
	expect(command).toBeDefined();
	if (!command) {
		throw new Error("sdl:objective:stack-impl was not registered");
	}
...
async function runObjectiveNext(
	...
	objectiveExtension(pi);
	const command = pi.commands.get("sdl:objective:next");
	expect(command).toBeDefined();
	const context = createContext(contextOptions);
	await command?.handler(args, context.ctx);
	return { pi, ...context };
...
describe("sdl:objective:stack-impl command", () => {
	test("registers the skill-backed wrapper command", () => {
		const pi = new FakePi();

		objectiveExtension(pi);

		expect(pi.commands.has("sdl:objective:stack-impl")).toBe(true);
	});
	...
	test("explicit slug bypasses objective list, git evidence, and recursive slash dispatch", async () => {
		await withStackImplSkill(async ({ skillPath, skillDir }) => {
			...
			expect(result.pi.sentUserMessages[0]?.startsWith("/sdl:objective:stack-impl")).toBe(false);
	...
	test("zero active Objectives sends no prompt", async () => {
		...
		result.pi.assertDone();
		expect(result.notifications).toEqual([
			{ message: "No active Objectives. Create one with /sdl:objective:create.", level: "info" },
		]);
		expect(result.selections).toEqual([]);
		expect(result.pi.sentUserMessages).toEqual([]);
	});
...
describe("objective picker suggestion", () => {
	...
	test("sdl:objective:close uses normal changed-first selection, not compact suggestion UX", async () => {
		const result = await runObjectiveCommand("sdl:objective:close", "", [
			listStep(["alpha", "bravo", "charlie"]),
			diffStep("M\t.ji/objectives/bravo/objective.md\n"),
			statusStep(""),
		]);
...
describe("objective command shared selection policy", () => {
	...
	test("sdl:objective:next completions return fast active Objective slug candidates", async () => {
		const { pi, items } = await objectiveCommandCompletions("sdl:objective:next", "", [
	...
	test("slug completions reject unsupported multi-arg input without loading candidates", async () => {
		const { pi, items } = await objectiveCommandCompletions(
			"sdl:objective:next",
			"alpha bravo",
			[],
		);
...
`,
	...
	test("sdl:objective:next fallback prompt requires a work-left estimate", async () => {
		const result = await runObjectiveCommand("sdl:objective:next", "bravo");
	...
	test("sdl:objective:next prompt preauthorizes clear tracking-gate updates", async () => {
		const result = await runObjectiveCommand("sdl:objective:next", "bravo");
	...
	test("objective-update prompt includes the post-selection evidence workflow reminder", async () => {
		const result = await runObjectiveCommand("sdl:objective:update", "bravo");
	...
	test("sdl:objective:close fallback and prompt include closure confirmation guidance", async () => {
		const result = await runObjectiveCommand("sdl:objective:close", "bravo");
	...
	test("sdl:objective:next prompt does not include the objective-update evidence workflow reminder", async () => {
		const result = await runObjectiveCommand("sdl:objective:next", "bravo");
