...
describe("handoff-tab extension", () => {
	test("registers handoff-tab command and launch tool when tool support exists", () => {
		...
		expect(pi.commands.get("ns:ccc:handoff-tab")?.description).toBe(
	...
	test("handoff-tab command queues create prompt with content-derived slug instructions", async () => {
		await withHandoffCreateSkill(async ({ skillPath, repoDir }) => {
			...
			expect(result.notifications).toEqual([
				{
					message: "Starting ns:ccc:handoff-tab workflow with content-derived slug…",
					level: "info",
				},
			]);
			...
			expect(prompt).toContain(
				`ns handoff create --slug <returned-slug> --branch ${BRANCH} --file /dev/stdin`,
			);
			expect(prompt).toContain(
				"After `ns handoff create` succeeds, call handoff_tab_launch with `branch` set",
			);
			expect(prompt).toContain(`/ns:handoff:pickup --branch ${BRANCH} <returned-slug>`);
		});
	});

	test("handoff-tab command delegates slug collision handling to generated prompt", async () => {
		const result = await runCommand("ns:ccc:handoff-tab", "finish handoff tab implementation", [
			branchStep(),
			cmuxIdentifyStep(),
		]);
		...
		expect(result.notifications).toEqual([
			{
				message:
					"handoff-create skill was not found; using fallback ns:ccc:handoff-tab workflow prompt for a content-derived slug.",
				level: "warning",
			},
		]);
		expect(result.pi.sentUserMessages).toHaveLength(1);
		const prompt = result.pi.sentUserMessages[0] ?? "";
		expect(prompt).toContain("If `ns handoff create` reports an existing artifact");
	...
	test("handoff-tab launch tool opens a focused pickup cmux tab", async () => {
		const pi = new FakePi([
			...
			step("cmux", [
				...
				"window-1",
				"--",
				"pi --provider anthropic --model claude-sonnet --thinking medium '/ns:handoff:pickup --branch feature/handoff finish-widget'\n",
			]),
		]);
		handoffExtension(pi);
		...
		expect(result.content[0]?.text).toContain(
			"Command: pi --provider anthropic --model claude-sonnet --thinking medium '/ns:handoff:pickup --branch feature/handoff finish-widget'",
		);
		expect(result.details).toEqual({
			type: "launched",
			branch: BRANCH,
	...
	test("handoff-tab launch tool reports manual recovery when rename fails after surface creation", async () => {
		...
		expect(result.content[0]?.text).toContain(
			"Manual recovery: run pi --thinking medium '/ns:handoff:pickup --branch feature/handoff finish-widget'",
		);
	});
		...
		const pi = new FakePi([
			...
			step(
				...
				[
					...
					"window-1",
					"--",
					"pi --thinking medium '/ns:handoff:pickup --branch feature/handoff finish-widget'\n",
				],
				{
					code: 2,
					stderr: "send failed",
...
describe("handoff-tab pure helpers", () => {
	...
	test("handoff-tab prompt requires content-derived slug and launch tool ordering", () => {
		...
		expect(prompt).toContain("This is a /ns:ccc:handoff-tab request.");
		expect(prompt).toContain(`- Branch: ${BRANCH}`);
		expect(prompt).toContain(
		...
		expect(prompt).toContain("Do not derive the entry name from the raw continuation focus.");
		expect(prompt).toContain(
			"If `ns handoff create` reports an existing artifact, stop; do not overwrite and do not open a cmux tab.",
		);
		expect(prompt).toContain(
			"After `ns handoff create` succeeds, call handoff_tab_launch with `branch` set",
		);
