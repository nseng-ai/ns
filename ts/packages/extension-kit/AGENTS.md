# Extension Kit Agent Instructions

Rules for editing under `@nseng-ai/extension-kit`. Read the root `AGENTS.md` and `ts/AGENTS.md` first; this file adds the kit admission test for its exports.

## Kit admission test

Before adding or renaming a barrel export:

- **Tool vocabulary only.** Export names use git/exec/worktree tool terms, never extension-domain words (`land`, `autobranch`, `objective`, `handoff`, …). The kit is agnostic about which extension owns domain behavior.
