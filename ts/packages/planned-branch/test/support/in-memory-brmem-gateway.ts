...
export class InMemoryPlannedBranchBrmemGateway implements PlannedBranchBrmemGateway {
	...
	async attachmentPresence(params: BrmemAttachmentParams): Promise<BrmemAttachmentPresenceResult> {
		...
		if (this.entries.has(entryKey(params.branch, params.key))) {
			return { type: "present", displayCommand: `brmem check ${params.key} --namespace ${PLAN_BRANCH_NAMESPACE} --branch ${params.branch} --format json` };
		}
		return { type: "absent" };
	}
	...
	async listAttachedPlans(params: BrmemCwdParams & { branch: string }): Promise<BrmemResult<AttachedPlanEntry[]>> {
		...
		const entries = [...this.entries.values()]
			...
			.map((entry) => ({ namespace: PLAN_BRANCH_NAMESPACE, key: entry.key, branch: entry.branch, refName: entry.refName }))
...
function defaultRefName(branch: string, key: string): string {
	return `refs/brmem/ns/${PLAN_BRANCH_NAMESPACE}/${branch.replaceAll("/", "---")}:${key}`;
}
