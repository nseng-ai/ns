...
export type PlannedApplyOperation = PlannedWriteOperation | PlannedSkipOperation | PlannedDeleteOperation | PlannedRemoveEmptyDirOperation;
...
export type SkillKindApplyOperationStatusResult = ProjectMutationOperationStatusRecord | SkillKindApplySkipStatusResult;

const MANAGED_OPENAI_POLICY = "policy:\n  allow_implicit_invocation: false\n";

export function buildSkillKindApplyPlan(inspection: SkillKindProjectInspection, skillName: string, kind: SkillInvocationKind): Result<SkillKindApplyPlan> {
...
export function inspectionAfterPlannedApply(inspection: SkillKindProjectInspection, plan: SkillKindApplyPlan): SkillKindProjectInspection {
...
export function skillAfterPlannedApply(skill: AregSkillKindSkillInspection, plan: SkillKindApplyPlan): AregSkillKindSkillInspection {
...
export function planSidecarOperations(skill: AregSkillKindSkillInspection, kind: SkillInvocationKind): Result<readonly PlannedApplyOperation[]> {
...
export function planPiSettingsOperation(skillName: string, kind: SkillInvocationKind, settings: PiSettingsData): Result<PlannedApplyOperation> {
...
export function plannedRemoveEmptyDirs(plan: SkillKindApplyPlan): readonly AregSkillKindRemoveEmptyDirPlan[] {
...
export function toApplyResult(operation: PlannedApplyOperation, hasAppliedOperation: boolean, hasRemovedEmptyDir: boolean): SkillKindApplyOperationResult {
	return {
		type: operation.type,
...
export function operationStatusesForPlans(plans: readonly SkillKindApplyPlan[], operationStatuses: readonly ProjectMutationOperationStatusRecord[]): readonly { skill: string; operations: readonly SkillKindApplyOperationStatusResult[] }[] {
	...
	return plans.map((plan) => ({
		...
		operations: plan.operations.map((operation) => {
			...
			const statusIndex = operationStatuses.findIndex((status, index) => !consumedStatusIndexes.has(index) && status.type === operation.type && status.path === operation.relativePath && status.description === operation.description);
			...
			return operationStatuses[statusIndex] ?? { type: operation.type, path: operation.relativePath, description: operation.description, status: "not_attempted" as const };
		}),
	}));
}
