...
describe("init config helpers", () => {
	...
	test("parses legacy areg.json agents", () => {
		expect(parseLegacyAregJsonAgents('{"agents":["codex","cursor"]}')).toEqual({
			ok: true,
			value: ["codex", "cursor"],
		});
		expect(parseLegacyAregJsonAgents("[]")).toMatchObject({
			ok: false,
			error: { message: "areg.json must contain a JSON object." },
		});
		expect(parseLegacyAregJsonAgents('{"agents":[]}')).toMatchObject({
			ok: false,
			error: { message: "areg.json field `agents` must be a non-empty string list." },
		});
	});

	test("renders and replaces only the top-level areg TOML section", () => {
		expect(renderAregSection(["codex", "claude-code"])).toBe(
...
describe("init managed block helpers", () => {
	test("finds valid managed block bounds and rejects malformed variants", () => {
		expect(
			managedBlockBounds("prefix <s>body<e> suffix", { start: "<s>", end: "<e>" }, "file.md"),
		).toEqual({ ok: true, value: { start: 7, end: 17 } });
		expect(managedBlockBounds("no block", { start: "<s>", end: "<e>" }, "file.md")).toEqual({
			ok: true,
			value: null,
		});
		expect(
			managedBlockBounds("<s>one<s><e>", { start: "<s>", end: "<e>" }, "file.md"),
		).toMatchObject({ ok: false });
		expect(managedBlockBounds("<e>bad<s>", { start: "<s>", end: "<e>" }, "file.md")).toMatchObject({
			ok: false,
		});
	});
