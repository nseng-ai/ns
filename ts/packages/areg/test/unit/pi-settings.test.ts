...
describe("Pi settings parser", () => {
	...
	test("refuses non-directory Pi directory states", () => {
		expect(parsePiSettings({ type: "file" }, { type: "missing" })).toEqual({
			ok: false,
			error: { code: "path_not_directory", message: ".pi exists but is not a directory." },
		});
		expect(parsePiSettings({ type: "other" }, { type: "missing" })).toEqual({
			ok: false,
			error: { code: "path_not_directory", message: ".pi exists but is not a directory." },
		});
	});
	...
	test("refuses symlinked settings file", () => {
		expect(
			parsePiSettings({ type: "directory" }, { type: "symlink", target: "../settings.json" }),
		).toEqual({
			ok: false,
			error: {
				code: "path_symlink",
	...
	test("refuses non-file settings states", () => {
		expect(parsePiSettings({ type: "directory" }, { type: "directory" })).toEqual({
			ok: false,
			error: { code: "path_not_file", message: ".pi/settings.json exists but is not a file." },
		});
		expect(parsePiSettings({ type: "directory" }, { type: "other" })).toEqual({
			ok: false,
			error: { code: "path_not_file", message: ".pi/settings.json exists but is not a file." },
		});
		expect(
			parsePiSettings({ type: "directory" }, { type: "unreadable", message: "permission denied" }),
		).toEqual({
			ok: false,
			error: { code: "path_not_file", message: ".pi/settings.json exists but is not a file." },
		});
	...
	test("requires skills to be an array of strings", () => {
		expect(
			parsePiSettings({ type: "directory" }, { type: "file", text: '{"skills":[1]}' }),
		).toEqual({
			ok: false,
			error: {
