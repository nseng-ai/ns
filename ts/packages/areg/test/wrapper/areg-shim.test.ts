...
const SHIM_CONFIG = {
	tool: "areg",
	cliRelPath: "ts/packages/areg/src/cli.ts",
	installHint: "just install-areg or just install-tools",
...
/** Renders the shared shim template into an installed `areg` the way `just install-areg` does. */
async function installShim(canonicalCheckout: string): Promise<string> {
	const dir = await makeTempDir("areg-shim-");
	const template = await readFile(SHIM_TEMPLATE, "utf8");
	const shimPath = join(dir, "areg");
...
async function makeFakeCheckout(options: { shouldIncludeNodeModules: boolean }): Promise<string> {
	...
	await mkdir(join(dir, "ts/packages/areg/src"), { recursive: true });
	await writeFile(join(dir, "ts/packages/areg/src/cli.ts"), `console.log("fake-cli: " + process.argv.slice(2).join(" "));
`, "utf8");
...
describe("areg shim", () => {
	test("renders the tool-specific values into the installed shim", async () => {
		...
		expect(rendered).toContain("cli_rel_path=ts/packages/areg/src/cli.ts");
		expect(rendered).toContain("install_hint='just install-areg or just install-tools'");
	});
	...
	test("runs the enclosing checkout's TypeScript CLI inside an asdl checkout", async () => {
		...
		expect(result.stdout).toContain("Usage: areg");
	});
	...
	test("falls back to the baked canonical checkout outside a checkout", async () => {
		const outsideDir = await makeTempDir("areg-shim-");
		const shimPath = await installShim(REPO_ROOT);
	...
	test("fails clearly when the checkout has no installed ts dependencies", async () => {
		...
		expect(result.stderr).toContain("areg");
	...
	test("fails clearly when no checkout is available", async () => {
		...
		expect(result.stderr).toContain("just install-areg");
