...
export const finalizeRunOperation = defineExecOperation({
	...
	compactOutput: {
		...
		buildCompact: ({ data, fullOutput }) => compactFinalizeRunResult(data, fullOutput),
	},
});
...
function compactFinalizeRunResult(data: unknown, fullOutput: PayloadReference): { type: "ok"; value: Record<string, unknown> } {
	const result = data as FinalizeRunResult;
	return {
		type: "ok",
		value: compactOperationResult({
			...
			details: { ready_to_stop: result.ready_to_stop, all_feedback_addressed: result.all_feedback_addressed, pr_number: result.pr_number },
		}),
	};
}
