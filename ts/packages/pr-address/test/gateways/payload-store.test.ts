...
async function collectErrorCases(root: string): Promise<Map<string, ObservedError>> {
	...
	actualByName.set("session_required", expectError(resolvePayloadSessionId(null, { env: {} })));
	actualByName.set("session_invalid_env", expectError(resolvePayloadSessionId(null, { env: { ASDL_PAYLOAD_SESSION_ID: "Bad Session" } })));
	actualByName.set("session_invalid_explicit", expectError(resolvePayloadSessionId("UPPER", { env: {} })));
...
describe("payload store environment and safety behavior", () => {
	test("fromEnvironment resolves ASDL_PAYLOAD_ROOT and ASDL_PAYLOAD_SESSION_ID", async () => {
		const tempDir = await makeTempDir();
		const root = join(tempDir, "env-root");

		const store = expectOk(await PayloadStore.fromEnvironment({ env: { ASDL_PAYLOAD_ROOT: root, ASDL_PAYLOAD_SESSION_ID: "env-session" } }));
	...
	test("fromEnvironment prefers an explicit session id over the environment", async () => {
		const tempDir = await makeTempDir();
		const root = join(tempDir, "env-root");

		const store = expectOk(
			await PayloadStore.fromEnvironment({ explicitSessionId: "explicit-session", env: { ASDL_PAYLOAD_ROOT: root, ASDL_PAYLOAD_SESSION_ID: "env-session" } }),
		);
