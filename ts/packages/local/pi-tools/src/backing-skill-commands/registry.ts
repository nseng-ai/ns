...
import {
	...
	type CommandBackedSkillRegistration,
	type CommandBackedSkillRegistrationKind,
} from "@ji/pi/commands";

const LOCAL_COMMAND_BACKED_SKILL_REGISTRY = [
	{
...
export function commandBackedSkillRegistrations(): readonly CommandBackedSkillRegistration[] {
	return LOCAL_COMMAND_BACKED_SKILL_REGISTRY;
}

export function commandBackedSkillSurface(skillName: string): string | undefined {
	return LOCAL_COMMAND_BACKED_SKILL_REGISTRY.find(
		(registration) => registration.skillName === skillName,
	)?.surface;
}
...
function registrationsOfKind(
	...
	return LOCAL_COMMAND_BACKED_SKILL_REGISTRY.filter((registration) => registration.kind === kind);
}
...
export function visibleCommandBackedReplacementSurfaces(): string[] {
	return LOCAL_COMMAND_BACKED_SKILL_REGISTRY.map((registration) => registration.surface);
}
