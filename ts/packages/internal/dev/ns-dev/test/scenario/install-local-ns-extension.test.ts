...
const BASE_FILES = {
	...
	"/repo/ts/packages/incubating/extensions/objectives/package.json": JSON.stringify({
...
describe("install-local-ns-extension", () => {
	...
	it("packs with pack:local, copies the tarball, and installs it as dev dependency", async () => {
		...
		expect(run.calls).toEqual([
			...
			{
				command: "npm",
				args: ["install", "--save-dev", "/repo/ts/packages/public/ns/dist/publish"],
				cwd: "/target",
			},
		]);
	...
	it("installs objectives and other non-extension workspace packages", async () => {
		...
		expect(run.calls).toEqual([
			{
				...
				args: [
					"pack",
					"/repo/ts/packages/incubating/extensions/objectives",
					"--pack-destination",
		...
		expect(parseJsonOutput(run)).toMatchObject({
			...
			data: {
				...
				packagePath: "/repo/ts/packages/incubating/extensions/objectives",
			},
		});
	});
