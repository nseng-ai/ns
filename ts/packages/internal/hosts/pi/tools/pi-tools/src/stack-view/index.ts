/**
 * Package surface for the `@internal/pi-tools/stack-view` subpackage. Exposes
 * the presentation model and status derivation (types.ts), the stack loader
 * (data.ts), and the plain-text snapshot renderer (render.ts). Interactive overlay internals
