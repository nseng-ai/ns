...
describe("runner subagent process dispatcher", () => {
	...
	test("passes optional model before runtime extension in terminal mode", async () => {
		...
		const running = dispatchRunnerSubagentProcess(
			pi,
			ctx,
			options({ modelSelection: { provider: "anthropic", modelId: "haiku", thinking: "minimal" } }),
			runner.dependencies,
		);
		const call = await waitForSpawn(runner.calls);
	...
	test("passes inherited model and non-off thinking to child Pi and progress metadata", async () => {
		...
		const running = dispatchRunnerSubagentProcess(
			...
			{
				cwd: "/repo",
				modelSelection: { provider: "anthropic", modelId: "claude-sonnet-4-5", thinking: "medium" },
			},
	...
	test("passes optional model to child Pi args in final-text mode", async () => {
		...
		const running = dispatchRunnerSubagentProcess(
			...
			ctx,
			finalTextOptions({
				modelSelection: { provider: "anthropic", modelId: "haiku", thinking: "minimal" },
			}),
			runner.dependencies,
		);
		const call = await waitForSpawn(runner.calls);
	...
	test("passes inherited provider with an unqualified requested model pattern", async () => {
		...
		const running = dispatchRunnerSubagentProcess(
			...
			{
				cwd: "/repo",
				modelSelection: { provider: "openai-codex", modelId: "gpt-5.6-sol", thinking: "minimal" },
			},
			finalTextOptions({
	...
	test("honors an explicit cross-provider model selection", async () => {
		const runner = createFakeRunnerSubagentDispatcher({
			sessionFile: "/tmp/runner-subagent.jsonl",
		});
		const running = dispatchRunnerSubagentProcess(
			...
			finalTextOptions({
				title: "Anthropic reviewer",
				modelSelection: { provider: "anthropic", modelId: "sonnet", thinking: "minimal" },
			}),
			runner.dependencies,
		);
		const call = await waitForSpawn(runner.calls);
	...
	test("hydrates startup launch metadata from the child session file", async () => {
		...
		expect(result.progress.launch).toEqual({
			...
			modelSelection: { provider: "openai-codex", modelId: "gpt-5.4-mini", thinking: "high" },
			thinkingLevel: "minimal",
	...
	test("session launch hydration read failures are nonfatal", async () => {
		...
		const running = dispatchRunnerSubagentProcess(
			...
			ctx,
			finalTextOptions({
				modelSelection: { provider: "openai-codex", modelId: "gpt-5.4-mini", thinking: "minimal" },
			}),
			runner.dependencies,
		);
		const call = await waitForSpawn(runner.calls);
