...
import {
	...
	jsonLine,
	type SpawnCall,
} from "@local-pi-tools/runner-subagents/testing";
			...
			findings: [
				{
					...
					severity: "high",
					validationHints: [
						"pnpm --dir ts vitest run packages/local-pi-tools/thermo-council/test/thermo-council.test.ts",
					],
				},
			],
			disagreements: [],
...
describe("thermo council extension", () => {
	...
	test("launches three read-only terminal-capture reviewer seats and renders a report", async () => {
		...
		for (const call of pi.runnerCalls.slice(0, 3)) {
			...
			expect(call.args.join("\n")).toContain(".sdl/reviews/thermonuclear-review.md");
		}
...
> {
	return new Map([
		...
		["git show HEAD:.sdl/reviews/thermonuclear-review.md", { stdout: "Review strictly.\n" }],
	]);
}
