...
const REMOVED_CORE_DIR_PATH = join(
	REPO_ROOT,
	"ts/packages/incubator/flow/src",
	["co", "re"].join(""),
);
const WORKTREE_SUPPORT_PATH = join(REPO_ROOT, "ts/packages/incubator/flow/src/ns/worktree.ts");
const PUSH_COMMAND_PATH = join(REPO_ROOT, "ts/packages/incubator/flow/src/ns/commands/push.ts");
const SUBMIT_COMMAND_PATH = join(REPO_ROOT, "ts/packages/incubator/flow/src/ns/commands/submit.ts");
const REGENERATE_PR_COMMAND_PATH = join(
	REPO_ROOT,
	"ts/packages/incubator/flow/src/ns/commands/regenerate-pr.ts",
);
const AUTOBRANCH_COMMAND_PATH = join(
	REPO_ROOT,
	"ts/packages/incubator/flow/src/ns/commands/autobranch.ts",
);
const BRANCH_LATEST_COMMIT_COMMAND_PATH = join(
	REPO_ROOT,
	"ts/packages/incubator/flow/src/ns/commands/branch-latest-commit.ts",
);
const AUTOSLOT_COMMAND_PATH = join(
	REPO_ROOT,
	"ts/packages/incubator/flow/src/ns/commands/autoslot.ts",
);
const LAND_COMMAND_PATH = join(REPO_ROOT, "ts/packages/incubator/flow/src/ns/commands/land.ts");
const PULL_TRUNK_COMMAND_PATH = join(
	REPO_ROOT,
	"ts/packages/incubator/flow/src/ns/commands/pull-trunk.ts",
);
const FLOW_PACKAGE_PATH = join(REPO_ROOT, "ts/packages/incubator/flow/package.json");
...
const FLOW_AUTOBRANCH_INTERNAL_PATH = join(
	REPO_ROOT,
	"ts/packages/incubator/flow/src/autobranch/dirty-worktree.ts",
);

const REMOVED_LOCAL_AUTOBRANCH_HELPERS = [
	["ts/packages/incubator/flow/src/autobranch", "branch-availability.ts"],
	["ts/packages/incubator/flow/src/autobranch", "branch-slugs.ts"],
	["ts/packages/incubator/flow/src/autobranch", "branch-slug-text.ts"],
	["ts/packages/incubator/flow/src/autobranch", ["latest", "commit", "autobranch.ts"].join("-")],
] as const;
