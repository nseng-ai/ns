...
export function loadGraphiteMetadataStatus(
	...
	const upstackCount = upstackCountFromTopology(
		parsed.topology,
		input.currentBranch,
		liveBranches.branches,
	);
	const stackTopologyCounts = {
		...(downstackCount === undefined ? {} : { downstackCount }),
		...(upstackCount === undefined ? {} : { upstackCount }),
	};
	...
	return {
		...
		...(downstackCount === undefined && upstackCount === undefined ? {} : { stackTopologyCounts }),
	};
}
