...
describe("Graphite metadata status lookup", () => {
	...
	test("omits the downstack count when an ancestor metadata row is missing", () => {
		...
		expect(status).toMatchObject({ type: "tracked", upstackCount: 0 });
		expect(Object.hasOwn(status, "downstackCount")).toBe(false);
	});
	...
	test("degrades to counting-once when the metadata topology contains a cycle", () => {
		...
		expect(status).toMatchObject({ type: "tracked", upstackCount: 1 });
		expect(Object.hasOwn(status, "downstackCount")).toBe(false);
	});
	...
	test("parses single-request worker protocol messages without request IDs", () => {
		...
		expect(
			graphiteMetadataWorkerResponseFromValue({
				type: "success",
				status: { ...trackedWithCounts, downstackCount: "2" },
			}),
		).toBeUndefined();
		expect(
			graphiteMetadataWorkerResponseFromValue({
				type: "success",
				status: { ...trackedWithCounts, upstackCount: -1 },
			}),
		).toBeUndefined();
	});
});
