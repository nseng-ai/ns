...
describe("sdl changes CLI", () => {
	...
	test("dirty worktree prints model bullets and raw status without mutation", async () => {
		...
		expect(run.context.textGenerationCalls[0]?.prompt).toContain("## branch\n\nfeature/demo");
		expect(run.context.textGenerationCalls[0]?.prompt).toContain("M src/app.ts\n?? notes.md");
	...
	test("git errors fail with command details", async () => {
		...
		expect(statusFailed.context.textGenerationCalls).toEqual([]);
	});
