...
import { FakeRoasterGitHubGateway, type GitHubGatewayOptions, type RoasterGitHubGateway } from "../../src/gateways/github.ts";
import type { PRChangedFile, PRDiscussionComment, PRInlineCommentInput, PRReviewComment } from "../../src/models.ts";
...
async function runRoaster(args: readonly string[], options: { readonly stdin?: string; readonly github?: RoasterGitHubGateway } = {}): Promise<RunResult> {
	const stdout: string[] = [];
	const stderr: string[] = [];
...
function failedEnvelope(): string {
	return JSON.stringify({ exit_code: 2, error_type: "harness_binary_missing", message: "claude not found" });
}
...
describe("roaster exec CLI", () => {
	...
	test("format-findings-comment renders findings from stdin", async () => {
		const run = await runRoaster(["exec", "format-findings-comment"], { stdin: findingsEnvelope([inlineFinding]) });
		expect(run.exitCode).toBe(0);
	...
	test("post-inline-findings no-ops for empty and failed run envelopes", async () => {
		const empty = await runRoaster(["exec", "post-inline-findings", "--pr-number", "47"], { stdin: findingsEnvelope([]), github: new UnexpectedInlineQueryGateway() });
		expect(empty.exitCode).toBe(0);
		expect(JSON.parse(empty.stdout)).toMatchObject({ postedCount: 0, fallbackOnlyCount: 0, apiError: null });

		const failed = await runRoaster(["exec", "post-inline-findings", "--pr-number", "47"], { stdin: failedEnvelope(), github: new UnexpectedInlineQueryGateway() });
		expect(failed.exitCode).toBe(0);
		expect(JSON.parse(failed.stdout)).toMatchObject({ postedCount: 0, fallbackOnlyCount: 0, apiError: null });
	});

	test("post-inline-findings posts inlineable findings and skips duplicate markers", async () => {
		const changedFiles = new Map<number, readonly PRChangedFile[]>([[47, [{ path: "app.py", status: "modified", patch: "@@ -1 +1 @@\n+new" }]]]);
		const firstGateway = new FakeRoasterGitHubGateway({ changedFilesByPr: changedFiles });
		const first = await runRoaster(["exec", "post-inline-findings", "--pr-number", "47"], { stdin: findingsEnvelope([inlineFinding]), github: firstGateway });
		...
		const reviewComments = new Map<number, readonly PRReviewComment[]>([[47, [{ author: "github-actions[bot]", body: markerBody }]]]);
		const duplicateGateway = new FakeRoasterGitHubGateway({ changedFilesByPr: changedFiles, reviewCommentsByPr: reviewComments });
		const duplicate = await runRoaster(["exec", "post-inline-findings", "--pr-number", "47"], { stdin: findingsEnvelope([inlineFinding]), github: duplicateGateway });
		expect(duplicate.exitCode).toBe(0);
		expect(JSON.parse(duplicate.stdout)).toMatchObject({ postedCount: 0, skippedDuplicateCount: 1 });
		expect(duplicateGateway.createdReviews()).toHaveLength(0);

		const humanReviewComments = new Map<number, readonly PRReviewComment[]>([[47, [{ author: "alice", body: markerBody }]]]);
		const humanQuotedGateway = new FakeRoasterGitHubGateway({ changedFilesByPr: changedFiles, reviewCommentsByPr: humanReviewComments });
		const humanQuoted = await runRoaster(["exec", "post-inline-findings", "--pr-number", "47"], { stdin: findingsEnvelope([inlineFinding]), github: humanQuotedGateway });
		expect(humanQuoted.exitCode).toBe(0);
		expect(JSON.parse(humanQuoted.stdout)).toMatchObject({ postedCount: 1, skippedDuplicateCount: 0 });
	});
	...
	test("post-inline-findings preserves fallback-only reasons and API errors", async () => {
		...
		const fallback = { path: "other.py", line: 10, severity: "info", summary: "Fallback", details: "Not changed." };
		const run = await runRoaster(["exec", "post-inline-findings", "--pr-number", "47"], { stdin: findingsEnvelope([inlineFinding, fallback]), github: gateway });
		expect(run.exitCode).toBe(0);
	...
	test("post-findings-comment creates and updates bot comments", async () => {
		...
		const created = await runRoaster(["exec", "post-findings-comment", "--pr-number", "47", "--run-url", "https://run"], { stdin: body, github: gateway });
		expect(created.exitCode).toBe(0);
		expect(created.stderr).toContain("posted findings comment");

		const comments = new Map<number, readonly (PRDiscussionComment & { readonly author: string })[]>([[47, [{ id: 1, body: "<!-- roaster:dignified-python -->\nold\n", author: "github-actions[bot]" }]]]);
		const updateGateway = new FakeRoasterGitHubGateway({ discussionCommentsByPr: comments });
		const updated = await runRoaster(["exec", "post-findings-comment", "--pr-number", "47"], { stdin: "<!-- roaster:dignified-python -->\nnew\n", github: updateGateway });
	...
	test("post-findings-comment rejects body without first-line marker", async () => {
		const run = await runRoaster(["exec", "post-findings-comment", "--pr-number", "47"], { stdin: "no marker\n" });
		expect(run.exitCode).toBe(1);
