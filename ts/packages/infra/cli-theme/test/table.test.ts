...
import { dim, paint } from "../src/palette.ts";
import { type Cell, cell, type Column, kv, renderTable } from "../src/table.ts";
...
describe("renderTable column alignment", () => {
	...
	test("styled cells with SGR escapes still align the trailing column", () => {
		...
		const offsets = data.map((line) => stripTerminalEscapes(line).indexOf(token));
	...
	test("header row is dimmed", () => {
		...
		expect(stripTerminalEscapes(lines[0] ?? "").startsWith("OBJECTIVE")).toBe(true);
	});
});
...
describe("renderTable truncation at caps.columns", () => {
	test("fill column clips overflowing content with a caps-aware ellipsis", () => {
		...
		expect(stripTerminalEscapes(dataLine).endsWith("…")).toBe(true);
	});
	...
	test("ascii caps degrade the ellipsis to dots", () => {
		...
		const long = "this-is-a-very-long-objective-slug";
		const lines = renderTable({ caps: c, columns, rows: [[cell(long, long)]] });
		expect(stripTerminalEscapes(lines[1] ?? "").endsWith("...")).toBe(true);
	});
});
...
describe("renderTable right alignment", () => {
	test("right-aligned cells pad on the left", () => {
		...
		const data = stripTerminalEscapes(lines[1] ?? "");
