...
describe("FakeBrmemGateway", () => {
	it("reads Base Namespace and named Namespace entries", async () => {
		...
		expect(
			await gateway.getEntry({ namespace: "base", branch: "feat/x", key: "scratch" }),
		).toMatchObject({
			type: "found",
			value: { content: "base content" },
		});
		expect(
			await gateway.getEntry({ namespace: "notes", branch: "feat/x", key: "missing" }),
	...
	it("normalizes generated and seeded UTC timestamps", async () => {
		...
		expect(generatedList.value.map((entry) => entry.updatedAt)).toEqual([
			"2026-01-01T00:00:01+00:00",
		]);
		expect(generatedList.value[0]?.updatedAt).not.toMatch(/(?:\.000)?Z$/u);
		const generatedCheck = await gateway.checkEntry({
			namespace: "base",
			branch: "main",
			key: "generated.md",
		});
		expect(generatedCheck).toMatchObject({
			type: "found",
			value: { headDate: "2026-01-01T00:00:01+00:00" },
		});
		...
		const seededCheck = await gateway.checkEntry({
			namespace: "base",
			branch: "seed",
			key: "seed.md",
		});
		expect(seededCheck).toMatchObject({
			type: "found",
			value: { headDate: "2026-02-03T04:05:07+00:00" },
		});
	});
	...
	it("mutates coherent snapshots without leaking caller-owned state", async () => {
		const gateway = new FakeBrmemGateway();
		expect(
			(await gateway.putEntry({ namespace: "base", branch: "main", key: "a", content: "A" })).type,
		).toBe("ok");
		expect(
			(await gateway.putEntry({ namespace: "base", branch: "main", key: "b", content: "B" })).type,
		).toBe("ok");
		...
		expect((await gateway.deleteEntry({ namespace: "base", branch: "main", key: "a" })).type).toBe(
			"ok",
		);
	...
	it("reads historical Entry content and diagnostics by commit", async () => {
		...
		const second = await gateway.putEntry({
			namespace: "handoff",
			branch: "feat/x",
			key: "resume.md",
			content: "second\n",
		...
		expect(
			await gateway.getEntry({ namespace: "handoff", branch: "feat/x", key: "resume.md" }),
		).toMatchObject({
			type: "found",
			value: { content: "second\n" },
		});
		expect(
			await gateway.getEntry({
				namespace: "handoff",
				branch: "feat/x",
				key: "resume.md",
	...
	it("copies snapshots and key globs with conflict behavior", async () => {
		...
		expect(entries.value.map((entry) => entry.key)).toEqual([
			"foo/body.md",
			"foo/sub/x.md",
			"keep.txt",
		]);
