...
interface HandoffSdlCommandOptions<S extends SdlCommandSchema, T> {
	name: string;
	summary: string;
	description: string;
	schema: S;
	...
	handler(ctx: HandoffCliContext, request: z.output<S>): Promise<ClinkrExit<T>>;
}
...
export function handoffSdlCommand<S extends SdlCommandSchema, T>(
	...
	return {
		...
		run: async (ctx: SdlExtensionApi, request: z.output<S>) => {
			const handoffContext = await createSdlHandoffContext(ctx);
			return await options.handler(handoffContext, request);
		},
	};
}
