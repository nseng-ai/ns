...
describe("handoff gc", () => {
	test("dry-run preserves deleted branch handoffs", async () => {
		...
		const data = (
			parseJsonOutput(run) as {
				data: {
					...
					dry_run: boolean;
					cancelled: boolean;
					entries: { slug: string; action: string }[];
				};
			}
		).data;
		...
		expect(Object.fromEntries(data.entries.map((entry) => [entry.slug, entry.action]))).toEqual({
			live: "kept_active",
			stale: "would_delete",
		});
		...
		expect(await getEntryContent(gateway, { key: "alpha_beta.md", branch: "feat/deleted" })).toBe(
			"underscore",
		);
	});
	...
	test("interactive accept and decline keep JSON stdout machine-readable", async () => {
		...
		await putHandoffEntry(acceptedGateway, {
			key: "stale.md",
			branch: "feat/deleted",
			content: "stale",
		});
		const accepted = runScenario(["gc"], {
			brmem: acceptedGateway,
		...
		expect(
			await getEntryContent(acceptedGateway, { key: "stale.md", branch: "feat/deleted" }),
		).toBeUndefined();

		const declinedGateway = new FakeBrmemGateway();
		await putHandoffEntry(declinedGateway, {
			key: "stale.md",
			branch: "feat/deleted",
			content: "stale",
		});
		...
		expect(parseJsonOutput(declined)).toMatchObject({
			data: { cancelled: true, would_delete_count: 1 },
		});
		expect(
			await getEntryContent(declinedGateway, { key: "stale.md", branch: "feat/deleted" }),
		).toBe("stale");
	});
	...
	test("no candidates skips prompt", async () => {
		...
		const run = runScenario(["gc"], {
			brmem: gateway,
			gitState: { existingBranches: ["feat/live"], currentBranch: { type: "detached" } },
		});
		expect(await run.exit).toBe(0);
