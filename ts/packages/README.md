...
Structure *inside* a container package — which units earn `ji.subpackages` rank, naming, and importer rules — is governed by [`docs/conventions/subpackage-conventions.md`](../../docs/conventions/subpackage-conventions.md) (ADR 0022/0023).

## Top-level packages

- `sdl/` is the SDL kernel and CLI package. It stays top-level by design.
- `capability-kit/` is the first-party Capability Kit substrate (`@ji/capability-kit`). It stays top-level by design and does not need a category wrapper.
...
- `infra/` contains below-SDK neutral packages such as core primitives, CLI framework support, Branch Memory, and Graphite support. The former transitional domain-primitives package has been deleted; shared first-party capability-building primitives now live under precise `@ji/capability-kit/*` subpaths.
...
- `hosts/` contains presentation/runtime hosts such as Pi and `jicc`.
...
- `local/` contains private, project-local Pi-native tools under the reserved `@internal/*` scope. They are not SDL capabilities, not public CLIs, and not distribution packages. They are registered only through this repository's `.pi/extensions/*` discovery adapters.
