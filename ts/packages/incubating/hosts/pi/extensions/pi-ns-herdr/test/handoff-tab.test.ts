...
import { createHandoffLaunchIntegration } from "@nseng-ai/pi-ns-handoffs/create-flow";
import type {
	CommandContext,
	HandoffCreateSkillLoader,
	HandoffExtensionAPI,
	ToolDefinition,
} from "@nseng-ai/pi-ns-handoffs/create-flow";
...
import { registerHerdrHandoffTab } from "../src/adapter/handoff-tab.ts";
import { isExactOptionalIntegrationAbsence } from "../src/adapter/extension.ts";
import { FakeHerdrGateway } from "@nseng-ai/pi-ns-herdr/testing";
...
describe("optional Handoffs integration absence classifier", () => {
	test("suppresses only a realistic exact curated-module miss", () => {
		const absent = Object.assign(
			new Error(
				"Cannot find package '@nseng-ai/pi-ns-handoffs/create-flow' imported from /repo/herdr/extension.js",
			),
			{ code: "ERR_MODULE_NOT_FOUND" },
		);
		const transitive = Object.assign(
			new Error(
				"Cannot find package 'broken-transitive' imported from /repo/node_modules/@nseng-ai/pi-ns-handoffs/create-flow.js",
			),
			{ code: "ERR_MODULE_NOT_FOUND" },
		);
