...
import registerHerdrPiExtension from "../src/adapter/extension.ts";
import {
	registerHerdrPlanSpaceImplCommand,
	registerHerdrPlanTabImplCommand,
} from "../src/adapter/impl-plan.ts";
import { handleHerdrSlotImplPlan } from "@nseng-ai/herdr/api";
import { createHerdrPiCommandApi } from "../src/adapter/pi-command-api.ts";
import { createCliHerdrGateway } from "@nseng-ai/herdr/api";
import { handleHerdrSlotImplPrompt, resolveImplPromptPayloadOptions } from "@nseng-ai/herdr/api";
import { registerHerdrPromptSpaceImplCommand } from "../src/adapter/impl-prompt.ts";
...
import {
	...
	writePlanStoreFile,
	savedPlanEntry,
} from "@nseng-ai/pi-ns-herdr/testing";
...
function herdrPlanTestContext(options: HerdrPlanTestContextOptions) {
	return {
		...
		pi: options.ctx,
		buildLaunchCommand: (prompt: string, profile: Parameters<typeof buildPiLaunchCommand>[1]) =>
			buildPiLaunchCommand(prompt, profile),
		resolveLaunchProfile: () => ({ thinkingLevel: "medium" as const }),
		formatBranchContextCommand: (key: string) => `/ns:branch-context:impl-attached-plan ${key}`,
		buildTrackedBranchLaunchCommand: (
			branchName: string,
			profile: Parameters<typeof buildTrackedBranchPayloadLaunchCommand>[1],
		...
		await handleHerdrSlotImplPrompt(
			{
				...
				git,
				resolveLaunchProfile: () => ({ thinkingLevel: "medium" as const }),
				buildTrackedBranchLaunchCommand: (branchName: string, profile) =>
					buildTrackedBranchPayloadLaunchCommand(branchName, profile),
			},
			{
...
): import("@nseng-ai/herdr/api").ResolvedHerdrSlotImplPlanOptions {
	return {
		planStoreRoot,
		slotClient: testSlotClient,
		git: new InMemoryGitGateway({ currentBranch: SOURCE_BRANCH }),
