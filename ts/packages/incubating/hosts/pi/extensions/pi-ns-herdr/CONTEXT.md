...
Prompt, session, and Saved Plan implementation are symmetric across space and tab destinations. A tab prompt resolves the explicit caller space through the Herdr Consumer Gateway caller-context operation (backed by Herdr's caller-aware `pane current --current` query) immediately after command acknowledgement, before idle waiting, Git inspection, interaction, or mutation, and creates a focused tab in that explicit caller space. Caller-space resolution fails closed; there is no environment or UI-focus fallback. Session commands create a visible model summary turn and prefill the matching prompt command in the editor for review; they share one pending-summary coordinator and never auto-submit or mutate Herdr, Git, Handoff, or Branch Memory state.

Herdr resource creation, labels, explicit caller targeting, Slot-backed prepared destinations, and pane process launch remain owned by `@nseng-ai/herdr` and are consumed through `@nseng-ai/herdr/api`. User-facing language says **space**, **tab**, and **caller space**; **workspace** is reserved for upstream Herdr mechanics such as workspace IDs.
