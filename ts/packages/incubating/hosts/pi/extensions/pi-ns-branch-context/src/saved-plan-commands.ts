...
import type { BranchContextExtensionOptions, CommandContext } from "./host-types.ts";
import type { SavedPlanObserver } from "./saved-plan-observer.ts";
...
- If ${GRILL_ASK_TOOL_NAME} is unavailable or returns ui_unavailable, stop, explain that structured grill UI is required, summarize current status, and do not save.
...
- If ${GRILL_ASK_TOOL_NAME} returns end_grill, stop, summarize resolved decisions, unresolved branches, and final recommendation, and do not save.
...
- Review the final Markdown plan for completeness, write it to a temporary file with Pi's built-in write tool, then run \`enriched-plan exec save --file <path> --format json\` as a standalone bash command; do not generate or pass a slug.
- Do not chain, redirect, or wrap the save command. Stop on collision or derivation failure. Remove the temporary file only after a successful save.
...
export function registerSavedPlanCommands(
	pi: BranchContextPiCommandApi,
	observer: SavedPlanObserver,
	_options: BranchContextExtensionOptions = {},
): void {
	registerCommandWithImmediateAck({
		...
		commandDefinition: {
			...
			handler: async (args, ctx) => {
				observer.arm(ctx.cwd);
				try {
					await handleWritePlanCommand(pi, args, ctx);
				} catch (error) {
					observer.disarm();
					throw error;
				}
	...
	registerCommandWithImmediateAck({
		...
		commandDefinition: {
			...
			handler: async (args, ctx) => {
				observer.arm(ctx.cwd);
				try {
					await handleWriteGrilledPlanCommand(pi, args, ctx);
				} catch (error) {
					observer.disarm();
					throw error;
				}
