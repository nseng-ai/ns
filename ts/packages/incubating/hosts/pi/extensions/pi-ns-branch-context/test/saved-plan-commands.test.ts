...
describe("enriched-plan-commands", () => {
	test("registers plans write and branch-context workflow commands without a custom save tool", () => {
		const pi = new FakePi();
	...
	test("ns:plan:save waits for idle, reads repo prompt, and dispatches the generated prompt", async () => {
		...
		expect(pi.sentUserMessages[0]).toContain("3–7 words in lowercase kebab-case");
