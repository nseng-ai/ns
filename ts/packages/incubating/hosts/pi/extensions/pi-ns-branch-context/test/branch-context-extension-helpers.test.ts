...
describe("buildWritePlanPrompt", () => {
	test("includes local plan store instructions without branch creation", () => {
		...
		expect(prompt).toContain(
			"enriched-plan exec save --slug '<slug>' --content-file '<exact path>' --format json",
		);
	...
	test("checked-in write-plan prompt policy is an intentional repo override", async () => {
		...
		for (const content of [DEFAULT_WRITE_PLAN_PROMPT_BODY, checkedInContent]) {
			...
			expect(content).toContain("3–7 words in lowercase kebab-case");
			expect(content).toContain("returned `data.slug` equals the supplied slug");
			expect(content).toContain(
...
describe("buildWriteGrilledPlanPrompt", () => {
	test("includes structured grill requirements and save/no-save contract", () => {
		...
		expect(prompt).toContain("enriched-plan exec save --slug '<slug>' --content-file");
