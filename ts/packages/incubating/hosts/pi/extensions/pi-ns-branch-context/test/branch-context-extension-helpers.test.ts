...
describe("buildWriteGrilledPlanPrompt", () => {
	test("includes structured grill requirements and save/no-save contract", () => {
		...
		expect(prompt).toContain("write_saved_plan_file");
		expect(prompt).toContain("grill_ask_round");
		expect(prompt).toContain("complete ordered current frontier");
		expect(prompt).toContain("At most five successfully submitted decision rounds");
		expect(prompt).toContain("Confirmation does not count");
		expect(prompt).toContain("Return to grilling recomputes the complete frontier");
		expect(prompt).toContain("explicit confirmed evidence");
		expect(prompt).toContain('"kind":"saved-plan","maxDecisionRounds":5');
