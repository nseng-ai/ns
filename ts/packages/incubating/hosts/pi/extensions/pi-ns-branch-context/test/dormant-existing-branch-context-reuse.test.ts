...
import type {
	BranchContextImplBranchContext,
	BranchContextImplBranchNewSessionOptions,
} from "../src/session/impl-branch-launch.ts";
import { runDormantGitExistingBranchContextReuse } from "../src/dormant-existing-branch-context-reuse.ts";
...
function fixture(entries: Array<{ branch: string; key: string }>, currentBranch = "current") {
	...
	const launchContext: BranchContextImplBranchContext = {
		cwd: "/repo",
		hasUI: true,
		ui: { setStatus() {} },
		sessionManager: { getSessionFile: () => "/sessions/source.jsonl" },
		async newSession(options?: BranchContextImplBranchNewSessionOptions) {
...
describe("private dormant Git existing-branch reuse orchestration", () => {
	test("preserves candidate selection, exact checkout, and fresh-session dispatch", async () => {
		for (const candidate of [
			...
			const result = await runDormantGitExistingBranchContextReuse({
				...options(subject),
	...
	test("preserves ambiguity and verification failures without launch", async () => {
		const subject = fixture([]);
		await expect(
			runDormantGitExistingBranchContextReuse({
				...options(subject),
