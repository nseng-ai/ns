...
This incubating package preserves the provider-independent `/ns:branch-context:*` and `/ns:plan:*` Pi commands and owns the plain-Git `/ns:git:branch-from-plan` and `/ns:git:branch-and-impl-from-plan` pair. It keeps Pi registration, prompt/status presentation, plan-save tooling, Grill activation, and session replacement out of the harness-independent Branch Context package. It consumes portable behavior through `@nseng-ai/branch-context/api` and `@nseng-ai/plans/api`, and uses neutral `@nseng-ai/pi-runtime/...` host contracts.
