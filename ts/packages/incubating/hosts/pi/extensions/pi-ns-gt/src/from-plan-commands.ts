...
function resolveBranchContextContext(
	...
	if (pi.exec !== undefined) {
		...
		return createBranchContextContext(commands, { cwd });
	}
...
export async function deriveCreateBranchContextPreview(
	...
	const branchCreation: BranchCreationMethod = "graphite";
...
async function createBranchContextFromPreview({
	...
	const selectedOperation = await selectBranchContextCreateOperationTarget({
		...
		git: context.git,
		brmem: context.brmem,
		isExplicitTargetBranch: preview.branchNameForCreation !== undefined,
	});
	...
	if (current.type !== "branch") {
		throw new Error(
			current.type === "detached"
				? "Graphite branch creation requires a named current branch."
				: current.error.message,
		);
	}
	const provider =
		extensionOptions.createBranchCreationProvider?.({
			context,
			parentBranch: current.branch,
		}) ??
		new GraphiteBranchCreationProvider({
			git: context.git,
			graphite: new RealGraphiteBranchGateway(context.commands),
			parentBranch: current.branch,
		});
...
export type GtExtensionOptions = BranchContextExtensionOptions;

export function registerGtCommands(
	pi: BranchContextPiCommandApi,
