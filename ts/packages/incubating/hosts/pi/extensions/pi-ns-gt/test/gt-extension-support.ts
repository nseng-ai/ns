...
import type { BranchCreationRequest } from "@nseng-ai/extension-kit/branch-creation";
...
export function branchContextExtensionTestOptions(
	fakes: BranchContextOperationFakes,
	entries: readonly BranchMemoryEntrySeed[] = [],
	useInMemoryGit = false,
): BranchContextExtensionOptions {
	return {
		branchContextOperations: fakes.operations,
		shouldResolveTargetBranchInPreview: true,
		createBranchCreationProvider: fakes.createBranchCreationProvider,
		createBranchContextContext(pi, cwd) {
			const commands = createPiCommandExecApi(pi);
			const realContext = createBranchContextContext(commands, { cwd });
			return {
				commands,
				git: useInMemoryGit
					? new InMemoryGitGateway({ currentBranch: SOURCE_BRANCH })
					: realContext.git,
...
export interface BranchContextOperationFakes {
	...
	createBranchCalls: BranchCreationRequest[];
	createBranchCreationProvider: NonNullable<
		BranchContextExtensionOptions["createBranchCreationProvider"]
	>;
...
export function createBranchContextOperationFakes(
	overrides: Partial<BranchContextOperations> & {
		createBranch?(
			...
			{ ok: true } | { ok: false; error: { code: string; message: string }; branchCreated: boolean }
		>;
	} = {},
): BranchContextOperationFakes {
	const loadPlanCalls: Array<Parameters<BranchContextOperations["loadBranchContextPlan"]>> = [];
	const createBranchCalls: BranchCreationRequest[] = [];
	...
	return {
		loadPlanCalls,
		createBranchCalls,
		createBranchCreationProvider({ context }) {
			return {
				id: "graphite-test",
				async createBranch(request) {
					createBranchCalls.push({ ...request });
					const overridden = await overrides.createBranch?.(request);
					if (overridden !== undefined) return overridden;
					const created = await context.git.createBranchAtStartPoint({
						cwd: request.cwd,
					...
					return created.ok
						? { ok: true }
						: { ok: false, error: created.error, branchCreated: false };
				},
			};
		},
		writePlanCalls,
...
export async function makeRepoPrompt(content = "# Plan"): Promise<string> {
	const dir = await makeTempDir();
