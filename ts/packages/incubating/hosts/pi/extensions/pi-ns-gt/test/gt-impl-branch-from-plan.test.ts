...
import registerGtExtension from "../src/extension.ts";
...
async function assertStrictSavedPlanFailure(
	...
	expect(pi.sentMessages.at(-1)?.content).toBe(
		[
...
describe("GT impl-branch-from-plan strict creation", () => {
	test("missing plan store ignores explicit, session, and current Attached Plan evidence without provider or mutation", async () => {
		await assertStrictSavedPlanFailure("missing-directory");
	});

	test("empty plan store ignores explicit, session, and current Attached Plan evidence without provider or mutation", async () => {
		await assertStrictSavedPlanFailure("no-plan-files");
	});

	test("creates with Graphite, checks out the exact branch, and dispatches Attached Plan implementation", async () => {
		const filePath = await makeNamedPlanFile();
		const pi = new FakePi([planSlugStep(DEFAULT_PLAN_CONTENT), gitCheckoutStep(PLAN_SLUG)]);
		const fakes = createBranchContextOperationFakes();
		registerGtExtension(pi, branchContextExtensionTestOptions(fakes, [], true));
		const command = pi.commands.get("ns:gt:impl-branch-from-plan");
		const context = createContext();
