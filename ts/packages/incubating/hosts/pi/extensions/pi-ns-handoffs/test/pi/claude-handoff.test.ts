...
} from "../../src/adapter/claude-command.ts";
import {
	deriveSourcePiSessionId,
	resolveSourcePiSessionId,
} from "../../src/adapter/investigation-sources.ts";
import {
	BRANCH,
	FakePi,
	ROOT,
...
describe("claude handoff command", () => {
	...
	test.each([
		...
		{
			name: "detached HEAD",
			branch: step("git", ["branch", "--show-current"], { stdout: "\n" }),
	...
	test("launch tool verifies the handoff exists and launches Claude with pickup instructions", async () => {
		...
		const result = await tool.execute(
			"tool-call",
			{ branch: BRANCH, slug: "fix-auth-flow" },
			undefined,
			undefined,
			context.ctx,
		);
		...
		expect(runClaude.invocations[0]?.prompt).toContain(
			`/ns:handoff:pickup --branch ${BRANCH} fix-auth-flow`,
		);
