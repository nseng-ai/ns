...
- `ts/packages/internal/hosts/pi/subagents/ns-pi-subagents/src/runner-subagents/curated-context.ts` uses `CuratedContextExecGit` for git evidence.
- `ts/packages/internal/hosts/pi/subagents/ns-pi-subagents/src/runner-subagents/subagent-process.ts` is the async-spawn adapter seam for runner subagents; module logic depends on injected process functions.
...
- The injected host `ctx.exec` author-facing result shape lives in `@nseng-ai/sdk`. The command-execution gateway lives in `@nseng-ai/ns-foundation/exec`: pure command types/helpers are re-exported from `@nseng-ai/ns-foundation/command`, and the real Node child-process adapter (`runCommand`, `NodeCommandExecApi`) lives in `@nseng-ai/ns-foundation/exec` itself.
...
Every repo-owned Pi slash command must acknowledge receipt synchronously, before awaiting `ctx.waitForIdle()` or starting slow work. Use `registerCommandWithImmediateAck` from `@nseng-ai/pi-runtime/commands/ack` at each command registration site instead of hand-writing per-command acknowledgements or wrapping the host.
