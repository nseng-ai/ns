...
import {
	...
	type ExtensionAPI,
	type ToolCallResult,
} from "../src/core/sessions/harness-session.ts";
...
describe("harness session extension", () => {
	...
	test("restores an existing custom harness session id entry", async () => {
		...
		await pi.emitSessionStart({
			...
			entries: [
				{
					type: "custom",
					customType: "ns-harness-session-id",
	...
	test("does not double-prefix commands that already set HARNESS_SESSION_ID", async () => {
		...
		await expect(
			pi.emitBash("HARNESS_SESSION_ID=manual ns address exec get-feedback 123"),
		).resolves.toEqual({
			command: "HARNESS_SESSION_ID=manual ns address exec get-feedback 123",
			result: undefined,
		});
		await expect(
			pi.emitBash("export HARNESS_SESSION_ID=manual\nns address exec get-feedback 123"),
		).resolves.toEqual({
