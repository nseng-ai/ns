...
import type { WorktreeGhStatus } from "@nseng-ai/pi-runtime/worktree-status";
import {
	deferred,
...
describe("worktree status extension registration and rendering", () => {
	...
	test("custom footer renders multiline worktree status as separate footer lines", async () => {
		...
		expect(footerLines.at(-1)).toBe("/ns:flow:changes running CLI command (23s)");
...
function createRootCheckoutFixture(): LinkedWorktreeFixture {
	const tempRoot = mkdtempSync(join(tmpdir(), "ns-worktree-footer-"));
