...
**Herdr Pi subpackage**:
The `@nseng-ai/herdr/pi` subpackage presents Herdr's resource-first space and tab operations plus `/ns:herdr:impl:prompt:space`, `/ns:herdr:impl:session:space`, `/ns:herdr:impl:plan:space`, and `/ns:herdr:impl:plan:tab`. The prompt and plan commands start implementation from fresh prompt payloads; the session command summarizes the caller's active conversation path into a directed prompt and fills the input box with a ready-to-send `/ns:herdr:impl:prompt:space` command. Prepared Herdr Launch and Pi launch remain destination/process startup mechanics beneath the prompt and plan workflows; `ns-impl` identifies prompt transport/storage for those workflows, and Handoff launch remains specific to the durable Handoff integration.
