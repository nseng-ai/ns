...
**Discovery adapter**:
A thin project-local extension file whose job is to register Pi commands or tools by importing implementation code from `ts/packages/incubating/hosts/pi/runtime/pi-runtime/src/` or from another owning package when the Pi implementation has been extracted. For extracted Pi-tool packages, extension `pi` subpackages, and host-adapter composition exports, the adapter imports the owning package through its package exports so `@nseng-ai/pi-runtime` does not become the tool or extension presentation consumer. A package that owns a complete integration may instead be discovered directly through its `pi.extensions` manifest.
*Avoid*: package export, shim as implementation, generated extension, host-to-tool registry.

**Engineered Pi implementation domain**:
A tested host-resident implementation area under `ts/packages/incubating/hosts/pi/runtime/pi-runtime/src/<domain>/` for project-local Pi behavior such as PR views, worktree status, terminal presentation, host-owned runtime helpers, and command registration helpers. Flow, Herdr, Handoff, Branch Context, and Objective Pi presentation are extracted into separate `@nseng-ai/pi-ns-*` host-adapter packages that consume their owning extensions' curated APIs. The broader Pi separation remains incomplete for other host-resident and extension-owned surfaces.
...
**Embedded structured-command invocation**:
The per-command Pi bridge that supplies a finite JSON request reader with no ambient stdin fallback, invocation-owned Clinkr `renderResult`/`echo` capture, complete settled non-TTY and non-ANSI rendering capabilities, typed progress presentation, and semantic confirmation/selection backed only by the active command context's `ctx.ui`. Required interaction fails closed without applicable UI. Pi sanitizes captured terminal escape and control sequences at its presentation seam before content reaches the TUI; raw commands keep their separate byte sinks. The bridge does not virtualize terminal streams, raw mode, key events, or a PTY.
*Avoid*: process stdin inheritance, ambient process output, physical-terminal capability inference, terminal emulation inside Pi.

**Shared CLI result presentation**:
The Pi-host presentation applied after an underlying CLI runner executes through `registerCliCommandExtension`: the bridge captures structured primary result and auxiliary echo text per invocation, derives completed compatibility `stdout`/`stderr` details at the host edge, persists their exact text in private OS-temporary `stdout.log` and `stderr.log` files, resolves the repository model policy's `pi.cli-command-result-summary` operation (default profile `fast`), and asks that model for a bounded, validated summary. Pi mechanically appends the absolute evidence paths. If policy resolution, generation, or validation fails, Pi presents the complete captured raw output plus those paths; if persistence fails, Pi presents complete raw output plus a storage warning and does not invoke the model. Parse, mapping, and positional-argument rejections remain deterministic and inline because no CLI runner executed. This is Pi host presentation and renderer safety: Clinkr continues to own command rendering and stream selection, while Foundation owns sink and byte adaptation.
*Avoid*: Clinkr summarization, model-authored evidence paths, interactive-model inheritance, raw-output deletion, Foundation host presentation.
