...
A curated `@nseng-ai/pi-runtime/...` package export for helper code intentionally reusable by other workspace packages, extension `pi` subpackages, or extracted Pi-tool packages, including command acknowledgement, command UI helpers, command I/O, command names, model-call and LM-JSON helpers, shared error/timer helpers, machine-envelope parsing, session replacement, harness-effective required-skill source capture and deferred expansion, terminal layout/presentation helpers, parity helpers, and Pi runtime/tool types. The skill helper consumes Pi's exact effective winner from `getSystemPromptOptions().skills`; explicitly repository-authoritative lookup remains separately named. The current export map is intentionally limited to these neutral/runtime/presentation families: `commands/*`, `models/*`, `parity/*`, `runtime/*`, `sessions/replacement`, `skills/*`, `terminal/*`, `shared/*`, and `worktree-status`, plus `worktree-status/extension`, which is a project-local extension entrypoint carried in the export map for `.pi/extensions` loading rather than a neutral helper family.
