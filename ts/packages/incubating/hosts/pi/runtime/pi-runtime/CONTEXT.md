...
**Herdr Pi subpackage**:
The `@nseng-ai/herdr/pi` subpackage presents Herdr's resource-first space and tab operations plus `/ns:herdr:impl:prompt:space`, `/ns:herdr:impl:session:space`, `/ns:herdr:impl:plan:space`, and `/ns:herdr:impl:plan:tab`. The prompt and plan commands start implementation from fresh prompt payloads; the session command continues a separately persisted clone of the caller's selected active conversation path. Prepared Herdr Launch and Pi launch remain destination/process startup mechanics beneath the workflow; `ns-impl` identifies prompt transport/storage only for the fresh prompt and plan workflows, and Handoff launch remains specific to the durable Handoff integration.
