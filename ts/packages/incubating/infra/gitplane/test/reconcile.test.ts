...
function artifacts(
	...
	return new InMemoryArtifactGateway({
		commits: { requested: { type: "found", value: commit } },
		commitInventories: [
			{
				commit,
				artifactRoot: "items",
...
test("matching retry replays the frozen plan without source or configuration reads", async () => {
	...
	expect(retryStore.snapshot()).toMatchObject({ attempts: [], events: [{ sequence: 1 }] });
});

test("conflicting pending work is refused before source reads or durable writes", async () => {
	const interrupted = new InMemoryMaterializationStoreGateway({
		failures: { insertEvent: failure },
	});
	...
	const forbiddenSource = new InMemoryArtifactGateway({ failures: { resolveCommit: failure } });
	const result = await reconcile(context(retryStore, forbiddenSource), {
		...options,
		targetCommitish: "different",
	});
	expect(result).toMatchObject({ type: "structural-failure", code: "attempt-conflict" });
	expect(forbiddenSource.operationLog()).toEqual([]);
...
test("post-CAS residue retries cleanup only and never replays source or artifact writes", async () => {
	const interrupted = new InMemoryMaterializationStoreGateway({
		failures: { deleteReconciliationAttempt: failure },
	});
...
test("source and pre-write structural failures leave no reconciliation residue", async () => {
	...
	expect(sourceFailureStore.snapshot()).toMatchObject({ attempts: [], errors: [] });
});
