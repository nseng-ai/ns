...
import {
	...
	type ReconciliationAttemptRecord,
...
class FaultStore extends InMemoryMaterializationStoreGateway {
	...
	override async insertReconciliationAttempt(
		record: ReconciliationAttemptRecord,
	): Promise<InsertResult> {
		if (this.shouldFail("insertReconciliationAttempt", "before"))
			return { type: "error", error: injected };
		const result = await super.insertReconciliationAttempt(record);
		return this.shouldFail("insertReconciliationAttempt", "after")
	...
	override async deleteReconciliationAttempt(request: {
		readonly sourceId: string;
		readonly attemptId: string;
	}): Promise<OperationResult> {
		return this.operation("deleteReconciliationAttempt", () =>
			super.deleteReconciliationAttempt(request),
		);
	}
}
