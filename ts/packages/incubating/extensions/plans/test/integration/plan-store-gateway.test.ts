...
describe("RealPlanStoreGateway", () => {
	...
	test("retries a final-path collision without overwriting the colliding file", async () => {
		const root = await mkdtemp(join(tmpdir(), "plans-real-collision-"));
		try {
			const collidingPath = join(root, "collision-plan--26-01-02T03-04-05--1.md");
			class CollidingPlanStoreGateway extends RealPlanStoreGateway {
				private injectedCollision = false;

				protected override async beforePublicationLink(): Promise<void> {
					if (this.injectedCollision) return;
					this.injectedCollision = true;
					await writeFile(collidingPath, "# Other Writer\n", "utf8");
				}
			}
			const gateway = new CollidingPlanStoreGateway();
			const publication = await gateway.publishBytesAtomic({
				directoryPath: root,
				fileNameForSequence: (sequence) => `collision-plan--26-01-02T03-04-05--${sequence}.md`,
				sequenceFromFileName: (fileName) => {
					const match = /--(\d+)\.md$/.exec(fileName);
					return match?.[1] === undefined ? undefined : Number(match[1]);
				},
				content: new TextEncoder().encode("# Published Plan\n"),
			});

			expect(publication.sequence).toBe(2);
			expect(await gateway.readTextFile(collidingPath)).toBe("# Other Writer\n");
			expect(await gateway.readTextFile(publication.filePath)).toBe("# Published Plan\n");
		} finally {
			await rm(root, { recursive: true, force: true });
		}
	});
