...
describe("durable Saved Plan selection", () => {
	test("resolves timestamped explicit paths with parsed filename details", async () => {
	...
	test("normalizes a home-relative explicit Local Plan Store path", async () => {
		...
		const filePath = join(directory, "home-relative-saved-plan--26-01-02T03-04-05--1.md");
		...
		expect(result).toMatchObject({ type: "resolved", plan: { filePath, format: "timestamped" } });
	});

	test("requires lexical and realpath containment and a regular .md file", async () => {
		const fixture = makeFixture();
		const outside = "/outside/canonical-saved-plan--26-01-02T03-04-05--1.md";
	...
	test("selects explicit when supplied and otherwise durable latest", async () => {
		const fixture = makeFixture();
		const explicit = join(fixture.directory, "canonical-saved-plan--26-01-01T03-04-05--1.md");
		fixture.store.writeFile(explicit, "# Explicit\n");
		await expect(
			resolveSelectedSavedPlanFile(commands, fixture.options(explicit)),
		).resolves.toMatchObject({
			type: "explicit",
			plan: { format: "timestamped", filePath: explicit },
		});
