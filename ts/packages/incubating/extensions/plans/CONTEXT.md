...
The curated `@nseng-ai/plans/api` surface used by downstream consumer packages and their tests for in-process saved-plan composition without broad package-root imports. It includes saved-plan path, evidence, selection, and the Saved Plan content-slug derivation operation and evidence type. The Saved Plan policy is owned beside that operation; the shared content-slug mechanism, model-policy resolution, and private prompt mechanics are owned by Extension Kit. Owning `@nseng-ai/plans` tests may still import the package root when covering root compatibility.
