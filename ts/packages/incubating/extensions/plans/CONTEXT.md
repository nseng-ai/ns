...
The curated `@nseng-ai/plans/api` surface used by downstream consumer packages and their tests for in-process saved-plan composition without broad package-root imports. It includes saved-plan path, evidence, selection, and the Saved Plan content-slug derivation operation and evidence type. The Saved Plan policy is owned beside that operation; the shared content-slug model-execution, normalization, validation, evidence, and private prompt mechanics are owned by Extension Kit. Callers resolve model policy before invoking the operation. Owning `@nseng-ai/plans` tests may still import the package root when covering root compatibility.
*Avoid*: generic content-slug API, variant seed, prompt helper, private source import, command shell, root barrel contract
