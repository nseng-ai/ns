...
The `/ns:herdr:impl:session:space [continuation-focus]` workflow summarizes the caller Pi session's active compaction-aware conversation path into one self-contained directed implementation prompt and fills the input box with a ready-to-send `/ns:herdr:impl:prompt:space <prompt>` command. It requires a persisted caller session, uses an explicit trimmed continuation focus to steer the summary, mutates nothing itself, and leaves branch, Slot, payload, and Herdr space creation to the prompt workflow when the composed command is sent. It is a visible prompt composer, not a session transfer: no session clone, Saved Plan, Branch Memory entry, or Handoff Artifact is produced.
*Avoid*: session clone, `pi --session` continuation, auto-submitted composed command, Handoff transfer, in-memory fallback, prompt implementation alias
