...
A Herdr `impl` workflow that implements a prompt or Saved Plan using the existing agent instructions and workflow behavior. `impl` is shorter, avoids collision with dispatch terminology for remote systems, and describes the outcome more accurately than `launch`; destination and process launch remain supporting mechanics. `/ns:herdr:impl:session:{space,tab} [focus]` forks the persisted source Pi session in a non-interactive process to produce a directed, self-contained implementation prompt, creates the destination branch through the ordinary prompt workflow, stores the complete prompt at immutable-per-branch `ns-impl/prompt.md`, verifies an exact Branch Memory read, and launches a prompt-free destination Pi whose launch command carries only a non-sensitive branch marker; on the destination's initial startup a Herdr-owned one-shot bootstrap verifies the checked-out branch, loads that entry directly from Branch Memory, and injects it as the destination session's first user prompt. After generation, the source session prints the prompt as a foldable TUI-only custom entry (collapsed preview by default, expandable in full; never part of the source session's LLM context) and presents a compact menu: implement now, load into the editor for review/edit, or cancel. Only explicit implement approval enters the branch/persistence/launch pipeline; loading into the editor lets the user edit the prompt, and cancellation or menu dismissal performs no implementation mutation. During direct implementation the source session's LLM context receives only acknowledgement, compact progress, identifiers, byte count, and actionable failures; the generated prompt never enters it. The retained Entry supports replay and diagnosis. A retry creates a collision-resolved branch and refuses any existing payload rather than overwriting it.
*Avoid*: visible summary turn, editor prefill except after explicit load-for-review approval, prompt text in progress or diagnostics, remote dispatch, new agent-behavior contract, process startup as the workflow outcome, Handoff Artifact transport for session implementation, temporary prompt-file transport, public bootstrap command
