...
A Herdr `impl` workflow that implements a prompt or Saved Plan using the existing agent instructions and workflow behavior. `impl` is shorter, avoids collision with dispatch terminology for remote systems, and describes the outcome more accurately than `launch`; destination and process launch remain supporting mechanics. `/ns:herdr:impl:session:{space,tab} [focus]` runs a private, tool-less `pi --fork` process against the persisted source session only to produce a directed, self-contained implementation prompt with repository-relative anchors. After generation, the source session prints the prompt as a foldable TUI-only custom entry (collapsed preview by default, expandable in full; never part of the source session's LLM context), identifies its checkout as source context, and presents a compact menu: implement on a new branch in an isolated Slot, load into the editor for review/edit, or cancel. Only explicit implement approval enters the branch/persistence/launch pipeline; loading into the editor lets the user edit the prompt, and cancellation or menu dismissal performs no implementation mutation.
