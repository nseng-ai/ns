...
local branch refs. Linked worktrees can therefore report missing or divergent gh-stack inventories over
...
Every successful result includes `worktreeGitDir`, the canonical worktree Git directory derived
from Git's absolute `--git-path gh-stack` result. Human output renders this provenance before the stacks,
including for an empty inventory. It identifies the inspected gh-stack view; it does not claim that this
worktree owns every branch or that peer gh-stack state agrees.
