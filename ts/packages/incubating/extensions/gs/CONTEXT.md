...
**GS command face**:
The provider-branded `ns gs` command group. `list` exposes Current-worktree gh-stack inventory with Provider worktree provenance; `restack-resolve` advances one local provider restack step; and `autobranch` moves dirty cached trunk or the invoking view's tracked top onto a checkpointed child. The mutating commands have thin `/ns:gs:*` Pi routers.
