...
describe("linked worktree local gh-stack inventory", () => {
	test("reads only the invoking worktree gh-stack state and reports its canonical Git directory", async () => {
		...
		await expect(gateway.readLocalInventory({ cwd: linked })).resolves.toEqual({
			ok: true,
			value: {
				worktreeGitDir: dirname(linkedStatePath),
				stacks: [
					{
						number: 3,
						base: "main",
		...
		await expect(gateway.readLocalInventory({ cwd: repository })).resolves.toEqual({
			ok: true,
			value: {
				worktreeGitDir: dirname(primaryStatePath),
				stacks: [
					{
						number: 2,
						base: "main",
	...
	test("does not fall back to another worktree when current-worktree gh-stack state is missing", async () => {
		...
		await expect(gateway.readLocalInventory({ cwd: linked })).resolves.toEqual({
			ok: true,
			value: { worktreeGitDir: dirname(linkedStatePath), stacks: [] },
		});
	});
});
