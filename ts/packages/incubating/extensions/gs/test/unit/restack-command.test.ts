...
import {
	...
	InMemoryGsRestackProviderGateway,
...
describe("GS restack-resolve", () => {
	...
	test("refuses version drift before mutation", async () => {
		...
		expect(scenario.provider.mutations).toEqual([]);
	});
	...
	test("requires --yes noninteractively and prompts on a TTY", async () => {
		...
		expect(allowed.provider.mutations).toEqual(["start-full"]);
	});
	...
	test.each([
		...
		expect(scenario.provider.mutations).toEqual([mutation]);
	});
	...
	test("continues exactly once to completion", async () => {
		...
		expect(scenario.provider.mutations).toEqual(["continue"]);
	});
	...
	test("bounds diagnostics and renders recovery last", async () => {
		...
		expect(scenario.provider.mutations).toHaveLength(1);
	});
});

function createScenario(options: InMemoryGsRestackOptions) {
	const provider = new InMemoryGsRestackProviderGateway(options);
	return {
		provider,
		context: { provider, git: new InMemoryGsRestackGitGateway(options) },
	};
}
