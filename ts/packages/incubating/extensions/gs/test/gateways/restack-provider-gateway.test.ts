...
import { GS_RESTACK_COMMAND_ENV, RealGsRestackProviderGateway } from "../../src/core/index.ts";
...
describe("real GS restack provider gateway", () => {
	test("uses only exact public argv and the command environment overlay", async () => {
		...
		const gateway = new RealGsRestackProviderGateway(commands, "/repo");
	...
	test("rejects malformed version output and bounds diagnostics", async () => {
		...
		const result = await new RealGsRestackProviderGateway(commands, "/repo").readVersion();
