...
describe("real GS restack gateway", () => {
	test("uses only exact public argv and the command environment overlay", async () => {
		const commands = new RecordingCommands([
			exited(0, { stdout: "gh stack version 0.1.0\n" }),
			exited(0),
			exited(0),
			exited(0),
		]);
	...
	test("rejects malformed version output and bounds diagnostics", async () => {
		const commands = new RecordingCommands([exited(0, { stdout: `${"x".repeat(2_000)}\n` })]);
...
interface ExitedOptions {
	readonly stdout?: string;
	readonly stderr?: string;
}

function exited(code: number, options: ExitedOptions = {}): ExecResult {
	return {
		type: "exited",
		code,
		signal: null,
