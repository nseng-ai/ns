...
export const gsAutobranchResultSchema = z.lazy(() =>
	z.strictObject({
		...
		effects: z.array(z.string().max(200)).max(EFFECT_LIMIT),
		diagnostic: z.string().max(DIAGNOSTIC_LIMIT).nullable(),
...
export async function runGsAutobranch(
	...
	data = { ...data, path };
	let sourceView: GsAutobranchProviderView | null = null;
	...
	return path === "trunk-bootstrap"
		? await runBootstrap(context, data, before, child, checkpointMessage)
		: await runExtension(context, data, before, sourceView!, child, checkpointMessage);
}
...
async function runBootstrap(
	...
	const committed = await context.checkpoint.commit(message);
	...
	return ok(completed(data, after.value, rel, effects, committed.ok ? committed.value : null));
}
...
async function runExtension(
	...
	data: GsAutobranchResult,
	before: GsAutobranchGitFacts,
	_sourceView: GsAutobranchProviderView,
	child: string,
	message: string,
) {
	...
	return ok(completed(data, observed.value, rel, effects, committed.value));
}
...
function completed(
	...
	return {
		...
		relationship: rel,
		effects: [...effects].slice(0, EFFECT_LIMIT),
