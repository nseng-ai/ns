...
import type { GsRestackGitGateway, GsRestackGitState } from "./restack-git.ts";
import {
	GS_RESTACK_PROVIDER_VERSION,
	type GsProviderDiagnostic,
	type GsRestackProviderGateway,
...
export interface GsRestackContext {
	readonly provider: GsRestackProviderGateway;
...
export async function runGsRestackResolve(
	...
	if (version.value !== GS_RESTACK_PROVIDER_VERSION) {
		return negative(`gh-stack ${GS_RESTACK_PROVIDER_VERSION} is required.`, {
			data: recover(
				data,
				"install-supported-provider",
				`Install gh-stack ${GS_RESTACK_PROVIDER_VERSION}, then rerun this command.`,
			),
		});
	}
	...
	const mutation =
		mode === "continue"
			? await context.provider.continue()
			: await context.provider.start(request.downstack ? "downstack" : "full");
	const after = await context.git.inspect();
	if (!after.ok) {
		return failure("restack-protocol-failed", "Could not inspect Git after provider mutation.", {
			...data,
	...
	if (after.state.operation === "rebase" && after.state.unmergedPaths.length > 0) {
		return negative("The provider stopped at a conflict.", {
			data: recover(
				{ ...data, outcome: "conflict-stopped" },
				"resolve-conflicts",
				"Resolve and stage the listed paths, then rerun this command.",
	...
	if (!mutation.ok) {
		return negative("The provider refused the restack mutation.", {
			data: recover(
				data,
				"inspect-provider-error",
				"Inspect the provider diagnostic, then rerun this command after correcting it.",
			),
		});
	}
	...
	return negative("The provider did not reach a classifiable completion or conflict stop.", {
		data: recover(
			data,
			"inspect-provider-error",
			"Inspect the provider diagnostic and Git state before rerunning this command.",
		),
	});
}
...
export function renderGsRestackHuman(data: GsRestackResult): string {
	...
	return [
		...
		...(data.diagnostic === null ? [] : [`Provider: ${data.diagnostic.termination}`]),
...
async function authorize(
	...
	if (!interaction.isInteractive()) {
		return usageError(
			...
			recover(data, "authorize-rewrite", "Rerun with --yes to authorize one provider mutation."),
		);
	}
	if (await interaction.confirm(`Run the ${data.mode} restack mutation?`)) return true;
	return negative("Restack was not authorized.", {
		data: recover(data, "authorize-rewrite", "Rerun and authorize the provider mutation."),
	});
}
...
function resultData(
	...
	diagnostic: GsProviderDiagnostic | null,
): GsRestackResult {
