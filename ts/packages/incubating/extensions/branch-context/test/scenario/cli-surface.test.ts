...
import { VERSION } from "../../src/core/cli.ts";
...
describe("branch-context CLI help, version, and dispatch pins", () => {
	...
	test("pins --runtime output", async () => {
		...
		expect(run.stdout.join("")).toBe(
			"runtime: typescript\nentry_point: @nseng-ai/branch-context bin branch-context -> ts/packages/incubating/extensions/branch-context/(no package bin)\n",
		);
		expect(run.stderr.join("")).toBe("");
		expectNoGitOrBrmemCalls(run);
	});
