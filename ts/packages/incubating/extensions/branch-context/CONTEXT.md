...
The separate `@nseng-ai/pi-ns-branch-context` package that owns concrete Pi slash-command registration, prompt/status presentation, Saved Plan authoring prompts, Grill activation, and fresh-session orchestration while consuming Branch Context behavior through `@nseng-ai/branch-context/api` and Saved Plan behavior through `@nseng-ai/plans/api`. Exact-byte Saved Plan persistence is delegated to the hidden Plans CLI file-input operation; it is not a Pi tool or Branch Context behavior. Branch Context resumption session artifacts remain separate from Saved-Plan Selection. Command-name constants remain portable extension API metadata so Skill Exposure and Herdr can refer to the stable surfaces without depending on a host adapter. Project-only Grill UI names do not belong in the Branch Context API.
