...
TypeScript package for the Address Extension API and `ns address exec ...` command face: PR feedback report download plus shared PR feedback primitives.
...
`@nseng-ai/pr-feedback/api` is the curated in-process Extension API for PR-feedback consumers. Consumers that handle PR lookup, review, discussion-comment, review-thread, review-thread mutation, or Address check payloads should import the gateway and DTO vocabulary from this subpath instead of the kit `@nseng-ai/extension-kit/github/pr-feedback` contract, Pi modules, command operation schemas, or private source paths.

Current export classification:

- **Stable Address Extension API:** `PrAddressGithubGateway`; PR lookup/review/discussion/review-thread DTOs; review-thread reply/resolve DTOs; feedback failure/options/operation types. These are owned by Address as the projected PR-feedback seam over the kit's canonical `GithubPrFeedbackGateway`.
