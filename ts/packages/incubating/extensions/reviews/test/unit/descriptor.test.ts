...
describe("reviews extension descriptor", () => {
	test("owns exactly the canonical reviews route subtree", async () => {
		expect(await readdir(descriptor.commandDirectory)).toEqual(["reviews"]);
		expect(
			await readdir(
