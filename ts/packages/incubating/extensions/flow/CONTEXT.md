...
The `executeLanding` entry point on the **Land subpackage API** that owns the full `LandingRequest` lifecycle — discovery, preflight planning, confirmation, pre-merge preparation, merge, Post-Merge Stack Reconciliation, landed-branch cleanup, and post-landing managed-slot cleanup under the closed resource-disposition policy (`preserve` / `free`) — and returns a `LandingExecutionResult` whose completed and failed variants carry the same observed-fact `LandingExecutionReport`. After each successful PR merge, Flow performs the one **Post-Merge Stack Reconciliation** operation for the remaining stack before proceeding or completing; remaining selected branches and descendant roots are target-planning details, not separate operations. Both policies require reconciliation. `preserve` then retains landed local branches and the current managed slot; `free` runs **Landed-Branch Cleanup** and only frees the slot after that pass succeeds. The executor reports reconciliation as `post-merge-stack-reconciliation`; the separate `landed-branch-cleanup` phase completes only after all selected landed-branch cleanup attempts finish, including a successful operation that changes no cleanup facts. A halt contributes no completed phase for the interrupted operation; the canonical failed-result builder appends its failed phase once. A selector-capable host may approve confirmation with a chosen cleanup policy after presenting keep (default), free, and cancel choices; explicit flags and non-selector hosts retain their existing behavior.
*Avoid*: cleanup policy controlling reconciliation, phase synthesis from plan shape or cleanup fact counts, failed phases returned as completed operation phases, slot cleanup before landed-branch cleanup, gateway-level `LandResult` widening, second execution report model, Flow-owned cleanup ordering
...
**Stack Landing Plan**:
The renderer-independent plan produced for a **Stack Landing Target**, including branch plans, PR submit requirements, managed-slot conflicts, and target-planning details for required **Post-Merge Stack Reconciliation**.
