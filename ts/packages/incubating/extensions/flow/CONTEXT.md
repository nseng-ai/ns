...
The Flow ownership boundary for public `ns flow gt autobranch` behavior. The provisional `/ns:flow:gs:autobranch` and `/ns:flow:gs:autoslot` are not part of the **Flow Command Face**: they are generic Skill-Backed Commands whose exact effective `ns-flow-gs-autobranch` and `ns-flow-gs-autoslot` skills temporarily specify provider-native dirty trunk bootstrap and dirty tracked-top extension while evidence is gathered for future provider-specific imperative Flow code. Autoslot repeats that full procedure, then composes `ns slot checkout --current` only after verified provider state, checkpoint success, and clean status; Slot failure preserves the durable provider child as a partial failure.
*Avoid*: Herdr extension public command owner, plain branch helper, Graphite primitive, shipped `ns flow gs` command, universal provider transaction, slash-command-to-slash-command composition
