...
The Flow ownership boundary for public `ns flow gt autobranch` behavior. Native GS autobranch belongs to the GS command face and its former provisional Flow registration is retired. The remaining `/ns:flow:gs:autoslot` is not part of the **Flow Command Face**: it is a generic Skill-Backed Command whose exact effective `ns-flow-gs-autoslot` skill temporarily embeds provider-native autobranch before composing `ns slot checkout --current`. Slot failure preserves the durable provider child as a partial failure. This provisional composition does not make Flow the GS lifecycle owner.
