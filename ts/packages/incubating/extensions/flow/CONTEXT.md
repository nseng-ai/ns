...
The `executeLanding` entry point on the **Land subpackage API** that owns the full `LandingRequest` lifecycle — discovery, preflight planning, confirmation, pre-merge preparation, merge, per-merge maintenance, post-target reconciliation, optional **Upstack Continuation**, landed-branch cleanup, and post-landing managed-slot cleanup under the closed cleanup policy (`preserve` / `free`) — and returns a `LandingExecutionResult` whose completed and failed variants carry the same observed-fact `LandingExecutionReport`. Both policies perform required maintenance between selected PRs and reconcile surviving branches after the final selected PR. `preserve` then retains landed local branches and the current managed slot; `free` runs landed-branch cleanup and only frees the slot after that pass succeeds. **Upstack Continuation** runs after reconciliation and preserves the invoking slot. The executor records each operation as it finishes: `post-target-maintenance` owns reconciliation of surviving remaining landing branches, `descendant-maintenance` owns descendant reconciliation, and `merge-maintenance-cleanup` completes only after all selected landed-branch cleanup attempts finish, including a successful operation that changes no cleanup facts. A halt contributes no completed phase for the interrupted operation; the canonical failed-result builder appends its failed phase once. For an execute-mode managed-slot landing whose flag-derived policy is `preserve` and which is not continuing upstack, a selector-capable host may approve confirmation with a chosen cleanup policy after presenting keep (default), free, and cancel choices; explicit flags and non-selector hosts retain their existing behavior.
