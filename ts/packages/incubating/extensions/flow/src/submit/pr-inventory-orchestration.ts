...
export type PrInventoryProgressListeners = Pick<
...
export interface PrMetadataReplacementOptions {
	...
	generation?: Extract<PrInventoryGenerationResolution, { ok: true }>;
...
export async function preparePrMetadataReplacement(
	...
	const generation =
		options.generation ??
		(await resolvePrInventoryGeneration({
			env: options.env,
			cwd: options.cwd,
			git: options.git,
...
export function formatPromptSourceLabel(source: PromptSource): string {
	switch (source.type) {
		case "builtin":
			return "built-in flow.submit.pr-inventory";
		case "repo":
			return "repository flow.submit.pr-inventory";
		case "env":
			return "environment override flow.submit.pr-inventory";
	}
}

export function assemblePrInventoryBody(input: {
	inventory: string;
	source: PrMetadataReplacementSource;
	promptSource: PromptSource;
	modelSelection: ModelSelection;
	...
	const disclosure = `*Automatically generated by \`${command}\` from the diff and commit headlines, without author steering, interview, or approval. It may omit intent, rationale, constraints, or context not visible in that evidence.*`;
