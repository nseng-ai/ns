...
Rerun the failing check command shown in the context. After it passes, rerun the recorded `ns flow gt submit` invocation. If the correct fix is ambiguous, ask the user for steering instead of changing the check.
