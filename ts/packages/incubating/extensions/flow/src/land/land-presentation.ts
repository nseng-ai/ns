...
export function usage(): string {
	return [
		...
		"",
		"Without [workflow].stack-provider, lands the current branch's GitHub PR into Git trunk.",
		'With stack-provider = "graphite", lands the current Graphite stack into trunk: the stack path lands bottom branch through current branch, one PR at a time, and maintains descendants when possible.',
		"Graphite stack mode requires a clean repo, non-draft open PRs, bottom PR based on gt trunk, and no landing-branch manual worktree conflicts. Descendant reconciliation after landing is required for full completion; descendant worktree conflicts require explicit consent (confirmation or --yes) and finish as a nonzero partial completion with deferred descendant maintenance.",
