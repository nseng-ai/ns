...
export type LatestCommitTransactionResult =
	...
	| { ok: false; kind: "transaction_git_trunk_missing" }
	| { ok: false; kind: "transaction_git_trunk_check_failed"; error: string }
...
export async function runLatestCommitAutobranchTransaction(
	...
	switch (upstream.type) {
		...
		case "git_trunk_missing":
			return { ok: false, kind: "transaction_git_trunk_missing" };
...
>()({
	...
	transaction_git_trunk_missing: {
		verdict: "failure",
		message: () =>
			"Could not re-check the Git trunk from cached `refs/remotes/origin/HEAD` for the synchronized source branch before moving the latest commit. Refresh it with `git remote set-head origin --auto`, or set it explicitly with `git remote set-head origin <branch>`, then retry.",
	},
	transaction_git_trunk_check_failed: {
		verdict: "failure",
		message: (failure) =>
			`Could not re-check the Git trunk from cached \`refs/remotes/origin/HEAD\` for the synchronized source branch before moving the latest commit.\n${failure.error}\nRefresh it with \`git remote set-head origin --auto\`, or set it explicitly with \`git remote set-head origin <branch>\`, then retry.`,
	},
