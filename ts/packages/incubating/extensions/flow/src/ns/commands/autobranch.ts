	...
	return defineCommand({
		...
		renderHuman: (result, caps) =>
			renderResultBlock(resolveThemeCaps(caps), {
				kind: "success",
				headline: `Created a ${providerLabel} branch from dirty worktree changes.`,
				cwd: result.cwd,
				body: result.summary,
			}),
		handler: async (ctx, request: AutobranchRequest) => {
			...
			return await runWithNsCommandIo(io, async (io) => {
				const result = await dispatchAutobranchCheckpoint(
					...
					{
						...createAutobranchDispatchEnv(ctx, args, slugModel.modelSelection, provider),
					...
					case "pending-worktree":
						return negative(
							renderPendingWorktreeFailure(caps, {
								error: result.error,
								cwd: ctx.cwd,
								commandLabel: `\`ns flow ${namespace} autobranch\``,
							}),
						);
					case "refused-clean":
						...
						return negative(
							renderResultBlock(caps, {
								kind: "refusal",
								headline: `\`ns flow ${namespace} autobranch\` requires pending worktree changes and did not run.`,
								cwd: result.snapshot.root,
								body: "Working tree is clean.",
								guidance: `Use \`ns flow ${namespace} branch-latest-commit\` to move the latest eligible commit to a new ${providerLabel} child branch.`,
							}),
						);
					case "flow": {
						...
						return negative(
							renderAutobranchFailureResultBlock({
								...
								cwd: result.snapshot.root,
								error: flow.error,
								refusalHeadline: `Did not create a ${providerLabel} branch from dirty worktree changes.`,
								failureHeadline: `Could not create a ${providerLabel} branch from dirty worktree changes.`,
							}),
						);
					}
				}
...
export const flowAutobranchCommand = createFlowAutobranchCommand("graphite");
