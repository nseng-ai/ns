...
export const flowPushCommand: NsCommand = defineCommand({
	...
	renderHuman: (result, caps) =>
		renderResultBlock(resolveThemeCaps(caps), {
			...
			cwd: result.cwd,
			guidance:
				"For Graphite-tracked PR branches, prefer `ns flow gt submit`; if this push moved a PR branch outside Graphite, run `gt get` or `gt sync` before submitting again.",
		}),
...
async function runPush(ctx: NsExtensionApi) {
	...
	if (!status.ok) {
		return negative(
			renderGitResultBlock(caps, {
				...
				guidance:
					"Inspect the Git output, fix the repository state, or use `ns flow gt submit` / `/ns:flow:gt:submit` for the Graphite submit flow when appropriate.",
			}),
		);
	}
	...
	if (!status.isClean) {
		return negative(
			renderGitResultBlock(caps, {
				...
				guidance:
					"Commit or stash outstanding changes first, or use `ns flow gt submit` / `/ns:flow:gt:submit` for the Graphite submit flow when appropriate.",
			}),
		);
	}
	...
	return negative(
		renderGitResultBlock(caps, {
			kind: "failure",
			headline:
				"`git push` failed. The branch is likely out of sync or needs the Graphite submit flow; use `ns flow gt submit` / `/ns:flow:gt:submit` for Graphite-tracked PR branches.",
			command: "git push",
			cwd: ctx.cwd,
			result: pushResult,
		}),
