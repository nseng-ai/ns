...
This command requires a clean worktree. The latest commit is eligible when the source has no upstream, is locally ahead of its locally known upstream, or is exactly synchronized on a non-trunk branch. Remote-ahead, diverged, and exactly synchronized configured repository trunk states are refused. Upstream checks use only local tracking refs and do not fetch.
	...
	defineCommand({
		...
		handler: async (ctx, request: BranchLatestCommitRequest) => {
			...
			const dispatched = await dispatchAutobranchCheckpoint(
				{ mode: "require-clean" },
				await createAutobranchDispatchEnv(ctx, args, model.modelSelection),
			);
