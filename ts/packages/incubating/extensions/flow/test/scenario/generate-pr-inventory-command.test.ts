...
function bodyInspectingEditResponse(): ScriptedExecResponse {
	return {
		match: (call: ExecCall) => {
			...
			expect(body).toContain("Prompt-owned disclosure.");
			expect(body).not.toContain("Automatically generated by `ns flow generate-pr-inventory`");
			expect(body).toContain("Prompt: built-in flow.submit.pr-inventory");
			expect(body).toContain("Model: `openai-codex/gpt-5.6-luna-fast`");
...
describe("project-local generate-pr-inventory extension behavior", () => {
	test("confirms destructive complete replacement and edits exact generated metadata", async () => {
		...
		expect(confirmation).toContain("Generate and replace complete PR metadata?");
