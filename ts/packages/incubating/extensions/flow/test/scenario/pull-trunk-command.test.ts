...
describe("flow pull-trunk command outcomes", () => {
	test("successful fetch exits 0 on stdout with a house-style result block", async () => {
		...
		expect(stdout).toContain("Pulled local repository trunk branch `main` only.");
	...
	test("repository trunk failure exits 1 on stderr and surfaces stderr", async () => {
		const exec: ScriptedExecResponse[] = [
			{
				match: "git symbolic-ref refs/remotes/origin/HEAD",
				result: { code: 1, stderr: "fatal: no repository trunk\n" },
			},
		];
		const run = runFlowPullTrunkCommandWithFakes({ state: { exec } });
	...
	test("missing configured upstream stops before worktree inspection", async () => {
		...
		expect(run.stdout.join("")).toBe("");
		const stderr = stripAnsi(run.stderr.join(""));
		expect(stderr).toContain("Repository trunk `main` has no configured Git upstream");
