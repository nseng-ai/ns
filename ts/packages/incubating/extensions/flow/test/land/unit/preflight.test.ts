...
describe("@nseng-ai/flow land stack preflight planning", () => {
	...
	test("refuses before mutation when GitHub reports an open PR based on a landing head that the provider does not know", async () => {
		...
		// deletes, restacks, or submits were attempted.
		expect(git.snapshotBackupRefsCalls).toEqual([]);
		expect(github.squashMergePullRequestCalls).toEqual([]);
	...
	test("accepts remote dependents that the provider already reports as descendants or landing branches", async () => {
		const { context } = createInMemoryLandContext({
			git: {
				localBranches: [
					{ name: "feature-a", sha: SHA_A },
					{ name: "feature-child", sha: SHA_B },
				],
			},
			graphite: {
				stackShape: stackSnapshot({
					...
					landingBranches: ["feature-a"],
					descendantBranches: ["feature-child"],
					descendantRootBranches: ["feature-child"],
				}),
			},
			github: {
				pullRequests: [
		...
		expect(plan).toMatchObject({
			type: "success",
			value: {
				descendantMaintenance: { type: "auto", branches: ["feature-child"] },
			},
		});
	});
});
