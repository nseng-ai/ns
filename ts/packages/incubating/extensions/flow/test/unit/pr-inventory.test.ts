...
describe("PR inventory helpers", () => {
	...
	test("accepts Markdown callouts and disclosure-like prose as ordinary body content", () => {
		const body = [
			"Summary",
			"",
			"> [!IMPORTANT]",
			...
			"",
			"Evidence inputs: described by the selected presentation guidance.",
		].join("\n");

		expect(parsePrInventoryOutput(`Inventory title\n\n${body}`)).toEqual({
			ok: true,
			inventory: { title: "Inventory title", body },
		});
	});
