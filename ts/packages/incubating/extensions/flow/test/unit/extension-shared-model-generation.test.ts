...
const FLOW_MODEL_GENERATION_HELPER_PATH = join(
	REPO_ROOT,
	"ts/packages/incubating/extensions/flow/src/ns/model-generation.ts",
);
...
describe("project extension shared model generation helper", () => {
	...
	test("delegates changes summaries with package-owned model selection", async () => {
		...
		const result = await sharedModule.prepareFlowChangesSummary(
			...
			},
			{
				...
				status: " M ts/packages/incubating/extensions/flow/src/ns/commands/changes.ts\n",
				diff: "diff --git a/ts/packages/incubating/extensions/flow/src/ns/commands/changes.ts b/ts/packages/incubating/extensions/flow/src/ns/commands/changes.ts\n",
			},
		);
	...
	test("delegates checkpoint messages with flow-selected checkpoint model", async () => {
		...
		const result = await sharedModule.prepareFlowCheckpointMessage(
			...
			{
				status: " M ts/packages/incubating/extensions/flow/src/ns/commands/cp.ts\n",
				diff: "diff --git a/ts/packages/incubating/extensions/flow/src/ns/commands/cp.ts b/ts/packages/incubating/extensions/flow/src/ns/commands/cp.ts\n",
			},
		);
	...
	test("keeps checkpoint repair delegated to the package helper", async () => {
		...
		const result = await sharedModule.prepareFlowCheckpointMessage(
			...
			{
				status: " M ts/packages/incubating/extensions/flow/src/ns/commands/autobranch.ts\n",
				diff: "diff --git a/ts/packages/incubating/extensions/flow/src/ns/commands/autobranch.ts b/ts/packages/incubating/extensions/flow/src/ns/commands/autobranch.ts\n",
			},
		);
...
async function loadSharedModelGenerationModule(): Promise<SharedModelGenerationModule> {
	const sharedModule = await import(FLOW_MODEL_GENERATION_HELPER_PATH);
