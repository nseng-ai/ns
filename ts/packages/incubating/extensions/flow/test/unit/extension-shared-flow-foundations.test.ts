...
const WORKTREE_SUPPORT_PATH = join(
	REPO_ROOT,
	"ts/packages/incubating/extensions/flow/src/ns/worktree.ts",
);
const PUSH_COMMAND_PATH = join(
	REPO_ROOT,
	"ts/packages/incubating/extensions/flow/src/ns/commands/push.ts",
);
...
const AUTOBRANCH_COMMAND_PATH = join(
	REPO_ROOT,
	"ts/packages/incubating/extensions/flow/src/ns/commands/autobranch.ts",
);
const BRANCH_LATEST_COMMIT_COMMAND_PATH = join(
	REPO_ROOT,
	"ts/packages/incubating/extensions/flow/src/ns/commands/branch-latest-commit.ts",
);
const AUTOSLOT_COMMAND_PATH = join(
	REPO_ROOT,
	"ts/packages/incubating/extensions/flow/src/ns/commands/autoslot.ts",
);
const LAND_COMMAND_PATH = join(
	REPO_ROOT,
	"ts/packages/incubating/extensions/flow/src/ns/commands/land.ts",
);
const PULL_TRUNK_COMMAND_PATH = join(
	REPO_ROOT,
	"ts/packages/incubating/extensions/flow/src/ns/commands/pull-trunk.ts",
);
const FLOW_PACKAGE_PATH = join(REPO_ROOT, "ts/packages/incubating/extensions/flow/package.json");
...
const REMOVED_LOCAL_AUTOBRANCH_HELPERS = [
	["ts/packages/incubating/extensions/flow/src/autobranch", "branch-availability.ts"],
	["ts/packages/incubating/extensions/flow/src/autobranch", "branch-slugs.ts"],
	["ts/packages/incubating/extensions/flow/src/autobranch", "branch-slug-text.ts"],
	[
		"ts/packages/incubating/extensions/flow/src/autobranch",
		["latest", "commit", "autobranch.ts"].join("-"),
	],
] as const;
...
describe("project extension shared flow foundations", () => {
	...
	test("flow CLI commands own their orchestration instead of importing the retired package", async () => {
		...
		expect(JSON.parse(flowPackage)).toMatchObject({
