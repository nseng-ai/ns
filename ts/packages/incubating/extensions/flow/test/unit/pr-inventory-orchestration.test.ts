...
describe("PR metadata replacement", () => {
	test.each([
		["submit", { type: "builtin" }, "built-in flow.submit.pr-inventory"],
		[
			"generate-pr-inventory",
			{ type: "repo", path: "/private/repo/.ns/prompt.md" },
			"repository flow.submit.pr-inventory",
		],
		[
			"generate-pr-inventory",
			{ type: "env", path: "/secret/custom.md" },
			"environment override flow.submit.pr-inventory",
		],
	] as const)(
	...
	test("keeps standardized provenance at the bottom of prompt-generated content", async () => {
		...
		const textGeneration = new ScriptedTextGenerator([
			{
				...
				text: [
					"Generated title",
					"",
					"Custom prompt-owned disclosure",
					"",
					"Generated summary",
					"",
					"<details>",
