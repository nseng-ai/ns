...
export const objectiveCommandSpecs: ObjectiveCommandSpec[] = [
	...
	defineObjectiveCommandSpec({
		...
		fallbackPrompt:
			"The objective-autorun skill was not found among loaded Pi skills. Follow the repository's Objective Runner workflow anyway for the explicit Objective below: drive it through repeated decomposed runner steps (ns objective exec runner-begin, one dispatched subagent, ns objective exec runner-finish), reading each Runner Checkpoint and making an explicit continue, recover, or stop decision between steps. Implementation children inherit the parent provider, model, and thinking policy by default; only the named provider-local cheap route may be selected before dispatch, and failure never authorizes reactive rerouting. Route tracking through objective-update between steps only. Every implementation child and Runner step is local-only and external-write-forbidden. Parent publication is off by default; use it only through an implemented parent-only publisher after a committed checkpoint and tracking judgment, when durable Runner Policy plus exact human-confirmed Objective/branch/existing-PR/head binding authorize this invocation. Never substitute raw push or PR mutation, and never create a PR, submit/restack, force-push, merge, land, or deploy.",
		actionPrompt:
