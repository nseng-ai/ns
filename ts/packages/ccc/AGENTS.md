...
CCC owns orchestration that is often invoked from both SDL CLI commands and Pi slash-command mirrors. When adding or fixing human-facing progress for those workflows, prefer the existing `CommandIo` abstraction from `@sdl/core/command-io` instead of inventing a command-local progress sink or relying only on Pi status.

Use this pattern when a workflow can run through `sdl ...` and `/sdl:...` or another Pi mirror:

- Thread a `CommandIo` (usually named `progressIo` or `io`) through lower orchestration code that owns the long-running phases.
...
- In Pi-rendered flows that already use `pi.sendMessage(...)` custom messages, do not duplicate the same progress through `CommandIo`; use `CommandIo` as the fallback when no renderer/live message path exists.
...
- `src/autoslot.ts` for a compact CLI `CommandIo` adapter.
...
See also `docs/pi/extension-command-checklist.md` for Pi-only progress helpers and the boundary between `CommandIo`, status, and transcript messages.
