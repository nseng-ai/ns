...
import {
	...
	stripLineEnding,
} from "@asdl/core/markdown-frontmatter";
...
describe("markdown frontmatter bounds", () => {
	...
	test.each(["", "\n---\n---\n", " ---\n---\n", "--- \n---\n", "# Body\n---\n"])(
		"returns not_found when opening fence is not exact first line: %j",
		(source) => {
	...
	test("returns missing_closing_fence when exact opening has no exact later close", () => {
		expect(splitMarkdownFrontmatter("---\ntitle: Demo\n--- \nBody\n")).toEqual({
			type: "missing_closing_fence",
		});
	});

	test.each(["---\ntitle: Demo\n ---\n", "---\ntitle: Demo\n----\n", "---\nprose --- here\n"])(
		"does not treat near fences as closing fences: %j",
		(source) => {
