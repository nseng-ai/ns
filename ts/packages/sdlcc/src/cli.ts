...
import { startAppShell } from "./app-renderer.ts";
...
export function buildCli(): ClinkrGroup<SdlccCliContext> {
	...
	root.command(rawCommand({ name: "stack-map", description: "Open the dashboard shell focused on Stack Map.", summary: "Open sdlcc focused on Stack Map.", schema: z.object({}), run: async (ctx) => { await ctx.startStackMap(); return 0; } }));
...
export async function runSdlccCli(args: readonly string[], deps: SdlccCliDeps = {}): Promise<number> {
	...
	const startApp = deps.startApp ?? deps.startTui ?? (() => startAppShell({ initialTabId: "dashboard", cwd, env, runCommand }));
	const startStackMap = deps.startStackMap ?? (() => startAppShell({ initialTabId: "stack-map", cwd, env, runCommand }));
