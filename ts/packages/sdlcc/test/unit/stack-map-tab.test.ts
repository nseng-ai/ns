...
describe("stackMapTabModule.interpretKey mapping", () => {
	test("maps movement keys to action intents", () => {
		expect(stackMapTabModule.interpretKey(rowsState(), { name: "down" })).toEqual({ type: "action", action: { type: "move-selection", delta: 1 } });
		expect(stackMapTabModule.interpretKey(rowsState(), { name: "up" })).toEqual({ type: "action", action: { type: "move-selection", delta: -1 } });
	});

	test("maps q and escape to quit", () => {
		expect(stackMapTabModule.interpretKey(rowsState(), { name: "q" })).toEqual({ type: "quit" });
		expect(stackMapTabModule.interpretKey(rowsState(), { name: "escape" })).toEqual({ type: "quit" });
	});

	test("maps the cmux key to an activate-cmux effect", () => {
		expect(stackMapTabModule.interpretKey(rowsState(), { name: "c" })).toEqual({ type: "effect", effect: { type: "activate-cmux" } });
	});

	test("maps chooser confirmation to an activate-choice effect", () => {
		const chooserState: StackMapState = {
			...rowsState(),
			mode: { type: "cmux-choice", branch: "feature/current", choices: [{ type: "open-new", branch: "feature/current" }], selectedIndex: 0 },
		};
		expect(stackMapTabModule.interpretKey(chooserState, { name: "enter" })).toEqual({ type: "effect", effect: { type: "activate-choice" } });
	});
