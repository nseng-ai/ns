...
describe("objectiveTabModule.loadModel", () => {
	test("loads and parses a valid objective list envelope", async () => {
		...
		const runCommand = async (command: string, args: readonly string[], options: StackMapCommandOptions): Promise<StackMapCommandOutput> => {
		...
		expect(calls).toEqual([
			{ command: "objective", args: ["list", "--minimal", "--format", "json"], options: { cwd: "/repo", timeoutMs: 10_000 } },
		]);
	});

	test("throws when the objective CLI exits non-zero", async () => {
		const runCommand = async (): Promise<StackMapCommandOutput> => ({ code: 3, stdout: "", stderr: "not in a repo" });
	...
	test("throws when the envelope payload is invalid", async () => {
		const runCommand = async (): Promise<StackMapCommandOutput> => ({ code: 0, stdout: envelope({ trunk_branch: "master" }), stderr: "" });
...
describe("objectiveTabModule.render", () => {
	test("renders a cursor on the selected row and a detail line", () => {
		const lines = objectiveTabModule.render(LIST, { selectedSlug: "beta" }, { width: 80, height: 24 });
	...
	test("renders an empty-state message when there are no records", () => {
		const lines = objectiveTabModule.render({ ...LIST, records: [] }, { selectedSlug: undefined }, { width: 80, height: 24 });
