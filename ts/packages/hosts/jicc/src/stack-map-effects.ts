...
const JICC_CLI_ENTRYPOINT_PATH = fileURLToPath(new URL("./cli.ts", import.meta.url));
...
export function createStackMapCmuxActivationExecutor(
	...
	return {
		...
		async openNew(branch, slot) {
			...
			const description = `jicc cmux workspace for ${target.branchName}`;
...
export function buildJiccCmuxReportBootstrapCommand(
	cliEntrypointPath: string = JICC_CLI_ENTRYPOINT_PATH,
): string {
