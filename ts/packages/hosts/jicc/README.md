...
`jicc` opens a full-screen OpenTUI stack map for this repository. It shows the current Graphite branch graph, slot assignments, and strong cmux tab matches in one branch-oriented surface.
...
```bash
bun ts/packages/hosts/jicc/src/cli.ts
```

Package-local run:

```bash
pnpm --dir ts --filter jicc run stack-map
```
...
Use `jicc cmux report --json` for machine-readable success/failure output.
