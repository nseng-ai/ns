...
- neutral helper subpaths exported as `@nseng-ai/pi/...` for other workspace packages such as the Herdr capability, Objective, branch-context, and autobranch;
...
`@nseng-ai/pi/shared/*` exports are curated neutral infrastructure helpers for Pi-hosted code and extracted Internal Pi-tool packages. A Internal Pi-tool package may import helpers such as `@nseng-ai/pi/shared/command-exec` or `@nseng-ai/pi/shared/gh-command` when the helper is host/runtime infrastructure. Do not invert the dependency by making `@nseng-ai/pi` import Internal Pi-tool packages, and do not move tool-specific PR feedback/watch/preview domain behavior into `@nseng-ai/pi/shared/*` just to deduplicate it.
...
- `ts/packages/internal/ns-pi-subagents/src/runner-subagents/curated-context.ts` uses `CuratedContextExecGit` for git evidence.
- `ts/packages/internal/ns-pi-subagents/src/runner-subagents/subagent-process.ts` is the async-spawn adapter seam for runner subagents; module logic depends on injected process functions.
...
- The injected host `ctx.exec` author-facing result shape lives in `@nseng-ai/sdk`. The command-execution gateway lives in `@nseng-ai/foundation/exec`: pure command types/helpers are re-exported from `@nseng-ai/foundation/command`, and the real Node child-process adapter (`runCommand`, `NodeCommandExecApi`) lives in `@nseng-ai/foundation/exec` itself.
...
- The helper implementation default is `none`, but repo-owned registrations must choose explicit acknowledgement delivery. Use `{ delivery: "message" }` normally for an above-fold dim transcript message; use `{ delivery: "status" }` only when transcript output is inappropriate and document that reason at the registration site.
...
- Never omit `options.delivery` for a repo-owned registration; the helper's `none` default is an implementation fallback, not repository policy.
