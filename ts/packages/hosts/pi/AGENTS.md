...
- The injected host `ctx.exec` author-facing result shape lives in `@nseng-ai/sdk/sdk`. The command-execution gateway lives in `@nseng-ai/foundation/exec`: pure command types/helpers are re-exported from `@nseng-ai/foundation/command`, and the real Node child-process adapter (`runCommand`, `NodeCommandExecApi`) lives in `@nseng-ai/foundation/exec` itself.
