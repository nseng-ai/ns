...
- After implementation and validation, close all confirmed automation review threads stack-wide with `sdl address exec close-review-threads --thread-ids-json '{"threadIds":["<THREAD_ID>"]}' --format json` unless the user instructs otherwise.
- Use `sdl address exec close-review-threads --thread-ids-json '{"threadIds":["<THREAD_ID>"]}' --body <BODY> --format json` for bulk review-thread replies after validation; single-thread `reply-review-thread` and `resolve-review-thread` remain available for one-offs.
