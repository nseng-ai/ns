...
**Herdr Pi subpackage**:
The `@nseng-ai/herdr/pi` subpackage presents Herdr's resource-first space and tab operations plus `/ns:herdr:impl:prompt:space`, `/ns:herdr:impl:plan:space`, and `/ns:herdr:impl:plan:tab`. Each `impl` command starts a responsible implementation attempt: inspect the input and repository, implement a valid request, or stop with a concrete blocker or clarification. Prepared Herdr Launch and Pi launch remain destination/process startup mechanics beneath that semantic contract; `ns-launch` remains prompt transport/storage, and Handoff launch remains specific to the durable Handoff integration.
