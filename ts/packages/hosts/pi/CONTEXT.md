...
`@ji/pi` is the unified private TypeScript workspace package for this repository's Pi runtime integration. It contains neutral Pi helper subpaths consumed by other workspace packages and the remaining host-resident project-local Pi extension implementations used by `.pi/extensions/*.ts` discovery adapters. Pi presentation for domain capabilities may instead live in capability `pi` subpackages or capability-pi packages stacked above `@ji/pi`; Pi-native standalone tools may live in Local Pi-tool packages. Those packages consume neutral host helpers while their discovery adapters import the owning package directly. CCC (`@ji/ccc`) remains the separate orchestration layer for repo-opinionated command-and-control workflows; the `@ji/ccc/pi` subpackage imports CCC core APIs and neutral `@ji/pi/...` helpers so there are no direct `@ji/ccc` imports from `@ji/pi/...` and no `@ji/pi` import or declaration of `@ji/ccc`.

## Language

**Unified Pi package**:
The private workspace package at `ts/packages/hosts/pi/` named `@ji/pi`. It replaces the former split between Pi command constants, neutral runtime helpers, and engineered project-local extension modules.
...
**CCC Pi subpackage**:
The `@ji/ccc/pi` subpackage that wires CCC workflows into Pi/cmux presentation by importing CCC core APIs and neutral `@ji/pi/...` helper subpaths. It is the home for CCC-specific Pi command registration, acknowledgement/progress wiring, prompt/session formatting, machine-envelope parsing, and slash-command formatting; `@ji/pi` itself still must not import or declare `@ji/ccc`.
...
The Pi-owned slash-command presentation for Branch Context workflows, including `/ji:branch-context:from-plan`, `/ji:branch-context:upstack-impl-from-plan`, `/ji:branch-context:impl-attached-plan`, and formatting an implementation launch command as `/ji:branch-context:impl-attached-plan <attached-key>` for Pi sessions or CCC Pi launch commands. Branch Context domain/API behavior stays in `@ji/branch-context/api`; saved-plan selection behavior stays in `@ji/plans/api`.
...
**Runner subagent**:
A fresh Pi subprocess launched by a parent extension with an isolated conversation and explicit return mode. The model-visible dispatch tool and its shared runtime, process, JSON-event, and presentation helpers live in `@internal/pi-tools/runner-subagents`; the package still consumes neutral `@ji/pi/...` runtime/tool helper subpaths where it needs Pi host types or agent-definition loading.
*Avoid*: queued slash command, child thread, transcript scrape, forcing `@ji/pi` to import the extracted dispatch package.

**Terminal helper surface**:
The neutral `@ji/pi/terminal/*` layout and presentation subpaths owned by the Pi host and intentionally consumed by extracted Pi-tool packages and orchestration packages. Keep this surface in `@ji/pi` unless a future extraction proves a smaller acyclic home without broad churn.
