...
**Unified Pi package**:
The private workspace package at `ts/packages/hosts/pi/` named `@ns/pi`. It replaces the former split between Pi command constants, neutral runtime helpers, and engineered project-local extension modules.
...
The Pi-owned slash-command presentation for Branch Context workflows, including `/ns:branch-context:from-plan`, `/ns:branch-context:upstack-impl-from-plan`, `/ns:branch-context:impl-attached-plan`, and formatting an implementation launch command as `/ns:branch-context:impl-attached-plan <attached-key>` for Pi sessions or CCC Pi launch commands. Branch Context domain/API behavior stays in `@ns/branch-context/api`; saved-plan selection behavior stays in `@ns/plans/api`.
...
The command-registration requirement that repo-owned Pi slash commands acknowledge receipt synchronously before waiting for idle state or starting slow work. Use `@ns/pi/commands/ack` helpers rather than hand-writing acknowledgement behavior.
...
**Runner subagent**:
A fresh Pi subprocess launched by a parent extension with an isolated conversation and explicit return mode. The model-visible dispatch tool and its shared runtime, process, JSON-event, and presentation helpers live in `@internal/pi-tools/runner-subagents`; the package still consumes neutral `@ns/pi/...` runtime/tool helper subpaths where it needs Pi host types or agent-definition loading.
*Avoid*: queued slash command, child thread, transcript scrape, forcing `@ns/pi` to import the extracted dispatch package.

**Terminal helper surface**:
The neutral `@ns/pi/terminal/*` layout and presentation subpaths owned by the Pi host and intentionally consumed by extracted Pi-tool packages and orchestration packages. Keep this surface in `@ns/pi` unless a future extraction proves a smaller acyclic home without broad churn.
