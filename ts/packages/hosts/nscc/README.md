...
`nscc` opens a full-screen OpenTUI stack map for this repository. It shows the current Graphite branch graph, slot assignments, and strong cmux tab matches in one branch-oriented surface.
...
That command owns Graphite metadata-store parsing on the Python side and returns selected branch rows, graph edges, assigned slot rows, and warnings. `nscc` separately reads cmux tab inventory with `cmux tree --json --all` and overlays tabs only when there is strong branch evidence.
...
```bash
bun ts/packages/hosts/nscc/src/cli.ts
```

Package-local run:

```bash
pnpm --dir ts --filter nscc run stack-map
```
...
- zero strong cmux tab matches: run `ns slot checkout <branch> --format json --no-clipboard` if needed, then `cmux new-workspace --name <branch> --description <text> --cwd <worktreePath> --command "bun <nscc source cli.ts> cmux report || true; exec ${SHELL:-/bin/zsh} -l"`;
...
Use `nscc cmux report --json` for machine-readable success/failure output.
