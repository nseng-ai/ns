# OpenTUI reference docs

<!--
Lineage: relocated from the vendored `opentui` skill (upstream `msmps/opentui-skill`,
lock skillPath `skill/opentui/SKILL.md`). Kept as nscc package docs; no upstream
sync obligation. See docs/agents/matt-pocock-skills.md for the upstream-melding
registry conventions.
-->

These are package-local reference docs for building nscc's terminal UI with
OpenTUI. They were vendored from `msmps/opentui-skill` and are maintained here
as ordinary package documentation. Reference paths below resolve relative to
this directory (e.g. `core/gotchas.md` lives at `references/core/gotchas.md`).
Consolidated reference for building terminal user interfaces with OpenTUI. Use decision trees below to find the right framework and components, then load detailed references.
