...
describe("buildStackMapModelFromGraph", () => {
	test("builds topology from sanctioned graph rows, slots, and needs_restack facts", () => {
		...
		expect(model.title).toBe("nscc stack map");
...
describe("loadStackMapModel", () => {
	test("queries sanctioned graph and cmux inventory in parallel", async () => {
		...
		expect(calls.sort()).toEqual([
			...
			"/repo$ ns slot gt exec stack-map-branches --format json",
		]);
		expect(model.trunk.children.map((branch) => branch.name)).toEqual([
			"feature/a",
			"feature/recent",
	...
	test("returns unavailable stack map when graph data fails boundary validation", async () => {
		...
		expect(model.diagnostics).toContain(
			"ns slot gt stack-map data was missing branches/trunk/current/edges/slots/warnings.",
		);
	});
});
...
describe("createStackMapCmuxActivationExecutor", () => {
	...
	test("opens a new workspace after peer API checkout when slot has no worktree path", async () => {
		...
		expect(calls.join("\n")).not.toContain("ns slot checkout");
	});
	...
	test("reports peer API checkout failures", async () => {
		...
		await expect(executor.openNew("feature/current")).resolves.toEqual({
			type: "failed",
			message: "ns slot checkout failed (no-available-slot): No free slots.",
		});
		expect(calls).toEqual([]);
	});
	...
	test("opens a new workspace in an existing slot worktree", async () => {
		...
		expect(calls).toHaveLength(1);
		expect(calls[0]).toContain(
			"/repo/slot-04$ cmux new-workspace --name feature/current --description nscc cmux workspace for feature/current --cwd /repo/slot-04 --command bun '",
		);
