...
The package does not expose or install a top-level `slot` executable. First-party users that need in-process access should use curated exports such as `@sdl/slot/api` rather than parsing command output.
...
- `sdl slot checkout -b NEW [BASE]` creates and checks out a new branch.
...
Machine-readable calls such as `--format json` and `--json-schema` do not write cd directives, so they do not move the parent shell. `--no-clipboard` skips clipboard writes only; it does not disable an active parent-shell `cd`.
