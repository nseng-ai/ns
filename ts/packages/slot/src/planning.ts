...
export type CurrentCheckoutPlan =
	| {
			...
			plan: CheckoutPlan;
			branchName: string;
			redirect: CurrentWorktreeRedirect | null;
...
export async function planCheckout(
	...
	if (match?.kind === "main")
		return { type: "branch_in_main_worktree", mainPath: match.worktree.path };
	...
	if (target === null)
		return {
			type: "pool_full",
			assigned: inventory.records.filter((record) => record.branch !== null),
		};
...
export async function planCurrentWtRedirect(
	...
	if (
		previous !== null &&
		previous !== options.movingBranch &&
		(await git.branchExists(previous))
	) {
		const conflict = worktrees.find(
			(worktree) => worktree.branch === previous && worktree.path !== options.cwd,
		);
...
export function inventoryWithoutCallerBranchOccupancy(
	inventory: SlotInventory,
	options: { cwd: string; movingBranch: string },
): SlotInventory {
	return {
		records: inventory.records.map((record) =>
			record.path === options.cwd && record.branch === options.movingBranch
				? { ...record, branch: null, operation: null }
				: record,
		),
		mainWorktree:
			inventory.mainWorktree?.path === options.cwd &&
			inventory.mainWorktree.branch === options.movingBranch
				? { ...inventory.mainWorktree, branch: null }
				: inventory.mainWorktree,
		branchOccupancies: inventory.branchOccupancies.filter(
			(occupancy) => !(occupancy.path === options.cwd && occupancy.branch === options.movingBranch),
		),
	};
}
...
export async function planCurrentCheckout(
	...
	if (currentBranch.type === "failure")
		return {
			...
			message: `Failed to determine current branch at ${options.cwd}: ${currentBranch.failure.message}`,
		};
	...
	const redirect = await planCurrentWtRedirect(git, {
		cwd: options.cwd,
		movingBranch: currentBranch.branch,
	});
	const adjustedInventory = inventoryWithoutCallerBranchOccupancy(inventory, {
		cwd: options.cwd,
		movingBranch: currentBranch.branch,
	});
	return {
