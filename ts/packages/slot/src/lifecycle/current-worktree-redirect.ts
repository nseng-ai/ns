...
export async function executeCurrentWorktreeRedirect(redirect: CurrentWorktreeRedirect, ctx: RepoSlotContext): Promise<SlotLifecycleFailure | null> {
	...
	return { error_type: "slot_allocation_error", message: `Failed to detach current worktree at ${ctx.repo.root} to ${action.ref}: ${failure.message}` };
}
