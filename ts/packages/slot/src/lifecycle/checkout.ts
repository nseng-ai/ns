...
export async function checkoutBranch(ctx: SlotCliContext, branchName: string, options: { shouldCreateBranch: boolean; base: string | null }): Promise<SlotCheckoutResult> {
	...
	return await executeCheckoutPlan(plan, ctx, { branchName, hasCreatedBranch, currentWtNote: null });
}
...
export async function checkoutCurrent(ctx: SlotCliContext): Promise<SlotCheckoutResult> {
	...
	return await executeCheckoutPlan(currentPlan.plan, ctx, { branchName: currentPlan.branchName, hasCreatedBranch: false, currentWtNote: currentPlan.currentWtNote });
}

async function executeCheckoutPlan(plan: ExecutableCheckoutPlan, ctx: SlotCliContext, options: { branchName: string; hasCreatedBranch: boolean; currentWtNote: string | null }): Promise<SlotCheckoutResult> {
