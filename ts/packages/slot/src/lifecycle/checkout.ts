...
export async function checkoutBranch(ctx: SlotCliContext, branchName: string, options: { shouldCreateBranch: boolean; base: string | null }): Promise<SlotCheckoutResult> {
	...
	if (options.shouldCreateBranch) {
		...
		if (options.base !== null && !(await repoCtx.git.branchExists(options.base))) return failure("base_missing", `Base branch '${options.base}' does not exist.`);
		const createFailure = await repoCtx.git.createBranch(branchName, options.base ?? "HEAD", { shouldForce: false });
	...
	return await executeCheckoutPlan(plan, repoCtx, { branchName, hasCreatedBranch, currentWtNote: null });
}
...
export async function checkoutCurrent(ctx: SlotCliContext): Promise<SlotCheckoutResult> {
	...
	const currentPlan = await planCurrentCheckout(repoCtx.git, { cwd: repoCtx.repo.root, mainRepoRoot: repoCtx.repo.mainRepoRoot });
	...
	return await executeCheckoutPlan(currentPlan.plan, repoCtx, { branchName: currentPlan.branchName, hasCreatedBranch: false, currentWtNote: currentPlan.currentWtNote });
}

async function executeCheckoutPlan(plan: ExecutableCheckoutPlan, ctx: RepoSlotContext, options: { branchName: string; hasCreatedBranch: boolean; currentWtNote: string | null }): Promise<SlotCheckoutResult> {
	if (plan.type === "reuse_assignment") {
		return ok({ slot_name: plan.record.slotName, branch_name: options.branchName, worktree_path: plan.record.path, already_assigned: true, created_branch: options.hasCreatedBranch, current_wt_note: options.currentWtNote });
	}
	if (plan.type === "branch_in_main_worktree") {
		return ok({ slot_name: "", branch_name: options.branchName, worktree_path: plan.mainPath, already_assigned: true, created_branch: options.hasCreatedBranch, current_wt_note: options.currentWtNote });
	}
	...
	return ok({ slot_name: plan.record.slotName, branch_name: options.branchName, worktree_path: plan.record.path, already_assigned: false, created_branch: options.hasCreatedBranch, current_wt_note: options.currentWtNote });
}
