...
This context captures Slot language for the Git-worktree-backed slot pool and the boundary between Slot command usage, in-process Peer API composition, SDL-owned shell integration, and Graphite-aware slot helpers.
...
*Avoid*: standalone `slot` executable, top-level slot shim, Peer API, private source import

**Slot Peer API**:
The curated `@sdl/slot/api` surface for sibling in-process consumers that need Slot behavior without invoking the CLI or importing private modules.
...
The rule that **Slot Peer API** checkout side effects are explicit opt-ins, while safe in-process defaults do not copy to the clipboard or write parent-shell navigation directives.
...
*Avoid*: child process `cd`, Peer API navigation, standalone Slot shell wrapper
...
*Avoid*: Graphite support package, Slot Peer API by default, CCC landing policy

**Slot Graphite Exec Helper**:
A hidden command-face helper under `sdl slot gt exec ...` that emits structured Graphite/Slot facts for skills or agents while staying outside the **Slot Peer API** until an in-process consumer proves the need.
*Avoid*: public human command, Peer API promotion, parsing `gt` display output
