...
describe("RealSlotGitGateway movement methods", () => {
	it("checks branch presence, creates branches, checks out, and detaches in a throwaway repo", async () => {
		...
		expect(await gateway.createBranch("feature/delete-me", "HEAD", { shouldForce: false })).toBeNull();
		expect(await gateway.deleteLocalBranch("feature/delete-me", { shouldForce: true })).toBeNull();
		expect(await gateway.branchExists("feature/delete-me")).toBe(false);
