...
describe("RealSlotGtGateway stack metadata adapter", () => {
	...
	fixtureIt("reports the current branch as untracked when no metadata row exists", async () => {
		await withMetadataDb(
			{ currentBranch: "feature/missing", rows: [trunk("master")] },
			async ({ gateway }) => {
				await expect(gateway.stack("/repo")).resolves.toMatchObject({
	...
	fixtureIt("reports a missing metadata database", async () => {
		await withMetadataDb(
			{ currentBranch: "feature/a", rows: [trunk("master")], shouldSkipDb: true },
			async ({ gateway, commonDir }) => {
				await expect(gateway.stack("/repo")).resolves.toMatchObject({
					type: "failure",
					failure: {
						message: `Graphite metadata store not found at ${join(commonDir, ".graphite_metadata.db")}`,
						returnCode: null,
					},
				});
			},
	...
	fixtureIt("ignores empty branch-name rows while preserving tracked branches", async () => {
		await withMetadataDb(
			{ currentBranch: "master", rows: [trunk("master"), row("")] },
			async ({ gateway }) => {
				await expect(gateway.stack("/repo")).resolves.toMatchObject({
					type: "stack",
		...
		await withMetadataDb(
			{
				...
				rows: [
					trunk("master", { children: '["feature/a"]' }),
					row("feature/a", { parent: "master", children }),
				],
			},
			async ({ gateway }) => {
				await expect(gateway.stack("/repo")).resolves.toMatchObject({
		...
		await withMetadataDb(
			{
				...
				rows: [
					trunk("master"),
					row("feature/a", { parent: "feature/b" }),
					row("feature/b", { parent: "feature/a" }),
				],
			},
			async ({ gateway }) => {
	...
	fixtureIt("detects missing ancestor rows", async () => {
		await withMetadataDb(
			{
				currentBranch: "feature/a",
				rows: [trunk("master"), row("feature/a", { parent: "feature/missing" })],
			},
			async ({ gateway }) => {
				await expect(gateway.stack("/repo")).resolves.toMatchObject({
					type: "stack",
		...
		await withMetadataDb(
			{
				...
				rows: [
					trunk("master", { children: '["feature/a"]' }),
					row("feature/a", { parent: "master", children: '["feature/b"]' }),
					row("feature/b", { parent: "feature/a", children: '["feature/a"]' }),
				],
			},
			async ({ gateway }) => {
				await expect(gateway.stack("/repo")).resolves.toMatchObject({
		...
		await withMetadataDb(
			{
				...
				rows: [
					trunk("master", { children: '["feature/a"]' }),
					row("feature/a", { parent: "master", children: '["feature/missing"]' }),
				],
			},
			async ({ gateway }) => {
				await expect(gateway.stack("/repo")).resolves.toMatchObject({
		...
		await withMetadataDb(
			{
				...
				rows: [
					trunk("master", { children: '["feature/a"]' }),
					row("feature/a", { parent: "master", children: '["feature/b", "feature/c"]' }),
					row("feature/b", { parent: "feature/a" }),
					row("feature/c", { parent: "feature/a" }),
				],
			},
			async ({ gateway }) => {
				await expect(gateway.stack("/repo")).resolves.toMatchObject({
	...
	fixtureIt.each([
		[
			"row-missing",
			[row("feature/a", { parent: "master" })],
			{ terminusState: "row_missing", terminus: "master", markedTrunks: [] },
		],
		[
			"unmarked",
			[row("master"), row("feature/a", { parent: "master" })],
			{ terminusState: "unmarked", terminus: "master", markedTrunks: [] },
		],
		[
			"multiple",
			[trunk("master"), trunk("other"), row("feature/a", { parent: "master" })],
			{ terminusState: "marked", terminus: "master", markedTrunks: ["master", "other"] },
		],
		[
			"different",
			[row("master"), trunk("other"), row("feature/a", { parent: "master" })],
...
async function withRunner(
	result: ReturnType<SqliteJsonRunner["run"]>,
	run: (gateway: RealSlotGtGateway) => Promise<void>,
): Promise<void> {
	await withTempDir(async (commonDir) => {
