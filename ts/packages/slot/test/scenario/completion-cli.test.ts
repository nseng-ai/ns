...
describe("slot completion CLI", () => {
	...
	it("installs zsh completion into redirected HOME", async () => {
		...
		const run = runScenario(["completion", "install", "--shell", "zsh", "--format", "json"], {
			env: { PATH: "/fake/bin", HOME: home },
		});
		expect(await run.exit).toBe(0);
		expect(parseJsonOutput(run)).toMatchObject({
			data: { shell: "zsh", rc_path: join(home, ".zshrc"), is_already_installed: false },
		});
	...
	it("installs bash completion into redirected HOME", async () => {
		const home = await makeHome();
		const run = runScenario(["completion", "install", "--shell", "bash", "--format", "json"], {
			env: { PATH: "/fake/bin", HOME: home },
		});
		expect(await run.exit).toBe(0);
		expect(parseJsonOutput(run)).toMatchObject({
			data: { shell: "bash", rc_path: join(home, ".bashrc"), is_already_installed: false },
		});
		await expect(readFile(join(home, ".bashrc"), "utf8")).resolves.toContain(
	...
	it("is idempotent and does not duplicate the completion marker", async () => {
		const home = await makeHome();
		const first = runScenario(["completion", "install", "--shell", "zsh", "--format", "json"], {
			env: { PATH: "/fake/bin", HOME: home },
		});
		expect(await first.exit).toBe(0);
		const second = runScenario(["completion", "install", "--shell", "zsh", "--format", "json"], {
			env: { PATH: "/fake/bin", HOME: home },
		});
	...
	it("does not disturb an existing shell integration block", async () => {
		...
		const run = runScenario(["completion", "install", "--shell", "zsh"], {
			env: { PATH: "/fake/bin", HOME: home },
		});
		expect(await run.exit).toBe(0);
		const rc = await readFile(join(home, ".zshrc"), "utf8");
	...
	it("shell and completion marker blocks coexist in either order", async () => {
		...
		const shellFirst = runScenario(["shell", "install", "--shell", "zsh"], {
			env: { PATH: "/fake/bin", HOME: firstHome },
		});
		expect(await shellFirst.exit).toBe(0);
		const completionSecond = runScenario(["completion", "install", "--shell", "zsh"], {
			env: { PATH: "/fake/bin", HOME: firstHome },
		});
		expect(await completionSecond.exit).toBe(0);
		const shellFirstRc = await readFile(join(firstHome, ".zshrc"), "utf8");
		expect(shellFirstRc.indexOf(shellIntegrationBeginMarker)).toBeLessThan(
			shellFirstRc.indexOf(completionBeginMarker),
		);

		const secondHome = await makeHome();
		const completionFirst = runScenario(["completion", "install", "--shell", "zsh"], {
			env: { PATH: "/fake/bin", HOME: secondHome },
		});
		expect(await completionFirst.exit).toBe(0);
		const shellSecond = runScenario(["shell", "install", "--shell", "zsh"], {
			env: { PATH: "/fake/bin", HOME: secondHome },
		});
		expect(await shellSecond.exit).toBe(0);
		const completionFirstRc = await readFile(join(secondHome, ".zshrc"), "utf8");
		expect(completionFirstRc.indexOf(completionBeginMarker)).toBeLessThan(
			completionFirstRc.indexOf(shellIntegrationBeginMarker),
		);
	});
});
