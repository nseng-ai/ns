...
describe("slot shell CLI", () => {
	...
	it("installs the bash marker block into redirected HOME", async () => {
		const home = await makeHome();
		const run = runScenario(["shell", "install", "--shell", "bash", "--format", "json"], {
			env: { PATH: "/fake/bin", HOME: home },
		});
		expect(await run.exit).toBe(0);
		expect(parseJsonOutput(run)).toMatchObject({
	...
	it("adds a separating newline when existing rc content has no trailing newline", async () => {
		...
		const run = runScenario(["shell", "install", "--shell", "zsh"], {
			env: { PATH: "/fake/bin", HOME: home },
		});
		expect(await run.exit).toBe(0);
		await expect(readFile(join(home, ".zshrc"), "utf8")).resolves.toMatch(
