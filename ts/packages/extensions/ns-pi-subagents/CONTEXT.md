...
**Fleet**:
The session-local collection of logical tool runs and tasks, including progress, diagnostics, session evidence, and worktree observations exposed through `ns:agents:*` Pi UI. One Subagent Tool invocation creates one Fleet run with one task per requested position. Fleet names the session-tree view and is independent from Agent Type and Execution Architecture.
