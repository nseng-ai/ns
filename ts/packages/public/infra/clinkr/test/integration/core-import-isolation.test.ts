...
// ns house-style display now lives outside Clinkr in `@nseng-ai/ns-foundation/cli-theme`; Clinkr's only display-adjacent
...
import { fileForReport, literalSpecifierUsesOf, sourceFilesUnder } from "@nseng-ai/clinkr/testing";
...
const DISPLAY_SUBPATHS = ["@nseng-ai/ns-foundation/cli-theme", "@nseng-ai/clinkr/stream"];
...
function productionCliThemeImportOffenders(): readonly BoundaryOffender[] {
	...
	for (const file of sourceFilesUnder(SRC_DIR)) {
		const source = readFileSync(file, "utf8");
		for (const { specifier } of literalSpecifierUsesOf(source)) {
			if (!isPackageOrSubpathSpecifier(specifier, "@nseng-ai/ns-foundation/cli-theme")) continue;
			offenders.push({
				file: fileForReport(file),
				specifier,
...
describe("clinkr display import boundary", () => {
	test("literal scanner sees static imports, re-exports, side-effect imports, and dynamic imports", () => {
		const source = `
			...
			import "@nseng-ai/ns-foundation/cli-theme";
		...
		expect(literalSpecifierUsesOf(source)).toEqual([
			...
			{ specifier: "@nseng-ai/ns-foundation/cli-theme", kind: "static-import" },
