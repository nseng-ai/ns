...
describe("copy operation", () => {
			...
			entries: [
				{
					...
					content: "scratch\n",
					headSha: "base-head",
					blobSha: "blob-scratch",
				},
			],
		});
		const run = runScenario(
			["copy", "--base", "--from-branch", "master", "--to-branch", "feat/x"],
			{ gateway },
		);
		expect(await run.exit).toBe(0);
		const output = run.stdout.join("");
		expect(output).toContain(
			"Copied 1 Entry in Base Namespace from Branch master to Branch feat/x.",
		);
	...
	it("treats --namespace base as Base Namespace and still rejects an explicit scope conflict", async () => {
		const alias = runScenario(
			...
			],
			{
				fake: {
					entries: [
						{ namespace: "base", branch: "master", key: "a.md", content: "A", headSha: "head" },
					],
				},
			},
		);
	...
	it("validates scope and values with stable error types before gateway mutation", async () => {
		...
		expect(parseJsonOutput(missingFrom)).toMatchObject({
			...
			message: expect.stringContaining("fromBranch"),
		});
		...
		expect(parseJsonOutput(missingTo)).toMatchObject({
			...
			exitCode: 2,
			errorType: "invalid-request",
			message: expect.stringContaining("toBranch"),
		});
	});
	...
	it("supports overwrite for full-snapshot and key-glob Namespace Copies", async () => {
		...
		expect(
			await runScenario(["get", "source.md", "--namespace", "notes", "--branch", "feat/x"], {
				gateway: full,
			}).exit,
		).toBe(0);
		expect(
		...
		expect(
			await runScenario(["get", "foo/sub/x.md", "--namespace", "notes", "--branch", "feat/x"], {
				gateway: glob,
			}).exit,
		).toBe(0);
		expect(
	...
	it("keeps dry-run non-mutating while still checking conflicts", async () => {
		...
		const after = runScenario(["get", "source.md", "--namespace", "notes", "--branch", "feat/x"], {
			gateway,
		});
	...
	it("maps gateway failures and missing source SHA preflight to public failures", async () => {
		...
		const missingShaGateway = new MissingCheckGateway({
			entries: [{ namespace: "base", branch: "master", key: "source.md", content: "source\n" }],
		});
		const missingSha = runScenario(
			["copy", "--base", "--from-branch", "master", "--to-branch", "feat/x", "--format", "json"],
