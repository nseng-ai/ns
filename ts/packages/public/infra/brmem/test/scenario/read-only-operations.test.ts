...
describe("read-only brmem operations", () => {
	...
	it("propagates ANSI capability to the list table renderer", async () => {
		const color = runScenario(["list", "--branch", "feat/x"], {
			fake: { currentBranch: "feat/x", entries: seededEntries },
			canEmitAnsi: true,
		});
		expect(await color.exit).toBe(0);
		expect(color.stdout.join("")).toContain(String.fromCharCode(0x1b));

		const plain = runScenario(["list", "--branch", "feat/x"], {
			fake: { currentBranch: "feat/x", entries: seededEntries },
			canEmitAnsi: false,
		});
		expect(await plain.exit).toBe(0);
		expect(plain.stdout.join("")).not.toContain(String.fromCharCode(0x1b));
	});
});
