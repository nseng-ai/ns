...
const copyRequestSchema = z.object({
	namespace: cliOption(z.string().optional().describe("Namespace to copy."), {}),
	base: cliOption(z.boolean().default(false).describe("Copy Base Namespace."), {}),
	fromBranch: cliOption(z.string().describe("Source branch."), {}),
	toBranch: cliOption(z.string().describe("Destination branch."), {}),
	overwrite: cliOption(z.boolean().default(false).describe("Overwrite destination Entries."), {}),
	dryRun: cliOption(
		z
			.boolean()
			.default(false)
	...
	keyGlob: cliOption(z.string().optional().describe("Entry Key glob filter."), {}),
});
...
type CopyPlanItem = z.infer<typeof copyPlanItemSchema>;
type CopyResult = z.infer<typeof copyResultSchema>;

async function runCopy(ctx: BrmemCliContext, request: CopyRequest) {
	...
	if (sourceEntriesResult.type === "error") return gatewayFailure(sourceEntriesResult.error);
	...
	if (destinationEntriesResult.type === "error")
		return gatewayFailure(destinationEntriesResult.error);
	...
	if (copied.type === "error") {
		if (copied.error.code === "copy-conflict") {
			return negative(copied.error.message, { data: result });
		}
		return gatewayFailure(copied.error);
	}
	return ok(result);
}

function renderCopy(result: CopyResult): string {
...
type CopyPlanResult =
	...
	| { type: "failure"; failure: FailureOutcome };
