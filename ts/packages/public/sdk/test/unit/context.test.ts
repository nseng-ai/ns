...
describe("real ns command context", () => {
	...
	test("overlays per-command environment without dropping the host execution environment", async () => {
		const ctx = createRealNsCommandContext({
			cwd: process.cwd(),
			env: {},
			execEnv: { PRESERVED: "host", OVERRIDDEN: "host", REMOVED: "host" },
			textGenerator: {
				generateText: async () => ({ ok: false, error: "not used" }),
			},
		});
		...
		const result = await ctx.exec(
			...
			[
				"-e",
				"process.stdout.write(JSON.stringify({preserved: process.env.PRESERVED, overridden: process.env.OVERRIDDEN, added: process.env.ADDED, removed: process.env.REMOVED}))",
			],
			{ env: { OVERRIDDEN: "command", ADDED: "command", REMOVED: undefined } },
		);
