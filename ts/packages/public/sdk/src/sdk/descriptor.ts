...
export interface ExtensionDescriptor {
	readonly description: string;
	/** Absolute root of this extension's Clinkr filesystem command tree. */
...
const extensionActivationSchema: z.ZodType<ExtensionActivation> = z
	.strictObject({
		...
		consumerDirs: z.array(activationConsumerDirSchema).optional(),
	})
	.transform((activation) => optionalEntries(activation));
...
function isSingleLevelTwoMarkdownSection(instructions: string): boolean {
	...
	return (
		...
		remainingLines.every((line) => !line.startsWith("## "))
	);
}
...
export function validateExtensionDescriptor(
	...
	const field = issue?.path.join(".") || "default export";
	return {
		ok: false,
		message: `Invalid ${label}: ${field} ${issue?.message ?? "did not match the expected shape."}`,
	};
}
