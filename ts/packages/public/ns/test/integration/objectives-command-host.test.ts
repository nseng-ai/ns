...
describe("real ns host Objectives command loading", () => {
	...
	test.each(["human", "json", "md"] as const)("renders Objective list in %s format", (format) => {
		const run = runNsWithObjectiveImportLog(["objective", "list", "--format", format]);

		expect(run.status, run.stderr).toBe(0);
		if (format === "json") {
			expect(JSON.parse(run.stdout)).toMatchObject({ status: "success", exitCode: 0 });
	...
	test("publishes all four outcome statuses without running the selected handler", () => {
		...
		const statuses = JSON.stringify(JSON.parse(run.stdout)).match(
			/"const":"(success|negative|failure|usage-error)"/gu,
		);
	...
	test("a malformed unrelated filesystem mount warns without bricking Objectives", () => {
		...
		expect(run.stderr).toContain(
			'clinkr: source "project:@test/bad-objective-neighbor" failed to open scope <root>',
		);
		expect(run.stderr).toContain("clinkr: incomplete command pair at broken");
		expect(run.stderr).toContain(join(project, "bad-extension", "src", "ns", "commands", "broken"));
		expect(run.objectiveImplementations).toEqual(["cli/objective/check/command.ts"]);
	});
});
...
function createProjectWithMalformedUnrelatedMount(): string {
	...
	const brokenCommandDirectory = join(extensionDirectory, "src", "ns", "commands", "broken");
	mkdirSync(brokenCommandDirectory, { recursive: true });
	writeFileSync(
		join(brokenCommandDirectory, "metadata.ts"),
		`export function metadata() { return { description: "Missing its command module." }; }\n`,
	);
	writeFileSync(
