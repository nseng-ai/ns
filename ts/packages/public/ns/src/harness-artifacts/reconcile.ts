...
export interface RunHarnessArtifactReconcileRequest {
	...
	/**
	 * Seam over first-party catalog source-root discovery. Defaults to
	 * `resolveFirstPartyCatalogSourceRoot`, which returns `undefined` in a checkout-free
	 * install because the published package ships no first-party skill sources.
	 */
	resolveFirstPartySourceRoot?: () => string | undefined;
}
...
export async function runHarnessArtifactReconcile(
	...
	const desired = desiredHarnessArtifacts({
		...
		resolveFirstPartySourceRoot:
			request.resolveFirstPartySourceRoot ?? resolveFirstPartyCatalogSourceRoot,
					...
					deletionAuthority: extensionSelection.value.isTargeted
						...
						: {
								...
								preserveRemovedSources:
									...
									loadedDescriptors.diagnostics.length > 0 ||
									desired.isFirstPartyCatalogSourceUnavailable,
							},
				}),
...
interface DesiredHarnessArtifactPlan {
	readonly artifacts: readonly DesiredHarnessArtifact[];
	/**
	 * True when the first-party catalog was in scope but its source root could not be
	 * resolved. That is the permanent condition of a checkout-free install: the published
	 * `@nseng-ai/ns` tarball ships no first-party skill sources. It must not abort the
	 * reconcile — module-declared artifacts still reconcile — and it must not let already
	 * provisioned first-party manifest entries be classified as removed sources.
	 */
	readonly isFirstPartyCatalogSourceUnavailable: boolean;
}
...
}): DesiredHarnessArtifactPlan {
	const firstParty = firstPartyDesiredArtifacts(input);
	return {
		artifacts: [
			...firstParty.artifacts,
		...
		isFirstPartyCatalogSourceUnavailable: firstParty.isFirstPartyCatalogSourceUnavailable,
	};
}
...
function firstPartyDesiredArtifacts(input: {
	...
	if (!input.shouldIncludeFirstPartyArtifacts) {
		return { artifacts: [], isFirstPartyCatalogSourceUnavailable: false };
	}
	const firstPartySourceRoot = input.firstPartySourceRoot ?? input.resolveFirstPartySourceRoot();
	if (firstPartySourceRoot === undefined) {
		return { artifacts: [], isFirstPartyCatalogSourceUnavailable: true };
	}
	return {
		artifacts: listFirstPartySkillArtifacts().map((artifact) => ({
			artifact,
			sourceRoot: firstPartySourceRoot,
			sourceVersion: FIRST_PARTY_SKILL_CATALOG_SOURCE_VERSION,
		})),
		isFirstPartyCatalogSourceUnavailable: false,
	};
}
