...
- **User lifecycle** uses `RealUserArtifactActivationGateway`. Install, update, and uninstall reconcile bundled skills into the user roots of the `supported_harnesses` configured in the user `ns.toml`. This reconciliation is targeted to the package being administered, preserving other packages and first-party entries; changing `supported_harnesses` alone does not reconcile existing declarations. Identifiable uninstall keeps the declaration as deletion authority until prepared artifact removals apply successfully. User npm update discovers and preflights artifacts from separately staged candidate package bytes before canonical package promotion.
