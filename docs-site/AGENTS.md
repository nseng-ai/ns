...
This directory is the published documentation site for ns (nonslop engineering, `nseng.ai`). The repo's CLI cutover to `ns` has landed; site copy is ns-first and must not reintroduce the retired `ji`/`sdl` names.
