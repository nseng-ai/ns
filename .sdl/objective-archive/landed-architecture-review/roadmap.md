...
- [x] Assess and decide the cmux command-suite architecture seam.
      ...
      Evidence: the cmux suite is several workflow Modules rather than one coherent deep Module. The Python workspace-summary path already has a deep `CmuxGateway`/exec seam. Historical note: this slice consolidated the then-named `/cmux-slot:dispatch-plan` (current `/cmux:workspace:dispatch-plan`) onto the shared `cmux/slot.ts` and `cmux/worktree-description.ts` slot-launch Module, deleting duplicate checkout/open/worktree-description implementation from `slot-dispatch-plan.ts`. Validation: `bun run --cwd ts/packages/pi-extensions check`; `bun test ts/packages/pi-extensions/test/cmux.test.ts`.
