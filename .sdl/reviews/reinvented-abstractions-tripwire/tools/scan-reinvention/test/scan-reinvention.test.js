...
import { parseAddedLines } from "../src/git-diff.ts";
...
class FakeScannerIo {
    ...
    constructor(options) {
        ...
        this.added = new Map(Object.entries(options.added ?? {}).map(([file, lines]) => [file, new Set(lines)]));
    }
    ...
    async addedLines(diffBase, file) {
        ...
        return { ok: true, value: this.added.get(file) ?? new Set() };
    }
    async readFile(path) {
