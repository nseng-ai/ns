...
- Escalation path for duplicated command-author helpers: first copy/localize while one command proves the seam, then extract common helpers inside `.sdl/extensions/` when that keeps project-local authoring readable; only promote them to `@sdl/sdl/sdk` after multiple extensions have proven the API shape or a documented single-command necessity justifies making it public author API.
