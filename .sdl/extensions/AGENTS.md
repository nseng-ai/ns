...
- Escalation path for duplicated command-author helpers: first copy/localize while one command proves the seam, then extract extension-owned policy inside `.sdl/extensions/` when that keeps project-local authoring readable. Repeated need for SDL-owned behavior must be promoted into a deliberate `@sdl/sdl/sdk` facade before checked-in extensions import SDL implementation subpaths.
