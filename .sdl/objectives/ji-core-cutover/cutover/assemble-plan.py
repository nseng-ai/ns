#!/usr/bin/env python3
"""Assemble and validate cutover-plan.json for the sdl→ji core cutover.
...
engine work-list, anchor coverage, and skip-audit. Run: uv run python assemble-plan.py
"""
...
def post_mv(path: str) -> str:
    if path == "sdl.toml":
        return "ji.toml"
    if path == ".pi/extensions/sdl.ts":
        return ".pi/extensions/ji.ts"
    if path.startswith(".sdl/"):
        return ".ji/" + path[len(".sdl/"):]
    return path
...
def main() -> int:
    ...
    mv_only = {k for k in anchors.get("mvOnly", {}) if k not in (".sdl", "sdl.toml", "comment")}
    ...
    for cls, files in auto.items():
        ...
        rule_map = {
            ...
            "dottree": anchors["pathRules"].get(".sdl/extensions/**, .sdl/reviews/**, .sdl/prompts/**, .sdl/pi/**", ""),
    ...
    plan = {
        "comment": "Locked args content for refactor-swarm-workflow, chunked into sequential invocations. All paths are POST-MV (.ji/…, ji.toml, .pi/extensions/ji.ts). Invoke each chunk as Workflow({name:'refactor-swarm-workflow', args:{brief, simple, complex, invariants, repoRoot, model:'sonnet', complexModel:'sonnet'}}) — invariants ONLY on the final chunk (empty engine runs skip invariants).",
        "brief": brief,
        "model": "sonnet",
        "complexModel": "sonnet",
        ...
        "baselines": {
            "atSdlFileCount": int((LISTS / "baseline-atsdl-filecount.txt").read_text().strip()),
            "srcSdlOccurrences": int((LISTS / "baseline-srcsdl-occurrences.txt").read_text().strip()),
        },
        "skips": [
    ...
    print(f"OK cutover-plan.json: {n_simple} simple, {n_cs} changesets, {n_cohort} cohorts, "
          f"{len(plan['skips'])} skips, {len(invariants)} invariants, "
