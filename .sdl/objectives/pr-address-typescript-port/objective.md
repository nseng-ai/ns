...
- Run-from-source wrapper distribution for installed skill usage: the installed shim may require an asdl checkout with `ts/node_modules` and does not need a bundled checkout-free JavaScript artifact or npm registry publish for cutover.
...
- Public skill docs, wrapper behavior, README/developer docs, and distribution instructions point to TypeScript paths: local checkout execution plus the accepted run-from-source installed shim. Checkout-free bundling is not required for this Objective.
...
- Distribution: revised 2026-06-13 after explicit challenge — checkout-free bundled JavaScript is no longer required for this Objective. The accepted installed wrapper model is the existing run-from-source bash shim: `ts/packages/pr-address/scripts/pr-address-shim`, installed to `~/.local/bin/pr-address` by `just install-pr-address`, which bakes the installing checkout path into `@@ASDL_CANONICAL_CHECKOUT@@`. It runs `node <checkout>/ts/packages/pr-address/src/cli.ts`, preferring the enclosing worktree (so each worktree runs its own sources) and falling back to the baked canonical checkout. There is no `local|prod` mode env var, no `uvx`/Python wrapper path, no npm publish requirement, and no bundle gate.
...
- Python end-state: `packages/asdl-pr-address` is fully deleted in-repo in the endgame stack once parity, run-from-source wrapper evidence, and plugin retirement evidence exist. Rollback after deletion is the external frozen PyPI artifact `asdl-pr-address==0.1.1` via `uvx`, not in-repo code.
...
- Materialized 2026-06-09: the wrapper's prod mode pinned `asdl-pr-address==0.1.0`, which was never published — only `0.1.1` exists on PyPI — so installed/prod invocation was broken. Superseded 2026-06-13: the wrapper was reworked into the run-from-source shim, so the broken `0.1.0`/prod-mode `uvx` pin no longer exists in the wrapper. PyPI `0.1.1` survives only as the post-deletion Python rollback reference (objective end-state), unrelated to wrapper invocation. The remaining limitation is accepted: the shim requires an asdl checkout with `ts/node_modules` (`just ts-install`) and does not run checkout-free.
- Checkout-free distribution risk is accepted rather than solved in this Objective. Avoiding bundle machinery also avoids bundle-specific risks: build correctness, runtime requirements of the bundled artifact, skill-directory size, and staleness between releases.
...
- What documentation and tests are still needed to make the run-from-source shim contract obvious to skill consumers and future maintainers?
