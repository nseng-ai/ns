...
- `docs/cli-surface-conformance-audit.md` and ADR 0015 classify `roaster exec publish-findings` as a raw-command conformance exception; `docs/roaster-pierre-diffs.md` and ADR 0007 document the `@pierre/diffs` parser boundary; `CONTEXT-MAP.md` and `ts/packages/roaster/CONTEXT.md` document Roaster vocabulary and current package identity.
