...
- Implement the real Pi-subprocess adapter in the Pi-coupled edge (the `runner-subagents` region of `@sdl/local-pi-tools`), composed into the exec command's context by host wiring.
