...
- Outcome/result discriminator mapping is resolved for Flow: keep mapping Flow-local/command-local with no production code change; `@sdl/cli-theme` owns only generic finite result-block layout grammar.
- Success-with-warnings rendering is resolved: keep warning routing, severity, aggregation, action-first notification text, and exit policy command-local/CCC-local; do not add a `success-with-warnings` result kind or theme helper unless future repeated normal CLI surfaces prove a domain-free visual-list primitive.
- Still open: which of the remaining non-Slot consolidation candidates — caps-resolution helper placement, table rendering, and status-to-intent helpers — belong in the theme package versus command-local helpers or a different CLI presentation utility layer. Slot navigation is resolved as Slot-local presentation over `@sdl/cli-theme` primitives rather than a theme-package promotion.
