...
- Still open: which of the remaining non-Slot consolidation candidates — success-with-warnings rendering, caps-resolution helper placement, table rendering, and status-to-intent helpers — belong in the theme package versus command-local helpers or a different CLI presentation utility layer. Slot navigation is resolved as Slot-local presentation over `@sdl/cli-theme` primitives rather than a theme-package promotion.
