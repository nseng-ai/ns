...
- [x] Assess outcome/result discriminator to result-block mapping across Flow commands. Decision: keep outcome-to-kind mapping Flow-local/command-local with no production code change; `@sdl/cli-theme` already owns the generic `ResultBlockKind` layout grammar, while Flow keeps `AutobranchFlowOutcome`, git/Graphite transcript mining, refusal guardrail classification, warning emission, and `ok(...)`/`failed(...)` routing at the command edge.
- [x] Assess success-with-warnings rendering. Decision: keep warning routing, severity, aggregation, action-first notification text, and exit policy command-local/CCC-local; do not add a `success-with-warnings` result kind or theme helper now.
- [x] Assess caps-resolution duplication. Decision: keep Flow's host-aware resolver Flow-local for now; `@sdl/clinkr` already owns generic caps/process/host-extension primitives, and `@sdl/cli-theme` should remain a pure consumer of injected `Caps`.
...
- [x] Assess table rendering. Decision: keep `@sdl/core/text-table` and `@sdl/cli-theme` `renderTable` intentionally distinct; do not add a Markdown table composer until repeated domain-free Markdown boilerplate or a concrete lower terminal-text primitive need appears.
- [x] Assess status-to-intent mapping helpers. Decision: keep theme-owned closed visual grammars (`ResultBlockKind`, `PhaseState`) in `@sdl/cli-theme`, but keep domain status policy, routing, exit behavior, and command-specific status cells local.
