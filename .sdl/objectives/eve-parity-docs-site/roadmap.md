...
- [x] Build the marketing home page structure.
      The static home page now has the non-content eve-style structure this Objective wanted: hero, feature cards, file-tree preview, installer/code-command preview, CTA band, and page metadata/OG/Twitter basics. Final sdl positioning/tagline copy remains deliberately deferred and the page still avoids per-feature animated visuals. Evidence: `pnpm --dir docs-site run build`, `just docs-check`, and `just dprint-check` passed.
