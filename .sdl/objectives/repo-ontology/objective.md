...
- Map-vs-tree count drift (open): `CONTEXT-MAP.md`'s Inventory Baseline says "19 repo-local packages" while 23 `ts/packages/*` package.json files are tracked. The map's Planned section and Inventory Baseline also predate the four new packages. Mitigation: a focused map catch-up row should re-derive the exact count and the undecided-package list, not guess.
