...
- [x] Add `.pi/prompts/objective-stack-impl.md` with argument hint `[objective-slug]` and current-session-only parent-orchestrator instructions.
- [x] Ensure the prompt is brmem-free, does not depend on legacy rewrite-brief documents, avoids stack-specific child terminal tools, and tells the parent agent to run one same-worktree child slice at a time.
- [x] Validate the TypeScript package and prompt Markdown formatting; TypeScript package validation for the helper/tool slices passes, and prompt Markdown formatting passes now that the prompt exists.
- [x] Prepare the work for manual user inspection while leaving the Objective open until the user explicitly requests closure.
