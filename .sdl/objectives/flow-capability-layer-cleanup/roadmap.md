...
- [ ] Move shared capability gateway result/error substrate into `@sdl/capability-kit`.
  - Extract capability-oriented `ErrorInfo` / `GatewayResult` / command-failure helpers into Capability Kit if they prove shared across capabilities. Current verified state no longer has `@sdl/core/submit`; Flow submit owns its submit-specific command-failure helper and imports generic result primitives from `@sdl/core/result`, while `ts/packages/sdl-capability-kit/src` does not yet own capability gateway shapes. Evidence: capability packages import capability gateway shapes from `@sdl/capability-kit`, not Flow-local submit helpers or core result aliases, where sharing is warranted.

- [ ] Rebaseline package tiers, import guards, and docs/context.
  - Update `sdl.tier`, dependency manifests, export maps, TypeScript style-guard/package-tier expectations, jiti aliases, and forward-looking context/docs so they match the new Flow/Capability Kit boundary. Current verified state has removed the old autobranch and submit package/export surfaces and their stale kernel aliases; remaining rebaseline should verify the broader tier/import/docs/context story after the capability gateway substrate decision. Evidence: relevant guard checks pass and parent `sdl-extension-architecture` can be updated with the child completion evidence if needed.
