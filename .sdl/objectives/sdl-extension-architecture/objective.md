...
## Architecture Model (ADR 0009 + 0012 + 0016)
...
- **The SDK:** the `@sdl/sdl` host/kernel + the distinct `sdl-sdk` author package, the concrete SDL extension API of host primitives.
- **Above the SDK — Capabilities:** flow, handoff, objective, branch-context, plans, address, slot, roaster, aretro, built on the shared `@sdl/capability-kit` (the **Capability Kit**). "Extension Kit" is a reserved name for a hypothetical future substrate for building *all* extensions, third-party included; it does not name the first-party kit.
...
- Use the grouped project-local flow extension under `.sdl/extensions/flow`, treating lifecycle commands such as `changes`, `cp`, `regenerate-pr`, `submit`, and `land` as repository-authored command entries rather than universal SDL built-ins.
  ...
  - Source layout: `.sdl/extensions/flow/` holds the extension manifest (`package.json`) and thin re-export command shims (each `src/commands/<name>.ts` is `export { default } from "sdl-flow/commands/<name>";`). The implementation — command bodies and the `shared/` helper tier — lives in the `sdl-flow` package at `ts/packages/capabilities/flow/src/{commands,shared}/`. Roadmap evidence cites the `ts/packages/capabilities/flow/...` implementation paths accordingly.
...
- Use each command migration to discover and refine the minimal public SDL extension SDK.
  - Begin from the current SDL extension API (`sdl-sdk`) surface where possible.
...
- Consolidate duplicated command-author code across the `flow` group through the project-local shared-helper tier and existing internal-migration-export subpaths, not new public SDK surface.
  ...
  - Route command-local reimplementations — PR-description machinery, generic exec/format/JSON helpers, GitHub-PR access, Graphite-submit mechanics, autobranch behavior, and CCC-CLI delegation boilerplate — through Flow-owned seams in the `sdl-flow` package (`ts/packages/capabilities/flow/src/shared/`, `src/submit/`, and `src/autobranch/`) plus neutral mechanics packages only where they remain policy-free.
  - Replace the checked-in `submit.ts` bundle with a readable hand-authored command that delegates to Flow-owned submit orchestration, removing the bundled-artifact liability flagged in `.sdl/extensions/AGENTS.md`.
  - Keep this consolidation within the internal-migration-export and project-local shared-helper tiers; do not promote new `sdl-sdk` author API as part of this track.
...
- `sdl-sdk` is now the concrete SDL extension API and current Public author API filler; do not expand or reshape its exports unless concrete migration evidence justifies changing the author surface.
...
- The PR-description machinery, GitHub-PR gateway, submit orchestration, Graphite-submit policy, and autobranch behavior that `regenerate-pr`, `submit`, and autobranch flows need are now Flow-owned in `sdl-flow`; `@sdl/capability-kit` owns shared capability result/error substrate such as `commandFailure`, while `@sdl/core`/`@sdl/graphite` retain only generic or protocol/mechanics helpers.
  - Revised (2026-06-24 and later): for command-author modules consumed **only** by flow, the accepted disposition is to relocate them into `sdl-flow` (`ts/packages/capabilities/flow/src/shared/`, `src/submit/`, or `src/autobranch/`) and import `@sdl/core/*`/`@sdl/graphite/*` directly only for neutral mechanics, rather than keep them in `@sdl/sdl` behind `@sdl/sdl/*` internal-migration-export subpaths. SDK-independent shared primitives such as checkpoint/worktree/temp/text-generation later moved into `@sdl/capability-kit`; the author SDK itself moved to `sdl-sdk`. See `updates/2026-06-24-relocate-flow-only-modules.md`, `updates/2026-06-24-domain-primitives-transitional-extraction.md`, `updates/2026-06-26-sdl-sdk-cutover.md`, and the `flow-capability-layer-cleanup` child Objective updates.
...
- A shared GitHub-PR gateway seam could still overreach the command-first evidence threshold if promoted too far. The current mitigation is to keep submit/PR-description GitHub PR gateway behavior Flow-owned and defer any public SDK or standalone GitHub-PR subpath until another consumer proves it.
- The former `@sdl/pi` ↔ `@sdl/ccc` package dependency cycle is de-risked for the Objective capability path: `@sdl/objective/api` owns the Objective domain consumed by `ccc`/`sdlcc`, `@sdl/objective` has no `@sdl/pi` dependency, `@sdl/pi` no longer declares/imports `@sdl/ccc`, and `just ts-guard` enforces the Objective-scoped Extension Dependency Graph. The final deferred autobranch/pi/sdl manifest cycle was retired by relocating branch-slug primitives to neutral `@sdl/core/branch-slug` and removing the `@sdl/autobranch` → `@sdl/pi` dependency. Remaining graph work is broader `ccc` clean-consumer conversion for capabilities that have not yet migrated.
...
- Which Flow-owned submit/PR-description helpers, if any, have earned enough cross-extension evidence to graduate into `sdl-sdk` or a neutral package? This track deliberately defers that promotion and keeps it as a later steer-first decision.
