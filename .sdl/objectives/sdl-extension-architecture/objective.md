...
- The former `@sdl/pi` ↔ `@sdl/ccc` package dependency cycle is de-risked for the Objective capability path: `@sdl/objective/api` owns the Objective domain consumed by `ccc`/`sdlcc`, `@sdl/objective` has no `@sdl/pi` dependency, `@sdl/pi` no longer declares/imports `@sdl/ccc`, and `just ts-guard` enforces the Objective-scoped Extension Dependency Graph. Remaining graph debt is the explicitly deferred `@sdl/autobranch` / `@sdl/branch-context` / `@sdl/pi` / `@sdl/sdl` manifest cycle plus broader `ccc` clean-consumer conversion for capabilities that have not yet migrated.
