...
- [x] Reset the SDL CLI kernel and restore `changes` as the first project-local extension.
  ...
  - Evidence: the kernel command registry no longer imports or registers privileged domain commands, SDL scenario/unit coverage exercises an empty built-in catalog and selected project-local `changes`, and the command later moved into the grouped flow extension as `ts/packages/extensions/flow/src/commands/changes.ts` / `sdl flow changes`. Verification included the full Vitest suite, TypeScript typecheck/lint/format checks, dprint check, and source searches for default command registration and stale built-in/mirror wording.
...
2. **flow-shared** — extracted into `ts/packages/extensions/flow/src/shared/`, consumed by ≥2 flow commands.
...
| A7 CCC CLI delegation (exec-adapter + stdio accumulation + exit→ok/fail)      | land, autoslot, pull-trunk                                                | flow-shared helper completed in `ts/packages/extensions/flow/src/shared/ccc-cli.ts`; three commands delegate shared I/O/result mapping there while keeping command-specific CCC inputs local                                                                            | target met without public SDK promotion                                                                      | low (helper only, never SDK)                    |
