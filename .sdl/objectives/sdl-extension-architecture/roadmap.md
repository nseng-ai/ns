...
| A2 Git repository ops (snapshot/status/diff/branch/commit)                    | cp, changes, autobranch, push; submit/regenerate-pr through package seams | flow-shared local mechanics in `shared/git.ts` + `worktree.ts`, plus package-owned Git gateways in `@sdl/sdl/submit`, `@sdl/sdl/pr-description`, `@sdl/sdl/checkpoint`, and `@sdl/sdl/pending-worktree`; `push` keeps command-local push policy while using the helper | target met for this track; no new public SDK surface                                                         | high (gateway shape proven ≥3 commands) — defer |
