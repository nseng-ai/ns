...
- [x] Extract repeated worktree helpers inside the project-local extension boundary without promoting new SDK surface.
  ...
  - Evidence: PR #2016 / branch `refactor-worktree-helpers` adds `.sdl/extensions/shared/worktree.ts` and rewires `.sdl/extensions/changes.ts`, `.sdl/extensions/cp.ts`, and `.sdl/extensions/autobranch.ts` to share git snapshot loading, env lookup, command detail formatting, pending-worktree errors, and checkpoint commit creation while continuing to import SDL types only from `@sdl/sdl/sdk`. This validates extension-owned shared helpers as the middle tier between one-off command-local duplication and public SDK promotion; it does not by itself justify a new kernel or SDK Git helper.
...
- [ ] Migrate land as a project-local Graphite/GitHub stack-landing command extension.
  ...
  - `land` differs from prior slices: there is no `sdl land` built-in to remove. It exists only as the Pi `/sdl:code:land` command delegating to the CCC land-stack orchestration (`@sdl/ccc/land`, ~3300 lines across `ts/packages/ccc/src/land-stack/`) with `PARTIAL` parity and no CLI/skill entry. This slice establishes a project-local `sdl land` surface and reworks the Pi mirror to delegate, closing the `cross-harness-parity` gap.
  - Use this slice to pressure-test whether a project-local command may keep depending on the `@sdl/ccc/land` orchestration boundary or whether landing/Graphite-stack behavior needs a promoted public SDK/kernel interface.
  - Evidence should cover selected command loading through the public CLI, the land outcome/failure matrix with faked `git`/`gt`/`gh` commands, a non-mutating preview/dry-run boundary, Pi `/sdl:code:land` (or flat `/sdl:land`) parity/registration, and docs for project-local availability.
...
- [~] Rework Pi SDL mirrors/adapters for project-local command ownership.
  ...
  - Current evidence: first-slice Pi changes kept only explicit `changes` mirrors; the `cp` slice restored flat `/sdl:cp`; the submit slice restored flat `/sdl:submit` only, left `/sdl:code:submit` and legacy submit aliases absent, and updated command-surface replacement/push guidance accordingly. The broader row remains active for later `regenerate-pr` and `land` migrations and any dynamic Pi discovery decision. The `land` migration must also decide the final Pi land mirror shape (`/sdl:code:land` vs flat `/sdl:land`) and update its parity metadata accordingly.
...
- [x] Document the emerging SDL kernel and extension SDK model.
  ...
  - Evidence: SDL README/context language now distinguishes the SDL kernel from project-local command policy, records project-local extensions as the current command-first mechanism, keeps future bundled extensions as deferred design space, and documents the command-first SDK promotion rule. The SDK reference states that `@sdl/sdl/sdk` is intentionally small, owns curated lower-package re-exports as first-party author vocabulary, and remains the authoritative export inventory. `CONTEXT-MAP.md` points readers to the updated SDL kernel/project-local/future-bundled vocabulary and resolves the SDK re-export ownership note. Pi docs state that SDL extension discovery is CLI-only today and exact `/sdl:*` mirrors are static engineered adapters requiring explicit package tests/parity metadata. `.sdl/extensions/AGENTS.md` records the readable single-file extension boundary, helper-promotion escalation path, and generated/bundled artifact caution. The checkpoint message cleanup records the intermediate pattern: repeated repo-local extension behavior can first move to `.sdl/extensions/shared/` while public SDK promotion remains deferred.
