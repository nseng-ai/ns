...
| ----------------------------------------------------------------------------- | ------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------ | ----------------------------------------------- |
| A1 Process exec + evidence                                                    | all                                                                       | public-sdk (`commandSucceeded`/`formatCommandEvidence`) + kernel `exec`; submit no longer carries local `spawn`; flow-shared failure formatting covers regenerate-pr/worktree/autobranch | flow-shared cleanup landed; later rows may fold submit's specialized errors as part of the readable rewrite  | graduated                                       |
| A2 Git repository ops (snapshot/status/diff/branch/commit)                    | cp, changes, autobranch, push; submit/regenerate-pr through package seams | mixed: flow-shared `worktree.ts` plus package-owned Git gateways in `@sdl/sdl/submit` and `@sdl/sdl/pr-description`; `push` still calls `ctx.exec("git", …)` directly                    | resolve the remaining flow-level seam/disposition; no new public SDK surface                                 | high (gateway shape proven ≥3 commands) — defer |
| A3 GitHub-PR access (`gh pr view`, details, patch-id)                         | submit, regenerate-pr                                                     | internal-export via `@sdl/sdl/submit` and `@sdl/sdl/pr-description`, both backed by `@sdl/core/submit`'s `RealGithubPrGateway`; no standalone `@sdl/sdl/github-pr` subpath               | target met through package-owned migration seams; standalone subpath deferred unless later evidence needs it | high — defer (steer-first)                      |
| A4 PR-description generation (managed region, prompt, truncation, validation) | submit, regenerate-pr                                                     | internal-export via `@sdl/sdl/pr-description` plus `@sdl/sdl/submit`; both consumers delegate to package-owned PR-description orchestration                                              | target met without adding public SDK surface                                                                 | medium                                          |
...
| A8 Checkpoint/worktree snapshot                                               | cp, autobranch (via `worktree.ts`); submit through `@sdl/sdl/checkpoint`  | internal-export plus flow-shared: submit uses `runCheckpointIfPending`/`RealCheckpointGateway`, while cp/autobranch use the flow worktree seam                                           | target met for checkpoint consumption; remaining generic Git disposition stays in A2                         | medium (overlaps A2)                            |
| A9 Scratch/temp prompt + message I/O (`fs`/`os`/`path`/`crypto`)              | submit, regenerate-pr, autobranch                                         | internal-export (`@sdl/sdl/temp-files`) + flow-shared scratch helper; submit's durable raw-log path remains intentionally local                                                          | target met for scratch lifecycle; durable logging remains out of scope                                       | low–medium                                      |
...
- [x] A1+A9 — Unify exec failure-formatting and scratch I/O foundations.
  ...
  - Evidence: `@sdl/sdl/temp-files` now exposes core temp-file primitives as an internal-migration export, `.sdl/extensions/flow/src/shared/scratch.ts` owns flow scratch lifecycle, and `.sdl/extensions/flow/src/shared/command-output.ts` owns shared command failure/details formatting for regenerate-pr and worktree/autobranch callers. `regenerate-pr` and worktree checkpoint messages use the scratch helper; the submit bundle no longer imports `node:child_process`, no longer defines a local spawn-backed `runCommand`, requires injected gateway runners, and routes its feasible PR-body/checkpoint temp files through `withFlowTemporaryFile` while leaving durable raw failure logs local. Focused unit/scenario coverage plus lint/type/format checks stayed green.

- [~] A2+A8 — Resolve the remaining Git-ops seam disposition after package-seam extraction.
  ...
  - Evidence so far: `submit` and `regenerate-pr` no longer carry private command-local Git runners; they delegate through package-owned internal migration seams in `@sdl/sdl/submit`, `@sdl/sdl/pr-description`, `@sdl/sdl/checkpoint`, and `@sdl/sdl/pending-worktree`. `cp`, `changes`, and `autobranch` still use the flow `worktree.ts` seam. Remaining decision/work: inspect the remaining flow-level direct Git use, especially `push`, and either route it through the flow seam or record why package-owned/per-command Git boundaries are now the accepted A2 disposition. No new public `@sdl/sdl/sdk` surface should be added.
...
- [x] A3 — Introduce one GitHub-PR access seam for the flow group.
  ...
  - Evidence: GitHub PR access now lives behind package-owned migration seams rather than command-local flow code. `@sdl/sdl/submit` and `@sdl/sdl/pr-description` both build on `@sdl/core/submit`'s `RealGithubPrGateway`; `regenerate-pr` and readable `submit` are represented as consumers; no public `@sdl/sdl/sdk` surface was added. A standalone `@sdl/sdl/github-pr` subpath was deferred because the package-owned runtime seams were enough for this track.
...
- [x] A4 — Consolidate PR-description generation behind the internal-export seam.
  ...
  - Evidence: `@sdl/sdl/pr-description` now owns PR-regeneration preparation/application and re-exports the relevant `@sdl/core/submit` PR-description types/helpers, while `@sdl/sdl/submit` wires the same PR-description orchestration into submit. `.sdl/extensions/flow/src/commands/regenerate-pr.ts` delegates to the package seam instead of carrying local PR-description duplication, and `.sdl/extensions/flow/src/commands/submit.ts` uses the submit runtime seam. No new public SDK surface was added.
...
- [~] Replace the checked-in submit bundle with a readable delegating command.
  - Policy: direct execution after preview for implementation; ask before running real submit, restack, push, PR edit, or other external mutations.
  - Evidence so far: `.sdl/extensions/flow/src/commands/submit.ts` is now a readable hand-authored command that delegates submit orchestration through `@sdl/sdl/submit` and checkpointing through `@sdl/sdl/checkpoint`, while keeping command-local terminal output and submit-failure summarization policy in the extension. The prior ~3017-line bundle is gone from the flow command. The stale runtime import defect was repaired by routing submit orchestration symbols in `ts/packages/sdl/src/submit.ts` through `@sdl/graphite/submit` while retaining `RealGithubPrGateway` from `@sdl/core/submit`.
