...
- [x] Retire obsolete broad/deep sibling imports and record the final boundary.
  - Evidence: `ccc` and Pi extension source and sibling tests now consume branch-context/plans behavior through `@sdl/branch-context/api` and `@sdl/plans/api`; final sibling-boundary search over `ts/packages/ccc/src`, `ts/packages/ccc/test`, `ts/packages/pi-extensions/src`, and `ts/packages/pi-extensions/test` returns no broad root imports. `@sdl/branch-context` and `@sdl/plans` Peer APIs explicitly export the sibling test helpers needed for fake contexts, saved-plan store fixtures, and slug-model prompt fixtures. Package context docs record that sibling runtime packages and their tests use Peer API subpaths, while owner-package tests may import package roots for compatibility coverage and branch-context may intentionally depend on plans for saved-plan semantics. See `updates/2026-06-25-050451-sibling-peer-api-test-boundary.md`.
