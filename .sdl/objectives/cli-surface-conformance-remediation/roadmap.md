...
- [ ] **Area (a) — danger-tier safety remediation (land-now).** Add confirmation
      gating to the unblocked destructive commands, modeled on `handoff delete`/
      `gc`. With scenario tests per command (interactive confirm, `--yes`/`--force`
      bypass, non-interactive `usageError`).
  - `branch-context exec delete`, `brmem delete`: add `--yes`/`-y` +
