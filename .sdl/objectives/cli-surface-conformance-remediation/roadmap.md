...
- [~] **Area (d) — exit-semantics remediation.** Re-verify the flagged commands against current source, then apply the decision table: requested-target/action miss → `negative`; bad/missing arg → `usageError`; operational error → `failure`; harmless empty/predicate miss → `ok`. Do not carry old path evidence forward without a fresh probe. Landed evidence includes AREG mutation/apply failures, Vibechk read-only lookup/config/failure classification, `sdlcc cmux report` envelope migration, `ccc exec autobranch` envelope migration, and `roaster exec publish-findings` envelope migration. Remaining current raw-exit decisions are concentrated in `packagechk` and `vibechk run`.
