...
Rerun the exact failing check shown in the context. After it passes, rerun the original native `ns flow gt submit` invocation shown in the context. If the correct behavior is ambiguous, ask the user for steering instead of changing the check.
