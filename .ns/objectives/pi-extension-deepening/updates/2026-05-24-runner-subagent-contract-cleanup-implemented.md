...
- Updated `docs/pi/runner-subagent-helper.md` to describe the helper API, runtime internals, and generic `forked_pi_agent` tool separately.
...
- Added focused helper tests and strengthened `forked_pi_agent` tests so terminal-capture and diagnostic statuses do not look like final-text completion.
...
The runner-subagent docs now distinguish terminal-capture completion from final assistant text completion, and Objective-stack guidance no longer presents child-session / `runChildSession(...)` wording as current guidance. `forked_pi_agent` remains final-text oriented and continues to warn that non-`final-text` statuses require diagnostics/session inspection before completion is assumed.
