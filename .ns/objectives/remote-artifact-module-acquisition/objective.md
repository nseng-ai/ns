...
- **Decide fetched-module storage**: acquired npm modules use one private project per canonical package identity under `.ns/managed-extensions/npm/<package-name>/`, with the top-level module at that leaf's `node_modules/<package-name>`; the legacy shared npm project's `node_modules` tree is ignored. The managed location remains git-native and inspectable per the umbrella's no-hidden-database rule.
...
- **Storage-location mistakes are sticky:** npm acquisition now isolates each canonical package identity in its own private managed project so one package-manager operation cannot rewrite another extension's bytes; legacy shared-project bytes remain ignored in place.
