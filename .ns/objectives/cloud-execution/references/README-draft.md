...
- **Model keys** live as sensitive environment variables on the dispatch
  ...
  receives `AI_GATEWAY_API_KEY` and pins its model to the Vercel AI Gateway
  (`vercel-ai-gateway` / `anthropic/claude-opus-4.6`) through sandbox-global
  pi settings. A run receives only the key required by its configured harness.
