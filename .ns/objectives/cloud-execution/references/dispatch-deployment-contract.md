...
## Expected local builder notices

The deployability gate intentionally runs `vercel build --prod` locally. Vercel therefore
reports that the build is not running on Vercel and that hosted system environment variables
are unavailable. This is expected: the local prebuilt-artifact gate must remain independent
of those variables.

Vercel CLI may also describe lockfile version 9 as generated by pnpm 10 even though the
workspace is pinned to pnpm 11.8.0. `ts/package.json#packageManager` is authoritative, and
pnpm 11 accepts the checked-in lockfile. A future CLI release may update that label without
requiring any repository change.

Do not filter these notices or suppress stderr wholesale. The same output carries actionable
TypeScript diagnostics, artifact-closure failures, Workflow-manifest failures, and deployment
identity failures.
