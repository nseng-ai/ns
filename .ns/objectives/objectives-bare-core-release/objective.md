...
This clears the registry-artifact and acquisition-path dependency for `ship-objectives-to-customers` and `objectives-claude-onboarding-steelthread`. The latter still owns the documented, fresh-session Claude Code lifecycle journey and remains gated on its documentation launch slice. General release automation, self-update, extension fleet updates, and non-npm channels remain parked follow-ups.
