...
The repo is an all-TypeScript pnpm workspace. There are no tracked Python workspace packages (`packages/` is absent) and no Python `CONTEXT.md` targets. The product renamed from `sdl` to `ji` (ADR 0024) and then to `ns` (ADR 0026), both hard cutovers with no compatibility codepaths, and the workspace was restructured into role directories with container packages: `ts/packages/capabilities/` (address, aretro, branch-context, ccc, flow, handoff, objective, plans, roaster, slot), `ts/packages/capability-kit/`, `ts/packages/hosts/` (nscc, pi), `ts/packages/infra/` (brmem, clinkr, core), `ts/packages/kernel/`, `ts/packages/local/pi-tools/`, and `ts/packages/tools/` (areg, packagechk, vibechk). That is 21 tracked packages (`git ls-files 'ts/packages/*/package.json' | wc -l` = 21), named `@ns/*` with two naming exceptions: the unscoped `nscc` (`ts/packages/hosts/nscc`) and the local-space `@internal/pi-tools` (`ts/packages/local/pi-tools`). Former standalone identities are retired: `@sdl/sdl` became `@ns/kernel` (bin `ns`; subpath exports `./cli`, `./command-io`, `./context`, `./pi-text-generation`, `./sdk`), the standalone `sdl-sdk` package was re-absorbed as the `@ns/kernel/sdk` subpath, `sdlcc` became `nscc`, `@sdl/pr-address` became `@ns/address`, `@sdl/graphite` was absorbed into `@ns/capability-kit` (graphite subpackage; context at `ts/packages/capability-kit/src/graphite/CONTEXT.md`), the standalone `sdl-land` package was absorbed into `@ns/flow` as its `land` subpackage (exports `./land/api`, `./land/testing`), `autobranch` behavior lives in `@ns/flow` subpackages, and `@sdl/domain-primitives-transitional` was deleted along with the `transitional` package tier. The old directories hold only ignored `node_modules` leftovers on disk; none has tracked files.

- Root repo context: `CONTEXT.md` (titled "ns") — Objective-system vocabulary, the repo-wide Architecture Boundaries (Gateway / Domain logic) section, the Extension Layering cluster, and package-topology terms.
...
- `@ns/areg` — agent-resource bootstrap and skill-workflow vocabulary.
- `@ns/packagechk` — standalone package-name availability/claimability vocabulary.
- `@ns/aretro` — deterministic branch-retrospective evidence vocabulary.
...
- Do not split `@ns/core` into per-subpackage `CONTEXT.md` files. Keep it as a single file with H2 sections until a subpackage actually graduates to a standalone package (`clinkr` already graduated to the standalone `@ns/clinkr`).
...
- The post-`ns`-rename baseline is all-TypeScript: 21 tracked packages under `ts/packages/`, named `@ns/*` except the unscoped `nscc` and the local-space `@internal/pi-tools`, organized as role directories with container packages that carry manifest-declared subpackages (`ns.tier`, `ns.subpackages` in `package.json`). Twelve packages have present contexts (`handoff`, `brmem`, `ccc`, `pi`, `capability-kit/graphite`, `kernel`, `roaster`, `plans`, `branch-context`, `slot`, `objective`, `flow`); the rest are planned, undecided, or out-of-scope in the map.
- `CONTEXT-MAP.md` was substantially rebaselined to the `@ns` world by adjacent work (its Inventory Baseline says 21 packages; its Present section carries the kernel, objective, and flow contexts), but it has accumulated fresh drift concentrated in the land/flow-pi identities, two naming/count claims, and two stale Present link paths — see Risks.
...
- Inventory drift remains the dominant risk and has materialized again at larger scale since the last rebaseline: ADR 0024 (`sdl` → `ji`) followed by ADR 0026 (`ji` → `ns`) changed the namespace to `@ns/*` and the CLI to `ns`, and the container-package restructure moved capabilities under `ts/packages/capabilities/`, absorbed `graphite` into `@ns/capability-kit`, `sdl-land` into `@ns/flow`, `sdl-sdk` into `@ns/kernel/sdk`, and consolidated local Pi tools into `@internal/pi-tools`. Mitigation: fix obvious, source-backed drift inline when it is small/local and decision-free; handle broader drift as focused rebaseline/update phases against current source, never by silently widening an unrelated package session.
- Current `CONTEXT-MAP.md` drift (open): (1) the Inventory Baseline names `@ns/flow` as an unscoped naming exception, but `@ns/flow` is scoped — the only unscoped name is `nscc`; (2) the Present list includes `ts/packages/capabilities/land/CONTEXT.md` and an `sdl-land` Present entry, but that file and package are gone (land is now a `@ns/flow` subpackage), and relationship rows still describe standalone `sdl-land`; (3) the Present links for `@ns/roaster` and `@ns/branch-context` point at old pre-`capabilities/` paths; (4) the "Thirteen have present package context files" count does not match either its own 14-item list or the 12 tracked package context files; (5) the Planned entry `@ns/flow-pi` names a package that does not exist as tracked. Mitigation: a focused map catch-up slice re-derives the baseline and Present/Planned sections from `git ls-files`; the land/flow-pi items are decision-bearing (context-surface removal/re-scoping), so they go through a confirmed session rather than a silent fix.
- Packages without a recorded context decision (open): `@ns/address`, `@ns/clinkr`, `@ns/core`, `nscc`, plus the partial `@ns/capability-kit` (graphite-only coverage) and `@internal/pi-tools` (subpackage targets only) decisions. Mitigation: record an explicit decision per package rather than leaving silent absence.
- The rename left living domain docs still using the old name — the root context is titled "SDL Tools" and several context descriptions expand "Source Development Lifecycle". Whether living contexts should adopt `ns` naming is decision-bearing (ADR 0024 bans scrubbing *historical* prose, but these are current docs). Mitigation: treat as an open question; do not mass-rename without a confirmed session.
...
- Should `/CONTEXT-MAP.md` link into `@ns/core`'s H2 sections individually, or treat `@ns/core` as a single linked context? — *Provisional answer:* keep one `@ns/core` context entry with inline H2 anchors when it is authored; revisit during final readback. (`clinkr` is a standalone `@ns/clinkr` package, not a `@ns/core` H2 section.)
...
- Where should the remaining undecided packages land in the map — `@ns/address`, `@ns/clinkr`, `@ns/core`, `nscc`, kit-level `@ns/capability-kit`, and container-level `@internal/pi-tools`? Some may warrant a focused context; others may be accepted-from-adjacent or recorded as deliberately thin. Decide per package during the undecided-packages catch-up rather than defaulting them to Planned.
- How should the map's Planned slate be re-derived after the container-package restructure — in particular the phantom `@ns/flow-pi` target (Flow Pi presentation is now the `@ns/flow` `pi` subpackage) and whether `@internal/pi-tools/*` subpackage contexts should stay individually planned or collapse into one container-level decision?
- Should living domain docs adopt the `ns` name — e.g. the root context's "SDL Tools" title and "Source Development Lifecycle" expansions — or keep the old naming until a deliberate vocabulary session? (ADR 0024 forbids scrubbing historical prose; current living contexts are a separate decision.)
