...
The semantic boundary is now explicit: an `impl` workflow implements a prompt or Saved Plan. The name is shorter, avoids collision with remote-system dispatch terminology, and describes the workflow outcome more accurately than `launch`. The cutover preserves the existing agent instructions and workflow behavior. Prepared Herdr Launch, destination/process startup, and Pi launch mechanics remain supporting mechanics; prompt transport/storage now uses the `ns-impl` namespace. Handoff launch remains accurate for the durable `/ns:herdr:tab:handoff` workflow.
...
- `ts/packages/capability-kit/src/kit/tracked-branch-payload.ts` preserves the existing completion instructions while using implementation and launch vocabulary at their respective boundaries.
...
- `ts/packages/capabilities/herdr/test/herdr-impl.test.ts`, `impl-branch-basis.test.ts`, and `extension.test.ts`, plus `ts/packages/capability-kit/test/unit/tracked-branch-payload.test.ts`, cover implementation behavior, branch selection, exact registration, removed aliases, and preserved prompt instructions.
