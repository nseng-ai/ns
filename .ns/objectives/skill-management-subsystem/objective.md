...
- Ambient-drift fingerprint nudges (the commanded reconcile slice landed via the closed `npm-bundled-artifact-provisioning` Subobjective and now lives in the descriptor-activation lifecycle — `ns init` and the `ns extension` verbs; the drift nudge itself stays parked).
...
- **Graduated 2026-07-07** into the Subobjective `harness-artifact-vocabulary-reconciliation` (edge-tracked): the skill-workflow/vocabulary reconciliation sweep (`skillx`, `@nseng-ai/areg`, `npx skills`, repo skill conventions), including the bare-"artifact" collision cleanup.
- **Graduated 2026-07-07** into the Subobjective `remote-artifact-module-acquisition` (edge-tracked): remote acquisition sources for artifact-bearing modules (npm/git/local-path specs), carrying version-resolution-for-fetched-modules.
- Stale-after-upgrade detection, rename cleanup, and uninstall — manifest-enabled; disposed 2026-07-07 as a follow-on `## Work` row and **delivered 2026-07-10..12 cross-objective**: the removal machinery lives in `@nseng-ai/harness-artifacts` (`provision-removal.ts`) and the command surface shipped as the `ship-objectives-to-customers` descriptor-activation lifecycle (`ns init`, `ns extension install|uninstall|update`); see the roadmap row for evidence.
- **Retired 2026-07-07** (see `updates/20260707T150250Z-parked-breadth-dispositions.md`): marketplace/remote catalog discovery; project trust gating for provisioned artifacts (deliberate trusted-repo risk acceptance while ns is private); per-resource filtering/enable-disable (all-or-nothing provisioning is the contract).
...
- The package boundary may be awkward if the subsystem must be consumed by several first-party CLIs and extensions with different runtime/layering constraints; a shared core plus thin per-CLI bindings may be needed. Substantially de-risked: `@nseng-ai/harness-artifacts` lives at `ts/packages/capabilities/harness-artifacts`, and the descriptor-activation lifecycle became the proving multi-consumer — `ns init` and the `@nseng-ai/ns-init` `ns extension install|uninstall|update` verbs consume the same activation/transition/removal machinery (`project-harness-artifact-transitions.ts`, `provision-removal.ts`) that `ns skills` uses, with additive API growth and thin consumers. Residual risk is ongoing API shape, not package placement.
...
- Bare "artifact" was used elsewhere in ns vocabulary (handoff artifacts in `@nseng-ai/handoffs`, consumer artifacts in `docs/conventions/platform-and-consumer.md`, and AREG's own "managed artifacts" for invocation-kind overlay files — a direct collision inside the tool being re-read as Artifact Registry). **Resolved** by the closed `harness-artifact-vocabulary-reconciliation` Subobjective: the canonical qualified term is "harness artifact", root `CONTEXT.md` carries the harness-artifact/harness-overlay vocabulary cluster with Avoid entries, and AREG's overlay sense is renamed to **harness overlays** (`ts/packages/tools/areg/src/operations/skill-kind.ts`; no "managed artifacts" wording remains in areg src). Handoff and consumer artifacts stay owned by their own domains.
...
- Existing `skillx`, the `@nseng-ai/areg` CLI, `npx skills`, and repo skill conventions overlapped semantically. The prior AREG migration Objectives (`migrate-areg-and-ns-skills`, `areg-typescript-port`, `areg-ts-cli-cleanup`) have all closed, so re-platforming no longer competes with active migration plans. Largely reconciled by the closed `npm-bundled-artifact-provisioning` and `harness-artifact-vocabulary-reconciliation` Subobjectives: AREG's `npx skills` wrapping surfaces (the `npx-skills` gateway, `areg init` bootstrap-clone, `update-skills`, `skillx` temp-workspace) were removed — no `npx skills`/`npx-skills` reference remains in areg src — and the `skills-lock.json` parsing was pushed down into `@nseng-ai/harness-artifacts` (`src/skills-lockfile.ts`), leaving `check.ts` as the only lockfile-adjacent operation in areg (its `operations/` directory otherwise holds the doctor/skill-kind/skill-find/project-inspection inspector surfaces). AREG is now a standalone whole-project inspector that reads the shared install manifest and `skills-lock.json` as complementary sources. The residual overlap surface is the parked areg local-logic push-down row (invocation-kind apply planning, `check` drift detection, skill find), gated on a second runtime consumer.
