...
- [~] Subobjective `ns-skills-steelthread`: design and validate the `ns skills` thread end-to-end.
      Split out 2026-07-06 (see `updates/20260706T123757Z-split-steelthread-subobjective-and-umbrella-promotion.md`). The child owns the thread rows — design the artifact model / harness path table / provision plan / install manifest, then implement `ns skills list/path/install` provisioning one real ns-owned skill for `pi`/`claude-code`/`codex` with a per-file-hashed install manifest, zero `npx skills` dependency, consumed through the `@nseng-ai/ns-init` `SkillMaterializer` seam. Track progress in `.ns/objectives/ns-skills-steelthread/`; this row closes when that Subobjective closes and its evidence is synthesized here.
