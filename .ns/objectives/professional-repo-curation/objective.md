...
Three major foundations have landed. ADR 0044 renamed the extension tiers and moved all 11 ns extensions directly into the path-derived `ts/packages/incubator/` zone while keeping Extension Kit in the clean zone. Coordinated npm release `0.1.3` proved a colleague can install bare `@nseng-ai/ns`, acquire `@nseng-ai/objectives`, provision its Objective skills, and run `ns objective list` from a foreign repository without this checkout. ADR 0046 now governs the first-party skill tree: all 58 current skills have explicit support dispositions and stable family ownership, nested canonical sources, and flat harness identities, with `skills/README.md` as the authoritative contract and `pr-make-accountable` as the first public support warrant. Separately, approved ADR 0045 supersedes ADR 0044's flat package incubation zone: the Subobjective `package-disposition-and-host-ontology` owns the distinct three-disposition package ontology (`public`/`incubating`/`internal`), host-owned nesting, package-identity rules, and disposition dependency closure. Remaining work must finish the semantic rename, calibrate package contracts, complete the separate package child, ship the public presentation and PR Feedback quickstart, harden transfer boundaries, and perform the repository transfer.
...
- Preserve the completed ADR 0046 first-party skill-tree contract: 58 current skills — 1 public, 25 incubating, and 32 internal — under exact owner-nested canonical paths, except the approved top-level `brmem` and `slots` product boundaries. Runtime lookup remains the flat Harness Overlay; `npx skills` owns acquisition, installed-state lifecycle, and `skills-lock.json`; the repository directly owns canonical topology, overlays, and invocation metadata; vendored directories remain outside this ontology; and support disposition, family, identity, invocation mode, and metadata remain distinct. ns has no skill-management commands, package, provisioning manifest, or reconciliation interfaces; runtime Skill-Backed Commands remain.
- Synthesize the landed package reorganization from Subobjective `package-disposition-and-host-ontology`. ADR 0045 and the current tree use `public`/`incubating`/`internal` disposition roots, owner nesting (including `hosts/pi/`), global leaf/package-identity matching, and scope by disposition. The tree now has 31 workspace packages after deletion of the Skill Exposure package; `ts/packages/README.md` is authoritative and deferred Pi extraction remains.
- Preserve package disposition dependency closure: public depends only on public; incubating on public or incubating; internal on anything. Public `@nseng-ai/ns` no longer depends on incubating Branch Context or Harness Artifacts; Harness Artifacts was folded and its obsolete management surface has since been removed.
...
- **Curation:** Extension Kit and the 11 extensions use the settled terminology; the package tree carries ADR 0045's disposition ontology with every package explicitly classified by the approved destination map; `ts/packages/README.md` states the authoritative package contract; package disposition dependency closure is mechanically enforced; foundation packages carry verified cold-audience README contracts. The distinct ADR 0046 skill ontology remains complete for all 58 current first-party skills — 1 public, 25 incubating, and 32 internal — with `skills/README.md` authoritative, flat harness identities preserved, and the first public support warrant limited to `pr-make-accountable`.
...
- **Incomplete isolation.** ADR 0045's disposition dependency closure is landed and public `@nseng-ai/ns` no longer depends on Branch Context or Harness Artifacts. Remaining boundary risk is the deferred Pi extraction and any incubating-to-incubating product coupling that presentation must describe honestly.
- **Presentation ahead of evidence.** Root `README.md` is still a one-line placeholder, `why-ns.md` is absent, and PR Feedback's package README describes checkout-bound use. Do not publish stronger claims first. The public `pr-make-accountable` skill warrant covers operation with Git and authenticated `gh`; it is not evidence that PR Feedback is checkout-free.
- **Ontology conflation.** Skill support disposition and family ownership are distinct from package disposition and host ownership; neither inherits from the other. Within the skill system, support disposition, family, globally flat identity, invocation mode, and `metadata.internal` are independent classifications. `npx skills` owns lifecycle and lock state; repository files own topology, overlays, and invocation metadata; ns owns neither management surface.
