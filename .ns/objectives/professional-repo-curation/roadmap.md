...
- [x] Synthesize completed Subobjective `skill-disposition-and-owner-ontology`. ADR 0046 and authoritative `skills/README.md` now govern all 58 first-party skills: 1 public, 23 incubating, and 34 internal, with exact owner-nested canonical paths plus the `brmem`/`slots` exceptions, flat global identities and Harness Overlays, unchanged vendored directories, and no old mixed-layout fallback. `pr-make-accountable` is the first public support warrant and requires only Git plus authenticated `gh`; support disposition, family, identity, exposure, and metadata remain independent. `just`, integration, isolated, explicit exposure, and structural checks passed. `npx skills check` is excluded as evidence because it attempted an external vendored refresh and failed for two skills; its effects were restored and do not block the cutover.
- [~] Complete the separate package reorganization through Subobjective `package-disposition-and-host-ontology`. Approved ADR 0045 and its destination map supersede the two-zone/flat-incubator direction with `public`/`incubating`/`internal` package disposition roots, owner-nested ontology, leaf/package-identity rules, scope by disposition, and package disposition dependency closure — subsuming the former zone-invariant row, including the public `@nseng-ai/ns` dependencies on Branch Context and Harness Artifacts that the child must remove or fold. Remaining: complete the child's deferred Pi extraction and guards, land the coordinated stack, and synthesize the result here; do not infer package verdicts from the completed skill ontology.
