...
- [ ] Foundation README-driven development pass, in place, bottoms-up in dependency order: `infra/clinkr` → `infra/foundation` → `infra/brmem` → `sdk` → `capability-kit`. `objective-next` advances this row by creating the next package's Readme-Driven-Development Subobjective, starting with `clinkr` (smallest, zero internal deps) as the dry-run; it does not perform the package audit directly in this umbrella. Each Subobjective develops `references/README-draft.md` through a human-steered interrogative process, audits and explicitly dispositions contract mismatches in its roadmap, probes accidental complexity, discusses every proposed refactoring with the user before implementation (including refactoring proposed before the draft settles), reconciles implementation to the settled contract, promotes the draft to the package README, verifies it, and returns lessons and closure evidence for umbrella synthesis. These packages never move; this row calibrates the graduation gate before anything else uses it.
