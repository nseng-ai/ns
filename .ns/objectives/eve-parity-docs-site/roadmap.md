...
- [x] Integrate with the repo and Vercel deploy.
      ...
      Evidence: `pnpm --dir docs-site run build`, `just docs-check`, and `just dprint-check` passed; `just docs-check` re-verified green on trunk 2026-07-03 after the post-slice config/helper refactors and the `.sdl`→`.ji` cutover.
