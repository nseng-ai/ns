...
- A coordinated stack can provide reviewable commits while preserving an atomic landing boundary. The reorganization is implemented as four reviewable stack points: design, cutover preparation, `@nseng-ai/ns` product consolidation, and one atomic package-cutover boundary. Each point passed its validation before submission; the cutover landed on `master` as squash commit `1d3c65b47` on 2026-07-29 (recorded in update `2026-07-29T111539Z-cutover-and-objective-extraction-landed-on-master.md`).
...
- **Hidden Pi coupling.** The live risk of this Objective, now partially exercised: the `objectives` extension was fully extracted through its curated `@nseng-ai/objectives/api` into `@nseng-ai/pi-ns-objectives` and landed on `master` (`44612ff33`), proving one extension's behavior reachable through a curated harness-independent API. Four ns extensions (`flow`, `handoffs`, `branch-context`, `herdr`) still contain `src/pi/` subpackages, `./pi*` exports, and `@nseng-ai/pi-runtime` peers on trunk. Those subpackages may reach private domain or command internals no package API exposes; extraction may expose real API design work and must not be bypassed with private imports.
- **Large atomic conflict surface.** Retired for the cutover: the complete inventory moved in one atomic stack point, with whole-tree content preserved and measured (4984 tracked files before and after, identical blob multiset apart from changed path literals), and the boundary landed on `master` as a single squash commit (`1d3c65b47`) with no mixed old/new tree reaching trunk. It remains live separately for the deferred Pi extraction, which creates the remaining adapter packages and moves Pi code out of four extensions.
- **Identity ripple.** De-risked for the identities that changed on 2026-07-26 — `@nseng-ai/pi` → `@nseng-ai/pi-runtime`, `@internal/pi-editor-mods`, and `@internal/review-reinvention-scanner`. The `pi` rename was scripted with a `@nseng-ai/pi(?![-\w])` lookahead rather than a bare substring, which would have corrupted the `pi-editor-mods` and `pi-tools` occurrences. It also materialized in a form path checks did not cover: per-package test/build scripts and every tsconfig `extends` encoded their own depth, source-dev extension discovery silently stopped resolving until the SDK resolver was fixed, and 19 depth-coupled `import.meta.url` and fixture path literals needed resolving individually. Still live for the remaining `pi-ns-*` adapters the deferred extraction creates; `@nseng-ai/pi-ns-objectives` landed cleanly.
