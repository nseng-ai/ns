...
- **Hidden Pi coupling.** Still the live risk of this Objective, now exercised in the confirming direction by complete adapter boundaries rather than independently landed precursors. The Objectives extraction landed on trunk as `@nseng-ai/pi-ns-objectives` consuming only `@nseng-ai/objectives/api`. The Handoffs extraction is implemented in its current PR as `@nseng-ai/pi-ns-handoffs`: stable cross-package metadata remains on `@nseng-ai/handoffs/api`, the complete Pi implementation and tests move to the host adapter, Herdr consumes its declared `./handoff-launch` subpath, and Handoffs removes its Pi surface and runtime coupling. Branch Context now has equivalent end-to-end working-tree evidence through `@nseng-ai/pi-ns-branch-context`; Flow and Herdr's remaining coupling still need equivalent extraction evidence. Curated API promotions required by an extraction must land in the same PR that removes the extension's Pi surface; API-only transitional PRs are rejected as confusing ownership states. Remaining extractions may expose real API design work and must not be bypassed with private imports.
