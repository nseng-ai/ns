...
- **Hidden Pi coupling.** Now the live risk of this Objective, and carried rather than hypothetical: the implemented cutover defers the Pi extraction, so five ns extensions (`flow`, `handoffs`, `branch-context`, `objectives`, `herdr`) still contain `src/pi/` subpackages, `./pi*` exports, and `@nseng-ai/pi-runtime` peers, and nothing has yet tested whether their behavior is reachable through curated harness-independent APIs. Those subpackages may reach private domain or command internals no package API exposes. Extraction may therefore expose real API design work and must not be bypassed with private imports. The risk is unexercised, not reduced — the deferral moved it, it did not shrink it.
- **Large atomic conflict surface.** De-risked in the implemented reorganization: the complete inventory moves in one atomic stack point, with whole-tree content preserved and measured (4984 tracked files before and after, identical blob multiset apart from changed path literals). It remains live until that boundary lands, and remains live separately for the deferred Pi extraction, which creates nine packages and moves Pi code out of five extensions.
- **Identity ripple.** De-risked for the identities that changed on 2026-07-26 — `@nseng-ai/pi` → `@nseng-ai/pi-runtime`, `@internal/pi-editor-mods`, and `@internal/review-reinvention-scanner`. The `pi` rename was scripted with a `@nseng-ai/pi(?![-\w])` lookahead rather than a bare substring, which would have corrupted the `pi-editor-mods` and `pi-tools` occurrences. It also materialized in a form path checks did not cover: per-package test/build scripts and every tsconfig `extends` encoded their own depth, source-dev extension discovery silently stopped resolving until the SDK resolver was fixed, and 19 depth-coupled `import.meta.url` and fixture path literals needed resolving individually. Still live for the six `pi-ns-*` adapters the deferred extraction creates.
