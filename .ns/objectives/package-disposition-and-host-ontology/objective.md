...
- **Hidden Pi coupling.** Still the live risk of this Objective, now partially exercised in the confirming direction: the Objectives extraction landed on trunk as `@nseng-ai/pi-ns-objectives` consuming only `@nseng-ai/objectives/api`, demonstrating one integration is fully reachable through a curated harness-independent API. Four ns extensions (`flow`, `handoffs`, `branch-context`, `herdr`) still contain `src/pi/` subpackages, `./pi*` exports, and `@nseng-ai/pi-runtime` peers on trunk, and nothing has tested whether their behavior is reachable through curated APIs. Those subpackages may reach private domain or command internals no package API exposes; extraction may expose real API design work and must not be bypassed with private imports.
