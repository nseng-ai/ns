...
- [~] Draft and approve the superseding package-ontology ADR and complete destination inventory. ADR 0045 and `references/package-destination-map.md` classify all 26 baseline workspace manifests, define the 34-package target, settle Pi extraction and owner nesting, enumerate identity/dependency/release consequences, and provide the approved replacement direction for `professional-repo-curation`. The user explicitly approved the full map and ADR on 2026-07-25. Remaining: apply that replacement direction to the parent Objective through its own tracking workflow before implementation begins. No package move or publication was authorized.
