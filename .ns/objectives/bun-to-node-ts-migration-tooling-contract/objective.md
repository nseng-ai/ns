...
- use separate pnpm-managed package-manager surfaces: `ts/` as the TypeScript workspace and `retired website files` as a standalone docs surface, with lockfile, script, deployment, Node-engine, and patch-handling expectations that later implementation should satisfy;
...
- pnpm remains a better fit for this repository's TypeScript workspace shape than npm because workspace coordination matters more than minimizing tooling count. The package-manager contract uses separate pnpm surfaces for `ts/` and `retired website files`, not one root workspace.
...
- Package-manager policy decisions include documentation install/build and Vercel command migration, but docs content/template cleanup remains downstream Bun-reference reconciliation work.
...
- Package-manager migration uses separate pnpm-managed surfaces: `ts/` as a pnpm workspace for `packages/*`, and `retired website files` as a standalone pnpm surface. Root remains orchestration-only through directory-scoped pnpm commands.
