...
- `retired website files` remains a standalone pnpm-managed surface with its own `retired website files` and `packageManager: pnpm@<chosen-version>` in `retired website files`.
- The repository root remains orchestration-only. Root `justfile` commands should use explicit directory-scoped pnpm calls such as `pnpm --dir ts ...` and `pnpm --dir documentation ...`, rather than introducing a root pnpm workspace solely for command dispatch.
...
- Deployment command migration belongs in this pnpm contract: root `vercel.json` should use `pnpm --dir documentation install --frozen-lockfile` and `pnpm --dir documentation run build`; `retired website files` should use `pnpm install --frozen-lockfile` and `pnpm run build`.
...
- `cd documentation && pnpm install --lockfile-only --ignore-scripts` resolved the package as a standalone pnpm surface.
...
- pnpm workspace migration owns `ts/pnpm-workspace.yaml`, `ts/pnpm-lock.yaml`, `retired website files`, `packageManager` fields, Node `engines`, root `justfile` command migration, Vercel install/build command migration, and pnpm patch metadata translation.
