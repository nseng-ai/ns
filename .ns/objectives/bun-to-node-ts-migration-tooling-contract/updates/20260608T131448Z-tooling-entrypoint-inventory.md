...
- Package-manager and workspace surfaces:
  - `justfile` routes TypeScript and documentation workflows through Bun: `ts-install`, `ts-check`, `ts-test`, `docs-install`, `docs-dev`, `docs-build`, and `docs-check` all call `bun`; `js-test` aliases `ts-test`; `link-planned-branch` runs `bun link` in `ts/packages/planned-branch`.
  ...
  - `ts/bun.lock` and `retired website files` are the active lockfiles. No `pnpm-lock.yaml` or `pnpm-workspace.yaml` exists at the repository root, `ts/`, or `retired website files`.
...
- Deployment surfaces:
  - `retired website files` is a separate private Astro/Starlight package with `astro dev`, `astro build`, `astro preview`, and `astro check` scripts; it carries its own `typescript: 6.0.3` dev dependency.
  - Root `vercel.json` installs and builds the documentation with `bun install --cwd documentation --frozen-lockfile` and `bun run --cwd documentation build`.
  - `retired website files` uses `bun install --frozen-lockfile` and `bun run build` for the documentation-root deployment mode.
  - `retired website files` documents the Bun-based Vercel install/build commands.
...
- The pnpm workspace contract must decide how to represent the `ts/` workspace, whether and how to include the separate `documentation` surface, how to replace the two Bun lockfiles, how top-level `justfile` and Vercel commands should invoke package-manager operations, and how to preserve the patched Pi dependency behavior.
...
- Should the package-manager contract combine `ts/` and `documentation` under one pnpm workspace, or keep documentation as a related but separately migrated surface?
