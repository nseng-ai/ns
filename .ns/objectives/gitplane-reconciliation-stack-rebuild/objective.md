...
- One coherent materialization-store snapshot containing generation cursor, all current records including tombstones, lineage, and a Pending Plan. New planning requires a completed snapshot with no Pending Plan; a matching plan replays, conflicts refuse replacement, and post-CAS residue is cleanup-only.
...
- Atomic Pending Plan persistence and a complete Reconciliation Plan prevent source/config reinterpretation. Matching retry replays, conflicting work is refused before artifact writes, and post-CAS residue is cleanup-only.
