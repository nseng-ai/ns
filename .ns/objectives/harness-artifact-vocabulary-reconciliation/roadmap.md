...
- [ ] Settle the replacement term for AREG's "managed artifacts" overlay sense (working candidate: "kind overlays") against CONTEXT.md conventions, then execute the mechanical rename across `skill-kind.ts`, `skill-kind-apply-plan.ts`, tests, and output strings. Flag machine-facing renames.
