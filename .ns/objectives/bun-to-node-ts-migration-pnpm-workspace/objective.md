...
This child Objective owns the pnpm migration for the repository's TypeScript package-manager surfaces: `ts/` as a pnpm workspace for `packages/*`, `retired website files` as a standalone pnpm-managed docs surface, and root-level orchestration that delegates into those surfaces without becoming a TypeScript workspace itself.
...
- migrate `retired website files` install/build/deploy package-manager commands to pnpm while keeping it a standalone surface rather than folding it into the `ts/` workspace;
...
- `retired website files` uses pnpm for install/build/deploy commands while remaining a standalone package-manager surface;
- user-facing or agent-facing docs that explain active TypeScript workspace and documentation commands match the migrated pnpm workflow;
...
- The settled package-manager contract remains valid: `ts/` is a pnpm workspace, `retired website files` is a standalone pnpm surface, and the repository root stays orchestration-only.
...
- Deployment configuration may have Vercel or Astro constraints separate from local package-manager commands.
...
- Local, CI, Vercel, and README command surfaces were migrated to standalone pnpm. If Vercel root-directory mode later needs host-side package-manager selection help, evaluate an explicit Corepack activation prefix without turning the repository root into a workspace.

## Closure

Completed on 2026-06-08. The Objective's pnpm package-manager slice is done: `ts/` is a pnpm workspace for `packages/*`, `retired website files` is a standalone pnpm-managed docs surface, and the repository root remains orchestration-only through directory-scoped `justfile` and CI commands.

Completion evidence is recorded across Semantic Updates for active Bun surface inventory, `ts/` workspace migration, root orchestration migration, documentation migration, pnpm command documentation, and final dependency/command evidence consolidation. The final local validation pass ran `just dprint-check`, `just ts-check`, `just js-test`, `just docs-check`, and `just docs-build`; pnpm emitted the expected unsupported-engine warnings because the local shell used Node `v24.2.0`, below the documented Node `>=24.12.0` baseline used by CI.
