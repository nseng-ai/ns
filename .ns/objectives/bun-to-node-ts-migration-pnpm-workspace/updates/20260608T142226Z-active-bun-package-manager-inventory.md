...
- `ts/bun.lock` and `retired website files` are the Bun lockfiles to replace with pnpm lockfile state for their separate package-manager surfaces.
...
- `vercel.json`, `retired website files`, and `retired website files` contain active documentation install/build command contracts and should migrate to pnpm while preserving `retired website files` as a standalone surface.
...
- Migrate `ts/` to pnpm workspace metadata and lockfile state before changing documentation commands, so dependency and patch behavior is understood first.
...
- Update Vercel commands and `asdl-dev` README examples when the corresponding pnpm commands exist.
