...
> **Status:** historical founding analysis for the `composable-command-core` Objective. Its `hostable(...)` examples were superseded when the standalone-tier collapse trigger fired on 2026-07-19; the current public executable model is raw or directly branded clinkr. See [the Semantic Update](../updates/2026-07-19-standalone-hostable-layer-collapsed.md). Where this document and `objective.md` disagree, `objective.md` is current. This was written as a counter-proposal to the retired `sdk-invocation-context` Objective's README-draft contract (deleted with that Objective). Later iteration also moved `NsContext` to `{ catalog }` only, dropped `env` virtualization, kept the event vocabulary in the SDK (not clinkr), and dropped `Host` inheritance in favor of composition. See [context-primitives.md](./context-primitives.md) for the per-primitive value analysis that started this.
