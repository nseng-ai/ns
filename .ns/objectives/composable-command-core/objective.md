...
- **Public command execution is raw or clinkr.** A core command definition carries either a process-shaped raw run or a directly branded `clinkr(...)` run. ns reads clinkr metadata and routes it in-process; raw commands retain the subprocess floor. There is no standalone public hostable execution tier.
...
8. **Naming (settled 2026-07-19, corrected by the collapse update).** The combinator is `clinkr(...)` — honest implementation naming; clinkr is first-party and owns the presentation seam, and its types become SDK-grade public API knowingly. The composable API exports `defineCommand` and `clinkr` under their plain names from `@nseng-ai/sdk/command`; the briefly introduced `hostable` export was removed when it failed to earn an executable consumer. The legacy main-surface `defineCommand` remains untouched until the migration verdict.
...
- The composable command API in the SDK: `defineCommand` core plus directly branded `clinkr(...)` runs whose metadata is readable by the catalog. Clinkr conversation types remain SDK-owned plumbing. The API lands beside `NsExtensionApi`; the legacy surface is untouched until the port verdict.
...
1. The composable API (`defineCommand` + `clinkr`) is exported from the SDK with clinkr metadata visible to catalog routing, and the SDK ships the default events→terminal renderer.
