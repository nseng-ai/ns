...
- **Public command execution is descriptor-declared raw or ns-owned composition.** Every descriptor command entry says `kind: "raw-command"` or `kind: "ns-command"` before lazy loading. Raw commands remain process-shaped programs; ns commands load a flat `NsCommandDefinition` that the host mounts through Clinkr in-process. The host never inspects a loaded value or callable brand to discover its route, and there is no standalone public hostable execution tier.
...
5. **The subprocess floor and declared route.** Raw commands are normal programs — `process.cwd()`, `console.log`, own their argv — and retain process-shaped execution. An explicit descriptor `kind` is the sole routing declaration: `raw-command` preserves that contract, while `ns-command` loads a flat SDK definition for in-process host mounting. Nothing pretends an arbitrary callable is hostable.
...
8. **Naming and ownership (settled 2026-07-19, corrected by descriptor-owned routing).** Descriptor entries are explicitly `ns-command` or `raw-command`. The ns author contract is a flat `NsCommandDefinition`, normally created by `defineCommand(...)` from `@nseng-ai/sdk/command`; there is no `nsClinkrCommand`, command brand, `DefinedCommand`, or runtime notion of “composable.” Clinkr remains generic parser and presentation mechanics used by the ns host. The legacy main-surface `defineCommand` remains untouched until the migration verdict.
...
- The ns command API in the SDK: explicit descriptor `kind` variants plus a flat `NsCommandDefinition`/`defineCommand(...)` author contract whose metadata is carried directly. Omitted input schemas become `z.strictObject({})` at the host registration boundary. The ns conversation types remain SDK-owned plumbing layered over generic Clinkr mechanics. The API lands beside `NsExtensionApi`; the legacy surface is untouched until the port verdict.
...
1. Descriptor entries declare `ns-command` or `raw-command`; the flat `NsCommandDefinition`/`defineCommand` API is exported from the SDK with command metadata directly available to the declared ns route, and the SDK ships the default events→terminal renderer.
...
- **Risk — an ns-command declaration can lie.** A flat `NsCommandDefinition` can still import `node:fs` or touch `process.cwd()`. The descriptor kind is a reviewable routing contract, not proof of isolation; `submit`'s port (completion criterion 4) sets the precedent for how ambient needs are expressed honestly.
