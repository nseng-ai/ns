...
The current durable handoff system is layered over Branch Memory rather than implemented in one place. The inventory found one intended storage convention, several coordinated user-facing surfaces, and one compatibility/legacy signal that should be handled explicitly during contract design. The inventory has since been refreshed to include the landed CLI-only `handoff delete` surface for exact-slug single-handoff deletion.
...
| `asdl-handoff` CLI                | Implements `handoff list`, `handoff delete`, and `handoff gc`. `handoff list` reads the `handoffs` Namespace and summarizes branch, branch state, slug, key, Entry Locator, and updated timestamp. `handoff delete` removes one exact-slug handoff from the target branch, rejects `.md` storage-key suffixes as user-facing slugs, prompts unless forced, and can target non-current or locally deleted branches with `--branch`. `handoff gc` previews or deletes handoffs for deleted local branches. | `packages/asdl-handoff/src/asdl_handoff/cli/handoff/`; `packages/asdl-handoff/tests/scenario/test_handoff_cli.py`; `skills/handoff/references/diagnostics-admin.md`; `docs/pi/handoff-artifacts.md` |
...
- Single-handoff deletion is now a first-party CLI admin operation: use `handoff delete [--branch <branch>] [-f|--force] <slug>` for exact-slug deletion, keep `/handoff:delete` absent, and reserve direct `brmem delete --namespace handoffs` for storage diagnostics/recovery fallback.
...
- `uv run pytest packages/asdl-handoff/tests/scenario/test_handoff_cli.py` — passed after the `handoff delete` CLI/admin surface landed.
...
The first roadmap item remains complete and is now current through `handoff delete`: current handoff and Branch Memory usage has been inventoried across skills, Pi extension commands, the `asdl-handoff` CLI, the generic `brmem` storage primitive, tests, and non-authoritative live local examples.

The inventory now distinguishes `handoff delete` from `handoff gc`. `handoff delete` removes one exact-slug handoff, is CLI-only, can target an explicit branch, and uses prompt/force semantics; `handoff gc` remains deleted-branch cleanup.

The next useful semantic step remains contract design. Delete evidence does not by itself close the final Entry Key convention or the fate of `session-artifacts/handoffs/...` compatibility normalization.
...
- Exercise a normal pickup path and at least one failure/staleness path after the contract-alignment slice lands, including deleted branch, missing entry, ambiguous selection, overwrite risk, stale artifact content, and deletion/destructive cleanup risk as relevant.
