...
- **npm-module source model.** How the widened `HarnessArtifactEntry.source` / `ProvisionSourceProvenance` represents an npm-module source (package name + resolved path + version?), and how discovery resolves a module's `ns` declaration for non-extension npm packages vs the existing kernel extension-discovery path. Binds the shared core's public API — decide with the umbrella before building on it.
- **`ns update` command-surface placement** — this record vs kernel/extension-lifecycle (the umbrella's standing open placement question; this record ships the reconcile/provision slice either way).
