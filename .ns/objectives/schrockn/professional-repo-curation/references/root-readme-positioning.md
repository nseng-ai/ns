...
- **why-ns.md** — the manifesto. Persuades by argument: a concrete slop failure story,
  the model-capability-amplifies-damage claim (lead argument), the CPU/algorithm analogy
...
1. **The core (the headline claim).** Universal problems, addressed by four primitives —
   every team has the problem, no incumbent owns the solution: **objectives, handoffs, flow, pr-feedback**.
...
3. **Harness integrations — pi is the first example.** The toolkit is harness-neutral
   ...
   harness = writing adapters, not re-implementing the core.
4. **Tools.** Standalone executables needing no lifecycle adoption: herdr (a tool with a
   pi extension, not an ns extension), areg, packagechk. May ship their own
   harness extensions.
5. **Skills — packaged, version-controlled context.** The lowest adoption rung (one-file
   ...
   on-ramp. Framing guard: skills are how you try ns; the core is why you stay —
...
1. Hero: nonslop engineering; the four parts of the core as felt problems; "not another
...
- No second taxonomy: do not teach a lifecycle-stage framework (Intent → Context → …)
  in the README; the presentation taxonomy is the only one a reader learns.
