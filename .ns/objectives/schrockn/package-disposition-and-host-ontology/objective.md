...
- A coordinated stack can provide reviewable commits while preserving an atomic landing boundary. The reorganization is implemented as four reviewable stack points: design, cutover preparation, `@nseng-ai/ns` product consolidation, and one atomic package-cutover boundary. Each point passed its validation before submission; the cutover boundary landed on `master` (commit `1d3c65b47`).
...
- **Hidden Pi coupling.** Still the live risk of this Objective, now exercised in the confirming direction by complete adapter boundaries rather than independently landed precursors. The Objectives extraction landed on trunk as `@nseng-ai/pi-ns-objectives` consuming only `@nseng-ai/objectives/api`. Handoffs, Branch Context, Herdr, and Flow now have equivalent end-to-end implementation evidence on the current stack: each adapter owns its Pi registration, presentation, discovery, tests, and parity while consuming the corresponding extension through curated package interfaces; each extension removes its Pi surface and Pi Runtime coupling in the same atomic slice. Flow additionally confirms that a cohesive API can hide submit-recovery descriptor plumbing and stack-squash gateway construction while direct discovery preserves invocation-time fresh CLI loading. Curated API promotions required by an extraction must land in the same PR that removes the extension's Pi surface; API-only transitional PRs are rejected as confusing ownership states. Pi-native internal extractions and final structural enforcement remain, and future work must not bypass package interfaces with private imports.
- **Large atomic conflict surface.** Retired for the reorganization boundary: the complete inventory moved in one atomic stack point, whole-tree content was preserved and measured (4984 tracked files before and after, identical blob multiset apart from changed path literals), and the boundary landed on `master` as commit `1d3c65b47` with no mixed legacy state. It remains live separately for the deferred Pi extraction, which creates the remaining adapter and internal packages and moves Pi code out of four extensions.
- **Identity ripple.** De-risked for the identities that changed on 2026-07-26 — `@nseng-ai/pi` → `@nseng-ai/pi-runtime`, `@internal/pi-editor-mods`, and `@internal/review-reinvention-scanner`. The `pi` rename was scripted with a `@nseng-ai/pi(?![-\w])` lookahead rather than a bare substring, which would have corrupted the `pi-editor-mods` and `pi-tools` occurrences. It also materialized in a form path checks did not cover: per-package test/build scripts and every tsconfig `extends` encoded their own depth, source-dev extension discovery silently stopped resolving until the SDK resolver was fixed, and 19 depth-coupled `import.meta.url` and fixture path literals needed resolving individually. Still live for the remaining `pi-ns-*` adapters the deferred extraction creates; the landed `@nseng-ai/pi-ns-objectives` identity introduced no observed ripple.
- **Guard blind spots.** Half-addressed on 2026-07-26. Topology, identity, scope, and dependency closure are enforced by `NS_TS_PACKAGE_DISPOSITION_TOPOLOGY` and mutation-tested rather than merely observed passing. The focused structural checks for forbidden Pi surfaces in ns extensions are not written and are deferred with the extraction, so the ns-extension/Pi boundary currently has no mechanical enforcement at all. Two blind spots also proved real in practice: the retired hand-maintained public-package list had drifted silently because its assertions only run inside release commands, and the broken source-dev extension discovery was invisible to `just`.
- **Path-ordering fragility in the shared test lane.** Surfaced 2026-07-26. Two clinkr tests neutralized an ambient `COLORTERM` but not `FORCE_COLOR`, and reordering test files exposed the defect because the shared lane runs `isolate: false` and tests share a process. Both were fixed, but no general guard against ambient colour environment exists, so any future change that renames or reorders test paths can surface further latent cross-test coupling.
...
- **Parent-guidance contradiction.** De-risked in the stack: `professional-repo-curation`'s objective, roadmap, and always-loaded orientation are reconciled to the approved disposition ontology and distinguish the implemented branch state from what has landed on `master`, so active parent guidance no longer directs agents toward the flat-incubator destination. Historical ADR 0044 and immutable updates remain unchanged as time-in-place records.
