...
- one filesystem adapter owning traversal and dynamic ESM imports while lowering filesystem and programmatic sources into the same lazy topology model, routing traversal, validation owner, and command-dispatch runtime;

The public immutable-builder details in the following historical bullets were later superseded by the narrow scoped callback builder recorded in `decision-record.md`; immutable nodes, provenance, publication, and definition lifecycle are private implementation details. The heading and discussion-gated list below likewise capture the audit-time state rather than the current settled state.
