...
The autoslot slice is implemented on the current branch in commit `f94abe726`. Flow now owns checkout domain logic that invokes `ns slot checkout --format json` through the existing injected command-exec seam for current-commit and named-branch modes, validates the command envelope, preserves valid Slots domain failures, and turns execution or malformed-protocol failures into Flow-owned typed outcomes. A subsequent ownership correction removed Flow's temporary directive filesystem implementation: JSON controls stdout representation, while Slots owns the established parent-shell wrapper/environment protocol and returns structured inactive, written, or failed navigation evidence. Flow renders a failed directive write as a non-fatal warning after successful placement. The initially delivered high-level checkout Gateway was subsequently removed so tests fake the external command capability beneath Flow-owned policy.
