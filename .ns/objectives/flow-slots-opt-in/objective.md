...
- **Resolved contract — CLI-boundary side effects.** Flow owns two-mode current and named-branch checkout policy above its existing command-exec seam. Autoslot builds and invokes the Slots JSON command with clipboard copying disabled, validates the result envelope, and writes the established parent-shell cd directive only after validated success. Valid Slots failures preserve their `errorType` and message; execution and malformed-protocol failures use explicit Flow-owned typed errors rather than throwing through autoslot presentation. No high-level checkout Gateway substitutes for this Flow-owned logic.
