...
What you see now — do not copy: this Objective's `references/` and pre-rename updates use retired `@sdl/*`/`@ns/*`/`sdlcc` names (the scope is now `@nseng-ai/*`) — re-map before use. Every neutral structural-cleanup row is complete; the closure pass routed the remaining capability-owned/design-sensitive rows (Flow submit/topology, aretro, ccc cmux dispatch, objective validator, plan-attachment) to their owning capability context, not to neutral infra work here.
...
Closed: see objective.md `## Closure` and roadmap.md routed-at-closure rows.
