...
Foundation now exposes `defineClinkrAppCli` as a durable modern lifecycle beside the unchanged legacy `defineCli` path. The modern lifecycle reads package metadata once, prepares dependencies before constructing a fresh contextful `ClinkrApp`, propagates rewritten arguments, invocation context, structured stdin, and ANSI capability, supports handled preparation and application error policy, and owns direct process-entry handling. Injectable output sinks and Clinkr's in-process test runner use one scoped, Clinkr-owned process-writer interception primitive. Override-backed runs remain process-global and must be awaited sequentially; the primitive rejects overlapping or nested interception and restores replaced writers in `finally`.
...
- Clinkr's isolated interception suite covers string and byte normalization, per-stream interception, restoration after success and rejection, empty-request rejection, and actionable overlap rejection while preserving the outer owner's restoration path. Foundation's modern lifecycle suite covers package metadata/runtime diagnostics, preparation inputs and handled exits, fresh app construction, rewritten args/context, stdin JSON, ANSI rendering, stdout/stderr capture and restoration on success and throw, handled/declined errors, and `runIfMain`.
- Brmem's fake-driven scenarios preserve command behavior, status/exit envelopes, confirmation semantics, hidden-route invocation, and CLI metadata surfaces. The list scenario now proves ANSI capability through the production filesystem-backed command and renderer rather than a copied test renderer.
...
- `just ts-test-isolated`: 6 files, 21 tests
