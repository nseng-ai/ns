...
- **Runtime filesystem discovery.** The common authoring path depends on runtime traversal and ESM imports. Keep `metadata.ts`, group module top levels, and complete group `group()` definitions cheap; import a command's `command.ts` only after selection; centralize resolution in the filesystem adapter; and verify recursive lazy command behavior without introducing generated manifests or production codegen.
...
- The primary README and common authoring path are filesystem-first: a Clinkr-owned `src/cli/` directory contains only `app.ts` and Clinkr route files/directories, while domain operations, gateways, and reusable application modules remain outside it; direct route directories define the command path, `group.ts` marks named groups, and the required `metadata.ts` + `command.ts` pair defines named or default commands according to its directory peers. A group directory contains one eager `group.ts` exporting a cheap, complete `group(): ClinkrGroupDefinition`; it has no command metadata sidecar or lazy second group-definition function, and its children come from the filesystem. A command's eager, cheap `metadata.ts` exports explicitly typed `metadata(): ClinkrCommandMetadata`; its selected-only `command.ts` exports async `command()` authored with a generic `defineCommand({...})` helper so schemas drive handler and renderer inference. Selection includes command help, schema introspection, and option-value completion, all of which may load `command.ts` but never run its handler. `command.ts` may use ordinary top-level implementation imports because the module itself is selected-only. Top-level/group help and command-name completion import only eager command metadata and group definitions, not command implementation. Runtime discovery rejects incomplete command pairs and mixed invalid shapes, preserves the sole group shape and adjacent default-command pair, and requires all command/group files and directories to ship intact. It lowers through a filesystem adapter into the same immutable `ClinkrApp` and builder model—there is no code generation, manifest, compatibility model, or second command-dispatch implementation. The settled bootstrap is `createClinkrApp({ name, commandDirectory, version?, runtimeInfo?, completion? })`, where `commandDirectory` is an absolute filesystem path and the colocated Node 24+ form is `import.meta.dirname`. Context remains invocation-owned through context-free or contextful `app.run(...)`/`app.complete(...)`; Foundation creates a fresh app after `prepareRun` and may combine filesystem and programmatic commands through the builder seam. Public async immutable builders, terminal define/import, provenance checks, transactional loading, per-app success caching with retry, app-only execution/completion, and fresh Foundation apps remain the canonical lower seam and an advanced escape hatch for programmatic topology, extension mounting, custom loading, and framework integration. The exact command/group helper type spellings remain provisional, and the current constructor/group-execution model is still a clean-break migration mismatch.
...
- The outcome-and-rendering reconciliation cluster is approved. Clinkr owns one command/outcome model spanning `resultSchema`, `negativeSchema`, `failureSchema`, and `usageErrorSchema`; it drives handler outcome types, runtime validation, machine-envelope construction, and the top-level discriminated JSON Schema. Omitted command schemas require handler-returned outcomes to be bodyless, supplied command schemas require and validate handler data, and `z.any()` is the explicit untyped escape hatch. Clinkr-owned Commander and request-validation usage errors remain data-bearing independently of a command's `usageErrorSchema`, and automatic machine-schema generation composes those exact built-in alternatives with the command-owned usage contract. Bodyless `ok()` emits no human result body or JSON `data` field, while JSON still emits its status envelope. Outcome-schema violations are programmer errors that propagate to app crash policy. The public throwable `ClinkrFailure` API and Clinkr's special exception-to-failure conversion are approved for removal: expected operational failures are explicit returned outcomes, deliberate adaptation belongs in applications or Foundation, and unexpected exceptions propagate unchanged.
