...
**Approved disposition:** **Reconcile implementation and callers through the foundational clean break.** Introduce the immutable app/group/command builder runtime described in the decision record, then place a runtime filesystem adapter over it as the primary README and common authoring path. The app owns a transparent root scope and is the only executable/completion host; groups remain non-executable organization. Direct directories, cheap complete group definitions, and required eager command metadata modules drive recursive selection, while selected-only command modules lower definitions into builders. Migrate through Foundation and SDK rather than forcing every feature package to become an executable. Builders remain public as the advanced escape hatch, not the main README tutorial.
...
- direct filesystem hierarchy with one eager cheap `group.ts` per group and a required `metadata.ts` + `command.ts` pair per command: `metadata.ts` exports cheap typed metadata, while selected-only `command.ts` exports the definition authored through `defineCommand({...})` and may use ordinary top-level implementation imports;
...
- top-level/group help and command-name completion import only eager command metadata and complete group definitions; selected command help, schema introspection, and option-value completion load the selected definition without running its handler; shared in-flight loads, per-app success caching, retryable failures, and fresh Commander trees per run;
- strict filesystem validation rejects either half of an incomplete command pair and invalid mixed shapes; a group retains its one-file eager `group.ts` contract and may have an adjacent required pair as its default command; packaging preserves every command/group directory and both command files;
...
This approval now has user-authorized TypeScript reconciliation on the shared branch, while this record update remains documentation-only. It does not authorize a compatibility layer, two command-dispatch implementations, manual application argv pre-dispatch, or a manifest fallback. The README leads with filesystem command authoring and mentions builders only as a separately documented advanced escape hatch. Command/group files and directories must ship intact; bundling and single-file packaging remain an explicit risk that may require builders or a later adapter. The `app.ts` bootstrap API is settled in `references/decision-record.md`.
