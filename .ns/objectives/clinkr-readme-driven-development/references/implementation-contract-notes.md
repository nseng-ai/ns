...
- `confirmOrUsageError(...)` belongs to the modern command/outcome surface: it stays on `/app` during quarantine and moves to root with that surface at final cutover. It owns only translation from interaction states to modern outcomes.
