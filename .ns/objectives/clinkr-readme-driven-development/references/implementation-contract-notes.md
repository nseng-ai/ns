...
- Output sinks belong to one invocation. Structured/framework execution delivers one structured-text presentation to its configured sink; raw execution writes only to its configured stdout/stderr byte sinks. The standalone terminal adapter defaults both paths to the corresponding process streams.
- Embedding layers must adapt these explicit sinks rather than intercepting or allowing ambient process writes. Foundation owns the generic adapter from Clinkr's sink shapes to its CLI-entrypoint callbacks. Its raw-byte-to-text adapter must use one streaming UTF-8 decoder per invocation and flush it at the end; decoding each byte chunk independently can corrupt a multibyte code point split across writes.
- Pi owns the next host boundary: it captures the Foundation callbacks and presents the result through Pi messages/widgets instead of allowing the embedded CLI to write directly to the terminal. Pi must sanitize captured terminal controls before TUI presentation. This is renderer-specific host safety, not Clinkr rendering or Foundation adaptation: a clear-screen sequence that is valid in a standalone CLI can clear the physical terminal behind Pi, desynchronize Pi's differential renderer from the actual screen, and cause later TUI updates to be corrupted or drawn in the wrong place.
- The ownership statement for promoted documentation is: Clinkr owns command rendering and stream selection; Foundation owns sink-shape and streaming byte-to-text adaptation; Pi owns host presentation and renderer-specific safety.
...
- Migration mechanics (temporary): the rebuild is quarantined under `@nseng-ai/clinkr/app` (runtime, command definitions, Design A outcomes, confirmation helper) and `@nseng-ai/clinkr/app/testing` (in-process app test runs) so shared legacy modules stay at their pre-rebuild shape for existing consumers. Nothing under `src/app/` imports legacy `exit.ts`, `emit.ts`, `group.ts`, `completion.ts`, or legacy confirmation logic, and legacy modules do not import from `src/app/`. The retained modern interaction seam in `src/interaction.ts` is not legacy confirmation logic; app outcome policy may depend on its types. The roadmap's tail deletion row flips the root export to the new surface and removes these temporary subpaths; the README-draft's `/app` import paths convert to root at that flip.

## Interaction and confirmation ownership

- `ClinkrInteraction` is the application-controlled semantic seam. It remains a two-method interface for confirmation and interactivity; applications place it in invocation context, and Clinkr dispatch never constructs it or prompts automatically.
- `createClinkrInteraction(...)` and `resolveClinkrInteraction(...)` are the shared terminal-adapter construction path. They retain line parsing, default-answer, stderr prompt, retry, EOF/abort, injected-stdin, and TTY behavior without owning command outcomes.
- The package root is the final public entrypoint for the interaction interface and terminal adapter. Do not add an `interaction` subpath or re-export the constructors from the temporary `/app` quarantine.
- `confirmOrUsageError(...)` belongs to the modern command/outcome surface: it stays on `/app` during quarantine and moves to root with that surface at final cutover. Its simple form retains the default modern translation from interaction states to outcomes. Commands that require domain-specific semantics may provide explicit declined and aborted outcome mappings plus actionable non-interactive authorization metadata; the helper still owns the single interactivity gate and prompt while the command owns those mappings. Do not add a raw modern counterpart to the legacy confirmation helper.
- `requireInteractiveOrUsageError(...)` and `confirmInteractiveOrUsageError(...)` are legacy-exit migration helpers. Preserve them only until callers express non-interactive bypass/gating and confirmation results through modern command outcomes; do not carry them into the final root.
- `@nseng-ai/clinkr/testing` owns strict general interaction fakes because callers test application/domain behavior through the same seam without necessarily constructing a `ClinkrApp`. `@nseng-ai/clinkr/app/testing` remains scoped to app invocation capture.
- Before deleting legacy confirmation gates, add direct modern evidence for non-interactive refusal, confirmation, decline, abort, default-no requests, unexpected prompts, and unused scripted answers. Preserve at least one real terminal-adapter test for prompt/retry/EOF behavior and representative host-adapter evidence.
