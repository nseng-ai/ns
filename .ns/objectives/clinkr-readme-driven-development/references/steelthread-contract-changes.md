...
2. **Context-free typing.** `requiresContext: true` is the runtime-visible and type-level discriminant on contextful apps and structured command definitions; omission means context-free. `handler(request)` and `clinkr.run(args)` are real type/runtime contracts, while a contextful tree uses one homogeneous typed context and requires it per invocation. The repeated boolean is accepted so schema inference remains intact and dynamically loaded definitions can be validated without function-arity inspection or a synthetic null context.
