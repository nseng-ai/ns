...
- [~] Continuously classify or remove reintroduced raw optional-undefined candidates.
  ...
  Current state (verified 2026-07-03 at trunk `5668ac5`): repo-wide (`ts`) raw count is 2 — both `namespace?: string | undefined` request fields in `ts/packages/infra/brmem/src/ref-layout.ts` — with 96 typed contracts, 0 legacy markers, and 2450 normalization/check lines.
