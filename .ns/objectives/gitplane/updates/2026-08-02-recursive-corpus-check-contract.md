...
Raw working-tree inventory and candidate reads belong to a check-only `CorpusCheckGateway`. The canonical `ArtifactGateway` continues to expose valid artifact-domain boundaries and complete snapshots for reconciliation rather than admitting malformed corpus entries.
