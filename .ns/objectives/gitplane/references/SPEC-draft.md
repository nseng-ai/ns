...
3. If a Pending Plan exists, replay it or clean post-CAS residue before new planning. If the invocation requests a different target, complete the Pending Plan without replacing it, reread the completed materialization snapshot, and continue to the requested target in the same invocation. This recovery is bounded to the one inherited Pending Plan; a new plan that appears after recovery is a conflict.
...
Writes are non-transactional. Partial materialization can be visible before cursor CAS, but matching sequential retry replays the Reconciliation Plan with stable identities and converges. The Gateway protocol permits only one Pending Plan per source: serialized invocations atomically reuse or reject that plan before artifact writes. The engine completes inherited pending work before it plans a different requested target, and generation CAS protects each completion boundary, including ABA when a commit is revisited. The native SQLite adapter is a single-writer local/reference adapter in v1; concurrent SQLite writers or simultaneous replayers are unsupported. Source leases remain out of scope.
...
**Completion and visibility.** Successful generation CAS is the completed-materialization boundary. Consumers requiring snapshot freshness check cursor commit and generation. Post-CAS cleanup failure leaves recognizable attempt residue; a later invocation performs cleanup only and never replays materialization or events. Results report bounded lifecycle counts, prior/resulting cursor, `cursorAdvanced`, replay/cleanup-only indication, and inherited Pending Plan recovery directly; repair and obsolete event-reconstruction status are not retained.
...
**Pending Plan.** One unresolved Reconciliation Plan is permitted per source. Matching work replays it verbatim without source or configuration rereads. An invocation for a different target completes the inherited plan, rereads the completed snapshot, and then reconciles the requested target without replacing the inherited plan. Residue whose cursor already matches the plan's Resulting Cursor is cleanup-only. A new plan that appears after bounded recovery is a conflict. Existing incompatible pre-release stores are rejected with recreate guidance; v1 defines generation-aware cursor, Reconciliation Plan, and event records directly and provides no migration.
...
| `attempt-conflict`               | different requested target with a Pending Plan                  | Completes the existing plan, then reconciles the requested target in the same invocation without replacing either plan.          |
