...
2. Atomically read the complete Gitplane-owned materialization snapshot: cursor, all current records including tombstones, lineage, and any pending attempt. Operator-owned target rows are not planning authorities.
3. If an attempt is unresolved, replay an exact matching frozen plan, reject conflicting work, or clean post-CAS residue before deriving new work. Only a completed snapshot with no unresolved attempt is a valid planning baseline.
4. Validate the complete target corpus and derive one deterministic semantic plan from target facts, completed control state, and kind registration. V1 always performs a complete scan.
5. Atomically persist the complete frozen plan as the sole pending attempt for the source before materialization.
...
| `attempt-matching-replay`        | matching unresolved attempt                                     | Replays verbatim with stable event IDs/sequences.                                                                                |
| `attempt-conflict`               | different unresolved attempt                                    | Refused before artifact writes; existing attempt is not replaced.                                                                |
| `attempt-post-cas-residue`       | cursor advanced; attempt remains                                | Cleanup only.                                                                                                                    |
...
Lowercase Base32 uses the Crockford alphabet without padding. The frozen attempt fixes generation and attempt identity, so retries reproduce the same event ID and sequence while a later visit to the same commit and event type receives a distinct ID. Each inserted event receives a store-assigned monotonic sequence scoped to its source.
