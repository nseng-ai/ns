...
- [x] Cursor-diff reconciliation and `gitplane reconcile <commit>`: implemented complete cursor-tree→target-tree planning, move/revise/delete/restore and one-way classification transitions, deterministic idempotent control/target/event writes, durable retry baselines, cursor-last compare-and-set, cleanup-only recovery, and `--full` initial sync/repair across equal, older, and divergent targets.
  - Evidence: public-core fake scenarios cover generic/classified creation, moves and revisions, deletion/restoration, schema and classification rules, target-wide duplicate IDs, zero-write planning failures, deterministic ordering, every operational write boundary with retry, cursor mismatch/failure, cleanup recovery, competing-target refusal, and full-repair convergence. Focused Gitplane and SQLite checks, default/integration/isolated suites, TypeScript style guard, Objective validation, and final `just` pass.
