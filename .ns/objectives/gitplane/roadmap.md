...
- [ ] Recursive discovery, optional kind registration, and `gitplane check`: implement the settled validator-free, source-only contract with invocation-relative root resolution, symlink-safe nesting-first discovery, the fixed deterministic corpus finding set, optional classified-kind/schema registration, and completed-versus-operational CLI result handling without storage or history access.
