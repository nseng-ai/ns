...
- Deterministic recursive SHA-256 content digests and `gpr_` revision IDs, plus deterministic `gpe_` event IDs. Revisions are immutable content snapshots with a first-observed Git locator and file-digest manifest; raw bytes remain in Git.
...
- Artifact-domain access behind one canonical `ArtifactGateway`, with local creation, commit/history facts, discovery, complete recursive snapshots, and commit diffs; creation consumes only its narrowed create operation. Storage sits behind a complete operation-level `MaterializationStoreGateway`. One adapter owns both control records and target-table writes. SQLite is the test/local implementation, not a deployment commitment.
...
- Both workspace packages exist with the documented topology, package-local clock seam, canonical artifact/store gateways, in-memory fakes, and SQLite reference adapter.
...
- **First-observed Git locators depend on retained history.** Raw bytes become unavailable if the referenced commit is removed; object-store replication remains an explicit future upgrade.
