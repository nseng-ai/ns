...
- [x] Settle the layered extension-resolution contract and user configuration model. ADR 0051 specifies the single XDG config path and extensions-only interpretation, manifest package-name identities, reserved built-ins, same-scope collision exclusion, whole-package project replacement for shared identities, command-path precedence for different identities, failure isolation, and command availability versus project activation.
- [ ] Add user-scope extension discovery and catalog composition. Load and validate user descriptors separately from project descriptors; reserve built-in paths; suppress user candidates when a project declaration establishes the same package identity; then merge preinstalled/user/project command paths while preserving lazy selected-command loading and actionable source-labelled diagnostics. Starting seams: `ts/packages/public/sdk/src/extensions/registry.ts`, `command-registry.ts`, `declared-descriptors.ts`, and the existing `LoadNsCommandCatalogOptions.homeDir`/`env` inputs.
