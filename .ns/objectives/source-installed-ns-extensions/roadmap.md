...
- [x] Extend extension lifecycle operations with `--scope user`. Project remains the default. Local user install/list/update/uninstall use the single XDG config, canonical absolute declarations, byte-preserving optimistic edits, command-only descriptor validation, and no project activation. Until user-managed npm storage lands, npm mutation requests fail actionably while list isolates hand-authored npm declarations as unavailable rows.
