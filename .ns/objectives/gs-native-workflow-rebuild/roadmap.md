...
- [ ] GS-native restack-resolve vertical slice — use `code-gt-restack-resolve` as user-outcome evidence, experimentally settle whether gh-stack v0.1.0 public commands can restack and resume conflicts without unsafe implicit network/GitHub effects, then implement `ns gs restack-resolve`, its portable GS skill, and `/ns:gs:restack-resolve`. Add only the narrow provider adapters, schemas, semantic facts, fakes, and host integration this command proves necessary; preserve explicit scope, Slot safety, sequential conflict resolution, human escalation, no implicit abort, observed postconditions, and forward-only recovery without carrying Graphite-specific mechanics into GS.
  - Evidence: the GS README records observed provider behavior and starting-state/recovery semantics before implementation; tests cover clean completion, an existing interrupted rebase, resolvable and escalating conflicts, exact-version drift, malformed/provider failure, Slot conflicts, and partial or ambiguous mutation; CLI, skill, and Pi parity/routing checks pass.
