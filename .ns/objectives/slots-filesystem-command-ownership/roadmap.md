...
- [x] Migrate `slot checkout` command ownership and completion wiring in one PR, retaining `co` only as filesystem metadata. Runner checkpoint `34b8d415de68139f3afa9f3c36a5ebbfe2ef0712` colocated the typed command and completion assembly and moved checkout behavior coverage to the production filesystem harness; focused Slot typecheck and all 355 Slot tests passed.
- [x] Migrate `slot goto` command ownership in one PR. Runner checkpoint `a4f2cb869e243fd9b4c033753760ca23f0504181` colocated the typed command assembly, preserved format-sensitive shell directive behavior through the route-neutral adapter, and moved goto coverage to the production filesystem harness; focused Slot typecheck and all 354 Slot tests passed.
