...
What you see now: Herdr derives trunk from the cached origin/HEAD git fact (no `gt trunk`), only after an implementation command selects the Local-trunk basis. Objective Runner still gates on Graphite tracking, Flow trunk paths use `gt` for trunk discovery, Branch Context wires Graphite eagerly, and Flow submit assumes the current stack — all slated for removal or explicit selection.
...
Active slice: replace the Objective Runner Graphite-tracking gate, then continue through the other named ambient couplings in roadmap order.
