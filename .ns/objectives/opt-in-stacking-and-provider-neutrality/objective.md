...
- ns works fully with no stack provider configured across the in-scope workflows: Branch Context, Herdr, checkpointing, trunk operations, and single-branch submit/land/push operate on plain git plus GitHub gateways with no Graphite gateway constructed.
- The three in-scope named ambient couplings (Herdr startup, generic Flow trunk discovery, eager Branch Context wiring) are gone, each verified by tests that exercise the workflow without Graphite available. The Objective Runner coupling remains explicitly parked pending redesign of that mechanism.
