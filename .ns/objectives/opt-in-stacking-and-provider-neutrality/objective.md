...
Two external systems bound the design. The official `github/gh-stack` extension (revalidated at v0.1.0, with machine-readable `gh stack view --json`) remains the intended first alternate provider for reconciliation and publication in a follow-up Objective. By user-approved course change, this Objective also delivered its narrower local topology-inspection and branch-creation consumer for explicit GS Branch Context workflows; that does not imply submit, land, reconciliation, or publication parity. Jujutsu (jj, colocated mode) remains the extensibility stress test: we never build a jj adapter, but every neutral contract must be satisfiable by a motivated third party assembling jj support from extensibility surfaces alone.
...
- **Building gh-stack reconciliation or publication adapters.** Those remain follow-up work; this Objective's user-approved GS addition is limited to local topology inspection and branch creation for Branch Context (its `sync`/`link` semantics deliberately do not map 1:1 onto `gt`).
...
- `gh stack view --json` remains stable enough for the shipped v0.1.0 local topology/creation consumer, while `sync`/`link`/publication semantics remain follow-up design inputs. gh-stack is pre-1.0, so drift remains an active risk and must be contained inside the package-owned consumer gateway.
...
- **Scope creep toward the follow-up.** The user explicitly approved a course change to ship GS local topology inspection and branch creation now. Reconciliation, submit, land, and publication remain parked; command/docs boundaries must not imply parity beyond creation.
