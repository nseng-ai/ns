...
- Herdr dispatch remains explicitly Slots-required, while optional label enrichment remains successful and unprefixed when Slots is absent; complete ns extension API factory, configuration, or programming failures propagate before core operation or rename.
...
- **Assumption — complete ns extension API composition can serve Pi-hosted Herdr.** Every Herdr Pi host supplies a factory for the complete API shape used by CLI execution from the effective project and preinstalled catalog. Each relevant command handler constructs it from exact `ctx.cwd` before entering core; Herdr core receives only the resolved required boolean.
