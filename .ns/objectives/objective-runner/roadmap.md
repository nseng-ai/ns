...
- [x] Define `ChildSessionGateway` (streaming: event iterable + outcome promise, harness-neutral request/outcome) in `@ji/objective` and ship `FakeChildSessionGateway` in the package testing surface.
...
- [x] Ship the legacy blocking `ji objective exec runner-step <slug>` with `--recover`, `--guidance` (inline or `@path`), `--model`, `--timeout`; clinkr exit codes; checkpoint-only stdout and streamed stderr progress.
      Evidence: `src/ji/commands/exec-runner-step.ts`, scenario suite `test/scenario/exec-runner-step.test.ts`. The `--guidance` file-form question resolved as one flag accepting `@path`. Legacy per ADR 0024; deleted with the final slice.
...
- [x] Decompose the step into Pi-free bookends per ADR 0024: `ji objective exec runner-begin` (LBYL preconditions, report-path hygiene, step facts + subagent prompt) and `ji objective exec runner-finish` (fail-closed facts/report validation, unchanged gate, runner-owned commit, two-zone checkpoint), with the implementation session moved to a parent-dispatched harness subagent and the child report moved to a JSON file outside the worktree.
