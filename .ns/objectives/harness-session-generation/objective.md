...
- Direct execution is allowed when: the slice advances one open implementation row within its stated evidence and compatibility boundaries, including the foundation exec migration, foundation harness API/private engine/adapters, unified text-generation steelthread, provider-by-provider Reviews migration, or final cleanup/documentation.
...
- The real command channel does not currently distinguish timeout from caller cancellation. **Breaking migration decided**: replace foundation `ExecResult` with an exhaustive discriminated union for exited, spawn-failed, cancelled, and timed-out outcomes and migrate callers atomically; do not preserve legacy compatibility fields or widen `NsExecOptions`.
