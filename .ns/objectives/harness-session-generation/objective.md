...
- The final architecture and migration behavior are documented sufficiently for another capability to choose direct inference, isolated harness generation, or a read-only-agent session without learning provider CLI details.
...
- `runTurn()` is a sufficiently small primitive for isolated generation and Reviews' bounded repository-aware execution without persistent cross-process machinery. **Partially validated**: the prototype supports the small caller interaction, but its invented transcript protocol does not prove native sequential turns, concurrency/close semantics, or failed-turn history. Production should start single-turn unless research establishes real continuation plus a consumer need.
...
- A lowest-common-denominator result could discard Claude usage data, Codex diagnostics, structured output fidelity, cancellation distinctions, or Reviews' pinned parse taxonomy. **Partially de-risked**: bounded provider usage and structured transport ownership remain viable, but the prototype's generic `invalid-output` and flattened text-generation errors do not yet preserve invalid-JSON, invalid-envelope, invalid-findings, and empty-output stages. The production contract must retain typed stage/cause evidence even where a convenience narrows it to strings.
- Session and profile machinery could become shallower and more complex than the duplicated launchers it replaces. **Partially de-risked at interface level**: profile-specific factories and one small session interaction remain deep candidates, while a shared Harness marker, generic subprocess wrapper, provider-specific TextGenerator subclasses, capability discovery, and an optional profile bag remain rejected. The prototype also exposed shallow or leaky candidates that must not be promoted: subprocess `purpose`/schema fields in the domain contract, a brand with no semantic enforcement, fictional lifecycle commands, and a conditional output-mode generic requiring assertions.
- Claude Code may not have an enforceable repository-aware read-only mode comparable to Codex's sandbox. Existing Reviews grants `Bash,Read`, and prompt-level non-mutation is not a guarantee; this can force a narrower tool set, a renamed/weakened profile, or a different migration order.
- The real command channel does not currently distinguish timeout from caller cancellation in its result shape, while the prototype assumes both booleans are already available. Production must prove cancellation before/during spawn, timeout, process-group termination, startup failure, and evidence preservation behind an internal harness transport seam.
...
### Contract questions reopened by prototype critique

- Can Claude Code enforce the read-only-agent guarantee, and what native continuation state—if any—supports sequential turns without transcript invention?
- Should production sessions begin single-turn, serialize turns internally, or reject concurrency explicitly; what happens when close races a turn or a failed/cancelled turn precedes another?
- Should successful text/structured results be one non-generic discriminated union or separate named creation surfaces? The prototype's conditional `TMode` generic conflicts with the earlier no-payload-generic decision.
- Should a statically impossible profile factory be absent from a concrete harness module, reserving creation failures for environment/version preflight?
- What structured failure stage/cause is required below Reviews to reproduce its pinned parse taxonomy while keeping the common seven-kind failure classification?
- What internal command gateway can classify cancellation versus timeout and preserve raw evidence without exposing schema, resource-purpose, or profile semantics?

### Fog

- The eventual package and subpackage names should remain unset until the prerequisite research, interface depth, importer classes, and dependency-cycle pressure are resolved.

Still settled: streaming remains out of this contract revision; consumer-semantic Zod validation stays above provider transport parsing; capability discovery and an optional-bag profile interface remain rejected.
