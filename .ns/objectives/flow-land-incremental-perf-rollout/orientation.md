**Direction: Flow land performance changes land only as small, validated/evidence-recorded, git-revertible slices through this Objective.**

Getting to: each improvement prototyped in the unmerged `flow-land-perf-baselines` reference stack is freshly re-derived as its own revertible PR, gated by local validation and materially relevant Objective evidence before the next risky slice; telemetry and the fake-backed scenario counts stay the evidence backbone.
