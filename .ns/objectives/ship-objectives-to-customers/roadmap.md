...
- [~] **Dependency (owned by `checkout-free-sdl-distribution`):** make the `ns` CLI publishable and checkout-free — drop `private` on `@ns/kernel` and its required runtime workspace deps, replace the source-path module loader (`ts/packages/kernel/src/runtime/module-loader.ts`) so `@ns/objective` and its hidden `exec` surface are bundled, produce a bundle with no `ts/node_modules` precondition, and publish a versioned package to npm.
  - Evidence: a global/`npx` install of `ns` on a machine with no ns checkout runs `ns objective list` against a foreign repo.
  - Notes: tracked here as a consumed dependency; the work lives in the split Objective. Its bundle strategy and runtime-dependency triage are decided, and loader/catalog prep has started reducing the Objective discovery source-path assumption. Publish metadata, final build/package shape, checkout-dependent shim replacement, actual npm publish, and no-checkout install verification remain open.
- [ ] **Dependency (spans `checkout-free-sdl-distribution` + `skill-management-subsystem`):** bundle the objective skill dirs into the npm package and expose a minimal `ns skills install` that copies them into the correct per-harness roots (`.claude/skills/`, `.agents/skills/`). Copy-not-symlink (areg's symlink model does not fit customers).
  - Notes: consume the platform path table, do not rebuild it; this is the concrete op the `@ns/init` `SkillMaterializer` binds to. No `ns skills` command exists yet.
- [ ] Scaffold the `@ns/init` capability package: `SkillMaterializer` gateway interface + fake + labeled stub real-impl, and the managed-block/git-posture operation surface, surfaced as top-level `ns init` (wired like `@ns/objective`). Reuse `@ns/core/managed-region`; borrow areg `init` patterns without depending on it.
