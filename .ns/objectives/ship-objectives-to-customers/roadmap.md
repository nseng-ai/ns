...
- [ ] **Dependency (spans `checkout-free-sdl-distribution` + `skill-management-subsystem`):** bundle the objective skill dirs into the npm package and expose a minimal `ji skills install` that copies them into the correct per-harness roots (`.claude/skills/`, `.agents/skills/`). Copy-not-symlink (areg's symlink model does not fit customers).
  - Notes: consume the platform path table, do not rebuild it; this is the concrete op the `@ji/init` `SkillMaterializer` binds to. No `ji skills` command exists yet.
- [ ] Scaffold the `@ji/init` capability package: `SkillMaterializer` gateway interface + fake + labeled stub real-impl, and the managed-block/git-posture operation surface, surfaced as top-level `ji init` (wired like `@ji/objective`). Reuse `@ji/core/managed-region`; borrow areg `init` patterns without depending on it.
