...
**Naming (ADR 0026, `rename-ji-to-ns`):** the shipped customer surface is the `ns` CLI, and the core cutover has landed — the repo's binary is now `ns`, consumer dirs are `.ns/` (`.ns/objectives/`, `.ns/extensions/`), config is `ns.toml`, and the package-scope sweep renamed the packages this Objective touches to `@ns/kernel` (`ts/packages/kernel`), `@ns/objective` (`ts/packages/capabilities/objectives`), and `@ns/core` (`ts/packages/infra/foundation`). All new surface this Objective builds stays ns-named — `ns init`, `ns skills`, `ns objective …`, `ns:objectives:*` block markers, the `@ns/init` package. The rename changed names only: the CLI is still run-from-source (the kernel bin points at raw `src/cli/index.ts`), so the checkout-free gap is unchanged.
