...
The Pi CLI bridge now provides invocation-scoped semantic confirmation and selection callbacks. When Pi has applicable UI, these operations delegate only to `ctx.ui.confirm` and `ctx.ui.select` while preserving activity phases. Confirmation returns the named `confirmed`, `declined`, or `cancelled` state. Selection returns `selected` with a value or the named `cancelled` state. When UI or a requested operation is unavailable, the callback throws an explicit error without touching terminal input or UI.

The shared host contract requires confirmation and selection from every host. Clinkr is the canonical owner of `ConfirmationResult` and `SelectionResult<T>`, and the SDK exposes host operations that reuse those result types. The contract exposes only these semantic operations and their named results; a missing host capability remains an error, not a user outcome. It does not expose terminal streams, line readers, raw mode, key events, cursor state, resize behavior, or other terminal machinery. The standalone host provides terminal confirmation and explicit unsupported-operation errors where it cannot interact.

## Objective Impact

The fourth roadmap item is complete. Runner checkpoint `710a8f06b03cced1d40b3da207228610ac6d163f` records the implementation and passed the runner gate. Every ns host must now provide both semantic interaction operations. The Pi host maps user outcomes to the Clinkr-owned named states and reports an error in headless or operation-missing contexts.
