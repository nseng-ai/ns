...
- Specifically watch for the repeated-integration-setup-for-localized-logic anti-pattern: a test (often a per-case loop) that stands up integration setup once per case — temp project, real extension-catalog discovery + dynamic import, real repo, etc. — when the thing under test is localized domain/bridge logic reachable through an injected seam. Split it into many cheap fake-seam scenario/unit cases plus meaningful representative integration coverage for each distinct real boundary surface involved, rather than re-running the backend setup per case.
...
- Move retained integration coverage to the suite's established integration location. For TypeScript packages, use `ts/packages/<package>/test/integration/**/*.test.ts` (the shared Vitest globs, generated by `testGlobsFor` in `ts/vitest.shared.ts`, also cover grouped packages at `ts/packages/<group>/<package>/test/integration/**/*.test.ts` and review tools at `.ns/reviews/*/tools/*/test/integration/**/*.test.ts`; `integration/` must sit directly under the package `test/` root to be discovered) and the existing `pnpm --dir ts run test:integration` / `just ts-test-integration` lane. For Python packages, follow the package's `tests/integration/` convention when an analogous split is needed.
...
3. **Preserve confidence before claiming speed.** For TypeScript migrations, inventory each confidence claim and apply the canonical [cross-product coverage model](../../../ts/TESTING.md#cross-product-coverage-model): keep application behavior claims in default-path coverage through fakes, injected command/sqlite/process seams, scenario fakes, or existing manual time helpers, and retain meaningful representative integration coverage for every distinct real adapter/runtime surface involved.
...
- Package renames and directory restructures can silently undo completed migrations: the lane split is enforced only by Vitest include/exclude globs (generated by `testGlobsFor` in `ts/vitest.shared.ts`: `packages/*/test/integration/**`, `packages/*/*/test/integration/**`, and `../.ns/reviews/*/tools/*/test/integration/**`), so an `integration/` directory nested any deeper — for example `test/exec/integration/` — falls back into default discovery with no failure signal. Proven on 2026-07-03: the `@sdl/*` → `@nseng-ai/*` package restructure returned twelve real-boundary tests to the default lane; restored on 2026-07-04 (see `updates/` and the roadmap).
