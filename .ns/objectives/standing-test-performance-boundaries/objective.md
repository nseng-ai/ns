...
- Specifically watch for the repeated-integration-setup-for-localized-logic anti-pattern: a test (often a per-case loop) that stands up integration setup once per case — temp project, real `.ns/extensions` discovery + dynamic import, real repo, etc. — when the thing under test is localized domain/bridge logic reachable through an injected seam. Split it into many cheap fake-seam scenario/unit cases plus one integration smoke at the real boundary, rather than re-running the backend setup per case.
...
- Move retained integration coverage to the suite's established integration location. For TypeScript packages, use `ts/packages/<package>/test/integration/**/*.test.ts` (the shared Vitest globs also cover grouped packages at `ts/packages/<group>/<package>/test/integration/**/*.test.ts`; `integration/` must sit directly under the package `test/` root to be discovered) and the existing `pnpm --dir ts run test:integration` / `just ts-test-integration` lane. For Python packages, follow the package's `tests/integration/` convention when an analogous split is needed.
- Treat shared-cache safety as a separate boundary from integration. TypeScript tests whose subject genuinely requires Vitest module-state or process-global mutation belong under `test/isolated/` and run with `just ts-test-isolated` in a separate CI job. Prefer injected seams and supported auto-restored stubs first; isolation is containment for irreducibly ambient behavior, not a general slow-test lane or an integration synonym. The default `just` entrypoint deliberately omits this lane.
- Add or refine narrow testing seams when an existing production boundary makes default-path tests depend on real systems. Prefer explicit gateway injection, in-memory fakes, command/process/sqlite adapters, and existing `@nseng-ai/foundation` `Clock` (`@nseng-ai/foundation/clock`) / `TimerScheduler` (`@nseng-ai/foundation/timers`) seams for TypeScript time-sensitive behavior.
...
- After package renames or directory restructures that touch test trees, re-verify lane placement: confirm moved integration files are still discovered by the integration config and excluded from the default config, rather than assuming a completed split stays completed.
...
- Do not move a test to the isolated lane merely to silence a guard. Remediate ambient state through injection, explicit env/cwd, manual time helpers, supported stubs, or an owned lifecycle seam first; isolate only when ambient behavior is the contract.
...
- Each implementation pass should treat the Objective itself as a self-learning loop: when the work reveals a reusable lesson, stale instruction, missing guardrail, or over-specific rule, update the Objective or roadmap so future agents inherit that learning instead of rediscovering it.
...
6. **Update the standing guidance with lessons learned.** At the end of an implementation slice, revise this Objective when the slice proves a better heuristic, invalidates an assumption, exposes a missing non-goal, shows a pattern worth adding, or makes existing guidance too broad or too narrow. Lessons may be additions, removals, or edits to Scope, Non-Goals, Definition of Progress, Runner Policy, Implementation Guidance, Assumptions/Risks, Open Questions, roadmap guidance, or a new Semantic Update. Do not preserve stale text just because it was useful for an earlier slice.
7. **Guard the shared-cache contract, not just lane globs.** A fast `isolate: false` posture depends on test behavior, not directory names alone. Once an inventory proves a narrow family of contaminating operations, enforce that vocabulary outside `test/isolated/` with adversarial style-guard cases and actionable alternatives. Keep the isolated lane explicit, separately discovered, and separately reported in CI; never infer that default `just` covered it.
...
- **Time and timeout behavior:** keep wall-clock reads (`Clock`) separate from one-shot timer scheduling (`TimerScheduler`). Default tests should advance the `@nseng-ai/foundation/time/testing` manual helpers (`createManualClock()`, `createManualTimerScheduler()`) rather than using sleeps, elapsed `Date.now()` assertions, or broad fake-timer state.
...
- **Per-case integration fan-out is a seam smell.** When a `for (const case of cases)` loop re-creates a temp project / real repo / real CLI loader per iteration, ask whether each iteration needs the real backend or just a seam. If the backend behavior is identical across cases and only localized inputs vary, move the fan-out onto a fake seam (for example, inject a fake `runCli`/gateway and assert routing, argument shaping, and output rendering) and keep one representative integration smoke. The flow Pi-extension slice is the model: `nsExtension(pi, { runCli })` in `ts/packages/capabilities/flow/src/pi/ns-extension.ts` lets default tests (`ts/packages/capabilities/flow/test/pi/ns-extension.test.ts`) assert `/ns:flow:<name>` → `ns flow <name>` routing per command with no temp project, while a single integration test (`ts/packages/kernel/test/integration/flow-extension-registry.test.ts`) proves the real loader imports every checked-in flow command entry.
- **Shared-cache contamination is not an integration boundary.** Module mocking/reset, Vitest fake timers, direct process env/cwd mutation, process-global listeners, and singleton lifecycle calls can make otherwise fake-backed tests order-dependent under `isolate: false`. First replace them with injection, manual schedulers, explicit inputs, auto-restored `vi.stubEnv()`, or owned lifecycle seams. Keep only focused tests of irreducibly ambient behavior in `test/isolated/`, with a style guard preventing those operations from drifting back into shared-cache lanes.
...
- Performance wins can be overstated when a change merely shifts cost from the default suite into integration or isolation; updates must distinguish those cases. Establishing containment and regression guards without a comparable post-change default measurement is a correctness/determinism result, not a speedup claim.
...
- Package renames and directory restructures can silently undo completed migrations: the lane split is enforced only by Vitest include/exclude globs (`packages/*/test/integration/**`, `packages/*/*/test/integration/**`), so an `integration/` directory nested any deeper — for example `test/exec/integration/` — falls back into default discovery with no failure signal. Proven on 2026-07-03: the `@sdl/*` → `@nseng-ai/*` package restructure returned twelve real-boundary tests to the default lane; restored on 2026-07-04 (see `updates/` and the roadmap).
...
- Should the integration lane get a structural guard (for example, a check that no `integration/` test directory sits outside the shared lane globs) so restructures cannot silently regress lane placement? Decide when fixing the 2026-07-03 lane regression.
