...
- [x] Make the local aggregate boundary explicit. Default `just` / `just check` runs core validation without integration, isolated, or the TypeScript style guard; the guard remains available through its dedicated recipe, opt-in `just ci`, and its separate non-draft CI job. Isolated remains explicit and outside `just ci`. This records command topology, not a measured speedup.
