...
Current implementation state: the already-implemented surface (`run`, `show`, `diff`, `runs`, the `claude` runner adapter, local bundle storage, Markdown reports, and local git/result-branch behavior) now lives in TypeScript as `@ji/vibechk` at `ts/packages/tools/vibechk/` (the TypeScript workspace package scope has since been renamed `@sdl` -> `@ji`). The earlier "migration pause" is over: the `vibechk-typescript-port` Objective that owned the language cutover has completed and is closed, the Python package was retired, and TypeScript is the documented default. Remaining v1 product feature work — `publish`, the `codex` and `pi` runner adapters, and a real GitHub publish smoke — now resumes on top of the TypeScript implementation rather than being blocked.
...
Implementation sequencing prioritized a thin, incomplete vertical slice of that workflow before hardening every supporting surface: run a plan in two prepared workdirs, inspect `vibechk diff`, and have Markdown evidence that can be manually pasted into a PR. That walking skeleton plus the store/listing surface now exist (carried through the TypeScript cutover); remaining work deepens runner parity and adds `publish`.
...
- Ship a standalone workspace package exposing the CLI command `vibechk`. The current home is the TypeScript package `@ji/vibechk` at `ts/packages/tools/vibechk/`.
...
- Run v1 runner adapters for `claude`, `codex`, and `pi` through subprocess/RPC-style integration as needed, stream raw transcript/event output incrementally, derive normalized metrics from each runner's available data, and persist partial bundles for failed runner exits. Currently only the `claude` adapter is registered; `codex` and `pi` remain to be added.
...
- Publish reports idempotently into GitHub PR descriptions via `gh`, preserving user prose outside the `vibechk` fence. (Not yet built.)
...
- Follow repo conventions for TypeScript package layout, Clinkr-style CLI construction, gateway/fake test seams, and Vitest scenario tests. The module outline in issue #434 is guidance, not a hard file-map requirement.
...
- Building runner-specific product surfaces or making Pi a special SDK-native v1 path; `claude`, `codex`, and `pi` should enter through the same uniform runner contract.
...
- `vibechk run --plan PATH [--workdir DIR] [--runner NAME] [--model NAME]` accepts `claude`, `codex`, and `pi` runner names, enforces the documented git preconditions, creates a unique 8-character run id, streams transcript/event output incrementally, persists all required bundle files, and prints the run id. (Today only `claude` resolves; `codex`/`pi` remain.)
...
- `vibechk publish` resolves all documented PR reference forms, checks required branches exist on the PR remote, fetches and edits the PR body through authenticated `gh`, replaces an existing `vibechk` fence idempotently, appends when absent, and short-circuits byte-identical publishes. (Not yet implemented.)
...
- A standalone `vibechk` workspace package remains the right home for v1 and does not need to be an `sdl` plugin; the scaffold and the TypeScript cutover validated that home with workspace wiring, help/version/runtime behavior, and repo validation passing.
- User-facing workflow feedback is more valuable early than complete infrastructure polish; the thin `run -> show/diff` walking skeleton, the store hardening, and `runs` listing already validate the local bundle loop, so remaining v1 work should deepen runner parity, add `publish`, and harden around the proven bundle/report shape.
- Users preparing baseline and treatment workdirs outside the tool is acceptable and keeps v1 composable.
- The `claude`, `codex`, and `pi` CLIs can each be driven non-interactively from the runner adapters in a supplied clean workdir, even if their output formats and available metrics differ. (Verified for `claude`; assumed for `codex`/`pi`.)
- Treating Pi through the uniform runner interface is acceptable for v1; direct Pi SDK session forking/resource-manifest evaluation can wait until after the implemented surface proves the bundle/report workflow.
- Eight-character hex run ids are sufficient for a local bundle store when regenerated on collision.
- Git branch and commit operations can be implemented through repo-conventional git seams without requiring Graphite at runtime; the TypeScript implementation uses a domain-shaped workdir/git gateway.
...
- Auto-staging and committing all workdir changes is powerful; the walking skeleton and TypeScript port de-risk this with clean-workdir checks, result-branch commits, switch-back behavior, and real git gateway coverage, while rollback behavior for mid-commit failures still needs hardening.
...
- The bundle schema may become a de facto public interface immediately, so v1 should avoid underspecified optional fields and keep future additions additive. Schema version 1 (snake_case `bundle.json`) is the current persisted contract.
