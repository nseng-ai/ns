...
- `model_profile` directly names an alias in root `ns.toml` `[models.profiles]`; it defaults to `fast` when omitted. The `fast` alias displays as a Tripwire, while every other configured alias displays as a deep review.
